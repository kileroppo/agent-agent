...
- M2 军团方向已校正：飞书“ A君·军团总管”是唯一日常派活、状态、交付与手机审批入口；Hermes 负责把飞书命令送到 Mac 并承载各 Agent 的角色、模型、技能与运行；A君提供连接授权、内容获取、组件托管、执行适配和故障恢复。普通一次性审批由飞书卡片与 A君审批记录闭环；仅新 Agent、扩权、外发、花钱、跨 Agent 长任务进入 Paperclip 的组织级任务、预算、审批与审计。Paperclip `2026.707.0` 已由本机 `launchd` 常驻，2026-07-21 在 `127.0.0.1:3100` 健康检查通过；其内置 HTTP Adapter 的低风险 heartbeat 闭环此前已验证。组织级飞书卡片会先把决定回写 Paperclip，再由 A君复核范围和有效期；真实手机点击仍待验收。
