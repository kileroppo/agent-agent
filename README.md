...
- 当前里程碑：**M5 并行 v2 已安全克隆到 live，真实发布仍关闭**。live 目标为 15 阶段 Pipeline、17 个 Routine 和 5 个无模型 HTTP 控制器：`daily`、`parallel`、`publisher`、`metrics`、`retrospective`。旧 v1 Pipeline、22 个 Case、Issue 和 Work Product 原样保留，旧草案已 superseded/cancelled。
...
- **本地执行能力增强已通过，但还没进入 live**：Pipeline `67/67`、内容插件候选源码 `0.4.9` 为 `97/97` 且 `check` 通过（`0.4.8` 仅为历史候选）、Paperclip 集成 `48/48`、Publisher Gateway `203/203`、A君全量自动化 `1004/1004`、岗位 Manifest `15/15`，Fake 全链 E2E `5/5`。Paperclip live 内容插件仍是 `0.4.7 ready`，`0.4.9` 尚未安装，当前 A君 `4321` 仍是旧进程；这些新增测试不能冒充 live 已加载。本轮源码根、隔离修理副本与候选发版的既有聚焦回归为 `139/139`。`task-store` 与飞书 completion watcher 的本机持久状态权限回归为 `15/15`：使用唯一临时文件、`wx`/0600、原子 rename 和 rename 后 chmod，既有 0644 文件会收敛为 0600，失败时不破坏旧文件或无关临时文件；本轮没有直接 chmod 真实数据。开放研究及其路由/Routine 契约定向 `29/29`，支持公开网页、动态网页、PDF、GitHub、Observation 换路、预算/重试/重规划和 Work Product 回写；恢复时会核验内嵌 Observation 与当前 assignment 的 Issue/Run 一致，跨 Issue/Run 注入失败关闭。外置可写 Git 源码根中的修复只能产出 `candidate_promoted` 并进入 `repair_candidate_awaiting_release`，不能冒充当前运行版本已经修好。Fake E2E 覆盖 15 阶段、并行峰值 4、重试、检查点恢复、退回、预算硬停、幂等发布、2h/24h/72h 模拟指标与复盘；这些均为本地 Fake 证据，零付费、零外部效果。
- **新的 A君不可变 Runtime Release 已冻结并通过启动验证，但尚未切 live**：clean source commit 为 `33aa25bd7ff7431d64467fca87866d299caa9857`；候选位于 `work/m5-runtime-releases/m5-8point-20260731-r2/ajun-runtime-release-v1-1c7f244ddaae055f336340ac0de566569012af3d39a6e66e8df528fda46ce0ef/`，`releaseHash=1c7f244ddaae055f336340ac0de566569012af3d39a6e66e8df528fda46ce0ef`、`payloadHash=7bd23d48db1f66583d854a28d420498e60e884e261a10171c4767e818156c910`、`entryCount=7571`、`manifestSha256=102daa78172a8857e7151d1a619d6868fb30f97cad1b48e8751ca93b5feb128c`、独立全目录哈希 `efc8967c6662b645f3018c0b6386231006f21b45f561a932e88df03799eb4b88`；冻结包以临时隔离状态启动，`/api/overview` 返回 200，SIGTERM 后确认退出。遗留 staging 与 final 均先通过 official validate，随后仅将自产 staging 移至 `~/.Trash/agent-army-staging-1c7f244ddaae055f-20260731`，可恢复，final 再次 validate 通过。旧 `025d4816…` 保留为历史失效候选，`0767604f…` dirty 候选继续隔离；没有修改 plist、没有重启现有 A君、没有激活发布。由于仍缺与旧运行版精确匹配的独立 rollback release/源码根，cutover/rollback 继续 blocked。
- **Run JWT 与恢复链已实现最小兼容层，不是 live 能力**：仓库提供 Paperclip `2026.722.0` 版本锁定的 `forwardRunJwt` 和一次性恢复 Approval 兼容补丁，合并测试 `15/15`；controller cutover 工具 `15/15`，其快照读写 TOCTOU 已通过同 fd、`O_NOFOLLOW`、dev/ino 复验、原子 no-replace 发布和固定原目录身份的清理器修复；post-link 父目录替换时原目录与替代目录均无残留、Paperclip PATCH 为 0，清理不完整会标记 `recoveryRequired`。清理器的 ready、cleanup、close 均有硬超时，卡死时按 `TERM → KILL → 确认退出` 收口；独立红队确认无残留监听器或子进程。A君 current-run 恢复 access provider 已实际 wire 进 server composition/metrics 请求级 Run 凭据作用域，provider composition 聚焦回归 `43/43`、相关 server/controller 回归 `84/84`。这些仍只是源码和本地测试：兼容补丁尚未 apply，live 控制器 adapterConfig 未启用 `forwardRunJwt`，当前 4321 PID `15246` 也早于本轮源码，未加载该 binding，不能把 `/api/overview` 200 当成本轮代码已上线。
...
- **11 个正式 Hermes Profile 已实际收敛，但 M5 尚未完成**：11 岗均已同步到 `stepfun/step-3.5-flash-2603` 及岗位最小 MCP/Feishu toolset；同步后的全军只读 dry-run 为 `0 drift`，11 份逐 Profile 备份目录权限均为 `0700`，同步过程 `gatewayActions=0`，没有启停 Gateway。岗位技能白名单随后指定只读检查为 `11/11 clean`。这些是 Profile 配置层证据；当前 A君 `4321` 仍是未重启的旧进程，Paperclip live 插件仍是 `0.4.7`，Publisher 仍关闭，真实 M5 Campaign 的 StepFun 视觉调用尚未执行，不能写成 M5 完成。完整分层证据见 [M5 验收账本](./docs/reviews/m5-high-autonomy-content-operations/acceptance.md)。
- 2026-07-30 微信聊天取件员正式按需上岗：A君自动采用“今天至现在、最多 200 条、增量刷新、同名会话选最近活跃”的默认范围，只有缺联系人/群名时才补问；真实读取前只要求一次逐次隐私确认。原文和发送者标识只在本机当前执行内存中流转，不进入 StepFun、DeepSeek、Paperclip、飞书、日志或报告。该岗位没有常驻监听，也不能发送微信消息。
