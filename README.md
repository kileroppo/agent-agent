...
- **2026-08-04 M1 本地 AI 全能力基础设施与 A君 live 接线已完成**：Qwen3.5-9B 的文本/视觉、Whisper ASR、Qwen3-TTS、Qwen3-Embedding/Reranker、带版本与访问范围过滤的本地知识索引，以及 MFLUX + FLUX.2 4B 的文生图、单图编辑和多参考图均已用真实本地样本通过；18081/18082 已由 LaunchAgent 守护，并在强制重启后自动恢复且 smoke 通过。统一回环网关位于 `integrations/local-ai`，提供能力状态、资源互斥、取消和逐请求批准的私网台式机转发，不复制 Paperclip 业务任务队列。A君 4321 已从干净提交 `087fd39…` 切到不可变 release `6a41ec6a…`，强制重启后 PID `22344` 的 cwd/entrypoint 仍指向该 release；live 概览回读本机 AI 11 项 E2E 为 `ready`，微信本机只读也为 `ready`。4070 软件端已补齐鉴权、Mac 来源 allowlist、附件/产物校验、单 GPU 队列、ComfyUI adapter 和 Windows/Linux bundle，模拟双进程链路与断线回 Mac 已通过；另一台电脑尚未提供系统和连接入口，因此真实 CUDA/FLUX E2E 未完成。视频生成与授权声音克隆保持失败关闭。见 [架构](./docs/architecture/local-ai-capability-system.md) 与 [验收账本](./docs/reviews/local-ai-capability-system/acceptance.md)。

- **11 个正式岗位已切到 DeepSeek**：AgentManifest、Hermes 映射、本机 11 Profile 与 Paperclip Adapter 均为 `deepseek/deepseek-v4-flash`，回退链为空；5 个常驻 Gateway 已重启，A君当前运行不可变 release `6a41ec6a…`（重启验收 PID `22344`），该 release 的 source commit 为 `087fd39…`。此前启动后的 Paperclip 对账为 11/11 DeepSeek；本次 AI 能力切换没有付费模型探针，配置与 live 对账不等于真实 DeepSeek 调用通过；M5 StepFun 生图、视觉和 TTS 是独立媒体能力，额度不足时继续失败关闭。
...
- 当前里程碑：**A君架构重构版与本机 AI 客户端已切到 fresh 不可变运行包，SQLite 与 Paperclip 既有状态目录保持不变，真实发布仍关闭**。A君强制重启验收 PID `22344` 从 release `6a41ec6a…` 运行；本轮没有 apply Paperclip 流程资源，也没有修改旧 v1 Pipeline、Case、Issue 或 Work Product。
...
- 2026-08-04 微信聊天取件员已随 A君 release `6a41ec6a…` 迁移到回环 `Qwen3.5-9B` OpenAI-compatible 服务：默认采用“今天至现在、最多 200 条、增量刷新、同名会话选最近活跃”，只有缺联系人/群名时才补问；首次确认生成绑定同一飞书会话、岗位、微信会话与固定范围的 30 分钟临时授权，最多 10 次，可撤销。原文和发送者只在本机内存及 `127.0.0.1:18081` 中流转，不进入 StepFun、DeepSeek、Paperclip、飞书、日志、台式机或报告；本机模型或 Vault 未就绪时不扣次数且不云端降级。live 概览已回读 `wechat-private-read=ready`，合成消息实模摘要不含原文和发送者标识；真实私聊仍需主人明确指定会话与范围后验收。该岗位没有常驻监听，也不能发送微信消息。
