...
- **11 个正式岗位已切到 DeepSeek**：AgentManifest、Hermes 映射、本机 11 Profile 与 Paperclip Adapter 均为 `deepseek/deepseek-v4-flash`，回退链为空；5 个常驻 Gateway 已重启，A君已切到不可变 release `80bd473f…`（PID `73653`），启动后 Paperclip 对账为 11/11 DeepSeek。本次没有付费模型探针，配置与 live 对账不等于真实 DeepSeek 调用通过；M5 StepFun 生图、视觉和 TTS 是独立媒体能力，额度不足时继续失败关闭。
...
- 当前里程碑：**A君架构重构版已切到 fresh 不可变运行包与 SQLite，Paperclip 流程资源与真实发布仍保持关闭**。A君验收 PID `52870` 从 release `389141e4…` 运行；Paperclip 仍是 15 阶段 Pipeline、17 个有效 Routine 和 5 个无模型 HTTP 控制器。旧 v1 Pipeline、22 个 Case、Issue 和 Work Product 原样保留，旧草案已 superseded/cancelled。
...
- 2026-08-02 微信聊天取件员升级为本机分析：A君默认采用“今天至现在、最多 200 条、增量刷新、同名会话选最近活跃”，只有缺联系人/群名时才补问；首次确认生成绑定同一飞书会话、岗位、微信会话与固定范围的 30 分钟临时授权，最多 10 次，可撤销。原文和发送者只在本机内存及 `127.0.0.1` 的 `qwen3:14b` 中流转，不进入 StepFun、DeepSeek、Paperclip、飞书、日志或报告；本机模型或 Vault 未就绪时不扣次数且不云端降级。当前 4321 的不可变包 `389141e4…` 已加载早期纵切和模型执行器，但尚未包含同范围跨任务复用、飞书原会话撤销、模型概览健康和 8 字原文片段防回显的最终加固；真实私聊继续保持未验收，最终加固须随下一版可追溯 release 一起切换。该岗位没有常驻监听，也不能发送微信消息。
