...
| 字段 | 内容 |
| --- | --- |
| 状态 | 已完成：PPTD/PPTX、不可变运行时、真实任务与三类 Work Product 均已验收 |
| 负责人 | 小办 / A君 |
| 最后更新 | 2026-08-07 |
...
新增 `office.presentation-package`，固定由 `office-assistant` 承接，并交付可编辑、自包含、可追溯的 PPTD、PPTX 与逐页视觉 QA。能力复用现有 Manifest、Hermes Profile、Paperclip Issue/Work Product、A君 MCP 和岗位授权，不新增队列、审批系统、控制台或常驻服务。结构化演示任务由 A君受控本地适配器确定性执行，Paperclip 记录治理与产物引用，不为该任务额外触发 Hermes/模型 Run。
...
- 所有路径必须位于当前 A君受控任务工作区，且为安全相对路径；拒绝绝对路径、`..`、符号链接、工作区外素材和已有目标覆盖。
...
- 真实任务：从小办入口创建一条公开样例任务，由 A君本地适配器完成，Paperclip 只记录脱敏运行元数据和三类 Work Product 引用；真实飞书消息按会话另行授权，不作为本地能力关闭门禁。
