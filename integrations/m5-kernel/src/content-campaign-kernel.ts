...
import { assertM5ControlPlane } from './control-plane.ts';
import { campaignNextAction as nextAction, ContentCampaignError, normalizeCampaignDraft as normalizeDraft, safeCampaignGrantView as safeGrantView, } from './campaign-domain.ts';
export { ContentCampaignError } from './campaign-domain.ts';
import { CampaignLifecycle } from './campaign-lifecycle.ts';
import { M5ParallelWorkCoordinator } from './parallel-work-coordinator.ts';
import { contentCampaignExecutionMethods } from './content-campaign-execution.ts';
...
export class ContentCampaignKernel {
    ...
    readonly publisher: Record<string, any> | null;
    readonly toolExecutor: Record<string, any> | null;
    readonly templateResolver: Record<string, any> | null;
    readonly activePipelineId: string | null;
    readonly activePipelineKey: string;
    readonly contentWorkspaceRoot: string | null;
    readonly allowLocalFixtureProvenance: boolean;
    readonly now: () => Date;
    readonly lifecycle: CampaignLifecycle;
    declare caseChain: (caseId: any) => Promise<any[]>;

    constructor({ controlPlane, definition, publisher = null, toolExecutor = null, templateResolver = null, activePipelineId = null, activePipelineKey = null, contentWorkspaceRoot = null, allowLocalFixtureProvenance = false, now = () => new Date(), }: any = {}) {
        if (!controlPlane || !definition)
            throw new ContentCampaignError('M5 内容活动缺少 Control Plane 或 Pipeline 定义。');
        ...
        this.lifecycle = new CampaignLifecycle({
            controlPlane: this.controlPlane as any,
            definition,
    ...
    async list() {
        ...
        const parents = rows.filter((item: any) => !item.parentCaseId && item.campaignGrant);
        return Promise.all(parents.map(async (item: any) => campaignView(item, {
    ...
    async reconcileParallelWork(dayCaseId: any) {
        const pipeline = await this.lifecycle.pipeline();
    ...
    async onM5WorkProductSynced({ pipelineCaseId, stageKey }: any = {}) {
        if (![
            'topic',
            'script',
            'evidence',
        ...
        const dayCase = chain.find((item: any) => item?.scheduledDate
            && !item.platform
            && !item.workBranch
            && item.parentCaseId);
        if (!dayCase)
            throw new ContentCampaignError('并行工作产物无法回溯到日期 Case。');
        return {
            reconciled: true,
            result: await this.reconcileParallelWork(dayCase.id),
        };
    }
    async get(caseId: any) {
    ...
    async getRawCase(caseId: any) {
        return this.lifecycle.parentCase(caseId);
    }
    async getAnyCase(caseId: any) {
        return this.lifecycle.case(caseId);
    }
    async assertCaseInActivePipeline(item: any) {
        await this.lifecycle.assertActivePipeline(item);
    }
    async transitionCase(item: any, toStageKey: any, reason: any) {
...
export interface ContentCampaignKernel {
    m5StageToolParameters(input: any): Promise<any>;
    executeM5MachineReview(input: any): Promise<any>;
    assertReplayableM5WorkProduct(input: any): Promise<any>;
}
Object.assign(ContentCampaignKernel.prototype, contentCampaignExecutionMethods);
function campaignView(item: any, related: any = {}, definition: any = {}) {
    ...
    const platformCases = descendants.filter((entry: any) => PLATFORMS.includes(entry.platform || platformFromCaseKey(entry.caseKey)));
    const done = platformCases.filter((entry: any) => entry.stage?.kind === 'done'
        || entry.terminalKind === 'done'
        || entry.stageKey === 'retrospective'
        || entry.status === 'done').length;
    return {
        campaignId: item.id,
...
function campaignFocus(parent: any, descendants: any, definition: any) {
    const active = descendants.filter((item: any) => item?.activeWork);
    const candidates = active.length ? active : [parent];
    const stageKeys = [...new Set(candidates.map((item: any) => item.stageKey).filter(Boolean))];
    const stages = stageKeys.map((key: any) => definition?.stages?.find((stage: any) => stage.key === key) || { key, name: key, owner: null });
    const stageName = stages.map((stage: any) => stage.name || stage.key).filter(Boolean).join('、')
        || parent.stage?.name
        || parent.stageKey
        || null;
    const owners = [...new Set(stages.map((stage: any) => stage.owner).filter(Boolean))];
    const owner = {
        agentIds: owners,
...
function m5ContentRecoveryInstruction(cases: any) {
    const recoveryStates = (Array.isArray(cases) ? cases : [])
        .map((item: any) => item?.contentRecovery)
        .filter((value: any) => value?.schemaVersion === 'agent.army/m5-content-recovery/v1')
        .sort((left: any, right: any) => Date.parse(right.updatedAt || 0) - Date.parse(left.updatedAt || 0));
    for (const recovery of recoveryStates) {
        const blocked = Object.values(recovery.stageRecoveries || {})
            ...
            .sort((left: any, right: any) => Date.parse(right.updatedAt || 0) - Date.parse(left.updatedAt || 0))
            .map((item: any) => item?.recoveryAction?.instruction)
            .find((value: any) => typeof value === 'string' && value.trim());
        if (blocked)
            return blocked;
    }
    const activeRevision = recoveryStates
        .map((recovery: any) => recovery.activePlanRevision)
        .find((revision: any) => revision?.schemaVersion === 'agent.army/m5-plan-revision/v1'
...
function flattenCases(items: any) {
    const result = [];
...
function descendantCases(parentId: any, rows: any) {
    const result = [];
    const queue = [parentId];
    while (queue.length) {
        const currentId = queue.shift();
        const children = rows.filter((item: any) => item.parentCaseId === currentId);
        result.push(...children);
        queue.push(...children.map((item: any) => item.id));
    }
    return result;
}
function childNodesFromTree(value: any) {
    return Array.isArray(value) ? value : [];
}
function caseRows(value: any) {
    return Array.isArray(value) ? value.filter(Boolean) : [];
}
function platformFromCaseKey(caseKey: any) {
