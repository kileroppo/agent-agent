...
import { getM5RoutineExecutionContract } from './routine-execution-contract.ts';
import { healthyM5StageWorkProducts, m5StageWorkProductCandidates, } from './stage-recovery-controller.ts';
import { safeId } from './content-campaign-primitives.ts';
import { replayM5StageWorkProduct, verifyPublishReceiptArtifact, m5WorkProductDrift, renderPlatformKey, confirmedM5ProviderReceipt, confirmedM5VoiceVariant, assertCompleteM5GrayVoiceVariants, validM5SocialCardPackageReceipt, m5HermesRouteExecution, m5HermesStrategy, m5HermesStageToolIds, executeM5Route, routeCaseInput, routeOutputHashes, } from './content-campaign-execution-support.ts';
const CONTENT_AUTONOMY_PLUGIN_KEY = 'agent-army.content-autonomy';
type CampaignMethod = (this: Record<string, any>, ...args: any[]) => any;
export const campaignExecutionRouteMethods: Record<string, CampaignMethod> = {
    async executeHermesStage({ assignment, task }: any = {}) {
        ...
        if (contract.executionTool?.kind !== 'agent_army_mcp'
            || contract.executionTool.id !== 'm5_stage_execute'
            || (!contract.pluginEntryTool && !contract.deterministicEntry)) {
            throw new ContentCampaignError(`M5 阶段 ${contract.stageKey} 必须调用 ${contract.executionTool?.id || '专用执行器'}，不能改走插件通用入口。`);
        }
        const targetCaseId = safeId(task?.input?.context?.pipelineCaseId, '当前 M5 指派缺少可信 Pipeline Case。');
        ...
        for (const item of chain) {
            ...
            try {
                ...
                throw new ContentCampaignError(`M5 ${contract.stageKey} 无法读取 Case ${item.id} 的 Work Product，已在工具调用前停止。`);
            }
            if (item.id === targetCaseId)
                targetOutputs = itemOutputs;
        ...
        if (stageCandidates.length > 1) {
            throw new ContentCampaignError(`M5 ${contract.stageKey} 阶段存在多个 Work Product 候选或未解决漂移，拒绝重放时自动选择。`);
        }
        ...
        if (contract.stageKey === 'parallel_image_generation') {
            ...
            const rawReceipt: any = receipt;
            const data = rawReceipt?.data && typeof rawReceipt.data === 'object' ? rawReceipt.data : rawReceipt;
        ...
        if (contract.stageKey === 'voice' && Array.isArray(parameters.voices)) {
            const receipts = await executeM5Route(routeExecution, () => Promise.all(parameters.voices.map(async (voiceParameters: any) => {
            ...
            const variants = Object.fromEntries(receipts.map((item: any) => [
                item.variantKey,
                item,
            ]));
        ...
        if (contract.stageKey === 'render') {
            const { receipts, socialCardPackage } = await executeM5Route(routeExecution, async () => {
                const receipts = await Promise.all(parameters.renders.map(async (render: any) => {
                    ...
                    if (propsWrite?.propsPath !== render.propsPath
                        || !/^sha256:[0-9a-f]{64}$/i.test(String(propsWrite?.checksum || ''))) {
                        throw new ContentCampaignError(`${render.composition} props 写入后没有返回同一路径和真实哈希，渲染未启动。`);
                    }
                    ...
                    if (receipt?.composition !== render.composition
                ...
                const socialCardPackage = rawSocialCards?.data && typeof rawSocialCards.data === 'object'
                    ? rawSocialCards.data
                    : rawSocialCards;
                if (!validM5SocialCardPackageReceipt(socialCardPackage)
            ...
            const outputs = Object.fromEntries(receipts.map((receipt: any) => [
