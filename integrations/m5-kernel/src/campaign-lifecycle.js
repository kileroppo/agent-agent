...
export class CampaignLifecycle {
  constructor({ controlPlane, definition, activePipelineId = null, activePipelineKey = null, now }) {
  ...
  async activateScheduledDayLocked() {
    ...
    const activeParents = cases.filter((item) => !item.parentCaseId && item.campaignGrant?.status === 'active');
  ...
  async approvalReadiness(item) {
    ...
    try {
      ...
      return { allowed:false, code:'preflight_failed', reason:safeText(error?.message, 500) || 'M5 启动前检查未通过。' };
    }
  }
  ...
  async assertActivePipeline(item) {
    const pipeline = await this.pipeline();
  ...
  async transition(item, toStageKey, reason) {
  ...
  async restoreExecutionCasesToDraft(cases) {
    for (const item of cases) {
      if (!item || item.stageKey !== 'cancelled') continue;
  ...
  async assertActivationAllowed(item, campaignGrant) {
    const pipeline = await this.pipeline();
    const cases = caseRows(await this.controlPlane.listPipelineCases(pipeline.id));
    const conflict = cases.find((entry) =>
      entry.id !== item.id && !entry.parentCaseId && entry.campaignGrant?.status === 'active',
    );
    if (conflict) {
    ...
    if (!companyId || !projectId) throw new ContentCampaignError('无法核验 Paperclip 公司或项目预算，活动保持未启动。');
    ...
    const policy = asList(overview?.policies).find((entry) =>
  ...
  async assertExecutionReady(pipeline) {
    ...
    for (const spec of routineSpecs) {
      ...
      const matches = routines.filter((item) => item.projectId === pipeline.projectId && String(item.description || '').includes(marker));
      if (matches.length === 0) {
      ...
      if (routine.status !== 'active') failures.push(`Routine ${spec.key} 当前状态为 ${routine.status || 'unknown'}，不是 active`);
    ...
    for (const stage of this.definition.stages.filter((item) => item.routineKey)) {
      ...
      if (!liveStage) failures.push(`Pipeline 缺少阶段 ${stage.key}`);
      else if (routine && liveStage.routineId !== routine.id) failures.push(`阶段 ${stage.key} 未绑定声明的 Routine ${stage.routineKey}`);
      else if (!contract) failures.push(`阶段 ${stage.key} 缺少唯一执行契约`);
    }
    if (failures.length) throw new ContentCampaignError(`M5 启动前检查未通过：${failures.join('；')}。`);
    try {
      return await this.controlPlane.readContentAutonomyApprovalSnapshot(contentPlugin);
    } catch (error) {
      throw new ContentCampaignError(`M5 无法锁定内容插件批准快照：${safeText(error?.message, 300) || 'unknown'}。`);
    }
  }
  ...
  async updateGrantWithRoutine(item, campaignGrant, routineEnabled) {
    ...
    try {
      ...
      if (routineState.changed) await this.setDailyRoutineEnabled(routineState.previousEnabled).catch(() => undefined);
      throw error;
    }
  }
  ...
  async setDailyRoutineEnabled(enabled) {
    ...
    if (previousEnabled === enabled) return { triggerId:trigger.id, previousEnabled, enabled, changed:false };
...
function agentInvocationBlockReason(agent) {
  ...
  if (!INVOKABLE_AGENT_STATUSES.has(agent.status)) return `Paperclip 状态为 ${agent.status || 'unknown'}`;
  ...
  if (adapterType === 'http') {
    ...
    try {
      if (!['http:', 'https:'].includes(new URL(url).protocol)) return 'HTTP adapter url 协议无效';
    } catch {
...
function hermesExecutionContractFailures(agent, contract) {
  const failures = [];
  if (agent?.roleId !== contract.agentId) failures.push(`Routine ${contract.routineKey} 必须绑定岗位 ${contract.agentId}`);
  ...
  if (!acceptedTaskTypes.includes(contract.taskType)) failures.push(`岗位 ${contract.agentId} 的 manifest 未声明任务类型 ${contract.taskType}`);
  ...
  for (const tool of ['paperclip_assignment_get', contract.completionTool]) {
    if (tool && !mcpTools.includes(tool)) failures.push(`岗位 ${contract.agentId} 的 MCP 工具契约缺少 ${tool}`);
  }
