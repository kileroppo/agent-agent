...
type RouteDescriptor = Readonly<{ strategy: string; toolIds: string[]; inputHash: string }>;
type RecoveryRecord = DynamicRecord | null;
type RouteExecutionInput = Readonly<{
  runId?: unknown;
  stageKey?: unknown;
  recovery?: RecoveryRecord;
  previousExecution?: DynamicRecord | null;
  strategy?: unknown;
  toolIds?: unknown;
  inputs?: unknown;
  now?: () => Date;
}>;
type ObservationDecisionInput = Readonly<{
  runId?: unknown;
  issueId?: unknown;
  observation?: DynamicRecord | null;
  action?: unknown;
  selectedToolId?: unknown;
  successCondition?: unknown;
  budget?: DynamicRecord | null;
  now?: () => Date;
}>;
...
export function routeExecutionDescriptor(value: unknown): RouteDescriptor | null {
  return normalizeRouteDescriptor(value);
}

export function routeDescriptorFingerprint(value: unknown): string | null {
  const descriptor = normalizeRouteDescriptor(value);
  if (!descriptor) return null;
  return sha256(stableJson(descriptor));
}

export function validM5RouteExecution(value: DynamicRecord | null | undefined): boolean {
...
export function assertChangedM5RecoveryRoute(
...
function normalizeDecisionBudget(value: DynamicRecord | null | undefined): Record<string, number> {
  const keys = [
    'stepsRemaining',
...
function routeChangedDimensions(previous: RouteDescriptor, current: RouteDescriptor): string[] {
...
function stableValue(value: unknown, depth = 0): unknown {
  ...
  return Object.fromEntries(
    Object.keys(record)
      ...
      .map((key) => [key, stableValue(record[key], depth + 1)]),
  );
}
