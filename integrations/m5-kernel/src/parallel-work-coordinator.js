...
export class M5ParallelWorkCoordinator {
  constructor({ controlPlane, pipelineId, maxConcurrency = 4 } = {}) {
  ...
  async reconcile(dayCaseId) {
    ...
    const refreshedBranches = await Promise.all(
      ingested.branches.map((item) => this.controlPlane.getCase(item.id)),
    );
    ...
    const verifiedBranches = allComplete
      ...
      : [];
    let joined = false;
    let join = await this.controlPlane.getCase(ingested.join.id);
    ...
    let dayCase = await this.controlPlane.getCase(day.id);
