...
export async function inspectContentAutonomyPluginReadiness({
  ...
  companyId: string;
  plugin: DynamicRecord;
  agents?: DynamicRecord[];
}>): Promise<string[]> {
...
function isSecretRef(value: unknown): value is DynamicRecord {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const record = value as DynamicRecord;
  if (record.type !== 'secret_ref' || !UUID.test(String(record.secretId || ''))) return false;
  return record.version === undefined
    || record.version === 'latest'
    || (Number.isInteger(record.version) && record.version > 0);
}

function validateBaseUrl(value: unknown, failures: string[]): void {
  try {
...
function validateVoices(value: unknown, failures: string[]): void {
...
function validateRates(value: unknown, failures: string[]): void {
  const record = value && typeof value === 'object' ? value as DynamicRecord : null;
  if (
    !value
    || typeof value !== 'object'
    || RATE_KEYS.some((key) => !Number.isFinite(Number(record?.[key])) || Number(record?.[key]) <= 0)
  ) {
    failures.push('StepFun 视觉、生图和 TTS 费率必须全部为正数；按负责人确认的真实费率补齐后重试');
  }
}

function validateRoleGrants(config: DynamicRecord, agents: DynamicRecord[], failures: string[]): void {
  ...
  for (const role of M5_CONTENT_ROLES) {
    ...
    const expected: readonly string[] = M5_ROLE_TOOL_BUNDLES[
      role as keyof typeof M5_ROLE_TOOL_BUNDLES
    ];
    if (
      !Array.isArray(actual)
      || actual.length !== expected.length
      || actual.some((tool: string) => !expected.includes(tool))
    ) {
...
function validateWorkspace(value: DynamicRecord | null | undefined, failures: string[]): void {
  if (
    !value
...
async function read(adapter: PaperclipAdapter, path: string): Promise<any> {
  return adapter.request('GET', path).catch(() => null);
}

function asList(value: unknown): DynamicRecord[] {
  if (Array.isArray(value)) return value as DynamicRecord[];
  const record = value && typeof value === 'object' ? value as DynamicRecord : null;
