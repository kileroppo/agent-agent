import { routeDescriptorFingerprint, routeExecutionDescriptor, validM5RouteExecution, } from './route-execution.ts';
...
function deriveM5StageRecoveryState({ assignment, contract, caseItem, contentCase, issue, runs, events, }: any = {}) {
    assertRecoveryInput({ assignment, contract });
    const caseContentRecovery = contentRecoveryValue(contentCase?.fields?.m5ContentRecovery || caseItem?.fields?.m5ContentRecovery);
    const rootStageRecovery = recoveryValue(caseContentRecovery?.stageRecoveries?.[stageRecoveryKey(caseItem?.id, contract.stageKey)], contract.stageKey);
    const caseRecovery = rootStageRecovery
        || recoveryValue(caseItem?.fields?.m5StageRecovery, contract.stageKey);
    const issueRecovery = recoveryValue(issue?.m5StageRecovery
        || issue?.metadata?.m5StageRecovery
        || issue?.executionPolicy?.m5StageRecovery, contract.stageKey);
    const eventRecoveries = listItems(events, 'events')
        .flatMap((event: any) => recoveryValuesFromEvent(event, contract.stageKey));
    const eventContentRecoveries = listItems(events, 'events')
        .flatMap((event: any) => contentRecoveryValuesFromEvent(event));
    const source = caseRecovery || issueRecovery || latestRecovery(eventRecoveries) || null;
    const history = uniqueHistory([
        ...(eventRecoveries.flatMap((item: any) => item.history || [])),
        ...(issueRecovery?.history || []),
        ...(caseRecovery?.history || []),
    ], contract.stageKey);
    const contentHistory = uniqueContentHistory([
        ...(eventContentRecoveries.flatMap((item: any) => item.history || [])),
        ...(caseContentRecovery?.history || []),
    ]);
    const failedRunIds = new Set(listItems(runs, 'runs')
        .filter((run: any) => FAILED_RUN_STATUSES.has(String(run?.status || run?.state || '').toLowerCase()))
        .map((run: any) => String(run?.id || run?.runId || '').trim())
        .filter(Boolean));
    const handled = history.some((item: any) => item.runId === assignment.runId);
    const fallbackAttempt = new Set([...failedRunIds, assignment.runId]).size;
    const stageAttempt = handled
        ? history.find((item: any) => item.runId === assignment.runId).stageAttempt
        : source
            ? nonNegativeInteger(source.stageAttempt) + 1
            : fallbackAttempt;
    const replanCount = Math.max(nonNegativeInteger(caseContentRecovery?.replanCount), ...eventContentRecoveries.map((item: any) => nonNegativeInteger(item.replanCount)), ...contentHistory.map((item: any) => nonNegativeInteger(item.replanCount)), nonNegativeInteger(source?.replanCount), ...history.map((item: any) => nonNegativeInteger(item.replanCount)));
    return {
...
function planM5StageFailureRecovery({ state, maxStageRetries = M5_STAGE_RECOVERY_LIMITS.maxStageRetries, maxReplansPerContent = M5_STAGE_RECOVERY_LIMITS.maxReplansPerContent, }: any = {}) {
...
function healthyM5StageWorkProducts(outputs: any, contract: any) {
...
function m5StageWorkProductCandidates(outputs: any, contract: any) {
    const expected = contract?.expectedWorkProduct;
    if (!expected)
        return [];
    const source = stageOutputValues(outputs);
    const normalized = normalizeStageOutputs(source);
    return source.filter((_: any, index: any) => {
        const item = normalized[index];
        if (item?.recordKind !== 'work_product' || item?.type !== 'artifact')
            return false;
...
function normalizeStageOutputs(outputs: any) {
    return stageOutputValues(outputs).map((item: any) => {
        if (item?.recordKind === 'work_product')
            return item;
        const metadata = item?.metadata;
        if (item?.kind !== 'work_product' || !metadata || typeof metadata !== 'object')
            return item;
...
function stageOutputValues(outputs: any) {
    return Array.isArray(outputs)
        ? outputs
        : Array.isArray(outputs?.items) ? outputs.items : [];
}
function validContentVersionWorkProduct(primary: any, nested: any) {
...
function validMachineReviewWorkProduct(primary: any, nested: any) {
    const value = primary || nested;
    return Boolean(value
        && typeof value === 'object'
        && !Array.isArray(value)
        ...
        && value.checks.every((check: any) => check?.passed === true));
}
function latestRecovery(items: any) {
    const values = Array.isArray(items) ? items : [];
    return values.reduce((latest: any, candidate: any) => {
        if (!latest)
            return candidate;
...
function uniqueRecoveryAction({ assignment, contract, replanCount }: any) {
    return {
...
function recoveryComment({ action, contract, assignment, stageAttempt, replanCount, detail, }: any) {
    return [
...
function recoveryValue(value: any, stageKey: any) {
    if (!value
        || typeof value !== 'object'
        || Array.isArray(value)
...
function recoveryValuesFromEvent(event: any, stageKey: any) {
    const found = [];
    const queue = [event];
    const seen = new Set();
    while (queue.length) {
...
function contentRecoveryValue(value: any) {
    if (!value
        || typeof value !== 'object'
        || Array.isArray(value)
    ...
    return {
        ...
        stageRecoveries: Object.fromEntries(Object.entries(value.stageRecoveries
            && typeof value.stageRecoveries === 'object'
            && !Array.isArray(value.stageRecoveries)
            ? value.stageRecoveries
            : {}).flatMap(([key, recovery]: any) => {
...
function stageRecoveryKey(caseId: any, stageKey: any) {
    const normalizedCaseId = String(caseId || '').trim();
    const normalizedStageKey = String(stageKey || '').trim();
    if (!/^[0-9a-f-]{36}$/i.test(normalizedCaseId)
...
function uniqueHistory(items: any, stageKey: any) {
    const byRun = new Map();
...
async function resolveM5ContentCase(governance: any, initialCase: any) {
    ...
    if (!campaignId
        ...
        throw new M5StageRecoveryError('M5 内容恢复必须从当前 Case 的 campaignId 和 scheduledDate 解析日期内容根。');
    }
    ...
    for (let depth = 0; depth < 8; depth += 1) {
        ...
        if (current.caseKey === expectedCaseKey
    ...
    throw new M5StageRecoveryError(`M5 当前 Case 无法回溯到日期内容根 ${expectedCaseKey}，拒绝使用分支级计数冒充整条内容计数。`);
}
function validPlanRevision(value: any) {
...
function validPlanRevisionConsumption(value: any) {
...
function uniqueContentHistory(items: any) {
    const byRun = new Map();
    for (const item of Array.isArray(items) ? items : []) {
        const runId = String(item?.runId || '').trim();
        const stageKey = String(item?.stageKey || '').trim();
        const action = String(item?.action || '').trim();
        if (!runId || !stageKey || !RECOVERY_ACTIONS.has(action))
            continue;
        byRun.set(runId, {
            runId,
            issueId: String(item.issueId || '').trim() || null,
...
function assertRecoveryInput({ assignment, contract }: any) {
...
function safeText(value: any, limit: any) {
