...
export function normalizeCampaignDraft(input: DynamicRecord, now: Date) {
  rejectSecrets(input);
...
export function requireCampaignGrant(item: DynamicRecord): DynamicRecord {
...
export function requireActiveCampaignGrant(item: DynamicRecord, now: Date): DynamicRecord {
...
export function safeCampaignGrantView(grant: DynamicRecord) {
  return {
...
export function samePluginApproval(left: DynamicRecord | null, right: DynamicRecord | null): boolean {
  return ['schemaVersion', 'pluginId', 'pluginKey', 'version', 'manifestHash', 'configHash']
    .every((key) => left?.[key] === right?.[key]);
}

export function campaignNextAction(status: unknown): string {
...
function rejectSecrets(input: unknown): void {
