...
import { assertM5ControlPlane, M5ControlPlane } from './control-plane.js';
import { inspectContentAutonomyPluginReadiness } from './paperclip-content-autonomy-preflight.js';
import { buildContentAutonomyApprovalSnapshot } from './paperclip-content-autonomy-snapshot.js';
...
export class PaperclipM5ControlPlane extends M5ControlPlane {
  constructor({ endpoint, definition, companyId = endpoint?.companyId } = {}) {
    super();
  ...
  listCaseOutputs(id) { return this.getCaseOutputs(id); }

  async listIssueRuns(id) {
    return list(await this.client.listIssueRuns(id)).map(normalizeRun).filter(Boolean);
  }

  async updateCampaignGrant(caseId, expectedVersion, campaignGrant) {
  ...
  async ingestCampaignDraft(pipeline, draft) {
  ...
  async ingestCampaignExecution(pipeline, campaign) {
  ...
  async verifyProviderAction({ operation, actionId, costEventId, runContext }) {
  ...
  async findCostActivity({ costEventId }) {
  ...
  async inspectExecutionReadiness({ pipeline, executionContracts }) {
  ...
  async getDailySchedule(pipeline) {
  ...
  async readContentAutonomyApprovalSnapshot(plugin = null) {
  ...
  countActiveParallelIssues(pipelineId) { return this.endpoint.countActiveParallelIssues(pipelineId); }
  runParallelRoutine(input) { return this.endpoint.runParallelRoutine({ ...input, branch:paperclipCase(input.branch) }); }
  linkCaseIssue(...args) { return this.endpoint.linkCaseIssue(...args); }
  completeParallelGateIssues(...args) { return this.endpoint.completeParallelGateIssues?.(...args); }
  async replaceCaseBlockers(...args) { return this.endpoint.replaceCaseBlockers(...args); }
  async ingestCase(pipelineId, value) { return normalizeCase(await this.endpoint.ingestCase(pipelineId, value)); }

  assertInterface() { return assertM5ControlPlane(this); }
}

export function normalizePaperclipCase(value) { return normalizeCase(value); }
export function normalizePaperclipWorkProduct(value) { return normalizeWorkProduct(value); }
...
function normalizeCaseTree(value) {
  if (Array.isArray(value)) return value.map(normalizeCase).filter(Boolean);
  const root = value?.case;
  if (!root) return [];
  const result = [];
  const queue = list(root.childGroups).flatMap((group) => list(group.cases));
  while (queue.length) {
    const current = queue.shift();
    const normalized = normalizeCase(current);
    if (normalized) result.push(normalized);
    queue.push(...list(current?.childGroups).flatMap((group) => list(group.cases)));
  }
  return result;
}

function normalizeWorkProduct(value) {
...
function normalizeEvent(raw) { return { id:raw?.id || null, type:raw?.type || raw?.action || null,
  createdAt:raw?.createdAt || null, summary:raw?.summary || raw?.message || null }; }
function normalizeCostActivity(raw) { return { id:raw.id, companyId:raw.companyId,
...
function normalizeBudgetPolicy(raw) { return { scopeType:raw.scopeType, scopeId:raw.scopeId,
...
function normalizePlugin(raw) { return { id:raw?.id || null,
...
function normalizePluginConfig(raw) { return raw?.configJson && typeof raw.configJson === 'object'
  ? structuredClone(raw.configJson) : null; }
function normalizeRoutine(raw) { return { id:raw?.id || null, projectId:raw?.projectId || null,
  description:String(raw?.description || ''), status:String(raw?.status || ''),
  assigneeAgentId:raw?.assigneeAgentId || null,
  schedules:list(raw?.triggers).filter((item) => item?.kind === 'schedule').map(normalizeSchedule) };
}
function normalizeSchedule(raw) { return { id:raw?.id || null, enabled:raw?.enabled === true,
  kind:String(raw?.kind || '') }; }
function normalizeAgent(raw) { return { id:raw?.id || null, status:String(raw?.status || ''),
...
function normalizeIssueLink(raw) { return { issueId:raw?.issue?.id || raw?.issueId || null,
  status:raw?.issue?.status || raw?.status || null, title:raw?.issue?.title || '',
  description:raw?.issue?.description || '' };
}
function rawCase(value) { return value?.case ?? value; }
function list(value) { return Array.isArray(value) ? value : Array.isArray(value?.items)
