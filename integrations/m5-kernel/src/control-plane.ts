...
export function assertM5ControlPlane<T>(value: T): T {
  const candidate = value && typeof value === 'object' ? value as Record<string, unknown> : {};
  const missing = M5_CONTROL_PLANE_METHODS.filter((name) => typeof candidate[name] !== 'function');
  if (missing.length) {
...
export function createFakeM5ControlPlane(overrides: Record<string, unknown> = {}): Record<string, unknown> {
