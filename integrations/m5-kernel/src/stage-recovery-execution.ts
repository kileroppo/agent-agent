...
const { limits: M5_STAGE_RECOVERY_LIMITS, Error: M5StageRecoveryError, records: { comment: recoveryComment }, input: { assert: assertRecoveryInput, positiveLimit, }, content: { resolve: resolveM5ContentCase }, collections: { listItems }, } = stageRecoveryState;
class M5StageRecoveryController {
    readonly governance: Record<string, any>;
    readonly maxStageRetries: number;
    readonly maxReplansPerContent: number;
    readonly workProductValidator: ((...args: any[]) => any) | null;
    readonly now: () => Date;

    constructor({ governance, maxStageRetries = M5_STAGE_RECOVERY_LIMITS.maxStageRetries, maxReplansPerContent = M5_STAGE_RECOVERY_LIMITS.maxReplansPerContent, workProductValidator = null, now = () => new Date(), }: any = {}) {
    ...
    async handleFailure(input: any = {}) {
        ...
        const decision = await new M5StageRecoveryLedger({
            ...
            now: this.now,
            snapshotLoader: (assignment: any) => this.snapshot(assignment),
        }).recordFailure(input);
    ...
    async applyIssueDecision({ assignment, contract, decision, replayed }: any) {
