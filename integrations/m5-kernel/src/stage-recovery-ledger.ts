...
import { assertChangedM5RecoveryRoute, createM5RouteExecution, routeDescriptorFingerprint, routeExecutionDescriptor, validM5RouteExecution, } from './route-execution.ts';
import { stageRecoveryState } from './stage-recovery-state.ts';
const { schemas: { stage: RECOVERY_SCHEMA, content: CONTENT_RECOVERY_SCHEMA, }, limits: M5_STAGE_RECOVERY_LIMITS, Error: M5StageRecoveryError, derive: deriveM5StageRecoveryState, plan: planM5StageFailureRecovery, workProducts: { healthy: healthyM5StageWorkProducts, candidates: m5StageWorkProductCandidates, }, records: { contentValue: contentRecoveryValue, key: stageRecoveryKey, uniqueAction: uniqueRecoveryAction, }, revision: { valid: validPlanRevision }, input: { assert: assertRecoveryInput, positiveLimit, safeText, }, content: { resolve: resolveM5ContentCase }, collections: { listItems }, conflict: isVersionConflict, } = stageRecoveryState;
export class M5StageRecoveryLedger {
    ...
    #now;
    #snapshotLoader;
    constructor({ governance, maxStageRetries = M5_STAGE_RECOVERY_LIMITS.maxStageRetries, maxReplansPerContent = M5_STAGE_RECOVERY_LIMITS.maxReplansPerContent, workProductValidator = null, now = () => new Date(), snapshotLoader = null, }: any = {}) {
    ...
    async recordFailure({ assignment, contract, task = null, summary, routeExecution = null }: any = {}) {
        ...
        for (let conflictAttempt = 0; conflictAttempt < 5; conflictAttempt += 1) {
            ...
            const handled = [...state.history, ...state.contentHistory]
                .find((item: any) => item.runId === assignment.runId);
            if (handled)
            ...
            try {
                await this.#governance.patchPipelineCaseFields(snapshot.contentCase.id, {
                ...
                if (conflictAttempt < 4 && isVersionConflict(error))
                    continue;
                throw error;
            }
        }
    ...
    async getActivePlanRevision({ pipelineCaseId, stageKey, pipelineCase = null, }: any = {}) {
        if (typeof this.#governance?.getPipelineCase !== 'function'
        ...
        if (currentCase?.id !== pipelineCaseId)
            return null;
    ...
    async consumeSystemPlanRevision({ pipelineCaseId, stageKey, runId, routeSummary, }: any = {}) {
        if (typeof this.#governance?.getPipelineCase !== 'function'
            || !validCaseId(pipelineCaseId)
            || !validStageKey(stageKey)
            || !String(runId || '').trim()
        ...
        for (let conflictAttempt = 0; conflictAttempt < 5; conflictAttempt += 1) {
            ...
            const existing = recovery.planRevisionConsumptions.find((item: any) => item.revisionId === revision.revisionId && item.runId === runId);
            if (existing)
    ...
    async #loadFailureSnapshot(assignment: any) {
        if (this.#snapshotLoader)
            return this.#snapshotLoader(assignment);
    ...
    async #resolveExistingProduct({ assignment, contract, task, snapshot }: any) {
        ...
        if (stageCandidates.length > 1) {
            throw new M5StageRecoveryError(`M5 ${contract.stageKey} 阶段存在多个 Work Product 候选或未解决漂移，拒绝自动选择。`);
        }
        if (stageCandidates.length === 1 && existingProducts.length !== 1) {
            throw new M5StageRecoveryError(`M5 ${contract.stageKey} Work Product 候选结构、Provider 或状态漂移；禁止自动恢复或覆盖。`);
        }
        if (existingProducts.length === 0)
            return null;
        if (!this.#workProductValidator) {
            throw new M5StageRecoveryError(`M5 ${contract.stageKey} 已有 Work Product，但完整漂移校验器不可用，禁止自动恢复。`);
        }
        try {
            ...
            throw new M5StageRecoveryError(`M5 ${contract.stageKey} Work Product 漂移：${(error as any)?.message || '完整校验失败'}；禁止自动恢复或覆盖。`);
        }
        return existingProducts[0];
    }
    #createFailureDecision({ assignment, contract, snapshot, state, summary, routeExecution, }: any) {
        ...
        const recoveryAction = decision.action === 'blocked'
            ...
            : null;
        const previousContentRecovery = contentRecoveryValue(snapshot.contentCase.fields?.m5ContentRecovery);
        return {
            runId: assignment.runId,
    ...
    #failureFields({ assignment, contract, snapshot, state, recordedDecision, summary, }: any) {
        const previousContentRecovery = contentRecoveryValue(snapshot.contentCase.fields?.m5ContentRecovery);
    ...
    #createPlanRevisionReceipt({ caseItem, contentCase, recovery, revision, stageKey, runId, routeSummary, }: any) {
        const previousExecution = [...recovery.planRevisionConsumptions]
            .reverse()
            .find((item: any) => item.revisionId === revision.revisionId)?.routeExecution || null;
    ...
    #assertFailureGovernance() {
        ...
        const missing = required.filter((name: any) => typeof this.#governance?.[name] !== 'function');
        if (missing.length) {
            throw new M5StageRecoveryError(`M5 阶段恢复缺少 Paperclip 原生能力：${missing.join('、')}。`);
        }
    }
}
function activeRevisionFor({ recovery, pipelineCaseId, stageKey }: any) {
    const revision = recovery?.activePlanRevision;
    if (!validPlanRevision(revision)
        || revision.failedCaseId !== pipelineCaseId
        || revision.nextRoute?.stageKey !== stageKey)
        return null;
    return structuredClone(revision);
}
function createPlanRevision({ assignment, contract, contentCase, failedCase, replanCount, summary, occurredAt, routeExecution, }: any) {
...
function latestSystemRouteExecution(contentRecovery: any) {
    return [...(contentRecovery?.planRevisionConsumptions || [])]
        .reverse()
        .map((item: any) => item?.routeExecution)
        .find(validM5RouteExecution) || null;
}
function defaultRejectedRouteExecution({ contract, contentCase, failedCase }: any) {
...
function systemControllerRouteInputs(targetCase: any, contentCase: any) {
    return {
        targetCase: systemControllerCaseInput(targetCase),
        contentCase: systemControllerCaseInput(contentCase),
    };
}
function systemControllerCaseInput(caseItem: any) {
    const fields = caseItem?.fields && typeof caseItem.fields === 'object'
        ? Object.fromEntries(Object.entries(caseItem.fields)
            .filter(([key]: any) => key !== 'm5ContentRecovery')
...
function withoutInternalDecisionFields(decision: any) {
    const { nextStageAttempt: _, ...persisted } = decision;
    return persisted;
}
function hasContentRootFields(caseItem: any) {
    return Boolean(String(caseItem?.fields?.campaignId || '').trim()
        && /^\d{4}-\d{2}-\d{2}$/.test(String(caseItem?.fields?.scheduledDate || '')));
}
function validCaseId(value: any) {
    return /^[0-9a-f-]{36}$/i.test(String(value || ''));
}
function validStageKey(value: any) {
