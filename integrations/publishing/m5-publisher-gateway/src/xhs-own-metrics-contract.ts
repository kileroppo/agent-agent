...
export const XHS_OWN_METRIC_CONTEXT_SCHEMA = 'agent.army/xhs-own-metric-context/v1';
export const XHS_OWN_METRIC_OBSERVATION_SCHEMA = 'agent.army/xhs-own-metric-observation/v1';
...
export function xhsOwnMetricCollectionKey(receiptId: any, checkpoint: any) {
    if (!RECEIPT_ID.test(String(receiptId || '')) || !CHECKPOINTS.has(checkpoint)) {
        throw coded('xhs_metric_collection_identity_invalid', '小红书本人指标只能使用有效 receiptId 与 2h、24h 或 72h 检查点。');
    }
    return `${receiptId}:${checkpoint}`;
}
export function normalizeXhsOwnMetricObservation({ trustedReceipt, checkpoint, collectionKey, collectedAt, trustedContext, observation, }: any = {}) {
    const receipt = validateReceipt(trustedReceipt);
    const expectedCollectionKey = xhsOwnMetricCollectionKey(receipt.receiptId, checkpoint);
    if (collectionKey !== expectedCollectionKey) {
        throw coded('xhs_metric_collection_identity_invalid', '小红书本人指标 collectionKey 必须精确绑定同一 receiptId 与检查点。');
    }
    const normalizedCollectedAt = timestamp(collectedAt, 'xhs_metric_collection_time_invalid');
    ...
    if (observationAgeMs < 0
        || observationAgeMs > MAX_OBSERVATION_AGE_MS) {
        throw coded('xhs_metric_observation_invalid', '小红书本人指标观察时间必须位于本次采集前五分钟内。');
    }
    const output: Record<string, any> = {
        ...
        source: {
            ...
            capturedAt: timestamp(observed.capturedAt, 'xhs_metric_observation_invalid'),
            rawMetrics: structuredClone(observed.metrics),
        },
    };
    return deepFreeze(output);
}
function validateReceipt(value: any) {
    if (!value
        ...
        throw coded('xhs_metric_receipt_invalid', '小红书本人指标必须绑定结构完整的可信 PublishReceipt。');
    }
    return {
...
function validateTrustedContext(value: any, receipt: any) {
    if (!sameKeys(value, CONTEXT_KEYS)
        ...
        throw coded('xhs_metric_trusted_context_invalid', '小红书本人指标缺少 Paperclip 背书的官方页面、账号和 selector 身份。');
    }
    return value;
}
function validateObservation(value: any) {
    if (!sameKeys(value, OBSERVATION_KEYS)
        ...
        throw coded('xhs_metric_observation_invalid', '小红书本人指标观察结果结构无效、包含额外字段或不来自获批官方页面。');
    }
    timestamp(value.capturedAt, 'xhs_metric_observation_invalid');
    return value;
}
function assertObservationIdentity(observation: any, context: any, receipt: any) {
    if (observation.origin !== context.origin
        ...
        throw coded('xhs_metric_observation_identity_mismatch', '小红书本人指标观察结果与回执账号、笔记或获批 selector 身份不一致。');
    }
}
function normalizeMetrics(metrics: any) {
    return Object.fromEntries(METRIC_KEYS.map((key: any) => [key, exactNonNegativeInteger(metrics[key])]));
}
function exactNonNegativeInteger(value: any) {
    ...
    if (!canonicalDigits && !groupedDigits)
        throw metricValueError();
...
function metricValueError() {
    return coded('xhs_metric_value_not_exact', '小红书本人指标必须是精确非负整数；缩写、区间、小数、空值或估算值不得写入快照。');
}
function timestamp(value: any, code: any) {
    if (typeof value !== 'string') {
...
function sameKeys(value: any, expected: any) {
