...
export class WorkspaceArtifactVerifier {
    ...
    constructor(rootPath: any) {
        if (!path.isAbsolute(rootPath))
            throw coded('invalid_workspace_root', '发布工作区必须使用绝对路径。');
        this.rootPath = rootPath;
    }
    async verify(relativePath: any, expectedChecksum: any) {
    ...
    async acquire(relativePath: any, expectedChecksum: any) {
        ...
        if (!absolute.startsWith(`${root}${path.sep}`))
            throw coded('media_path_escape', '发布文件逃逸了受控工作区。');
        ...
        try {
            ...
            if (!sourceStat.isFile())
                throw coded('media_not_regular_file', '发布文件必须是普通文件。');
            snapshotHandle = await fs.open(snapshotPath, constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL, 0o400);
            ...
            return {
                ...
                createReadStream() {
                    if (released)
                        throw coded('media_lease_released', '发布文件私有快照已经释放。');
...
async function writeAll(handle: any, chunk: any) {
    let offset = 0;
    while (offset < chunk.length) {
        const { bytesWritten } = await handle.write(chunk, offset, chunk.length - offset, null);
        if (bytesWritten === 0)
            throw coded('media_snapshot_write_failed', '发布文件私有快照写入失败。');
        offset += bytesWritten;
    }
}
function publicLeaseMetadata(lease: any) {
    return {
...
function safeRelativePath(value: any) {
