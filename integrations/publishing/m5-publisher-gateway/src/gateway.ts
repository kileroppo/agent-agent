import { coded, publishIdempotencyKey } from './policy.ts';
import { PublisherCostRecorder, createFakePublisherCostReporter, } from './cost-reporting.ts';
import { MetricCollectionExecution } from './metric-collection-execution.ts';
import { MetricInvocationRecovery } from './metric-invocation-recovery.ts';
import { PublishExecution } from './publish-execution.ts';
const PRODUCTION_GATEWAY_ACTIVATION = Symbol('production-gateway-activation');
export function createProductionPublisherGateway(options: any) {
...
export class PublisherGateway {
    ...
    connectors: any;
    costRecorder: any;
    metricCollectionExecution: any;
    metricConnectors: any;
    metricInvocationRecovery: any;
    mode: any;
    paperclipControl: any;
    processHardStop: any;
    publishExecution: any;
    repository: any;
    constructor({ repository, connectors, metricConnectors = null, artifactVerifier, paperclipControl, costReporter, costRecorder, approvalGuard = null, mode = 'fake', productionActivation = null, clock = () => new Date(), }: any) {
        ...
        if (!paperclipControl?.assertPublishAllowed || !paperclipControl?.pauseCampaignAndDisableCron) {
            throw coded('paperclip_control_required', 'Publisher Gateway 必须连接 Paperclip 控制适配器；活动授权真相和 Cron 控制不能保存在发布器内。');
        }
        if (mode === 'real'
            && typeof approvalGuard?.assertCapabilityAllowed !== 'function') {
            throw coded('publisher_approval_guard_required', '真实 Publisher Gateway 必须在每次外部调用前重新核验 connector capability 批准。');
        }
        ...
        this.clock = clock;
        if (costRecorder) {
            if (typeof costRecorder.assertCampaignBudget !== 'function'
                || typeof costRecorder.recordLocalZeroAttempt !== 'function') {
                throw coded('publisher_cost_reporter_required', 'Publisher 费用记录器契约无效。');
            }
            ...
            this.costRecorder = new PublisherCostRecorder({
                repository,
                costReporter: costReporter || (mode === 'fake' ? createFakePublisherCostReporter() : null),
                clock,
            });
        }
        ...
        this.publishExecution = new PublishExecution({
            ...
            assertConnectorApproval: (platform: any, capability: any, checkedAt: any) => this.assertConnectorApproval(platform, capability, checkedAt),
            pauseInPaperclip: (input: any) => this.pauseInPaperclip(input),
        });
        this.metricCollectionExecution = new MetricCollectionExecution({
            ...
            assertConnectorApproval: (platform: any, capability: any, checkedAt: any) => this.assertConnectorApproval(platform, capability, checkedAt),
            pauseInPaperclip: (input: any) => this.pauseInPaperclip(input),
            activateGlobalHardStop: (input: any) => this.activateGlobalHardStop(input),
        });
    ...
    async collectMetricSnapshot(input: any = {}) {
    ...
    async getReceipt(identifier: any) {
        ...
        return receipts.find((receipt: any) => (receipt.receiptId === identifier || receipt.idempotencyKey === identifier)) || null;
    }
    async getAttempt(idempotencyKey: any) {
        return (await this.repository.read()).attempts[idempotencyKey] || null;
    }
    async reconcileMetricInvocation(input: any = {}) {
    ...
    async assertConnectorApproval(platform: any, capability: any, checkedAt: any) {
        if (this.mode !== 'real')
            return;
    ...
    async pauseInPaperclip({ idempotencyKey, campaignId, reason, now }: any) {
        ...
        try {
            ...
            throw coded('paperclip_pause_failed_hard_stop', 'Paperclip 未确认 CampaignGrant 暂停和 Cron 关闭；Publisher Gateway 已全局硬停，禁止继续发布。');
        }
    }
    async activateGlobalHardStop(latch: any) {
...
function validateConnectorModes(connectors: any, mode: any) {
    const connectorModes = Object.values(connectors || {})
        .map((connector: any) => String(connector?.connectorMode || ''));
    if ((mode === 'real' && connectorModes.some((value: any) => !value.startsWith('real:')))
        || (mode === 'fake' && connectorModes.some((value: any) => value.startsWith('real:')))) {
        throw coded('publisher_connector_mode_mismatch', 'fake 与 real connector 不得在同一个 Publisher Gateway 中混用。');
    }
}
