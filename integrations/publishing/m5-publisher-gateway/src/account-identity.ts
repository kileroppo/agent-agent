...
export const PUBLISHER_ACCOUNT_IDENTITY_VERIFIER_SCHEMA = 'agent.army/publisher-account-identity-verifier/v1';
const HASH = /^sha256:[0-9a-f]{64}$/;
const REFERENCE = /^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,255}$/;
export function validateAccountIdentityVerifier(verifier: any) {
    if (verifier?.contract?.schemaVersion !== PUBLISHER_ACCOUNT_IDENTITY_VERIFIER_SCHEMA
        || verifier?.contract?.deterministic !== true
        || verifier?.contract?.source !== 'paperclip'
        || typeof verifier?.verify !== 'function') {
        throw coded('publisher_account_identity_verifier_required', '抖音官方 connector 必须注入由 Paperclip 背书的确定性账号身份核验器。');
    }
    return verifier;
}
export async function verifyDouyinAccountIdentity({ verifier, accountRef, openId, }: any = {}) {
...
function validVerificationResult(result: any, request: any) {
    ...
    if (Object.keys(result).some((key: any) => !allowed.has(key)))
        return false;
...
function accountMismatch() {
    return coded('account_mismatch', 'Paperclip 无法确认抖音 accountRef 与当前授权身份一致；平台 HTTP 未调用。');
}
