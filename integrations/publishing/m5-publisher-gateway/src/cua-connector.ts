import { renderBoundedBrowserPolicy } from './cua-policy.ts';
import { coded, STOP_REASONS } from './policy.ts';
export const CUA_RUNNER_SCHEMA = 'agent.army/cua-publisher-runner/v1';
export const CUA_PLATFORM_ORIGINS: Readonly<Record<string, string>> = Object.freeze({
...
const EXPECTED_PAGE_STATES: Readonly<Record<string, readonly string[]>> = Object.freeze({
...
export class CuaPlatformConnector {
    ...
    platform: any;
    runner: any;
    constructor({ platform, runner, enabled = false }: any) {
    ...
    async publish(request: any) {
        ...
        try {
            ...
            if (initial.stopReason)
                return stopped(origin, initial.stopReason);
            let finalObservation: any = null;
            for (const step of buildSteps(request)) {
                ...
                const inspected = inspectObservation(observation, origin, EXPECTED_PAGE_STATES[step.action]);
                if (inspected.stopReason)
                    return stopped(origin, inspected.stopReason);
    ...
    async readOwnMetrics() {
        throw coded('cua_metrics_disabled', `${this.platform} CUA 指标读取尚未实现和批准。`);
    }
}
export function buildPlatformCuaSessionPolicy({ platform, readableDirectory, profileMode = 'isolated_new', profileName = null, }: any) {
    const origin = CUA_PLATFORM_ORIGINS[platform];
    if (!origin)
        throw coded('unsupported_cua_platform', 'CUA 发布策略只支持抖音和小红书。');
...
function validateRunner(runner: any) {
    ...
    const actionsMatch = Array.isArray(contract?.allowedActions)
        ...
        && contract.allowedActions.every((action: any, index: any) => action === CUA_PUBLISH_ACTIONS[index]);
    if (contract?.schemaVersion !== CUA_RUNNER_SCHEMA
        ...
        throw coded('cua_runner_contract_mismatch', 'CUA runner 必须使用获批的命名隔离 Profile、版本化 selector bundle、无任意桌面权限且只能执行固定发布动作。');
    }
}
function validateRequest(request: any) {
    if (!request?.accountRef
        ...
        throw coded('invalid_cua_publish_request', 'CUA 发布需要账号引用、标题、正文、标签和审核哈希绑定的媒体 lease。');
    }
}
function buildSteps(request: any) {
    return [
        {
            action: 'upload_media',
            input: {
...
function inspectObservation(observation: any, expectedOrigin: any, allowedStates: any) {
...
function validPublishedResult(observation: any, expectedOrigin: any) {
    if (typeof observation?.externalContentId !== 'string'
...
function stopped(origin: any, stopReason: any) {
    return {
        state: 'stopped',
        stopReason,
        evidence: `${origin}/`,
