...
export class FilePublisherRepository {
    ...
    retryIntervalMs: any;
    sleep: any;
    staleLockSafetyMs: any;
    constructor(filePath: any, { clock = () => new Date(), hostname = os.hostname(), pid = process.pid, isProcessAlive = processIsAlive, nonceFactory = () => crypto.randomUUID(), sleep = (milliseconds: any) => new Promise((resolve) => setTimeout(resolve, milliseconds)), staleLockSafetyMs = DEFAULT_LOCK_OPTIONS.staleLockSafetyMs, retryIntervalMs = DEFAULT_LOCK_OPTIONS.retryIntervalMs, maxWaitMs = DEFAULT_LOCK_OPTIONS.maxWaitMs, }: any = {}) {
        if (!path.isAbsolute(filePath))
            throw new Error('发布账本必须使用明确的绝对路径。');
        if (!Number.isInteger(pid) || pid <= 0)
            throw new Error('发布账本锁 PID 必须是正整数。');
        if (typeof hostname !== 'string' || !hostname.trim())
            throw new Error('发布账本锁 host 不能为空。');
    ...
    async acquireLock() {
        ...
        while (true) {
            ...
            try {
                ...
                if (error?.code?.startsWith?.('publisher_ledger_lock_'))
                    throw error;
            ...
            if (elapsed >= this.maxWaitMs) {
                throw lockError('publisher_ledger_lock_timeout', '发布账本仍被其他进程占用或锁处于安全期内；已在有界等待后停止，未抢占该锁。', this.lockPath);
            }
    ...
    async tryCreateLock(owner: any) {
        ...
        try {
            ...
            await fs.unlink(temporary).catch((error: any) => {
                if (error?.code !== 'ENOENT')
                    throw error;
            });
        }
    }
    async inspectLock(lockPath: any = this.lockPath) {
        let before;
        try {
    ...
    lockDisposition(owner: any) {
        ...
        if (alive || age < this.staleLockSafetyMs)
            return 'wait';
        return 'recoverable';
    }
    async isolateStaleLock(observed: any) {
    ...
    async releaseLock(owner: any) {
        ...
        try {
            ...
            if (!released || !sameObservedLock(observed, released) || released.owner.nonce !== owner.nonce) {
    ...
    async restoreMovedLock(movedPath: any) {
        try {
            await fs.link(movedPath, this.lockPath);
            await fs.unlink(movedPath);
        }
        catch (error: any) {
            throw lockError('publisher_ledger_lock_restore_failed', '锁竞争期间检测到所有权变化，无法安全还原被隔离文件，操作已硬停。', this.lockPath, error);
        }
    }
}
function parseLockOwner(raw: any, lockPath: any) {
    let value;
    try {
...
function isCanonicalTimestamp(value: any) {
    const parsed = new Date(value);
    return !Number.isNaN(parsed.getTime()) && parsed.toISOString() === value;
}
function sameObservedLock(left: any, right: any) {
    return Boolean(left
        && right
        && left.dev === right.dev
        && left.ino === right.ino
...
function processIsAlive(pid: any) {
    try {
        process.kill(pid, 0);
        return true;
    }
...
function lockError(code: any, message: any, lockPath: any, cause: any = undefined) {
    const error = new Error(message, cause ? { cause } : undefined) as Error & Record<string, any>;
    error.code = code;
