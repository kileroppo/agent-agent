...
import { PublisherCostRecorder } from './cost-reporting.js';
import { coded } from './policy.js';
import { WECHAT_DRAFT_PLATFORM } from './wechat-wenyan-connector.js';
...
export function wechatDraftIdempotencyKey(request) {
  return [
...
export class WechatDraftGateway {
  ...
  async createDraftOnce(request, idempotencyKey) {
    ...
    try {
      ...
      const claim = await this.repository.update((state) => {
        ...
        if (state.attempts[idempotencyKey]) return { kind:'ambiguous' };
      ...
      if (result?.state !== 'draft_created'
        ...
        await this.failAmbiguous(
          request,
          idempotencyKey,
          coded('wechat_draft_result_unverified', '公众号草稿回执无法核验。'),
        );
      }
      ...
      try {
        ...
        await this.failAmbiguous(
          request,
          idempotencyKey,
          coded('wechat_draft_receipt_commit_failed', '公众号草稿外部成功，但本地回执未能安全落账。'),
        );
      }
  ...
  async assertAuthorized(request, checkedAt) {
    let result;
    try {
  ...
  async blockAttempt(idempotencyKey, reason) {
    await this.repository.update((state) => {
  ...
  async failAmbiguous(request, idempotencyKey, error) {
    await this.repository.update((state) => {
      const attempt = state.attempts[idempotencyKey];
      if (!attempt) return;
  ...
  async pauseBeforeExternal(request, reason, checkedAt) {
    try {
      const pause = await this.paperclipControl.pauseCampaignAndDisableCron({
        campaignId:request.campaignId,
        reason,
...
function containsForbiddenRequestField(value) {
  return value && typeof value === 'object' && !Array.isArray(value)
    ? Object.keys(value).some((key) => FORBIDDEN_REQUEST_FIELDS.has(key))
    : false;
}
...
function draftReceiptId(idempotencyKey) {
