...
import { CUA_SELECTOR_BUNDLE_SCHEMA, selectorBundleChecksum, validateApprovedProfileLease, } from './cua-trust-contracts.ts';
import { XHS_OWN_METRIC_CONTEXT_SCHEMA, XHS_OWN_METRIC_OBSERVATION_SCHEMA, XHS_OWN_METRIC_PAGE_KIND, normalizeXhsOwnMetricObservation, xhsOwnMetricCollectionKey, } from './xhs-own-metrics-contract.ts';
import { coded } from './policy.ts';
export const XHS_OWN_METRICS_CUA_RUNNER_SCHEMA = 'agent.army/xhs-own-metrics-cua-runner/v1';
...
const PAGE_STATES: Readonly<Record<string, string>> = Object.freeze({
...
const SECRET_LIKE_KEY = /(?:api.?key|authorization|bearer|cookie|credential|login.?state|password|secret|storage|token)/i;
export class XhsOwnMetricsCuaConnector {
    ...
    runner: any;
    selectorBundle: any;
    constructor({ runner, selectorBundle, profileLease, enabled = false, clock = () => new Date(), }: any = {}) {
    ...
    async readOwnMetrics(trustedReceipt: any, collectedAt: any, context: any = {}) {
        return this.collect({
            trustedReceipt,
            collectedAt,
    ...
    async collect(input: any = {}) {
        if (!this.enabled) {
            throw coded('xhs_metrics_cua_connector_disabled', '小红书本人指标 CUA connector 默认关闭，尚未获得单独启用批准。');
        }
        ...
        if (request.trustedReceipt.accountRef !== trust.profile.accountRef) {
            throw coded('xhs_metrics_cua_account_mismatch', '小红书本人指标回执账号与获批命名 Profile 不一致。');
        }
        ...
        try {
            inspectBaseObservation(session.observation, trust.selector.origin, 'ready');
    ...
    validateTrust(accountRef: any = this.profileLease?.accountRef) {
        const checkedAt = normalizeClock(this.clock);
        const checkedClock = () => checkedAt;
        const selector = validateMetricSelectorBundle(this.selectorBundle, checkedClock);
...
function validateCollectionRequest(input: any) {
    ...
    if (!receipt
        ...
        throw coded('xhs_metrics_cua_request_invalid', '小红书本人指标 CUA 只能读取结构完整的可信 PublishReceipt。');
    }
    const expectedCollectionKey = xhsOwnMetricCollectionKey(receipt.receiptId, input.checkpoint);
    if (input.collectionKey !== expectedCollectionKey) {
        throw coded('xhs_metrics_cua_request_invalid', '小红书本人指标 CUA collectionKey 与 receiptId/checkpoint 不一致。');
    }
    canonicalTimestamp(input.collectedAt, 'xhs_metrics_cua_request_invalid');
    return {
...
function validateMetricSelectorBundle(bundle: any, clock: any) {
    ...
    if (bundle?.schemaVersion !== CUA_SELECTOR_BUNDLE_SCHEMA
        ...
        throw coded('xhs_metrics_cua_selector_invalid', '小红书本人指标 CUA 缺少获批、未过期且哈希绑定的只读 selector bundle。');
    }
...
function validMetricSelectorMap(selectorMap: any) {
    try {
        if (containsSecretLikeField(selectorMap)
            ...
            || XHS_OWN_METRICS_CUA_ACTIONS.some((action: any) => (!hasExactKeys(selectorMap.actions[action], METRIC_SELECTOR_ACTION_KEYS)
...
function hasExactKeys(value: any, expectedKeys: any) {
    if (!value
        || typeof value !== 'object'
        || Array.isArray(value)
    ...
    return actualKeys.length === canonicalExpectedKeys.length
        && actualKeys.every((key: any, index: any) => key === canonicalExpectedKeys[index]);
}
function hasExactArrayKeys(value: any, expectedKeys: any) {
    const actualKeys = Object.keys(value).sort();
    const canonicalExpectedKeys = [...expectedKeys].sort();
    return actualKeys.length === canonicalExpectedKeys.length
        && actualKeys.every((key: any, index: any) => key === canonicalExpectedKeys[index]);
}
function containsSecretLikeField(value: any): boolean {
    if (!value || typeof value !== 'object')
        return false;
    return Object.entries(value).some(([key, item]: any) => (SECRET_LIKE_KEY.test(key) || containsSecretLikeField(item)));
}
function canonicalMetricSelectorMap(selectorMap: any) {
    return deepFreeze({
        ...
        actions: Object.fromEntries(XHS_OWN_METRICS_CUA_ACTIONS.map((action: any) => [
            action,
...
function validateRunner(runner: any, selector: any, profile: any) {
    const contract = runner?.contract;
    const allowedActions = Array.isArray(contract?.allowedActions)
        ? contract.allowedActions
        : [];
    if (contract?.schemaVersion !== XHS_OWN_METRICS_CUA_RUNNER_SCHEMA
        ...
        || allowedActions.some((action: any, index: any) => action !== XHS_OWN_METRICS_CUA_ACTIONS[index])
        || typeof runner?.beginSession !== 'function'
        || typeof runner?.perform !== 'function'
        || typeof runner?.endSession !== 'function') {
        throw coded('xhs_metrics_cua_runner_contract_mismatch', '小红书本人指标 runner 必须是命名 Profile、获批 selector、无桌面权限且只有五个只读动作。');
    }
}
function metricSteps(request: any, selector: any) {
...
function inspectStepObservation({ observation, action, request, trust }: any) {
    inspectBaseObservation(observation, trust.selector.origin, PAGE_STATES[action]);
    if (action === 'read') {
    ...
    if (['filter', 'open_detail', 'read_metrics'].includes(action)) {
        ...
        if (observation.externalContentId
            !== request.trustedReceipt.externalContentId)
            hardStop('unknown_page');
    }
    if (action === 'read_metrics') {
        if (observation.identityClaim?.kind !== 'page_identity_sha256'
            || observation.identityClaim?.value !== trust.profile.identityClaim.value
        ...
        canonicalTimestamp(observation.observedAt, 'xhs_metrics_cua_observation_invalid');
    }
}
function inspectBaseObservation(observation: any, expectedOrigin: any, expectedPageState: any) {
    if (observation?.kind === 'stop') {
        hardStop(HARD_STOP_REASONS.has(observation.reason)
            ? observation.reason
            : 'unknown_page');
    }
...
function hardStop(reason: any) {
    const error = coded(`xhs_metrics_cua_stopped_${reason}`, `小红书本人指标 CUA 已硬停：${reason}。`);
    error.stopReason = reason;
    error.hardStop = true;
    throw error;
}
function normalizeClock(clock: any) {
    const value = clock();
...
function canonicalTimestamp(value: any, code: any) {
