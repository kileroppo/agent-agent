...
import { BOUNDED_BROWSER_TOOLS, renderBoundedBrowserPolicy } from './cua-policy.ts';
...
function coded(code: any, message: any) {
    const error = new Error(message) as Error & Record<string, any>;
    error.code = code;
    return error;
}
function parseVersion(value: any) {
    const match = String(value || '').match(/(\d+)\.(\d+)\.(\d+)/);
    if (!match)
        return null;
    return match.slice(1).map(Number);
}
export function versionAtLeast(actual: any, minimum: any = MINIMUM_CUA_DRIVER_VERSION) {
...
export function validateLoopbackOrigin(origin: any) {
    let parsed;
    try {
    ...
    if (parsed.protocol !== 'http:' ||
        ...
        parsed.search ||
        parsed.hash) {
        throw coded('invalid_acceptance_origin', `Computer Use 验收只允许 http://${LOCAL_ACCEPTANCE_HOST}:<port>，禁止路径、查询串和外部域名。`);
    }
    return parsed.origin;
}
export async function validateReadableDirectory(directory: any) {
...
export async function resolveAcceptanceUploadPath(directory: any, candidate: any) {
    ...
    if (!relative || relative.startsWith('..') || path.isAbsolute(relative)) {
        throw coded('acceptance_upload_outside_directory', '测试文件必须位于获准的测试目录内，且不能用符号链接越界。');
    }
...
export function buildBoundedSessionPolicy({ origin, readableDirectory, browserExecutable = null }: any) {
...
export function evaluateCuaPreflight({ enabled, confirmation, version, permissions }: any) {
    if (!enabled || confirmation !== LOCAL_ACCEPTANCE_CONFIRMATION) {
        throw coded('local_cua_acceptance_disabled', `本地 Computer Use 验收默认关闭；需同时设置 M5_FAKE_CUA_ACCEPTANCE=1 和 --confirm ${LOCAL_ACCEPTANCE_CONFIRMATION}。`);
    }
    if (!versionAtLeast(version)) {
        throw coded('cua_driver_version_unsupported', `需要 cua-driver >= ${MINIMUM_CUA_DRIVER_VERSION}，当前为 ${version || 'unknown'}。`);
    }
    if (permissions?.source?.bundle_id !== 'com.trycua.driver') {
        throw coded('cua_identity_mismatch', '权限状态不是由 com.trycua.driver 的独立 CuaDriver 应用身份返回，拒绝继续。');
    }
    if (permissions?.accessibility !== true) {
        throw coded('accessibility_required', 'CuaDriver 尚未获得辅助功能权限；请由负责人手工授权后重试。');
    }
    if (permissions?.screen_recording !== true ||
        permissions?.screen_recording_capturable === false) {
        throw coded('screen_recording_required', 'CuaDriver 尚未获得屏幕录制权限；脚本已在启动浏览器和执行任何动作前停止。');
    }
    return {
        passed: true,
...
export function createLocalFixtureRequestHandler(html: any) {
