...
import { createPublisherRuntime } from './runtime.js';
...
export function createPublisherGatewayService({
  mode = 'disabled',
  host = DEFAULT_HOST,
  port = DEFAULT_PORT,
  workspaceRoot,
  ...
  if (normalizedMode === 'real' && productionComposition != null) {
    throw coded(
      ...
      '独立 Publisher service 禁止注入 real production composition；真实发布只能由 A君可信入口逐请求刷新 Paperclip 批准。',
    );
  }
  ...
  const getRuntime = async () => {
    ...
    runtimePromise = Promise.resolve().then(() => {
      ...
      if (normalizedMode === 'real') {
        throw coded(
          'standalone_real_publisher_denied',
          '独立 Publisher service 不承载真实发布；请使用会逐请求刷新 Paperclip 批准的 A君可信入口。',
        );
      }
...
export function createPublisherGatewayServiceFromEnv(env = process.env, options = {}) {
  ...
  return createPublisherGatewayService({
    ...
    workspaceRoot:mode === 'fake'
      ? configuredAbsolutePath(
        env.M5_PUBLISHER_WORKSPACE_ROOT,
...
export class HttpPaperclipPublisherControl {
  ...
  async assertPublishAllowed({ campaignId, checkedAt } = {}) {
    ...
    if (
      ...
      throw coded(
        'campaign_not_active',
        'Paperclip CampaignGrant 未激活、已过期、父 Case 阶段错误或每日 Cron 未启用。',
      );
    }
    return {
      campaignId:item.id,
  ...
  async pauseCampaignAndDisableCron({ campaignId, reason, idempotencyKey } = {}) {
    let item = await this.getCampaignCase(campaignId);
    let trigger = await this.getDailyTrigger(item);
    if (trigger.enabled !== false) {
      await this.request(
        'PATCH',
        `/api/routine-triggers/${encodeURIComponent(trigger.id)}`,
        { enabled:false },
      );
    }
    const grant = item.fields?.campaignGrant;
    if (grant?.status !== 'paused') {
      ...
      const patched = await this.request(
        'PATCH',
        `/api/cases/${encodeURIComponent(item.id)}`,
        {
    ...
    if (item.fields?.campaignGrant?.status !== 'paused' || trigger.enabled !== false) {
      throw coded(
        'paperclip_pause_unverified',
        'Paperclip 没有回读确认 CampaignGrant 已暂停且每日 Cron 已关闭。',
      );
    }
    return {
      campaignId:item.id,
  ...
  async getCampaignCase(campaignId) {
  ...
  async getDailyTrigger(item) {
    const detail = await this.request(
      'GET',
      `/api/routines/${encodeURIComponent(this.dailyRoutineId)}`,
    );
    const projectId = item.fields?.projectId || item.pipeline?.projectId || null;
    if (!projectId || detail?.projectId !== projectId) {
      throw coded(
        'publisher_routine_scope_mismatch',
        '每日 Routine 与 Campaign Case 不属于同一个 Paperclip Project。',
      );
    }
    const triggers = list(detail?.triggers).filter((entry) => entry.kind === 'schedule');
    if (triggers.length !== 1 || typeof triggers[0].enabled !== 'boolean') {
      throw coded(
        'publisher_daily_trigger_invalid',
        `每日 Routine 必须恰好有一个明确状态的 schedule trigger，当前为 ${triggers.length} 个。`,
      );
    }
    return triggers[0];
  }
  ...
  async request(method, requestPath, body) {
    ...
    if (!response.ok) {
      throw coded(
        'paperclip_control_unavailable',
        `Paperclip ${method} ${requestPath} 失败：HTTP ${response.status}。`,
      );
    }
    return parsed;
  }
...
async function routeRequest({
  ...
  try {
    ...
    if (request.method === 'GET' && receiptMatch) {
      ...
      if (!receipt) return json(response, 404, { error:'publish_receipt_not_found' });
...
async function authorizeWriteRequest({
  requestAuthorizer,
  request,
  action,
  campaignId,
  ...
  if (typeof requestAuthorizer?.authorize !== 'function') {
    throw coded(
      ...
      'Publisher 写接口缺少可信 Paperclip Run requestAuthorizer，Runtime 保持未初始化。',
    );
  }
  ...
  try {
    ...
    throw coded(
      'publisher_request_unauthorized',
      'Paperclip Run 写入授权核验失败。',
    );
  }
  if (result?.replayed === true) {
    throw coded(
      'publisher_authorization_replayed',
      'Paperclip 一次性发布授权已经使用，拒绝重放。',
    );
  }
  if (
    ...
    throw coded(
      'publisher_authorization_scope_mismatch',
      'Paperclip 写入授权与 action、Run、Issue 或 Campaign 不一致。',
    );
  }
  return result;
}
...
function httpStatus(error) {
  if (['invalid_json_body', 'request_body_too_large'].includes(error?.code)) return 400;
  if (error?.code === 'publisher_request_unauthorized') return 401;
  if (error?.code === 'publisher_authorization_scope_mismatch') return 403;
  if (error?.code === 'publisher_authorization_replayed') return 409;
  if (
    [
      'publisher_request_authorizer_required',
...
function normalizeMode(value) {
  ...
  if (!mode || mode === 'off') return 'disabled';
...
function validateLoopbackHost(value) {
  ...
  if (!LOOPBACK_HOSTS.has(host)) {
    throw coded(
      'publisher_service_host_denied',
      'Publisher Gateway 只能绑定 127.0.0.1、::1 或 localhost。',
    );
  }
  return host;
}
...
function configuredAbsolutePath(value, name, mode) {
...
function opaqueId(value, message) {
  const id = String(value || '').trim();
  if (!/^[A-Za-z0-9][A-Za-z0-9_-]{7,159}$/.test(id)) throw coded('invalid_paperclip_reference', message);
  return id;
}
