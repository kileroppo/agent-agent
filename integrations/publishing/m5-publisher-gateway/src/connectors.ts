...
export class FakePlatformConnector {
    ...
    publishCalls: any;
    scenarios: any;
    constructor(platform: any, scenarios: any = []) {
    ...
    async publish(request: any) {
        ...
        if (scenario.type === 'error')
            throw coded('fake_transport_failed', '假平台模拟传输失败。');
        if (scenario.type === 'stop') {
            if (!STOP_REASONS.includes(scenario.reason))
                throw new Error('假平台停止原因无效。');
    ...
    async readOwnMetrics(receipt: any, collectedAt: any) {
        this.metricCalls.push({ receiptId: receipt.receiptId, collectedAt });
        const hours = Math.max(0, Math.round((Date.parse(collectedAt) - Date.parse(receipt.publishedAt)) / 3600000));
