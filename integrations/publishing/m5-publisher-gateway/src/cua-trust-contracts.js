...
import { CUA_PLATFORM_ORIGINS } from './cua-connector.js';
...
export async function loadApprovedSelectorBundle({
  file,
  trustedRoot,
  approval,
  platform,
  ...
  if (
    ...
    throw coded(
      'cua_selector_bundle_path_invalid',
      'selector bundle 必须位于受信任的非根目录内。',
    );
  }
  ...
  return validateApprovedSelectorBundle(
    { ...document, approval:{ ...approval } },
    { platform, clock },
  );
}

export function validateApprovedSelectorBundle(
  bundle,
  {
    platform,
  ...
  if (
    ...
    throw coded(
      ...
      `${String(platform || 'unknown')} selector bundle 缺少有效、未过期且哈希绑定的 Paperclip 批准。`,
    );
  }
  if (!validSelectorMapShape(bundle?.selectorMap)) {
    throw coded(
      'cua_selector_bundle_invalid',
      'selector bundle 缺少账号核验、固定五步动作或真实回执定位契约。',
    );
  }
...
export function selectorBundleChecksum(bundle) {
...
export function validateApprovedProfileLease(
  lease,
  {
    platform,
    accountRef,
  ...
  if (
    lease?.schemaVersion !== CUA_PROFILE_LEASE_SCHEMA
    ...
    throw coded(
      'cua_profile_lease_invalid',
      `${String(platform || 'unknown')} 命名 Profile 缺少有效、未过期且账号绑定的 Paperclip lease。`,
    );
  }
  return Object.freeze({
...
function containsSecretMaterial(value) {
  ...
  return Object.entries(value).some(([key, item]) => (
    forbidden.test(key) || containsSecretMaterial(item)
  ));
}
