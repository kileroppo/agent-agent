...
export const PUBLISHER_COST_REPORTER_SCHEMA = 'agent.army/publisher-cost-reporter/v1';
const OFFICIAL_CONNECTOR_MODE = 'real:douyin_official_api';
const LOCAL_ZERO_MODE = /^(?:fake|real:(?:(?:douyin|xiaohongshu|xiaohongshu_own_metrics)_cua|wechat_wenyan_cli))$/;
...
export class PublisherCostRecorder {
    ...
    costReporter: any;
    repository: any;
    constructor({ repository, costReporter, clock = () => new Date(), }: any = {}) {
        if (!repository?.read || !repository?.update) {
            throw coded('publisher_cost_repository_required', 'Publisher 费用记录必须复用发布事务账本。');
        }
    ...
    async assertCampaignBudget({ campaignId, connectorMode, operation, checkedAt = this.clock(), }: any = {}) {
        ...
        try {
            ...
            throw coded('publisher_budget_unavailable', '无法从 Paperclip 核验活动剩余预算；平台连接器未调用。');
        }
        if (result?.campaignId !== input.campaignId
            || result?.hardStopEnabled !== true
            || typeof result?.allowed !== 'boolean'
            || !Number.isFinite(Number(result?.remainingAmountUsd))
            || Number(result.remainingAmountUsd) < 0) {
            throw coded('publisher_budget_unavailable', 'Paperclip 返回的活动预算状态无法验证；平台连接器未调用。');
        }
    ...
    async recordLocalZeroAttempt(input: any = {}) {
        const connectorMode = String(input.connectorMode || '');
        if (!LOCAL_ZERO_MODE.test(connectorMode)) {
            throw coded('publisher_cost_source_invalid', '只有 Fake、本机 CUA 或本机 Wenyan connector 可以固定记录零费用。');
        }
        return this.record({
            ...input,
            connectorMode,
    ...
    async recordOfficialTransportAttempt(input: any = {}) {
        if (input.connectorMode !== OFFICIAL_CONNECTOR_MODE) {
            throw coded('publisher_cost_source_invalid', '官方 API 实际费用只能绑定受支持的官方 connector。');
        }
        return this.record({
            ...input,
    ...
    async record(input: any) {
        ...
        const claimed = await this.repository.update((state: any) => {
        ...
        if (claimed.kind === 'conflict') {
            throw coded('publisher_cost_record_conflict', '同一费用幂等标识对应了不同金额或来源，拒绝覆盖。');
        }
        if (claimed.kind === 'ambiguous') {
            throw coded('publisher_cost_reporting_ambiguous', '费用上报结果未决，禁止重复计费；需要先核对 Paperclip 费用事件。');
        }
        ...
        let receipt;
        try {
            receipt = await this.costReporter.recordConnectorAttempt(Object.freeze({
                costRecordId,
            ...
            throw coded('publisher_cost_reporting_ambiguous', '费用上报未得到确定回执，禁止重复计费；需要先核对 Paperclip 费用事件。');
        }
        const reportRef = String(receipt?.reportRef || '');
        if (!REFERENCE.test(reportRef)) {
            throw coded('publisher_cost_reporting_ambiguous', '费用上报没有返回可核验回执，禁止重复计费。');
        }
        const committed = await this.repository.update((state: any) => {
            ...
            if (!current || current.state !== 'submitting' || !sameCharge(current, record)) {
                throw coded('publisher_cost_record_conflict', '费用记录在提交期间发生变化，已停止后续动作。');
            }
...
export function createFakePublisherCostReporter() {
    return Object.freeze({
        ...
        async assertCampaignBudget({ campaignId }: any) {
            return {
                campaignId,
                allowed: true,
                hardStopEnabled: true,
        ...
        async recordConnectorAttempt({ costRecordId, amountUsd }: any) {
            if (amountUsd !== 0) {
...
export function parseOfficialTransportCost(value: any, { operation, connectorMode = OFFICIAL_CONNECTOR_MODE, }: any = {}) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
        throw coded('publisher_transport_cost_unverified', `${operation} 缺少传输层结构化实际费用，拒绝猜测金额。`);
    }
    ...
    if (Object.keys(value).some((key: any) => !allowed.has(key))) {
        throw coded('publisher_transport_cost_unverified', `${operation} 的传输费用包含未授权字段。`);
    }
...
function validateReporter(reporter: any) {
    if (reporter?.contract?.schemaVersion !== PUBLISHER_COST_REPORTER_SCHEMA
        ...
        throw coded('publisher_cost_reporter_required', '真实 Publisher 必须注入确定性的 Paperclip 费用 reporter。');
    }
}
function normalizeBudgetInput(value: any) {
...
function normalizeCostRecord(value: any) {
    const source = String(value.source || '');
    const connectorMode = connectorModeValue(value.connectorMode);
    if ((source === 'local_zero' && !LOCAL_ZERO_MODE.test(connectorMode))
    ...
    if ((providerRequestId ? 1 : 0) + (receiptRef ? 1 : 0) !== 1) {
        throw coded('publisher_cost_reference_invalid', '费用记录必须且只能提供 providerRequestId 或 receiptRef。');
    }
    return Object.freeze({
...
function deterministicCostRecordId(record: any) {
    const identity = [
        record.campaignId,
...
function sameCharge(left: any, right: any) {
...
function normalizeUsd(value: any) {
...
function connectorModeValue(value: any) {
...
function operationValue(value: any) {
...
function reference(value: any, field: any) {
...
function optionalReference(value: any) {
    if (value == null || value === '')
        return null;
    return reference(value, 'reference');
}
function timestamp(value: any, field: any) {
