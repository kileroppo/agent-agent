...
export const IMMEDIATE_PUBLISH_RECOVERY_ACTION = 'reschedule_platform_case_for_current_date';
...
export function publishIdempotencyKey(request: any) {
    return [
        request.campaignId,
        request.platform,
...
export function validatePublishRequest(request: any, now: any = new Date()) {
    const errors: any[] = [];
    const executionDate = calendarDateInShanghai(now);
    const grant = request?.grant;
    if (!request?.campaignId)
        errors.push('缺少 campaignId。');
    if (grant?.schemaVersion !== M5_SCHEMA_IDS.CAMPAIGN_GRANT)
        errors.push('CampaignGrant 版本无效。');
    if (grant?.status !== 'active')
        errors.push('活动授权未激活。');
    if (grant?.themeScope !== 'AI Agent 实战')
        errors.push('活动主题范围无效。');
    if (!PLATFORMS.includes(request?.platform) || !grant?.platforms?.includes(request?.platform)) {
        errors.push('平台不在活动授权范围内。');
    }
    if (!grant?.accountRefs?.[request?.platform])
        errors.push('缺少平台账号引用。');
    if (grant?.totalPublishLimit !== 14 || grant?.dailyPublishLimitPerPlatform !== 1) {
        errors.push('活动发布上限无效。');
    }
    if (!Number.isInteger(grant?.budgetCents) || grant.budgetCents <= 0)
        errors.push('活动预算无效。');
    ...
    if ((grant?.allowedActions || []).some((action: any) => FORBIDDEN_ACTIONS.includes(action))) {
        errors.push('活动授权错误包含禁止动作。');
    }
    for (const action of REQUIRED_ACTIONS) {
        if (!grant?.allowedActions?.includes(action))
            errors.push(`活动未授权 ${action}。`);
    }
    if (FORBIDDEN_ACTIONS.some((action: any) => !grant?.prohibitedActions?.includes(action))) {
    ...
    if (!request?.reviewReport || request.reviewReport.status !== 'passed'
        || REQUIRED_REVIEW_CHECKS.some((check: any) => request.reviewReport?.checks?.[check] !== true)) {
        errors.push('机器审核七项检查没有全部通过。');
    }
    if (!request?.mediaPath || !request?.title || !request?.body)
        errors.push('缺少视频、标题或正文。');
    if (FORBIDDEN_CALLER_COST_FIELDS.some((field: any) => Object.hasOwn(request || {}, field))) {
        errors.push('发布调用方不得提供或覆盖费用字段。');
    }
    const expected = publishIdempotencyKey(request);
    if (request?.idempotencyKey && request.idempotencyKey !== expected)
        errors.push('幂等键与发布请求不一致。');
    return { passed: errors.length === 0, errors, idempotencyKey: expected };
}
export function receiptId(idempotencyKey: any) {
...
export type PublisherError = Error & Record<string, any>;
export function coded(code: any, message: any): PublisherError {
    const error = new Error(message) as PublisherError;
    error.code = code;
    error.isPublisherError = true;
    return error;
}
export function calendarDateInShanghai(value: any = new Date()) {
    ...
    const values = Object.fromEntries(parts.map((part: any) => [part.type, part.value]));
    return `${values.year}-${values.month}-${values.day}`;
}
function immediatePublishDateMismatch(scheduledDate: any, executionDate: any) {
    const error = coded('publisher_scheduled_date_mismatch', `当前连接器只允许即时发布；Case 日期 ${scheduledDate} 与上海执行日 ${executionDate} 不一致。`);
...
function validCalendarDate(value: any) {
