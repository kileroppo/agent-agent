...
import { parseOfficialTransportCost } from './cost-reporting.ts';
import { validateAccountIdentityVerifier, verifyDouyinAccountIdentity, } from './account-identity.ts';
...
export class DouyinOfficialApiConnector {
    ...
    httpRequest: any;
    maxUploadBytes: any;
    constructor({ enabled = false, httpRequest, credentialResolver, accountIdentityVerifier, costRecorder, maxUploadBytes = DEFAULT_MAX_UPLOAD_BYTES, clock = () => new Date(), }: any = {}) {
    ...
    async publish(request: any = {}) {
        ...
        const costContext: Record<string, any> = {
        ...
        if (!itemId || !createdVideoId) {
            throw coded('douyin_create_result_ambiguous', '抖音创建响应缺少内容 ID，结果不确定；必须暂停并人工核对，禁止自动重发。');
        }
        ...
        if (queried.stopped)
            return queried.stopped;
        const record = listFrom(queried.body).find((entry: any) => (stringValue(entry?.video_id) === createdVideoId
            && stringValue(entry?.item_id) === itemId));
        if (!record) {
            throw coded('douyin_publish_reconciliation_required', '抖音创建后无法核对相同 item_id 和 video_id；必须暂停并人工核对，禁止自动重发。');
        }
        return {
            state: 'published',
    ...
    async readOwnMetrics(receipt: any = {}) {
        this.assertEnabled();
        if (receipt.platform !== M5_PLATFORM_IDS.DOUYIN
            || !receipt.accountRef
        ...
        if (response.stopped) {
            const stopReason = normalizedMetricStopReason(response.stopped.stopReason);
            const error = coded(`douyin_metric_${stopReason}`, '抖音指标查询被安全门禁停止，等待 Paperclip 决定恢复动作。');
            error.hardStop = true;
        ...
        const record = listFrom(response.body).find((entry: any) => (stringValue(entry?.item_id) === receipt.externalContentId));
    ...
    assertEnabled() {
        ...
        if (typeof this.httpRequest !== 'function'
            ...
            throw coded('douyin_connector_dependencies_missing', '抖音官方 API 连接器必须注入 HTTP 传输、临时凭据解析器、Paperclip 账号核验器和确定性费用记录器。');
        }
        validateAccountIdentityVerifier(this.accountIdentityVerifier);
    }
    async resolveCredentials(accountRef: any, purpose: any) {
        let resolved;
        try {
    ...
    async callOfficial(request: any, { transportCode, platformCode }: any, costContext: any) {
        let response;
        try {
        ...
        let costReportingErrorCode: any = null;
        try {
            ...
            if (!safetyStopReason)
                throw error;
            costReportingErrorCode = safeErrorCode(error, 'cost_reporting_failed');
        }
...
function validatePublishInput(request: any, maxUploadBytes: any) {
    ...
    if (request.verifiedMedia?.immutableLease !== true
    ...
    if (!Number.isInteger(maxUploadBytes)
        || maxUploadBytes <= 0
        || request.verifiedMedia.bytes > maxUploadBytes) {
        throw coded('douyin_upload_too_large', '视频超过当前抖音单次上传安全上限；分片上传尚未实现，拒绝调用平台。');
    }
}
function buildCaption(request: any) {
    const lines = [
        stringValue(request.title),
        stringValue(request.body),
        ...(Array.isArray(request.tags)
            ? request.tags.map((tag: any) => stringValue(tag)).filter(Boolean).map((tag: any) => `#${tag}`)
            : []),
    ].filter(Boolean);
    const caption = lines.join('\n');
    if (!caption)
        throw coded('invalid_douyin_caption', '抖音发布文案不能为空。');
    if ([...caption].length > MAX_CAPTION_CODE_POINTS) {
        throw coded('douyin_caption_too_long', `抖音发布文案超过 ${MAX_CAPTION_CODE_POINTS} 个字符，拒绝静默截断。`);
    }
    return caption;
}
function officialErrorCode(body: any) {
...
function officialSafetyStopReason(response: any) {
    if (!response
    ...
    if (RISK_ERROR_CODES.has(errorCode))
        return 'risk_control';
    return null;
}
function safeErrorCode(error: any, fallback: any) {
    const value = String(error?.code || '');
    return /^[a-z][a-z0-9_]{1,127}$/.test(value) ? value : fallback;
}
function normalizedMetricStopReason(value: any) {
    if (value === 'identity_verification' || value === 'risk_control') {
        return value;
    }
    return 'unknown_page';
}
function listFrom(body: any) {
    const list = body?.data?.list ?? body?.data?.data?.list;
    return Array.isArray(list) ? list : [];
}
function evidenceFor(record: any, videoId: any) {
...
function publishedAt(record: any, clock: any) {
