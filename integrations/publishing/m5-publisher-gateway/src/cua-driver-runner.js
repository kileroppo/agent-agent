...
import { CuaDriverCliBridge } from './cua-driver-cli-bridge.js';
import {
  cuaSemanticSnapshot,
  findExactRef,
  findFileInputRef,
...
export class CuaDriverPublisherRunner {
  ...
  async beginSession(input = {}) {
    ...
    try {
      ...
      const initial = cuaSemanticSnapshot.normalize(
      ...
      if (!cuaSemanticSnapshot.verifyAccountIdentity(
        initial.raw,
        selectors.identity,
  ...
  async perform(input = {}) {
    ...
    const before = cuaSemanticSnapshot.normalize(
    ...
    if (beforeStop) return cuaSemanticSnapshot.stopObservation(session.origin, beforeStop);
    ...
    if (stopReason) return cuaSemanticSnapshot.stopObservation(session.origin, stopReason);

    session.nextActionIndex += 1;
    if (action === 'read_result') {
      const published = cuaSemanticSnapshot.publishedResult(
        after.raw,
        session.selectors,
        this.clock,
        input.input?.expectedTitle,
      );
      if (!published) return cuaSemanticSnapshot.stopObservation(session.origin, 'unknown_page');
      return {
        kind:'ok',
        pageState:'published',
        origin:after.origin,
