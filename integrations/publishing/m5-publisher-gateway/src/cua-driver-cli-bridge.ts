...
import { SUPPORTED_BROWSER_EXECUTABLES, renderBoundedBrowserPolicy, } from './cua-policy.ts';
import { MINIMUM_CUA_DRIVER_VERSION, versionAtLeast, } from './local-cua-acceptance.ts';
import { coded } from './policy.ts';
import { cuaSemanticSnapshot, findExactRef, findFileInputRef, findRef, findRichTextInputRef, parseBrowserPrepareResult, } from './cua-semantic-snapshot.ts';
...
export class CuaDriverCliBridge {
    approvalTokenProvider: any;
    browserPid: any;
    browserPidResolver: any;
    command: any;
    constructor({ command = 'cua-driver', browserPid = null, browserPidResolver = findBrowserPid, approvalTokenProvider = null, }: any = {}) {
        this.command = command;
        this.browserPid = browserPid;
        this.browserPidResolver = browserPidResolver;
        this.approvalTokenProvider = approvalTokenProvider;
    }
    async open({ sessionId, origin, readableDirectory, selectors, profileMode, profileName, }: any) {
        await verifyCuaPreflight(this.command);
        const browser = normalizeBrowserIdentity(await this.browserPidResolver(this.browserPid));
        ...
        if (typeof approvalToken !== 'string' || !approvalToken.trim()) {
            throw coded('browser_consent_required', '隔离浏览器需要可信宿主提供一次性 browser-approve 凭证；runner 不生成、记录或持久化该凭证。');
        }
        ...
        await fs.writeFile(policyPath, renderBoundedBrowserPolicy({
            origin,
            readableDirectory,
        ...
        const daemon: Record<string, any> = { exitCode: null };
        ...
        try {
            ...
            if (!targetId || !tabId)
                throw coded('browser_binding_missing', '无法取得隔离浏览器 capability。');
            ...
            const common: Record<string, any> = { target_id: targetId, tab_id: tabId, session: sessionId };
    ...
    async snapshot(session: any, snapshotFormat: any = 'semantic_v2', { query }: any = {}) {
    ...
    async upload(session: any, selector: any, file: any) {
        ...
        try {
            ...
            if (error?.code !== 'browser_ref_missing')
                throw error;
    ...
    async type(session: any, selector: any, text: any) {
        const semanticSnapshot = await this.snapshot(session);
        let ref;
        try {
            ...
            else if (!cuaSemanticSnapshot.text(semanticSnapshot).toLowerCase().includes(String(selector.label).toLowerCase())) {
                throw error;
            }
            const domSnapshot = await this.snapshot(session, 'dom_refs_v1');
    ...
    async click(session: any, selector: any) {
    ...
    async readManagementResult({ bridgeSession: session, origin, selectors, expectedTitle, attempts, intervalMs, sleep, }: any) {
...
async function verifyCuaPreflight(command: any) {
    const manifest = cuaSemanticSnapshot.parseJson(await run(command, ['manifest', '--pretty']), 'cua-driver manifest');
    const permissions = cuaSemanticSnapshot.parseJson(await run(command, ['permissions', 'status', '--json']), 'cua-driver permissions status');
...
async function findBrowserPid(explicitPid: any) {
    ...
    if (explicitPid)
        throw coded('unsupported_browser_pid', '指定 PID 不是已审核的 Chromium 主进程。');
    throw coded('supported_browser_not_running', '未发现已运行的 Chrome、Chromium 或 Edge 主进程。');
}
function normalizeBrowserIdentity(value: any) {
...
async function run(command: any, args: any, options: any = {}) {
...
async function cuaCall(command: any, socketPath: any, tool: any, input: any) {
...
async function waitForDaemon(command: any, socketPath: any, daemon: any) {
    let lastError = '';
...
async function waitForBrowserWindow(command: any, socketPath: any, preparedPid: any) {
...
function findBestBrowserWindowId(value: any, preparedPid: any) {
    const candidates: any[] = [];
    cuaSemanticSnapshot.walk(value, (item: any) => {
        if (item
            && typeof item === 'object'
            ...
            candidates.push({
                ...
                score: (item.is_on_screen === true ? 1000000000 : 0) + width * height,
            });
        }
        return false;
    });
    candidates.sort((left: any, right: any) => right.score - left.score);
    return candidates[0]?.windowId;
}
async function stopBridgeSession(session: any) {
