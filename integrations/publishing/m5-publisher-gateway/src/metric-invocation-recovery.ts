...
export class MetricInvocationRecovery {
    ...
    repository: any;
    constructor({ repository, paperclipControl, clock, pauseInPaperclip, activateGlobalHardStop, }: any) {
    ...
    async assertCurrentAuthorization(target: any, attemptId: any) {
        ...
        try {
            ...
            throw coded('metric_recovery_unauthorized', 'Paperclip 未确认负责人已核对该指标外部调用，账本保持 invoking。');
        }
    ...
    async reconcile(input: any = {}) {
        ...
        if (initial.recovery) {
            if (initial.recovery.authorizationId === target.authorizationId
            ...
            throw coded('metric_recovery_already_resolved', '该指标调用已经由另一份人工核对结论处理，拒绝覆盖。');
        }
        if (initial.attempt.state !== 'invoking') {
            throw coded('metric_recovery_not_required', '只有外部调用终止状态未确认的 invoking 指标 attempt 才允许人工核对恢复。');
        }
        if (!validMetricRecoveryClaimToken(initial.attempt.claimToken)) {
            throw coded('metric_recovery_claim_invalid', 'invoking 指标 attempt 缺少有效持久 claimToken，拒绝在无法确认调用所有权时套用人工核对结论。');
        }
        if (metricRecoveryAuthorizationOwner(initialState, target.authorizationId)) {
            throw coded('metric_recovery_authorization_reused', '该 Paperclip 人工核对授权已用于另一指标 attempt，拒绝跨检查点或跨活动重放。');
        }
        if (typeof this.paperclipControl.assertMetricRecoveryAllowed !== 'function') {
            throw coded('paperclip_metric_recovery_control_required', '指标未决调用恢复必须注入独立的 Paperclip 人工核对授权适配器。');
        }
        await this.assertCurrentAuthorization(target, initial.attempt.attemptId);
        const { authorization, checkedAt, } = await this.assertCurrentAuthorization(target, initial.attempt.attemptId);
        ...
        let committed;
        try {
            committed = await this.repository.update((state: any) => {
                const current = metricRecoveryTarget(state, target);
                if (current.recovery) {
                    return (current.recovery.authorizationId === target.authorizationId
                ...
                if (current.attempt.state !== 'invoking'
            ...
            if (target.conclusion === 'external_effect_verified') {
                await this.activateGlobalHardStop({
                    ...
                    controlError: String(error?.code || 'publisher_ledger_writeback_failure'),
                    activatedAt: this.clock().toISOString(),
                });
                throw coded('metric_recovery_writeback_failed_hard_stop', 'Paperclip 已暂停外部效果未决的指标调用，但核对账本回写失败；Publisher Gateway 已全局硬停。');
            }
            throw error;
        }
        if (committed.kind === 'already_resolved') {
            throw coded('metric_recovery_already_resolved', '该指标调用已由另一份人工核对结论处理，拒绝覆盖。');
        }
        if (committed.kind === 'authorization_reused') {
            throw coded('metric_recovery_authorization_reused', '该 Paperclip 人工核对授权已用于另一指标 attempt，拒绝跨检查点或跨活动重放。');
        }
        if (committed.kind === 'changed') {
            throw coded('metric_recovery_target_changed', 'Paperclip 授权核验期间指标 attempt 已变化，拒绝套用旧核对结论。');
        }
        return {
...
function normalizeMetricRecoveryInput(input: any) {
    ...
    if (!SAFE_REFERENCE_PATTERN.test(campaignId)
        ...
        throw coded('invalid_metric_recovery_request', '指标恢复必须绑定 Campaign、回执、固定检查点、负责人核对结论、Paperclip 授权与证据引用。');
    }
    return {
        campaignId,
...
function metricRecoveryTarget(state: any, target: any) {
    const receipt: any = Object.values(state.receipts).find((item: any) => (String(item.receiptId || '').toLowerCase() === target.receiptId.toLowerCase()));
    if (!receipt) {
        throw coded('publish_receipt_not_found', '指标恢复找不到指定发布回执。');
    }
    if (receipt.campaignId !== target.campaignId
        || !target.collectionKey.startsWith(`${receipt.receiptId}:`)) {
        throw coded('metric_recovery_scope_mismatch', '指标恢复的 Campaign、回执与 collectionKey 不属于同一可信发布记录。');
    }
    ...
    if (!attempt
        ...
        throw coded('metric_recovery_target_missing', '找不到与该可信发布记录精确绑定的指标 attempt。');
    }
    return {
        receipt,
        attempt,
...
function validMetricRecoveryClaimToken(value: any) {
    return typeof value === 'string'
        && SAFE_REFERENCE_PATTERN.test(value);
}
function metricRecoveryAuthorizationOwner(state: any, authorizationId: any) {
    return Object.values(state.attempts).find((attempt: any) => (attempt?.kind === 'metric_snapshot'
        && attempt?.metricRecovery?.authorizationId === authorizationId)) || null;
}
function validateMetricRecoveryAuthorization(value: any, expected: any) {
    if (value?.authorized !== true
        ...
        throw coded('metric_recovery_authorization_scope_mismatch', 'Paperclip 人工核对授权与当前指标 attempt、结论或证据范围不一致。');
    }
}
function metricRecoveryId(authorizationId: any) {
    return `metric_recovery_${crypto.createHash('sha256')
        .update(`${METRIC_RECOVERY_ACTION}:${authorizationId}`)
        .digest('hex')
        .slice(0, 32)}`;
}
function publicMetricRecovery(recovery: any) {
