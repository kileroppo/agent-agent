export function normalizeBoomSignal(input = {}) {
  ...
  if (!sourceUrl) throw new Error('爆款候选缺少可供小D读取的公开 HTTP(S) 来源。');
  if (!grade || !workId || !platform) throw new Error('爆款候选缺少等级、平台或作品编号。');
...
export async function dispatchBoomSignal(input, { missions } = {}) {
  if (!missions?.createBusinessMission) throw new Error('军团任务入口不可用。');
...
function nullableNumber(value) { const parsed = Number(value); return value === null || value === undefined || !Number.isFinite(parsed) ? null : parsed; }
function nonNegativeInteger(value) { return Math.max(0, Math.floor(number(value))); }
function isoDate(value) { const parsed = Date.parse(String(value || '')); return Number.isFinite(parsed) ? new Date(parsed).toISOString() : null; }
