...
type ScopeResolver = (approvalRef: string) => unknown | Promise<unknown>;
type VaultQueryInput = Readonly<{ chat: string; startTime: string; endTime: string; limit: number }>;
type VaultQuery = (input: VaultQueryInput) => Promise<JsonObject>;
type Clock = () => Date;
type NormalizedScope = Readonly<{ chatSelector: string; startTime: string; endTime: string; maxMessages: number }>;

export class WeChatLocalVaultAdapter implements ContentAcquisitionAdapter {
  readonly id = 'yichen-wechat-local-vault';
  readonly versionRef = 'controlled-v1';
  readonly capabilities = [CAPABILITY] as const;
  readonly accessMode = 'private_scoped' as const;
  readonly priorityClass = 'specialized' as const;
  readonly healthStatus: AdapterHealthStatus;
  readonly runtimeRequirements = ['wechat_chat_read'] as const;
  private readonly scopeResolver: ScopeResolver;
  private readonly runVaultQuery: VaultQuery;
  private readonly now: Clock;

  constructor({ scopeResolver, runVaultQuery = null, vaultCliPath = null, healthStatus = 'unavailable', now = () => new Date() }: Readonly<{
    scopeResolver?: ScopeResolver;
    runVaultQuery?: VaultQuery | null;
    vaultCliPath?: string | null;
    healthStatus?: string;
    now?: Clock;
  }> = {}) {
    ...
    this.healthStatus = ['healthy', 'degraded', 'unavailable'].includes(healthStatus)
      ? healthStatus as AdapterHealthStatus
      : 'unavailable';
  ...
  async acquire({ source, requestedCapabilities, requestingAgentId, taskId }: AdapterAcquireInput): Promise<AdapterAcquireResult> {
...
function approvalReference(source: string): string {
  let parsed: URL;
  try {
...
function validateScope(scopeValue: unknown, { approvalRef, requestingAgentId, taskId, now }: Readonly<{
  approvalRef: string;
  requestingAgentId: string;
  taskId?: string | null;
  now: Date;
}>): NormalizedScope {
  const scope: JsonObject | null = scopeValue && typeof scopeValue === 'object' ? scopeValue as JsonObject : null;
  ...
  if (scope.expiresAt && Date.parse(String(scope.expiresAt)) <= now.getTime()) {
  ...
  const start = Date.parse(String(scope.startTime));
  const end = Date.parse(String(scope.endTime));
...
function normalizeMessages(value: unknown, scope: NormalizedScope): Array<Readonly<Record<string, unknown>>> {
  ...
  return value.slice(0, scope.maxMessages).map((itemValue: unknown) => {
    const item: JsonObject = itemValue && typeof itemValue === 'object' ? itemValue as JsonObject : {};
...
function defaultRunVaultQuery({ vaultCliPath, chat, startTime, endTime, limit }: VaultQueryInput & Readonly<{ vaultCliPath: string | null }>): Promise<JsonObject> {
  ...
  return new Promise<JsonObject>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        resolve(JSON.parse(output) as JsonObject);
      } catch {
...
function cliTime(value: string): string {
...
function scopedError(code: string, message: string): Error & { code: string } {
