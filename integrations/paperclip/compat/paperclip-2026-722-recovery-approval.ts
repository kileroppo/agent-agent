...
export const RECOVERY_APPROVAL_KIND: any = 'metric_recovery_authorization_v1';
export const RECOVERY_APPROVAL_ROUTE_ORIGINAL_SHA256: any = 'bcb94462617c9c5e199b7af0da7f985d96470d56483434ebae9cde2bf4924a89';
export const RECOVERY_APPROVAL_ROUTE_PATCHED_SHA256: any = '30722c0bec8a53d4ce484df88ba91f4f4a4813f551be8a48db541ca6fe482dfd';
export const RECOVERY_APPROVAL_BACKUP_SUFFIX: any = '.agent-army-paperclip-2026.722.0-recovery-approval.bak';
const UUID_PATTERN: any = /^[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}$/i;
const SAFE_REF_PATTERN: any = /^[A-Za-z0-9][A-Za-z0-9._:@/-]{0,255}$/;
const ROUTE_RELATIVE_PATH: any = '@paperclipai/server/dist/routes/approvals.js';
const IMPORT_ANCHOR: any = `import { Router } from "express";
import { eq } from "drizzle-orm";
import { heartbeatRuns } from "@paperclipai/db";`;
const IMPORT_PATCH: any = `import { createHash, randomUUID } from "node:crypto";
import { Router } from "express";
import { and, eq, ne, sql } from "drizzle-orm";
import { approvals, heartbeatRuns, issueApprovals } from "@paperclipai/db";`;
const ERROR_IMPORT_ANCHOR: any = 'import { redactEventPayload } from "../redaction.js";';
const ERROR_IMPORT_PATCH: any = `${ERROR_IMPORT_ANCHOR}
import { conflict, forbidden, unprocessable } from "../errors.js";`;
const ROUTES_ANCHOR: any = 'export function approvalRoutes(db, options = {}) {';
const CREATE_NORMALIZATION_ANCHOR: any = `        const normalizedPayload = approvalInput.type === "hire_agent"
...
const CREATE_NORMALIZATION_PATCH: any = `        const recoveryPayloadRequested = isRecoveryApprovalPayload(approvalInput.payload);
        ...
        const normalizedPayload = approvalInput.type === "hire_agent"
            ...
            : recoveryPayloadRequested
                ...
                : approvalInput.payload;`;
const RESUBMIT_NORMALIZATION_ANCHOR: any = `        const normalizedPayload = req.body.payload
            ? existing.type === "hire_agent"
                ...
                : req.body.payload
            : undefined;`;
const RESUBMIT_NORMALIZATION_PATCH: any = `        const recoveryPayloadResubmitted = isRecoveryApprovalPayload(req.body.payload);
        ...
        const normalizedPayload = req.body.payload
            ? existing.type === "hire_agent"
                ...
                : recoveryPayloadResubmitted
                    ...
                    : req.body.payload
            : undefined;`;
const CREATE_ID_ANCHOR: any = `        const approval = await svc.create(companyId, {
            ...approvalInput,`;
const CREATE_ID_PATCH: any = `        const approval = await svc.create(companyId, {
            ...(recoveryApprovalId ? { id: recoveryApprovalId } : {}),
            ...approvalInput,`;
const ENDPOINT_ANCHOR: any = '    router.post("/approvals/:id/reject", validate(resolveApprovalSchema), async (req, res) => {';
const RUNTIME_HELPERS: any = `// agent-army:paperclip-recovery-approval:v1
...
`;
const ENDPOINT_PATCH: any = `    router.post("/approvals/:id/recovery-authorizations/consume", async (req, res) => {
...
${ENDPOINT_ANCHOR}`;
export function canonicalRecoveryScope(value: any): any {
    return canonicalJson(value);
}
export function hashRecoveryScope(scope: any): any {
    return `sha256:${crypto
        .createHash('sha256')
        .update(canonicalRecoveryScope(scope))
        .digest('hex')}`;
}
export function normalizeRecoveryApprovalPayload(payload: any, { now = new Date() }: any = {}): any {
    ...
    const actualScopeKeys: any = Object.keys(payload.scope).sort();
    if (actualScopeKeys.length !== scopeKeys.length
        || actualScopeKeys.some((key: any, index: any): any => key !== scopeKeys[index])) {
    ...
    if (!['no_external_effect', 'external_effect_verified']
    ...
    const collectionMatch: any = payload.scope.collectionKey.match(/^([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}):(2h|24h|72h)$/i);
    if (!collectionMatch
        || collectionMatch[1].toLowerCase()
            !== payload.scope.receiptId.toLowerCase()) {
        throw recoveryError('Recovery approval collectionKey 必须匹配 receiptId 与固定检查点。');
    }
    const nowMs: any = validDate(now).getTime();
    const expiresAtMs: any = Date.parse(payload.expiresAt);
    if (!Number.isFinite(expiresAtMs) || expiresAtMs <= nowMs) {
        throw recoveryError('Recovery approval expiresAt 必须是未来时间。');
    }
    const scope: any = JSON.parse(canonicalRecoveryScope(payload.scope));
...
export function bindRecoveryApprovalPayload(payload: any, approvalId: any): any {
    const normalizedApprovalId: any = nonEmptyString(approvalId);
...
export function decideRecoveryConsume(payload: any, { scopeHash, runId, agentId, now = new Date(), }: any = {}): any {
    if (!isRecord(payload) || payload.governanceKind !== RECOVERY_APPROVAL_KIND) {
        return { action: 'deny', reason: 'kind' };
    }
    if (payload.scopeHash !== scopeHash)
        return { action: 'deny', reason: 'scope' };
    if (nonEmptyString(payload.revokedAt))
        return { action: 'deny', reason: 'revoked' };
...
export function decideRecoveryRevoke(payload: any): any {
    if (!isRecord(payload) || payload.governanceKind !== RECOVERY_APPROVAL_KIND) {
        return { action: 'deny', reason: 'kind' };
    }
    if (nonEmptyString(payload.consumedAt) || nonEmptyString(payload.consumedByRunId)) {
...
export function decideRecoveryConsumeBinding({ actor, approval, scope, run, link, }: any = {}): any {
    if (actor?.type !== 'agent'
    ...
    const context: any = isRecord(run?.contextSnapshot) ? run.contextSnapshot : {};
    const runIssueId: any = nonEmptyString(context.issueId) || nonEmptyString(context.taskId);
    ...
    if (link?.companyId !== approval.companyId
...
export function buildRecoveryApprovalRouteSource(source: any): any {
    let patched: any = String(source);
    ...
    patched = replaceExactly(patched, CREATE_NORMALIZATION_ANCHOR, CREATE_NORMALIZATION_PATCH);
    patched = replaceExactly(patched, CREATE_ID_ANCHOR, CREATE_ID_PATCH);
    patched = replaceExactly(patched, RESUBMIT_NORMALIZATION_ANCHOR, RESUBMIT_NORMALIZATION_PATCH);
    patched = replaceExactly(patched, ENDPOINT_ANCHOR, ENDPOINT_PATCH);
    return patched;
}
export function patchRecoveryApprovalRouteSource(source: any): any {
    const text: any = String(source);
    const currentSha: any = sha256(text);
    if (currentSha === RECOVERY_APPROVAL_ROUTE_PATCHED_SHA256)
        return text;
    if (currentSha !== RECOVERY_APPROVAL_ROUTE_ORIGINAL_SHA256) {
        throw recoveryError('Paperclip approvals route源码SHA或补丁锚点不匹配，拒绝修改未知版本。');
    }
    const patched: any = buildRecoveryApprovalRouteSource(text);
    if (sha256(patched) !== RECOVERY_APPROVAL_ROUTE_PATCHED_SHA256) {
        throw recoveryError('Paperclip approvals route目标SHA不匹配，拒绝写入。');
    }
    return patched;
}
export async function resolveRecoveryApprovalTarget({ paperclipEntry }: any): Promise<any> {
    ...
    if (paperclipPackage.version !== PAPERCLIP_VERSION) {
        throw recoveryError(`只允许 Paperclip ${PAPERCLIP_VERSION}，当前为 ${paperclipPackage.version || 'unknown'}。`);
    }
    ...
    if (serverPackage.name !== '@paperclipai/server'
        || serverPackage.version !== PAPERCLIP_VERSION) {
        throw recoveryError('Paperclip server包名或版本不匹配。');
    }
    const routeFile: any = path.join(nodeModulesRoot, ROUTE_RELATIVE_PATH);
...
export async function applyRecoveryApprovalPatch(options: any): Promise<any> {
    const targets: any = await resolveRecoveryApprovalTarget(options);
    const result: any = await applyVersionLockedFile({
        ...
        patch: (current: any): any => Buffer.from(patchRecoveryApprovalRouteSource(current.toString('utf8'))),
    });
    return { ...result, ...targets };
}
export async function rollbackRecoveryApprovalPatch(options: any): Promise<any> {
    const targets: any = await resolveRecoveryApprovalTarget(options);
...
function replaceExactly(source: any, anchor: any, replacement: any): any {
...
async function packageRootForEntry(entryValue: any, expectedName: any): Promise<any> {
    ...
    for (;;) {
        const record: any = await readPackage(path.join(current, 'package.json'), {
            optional: true,
        });
...
function recoveryError(message: any): any {
