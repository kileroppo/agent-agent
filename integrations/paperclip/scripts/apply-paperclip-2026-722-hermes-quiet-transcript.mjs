...
const CONFIRMATION = 'I_ACCEPT_PAPERCLIP_2026_722_HERMES_QUIET_TRANSCRIPT_PATCH';
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const result = await applyCompatibilityPatch({
...
function parseArgs(args) {
  const allowed = new Set(['paperclip-entry', 'confirm-version', 'confirm-apply']);
  const result = {};
...
if (import.meta.url === invokedPath) {
  try {
    ...
    process.stderr.write(`${String(error?.message || 'Hermes安静转录补丁应用失败。')}\n`);
    process.exitCode = 1;
  }
}
