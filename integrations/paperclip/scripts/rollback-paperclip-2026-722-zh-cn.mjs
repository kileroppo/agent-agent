...
const CONFIRMATION = 'I_ACCEPT_PAPERCLIP_2026_722_ZH_CN_ROLLBACK';
...
export async function main(argv = process.argv.slice(2)) {
  ...
  return {
    ...
    uiFile:result.uiFile,
    htmlFile:result.htmlFile,
    uiBackupFile:result.uiBackupFile,
    htmlBackupFile:result.htmlBackupFile,
  };
}
...
if (import.meta.url === invokedPath) {
  try {
    ...
    process.stderr.write(`${String(error?.message || 'Paperclip简体中文补丁回滚失败。')}\n`);
    process.exitCode = 1;
  }
}
