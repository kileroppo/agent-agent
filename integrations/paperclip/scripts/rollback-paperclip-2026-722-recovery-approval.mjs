...
const CONFIRMATION = 'I_ACCEPT_PAPERCLIP_2026_722_RECOVERY_APPROVAL_ROLLBACK';
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (options['confirm-rollback'] !== CONFIRMATION) {
    throw new Error(
      `未回滚：必须显式传入 --confirm-rollback ${CONFIRMATION}。`,
    );
  }
  const result = await rollbackRecoveryApprovalPatch({
...
if (import.meta.url === invokedPath) {
  try {
    ...
    process.stderr.write(
      `${String(error?.message || 'Recovery approval兼容补丁回滚失败。')}\n`,
    );
    process.exitCode = 1;
  }
}
