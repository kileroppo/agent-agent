...
const CONFIRMATION = 'I_ACCEPT_PAPERCLIP_2026_722_BINARY_RPC_PATCH';
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const result = await applyCompatibilityPatch({
    ...
    pluginEntry:required(options['plugin-entry'], '--plugin-entry 缺失。'),
  });
  return {
    status:result.status,
    changed:result.changed,
    hostFile:result.hostFile,
...
if (import.meta.url === invokedPath) {
  try {
    ...
    process.stderr.write(`${String(error?.message || '兼容补丁应用失败。')}\n`);
    process.exitCode = 1;
  }
}
