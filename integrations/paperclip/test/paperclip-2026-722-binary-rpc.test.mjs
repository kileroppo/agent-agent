...
test('0.4.9使用当前StepFun SHA，0.4.8保留为历史候选且0.4.7回滚链保留旧SHA', async (context) => {
  assert.equal(
  ...
  const pluginRoot = path.join(root, 'content-autonomy-bundle-0.4.9-fixture');
  ...
  const pluginEntry = path.join(pluginRoot, 'src', 'worker.js');
  ...
  await fs.copyFile(
    new URL('../plugins/content-autonomy/src/stepfun-tools.js', import.meta.url),
