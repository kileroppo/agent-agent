import { createGoalSchema, createProjectSchema, createRoutineSchema, createRoutineTriggerSchema, pipelineStageConfigSchema, pipelineStageKindSchema, upsertBudgetPolicySchema, } from '@paperclipai/shared';
import { z } from 'zod';
...
export function validateDefinition(definition: any) {
    ...
    for (const stage of parsed.stages) {
        if (stageKeys.has(stage.key))
            throw new Error(`重复阶段 key: ${stage.key}`);
        ...
        if (stage.routineKey) {
            if (routineKeys.has(stage.routineKey))
                throw new Error(`重复 Routine key: ${stage.routineKey}`);
    ...
    if (parsed.stages.at(-4)?.key !== 'retrospective'
    ...
    for (const stage of parsed.stages.filter((item: any) => item.review)) {
        for (const destination of Object.values(stage.review!)) {
            if (!stageKeys.has(destination))
                throw new Error(`review 路由指向未知阶段: ${destination}`);
        }
    }
    return parsed;
}
export function validateCompiledStage(stage: any) {
    pipelineStageKindSchema.parse(stage.kind);
    pipelineStageConfigSchema.parse(stage.config);
    return stage;
}
export function validateRoutinePayload(payload: any) {
    return createRoutineSchema.parse(payload);
}
export function validateTriggerPayload(payload: any) {
    return createRoutineTriggerSchema.parse(payload);
}
export function validateGoalPayload(payload: any) {
    return createGoalSchema.parse(payload);
}
export function validateProjectPayload(payload: any) {
    return createProjectSchema.parse(payload);
}
export function validateBudgetPayload(payload: any) {
