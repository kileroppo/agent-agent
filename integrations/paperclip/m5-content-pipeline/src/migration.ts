...
import { buildBootstrapPlan } from './plan.ts';
import { APPLY_CONFIRMATION, applyBootstrap, dryRunBootstrap, } from './bootstrap.ts';
import { buildCampaignCaseBatch, ingestCampaignDraftCase, } from './contract.ts';
import { FakePaperclipAdapter } from './adapters/fake.ts';
export const M5_V2_MIGRATION_CONFIRMATION = 'I_CONFIRM_M5_V2_PIPELINE_MIGRATION_WITH_DRAFT_CAMPAIGN_CRON_OFF_AND_ROLLBACK';
export const M5_V2_CLONE_CUTOVER_CONFIRMATION = 'I_CONFIRM_M5_V2_CLONE_CUTOVER_WITH_V1_RETAINED_V2_DRAFT_ONLY_AND_CRON_OFF';
const TERMINAL_OR_IDLE_STAGES = new Set(['draft', 'campaign_active', 'done', 'cancelled']);
export async function inspectM5V2Migration({ adapter, legacyPipelineId, definition, backup, }: any = {}) {
    ...
    const cases = rows(caseDocument).map((item: any) => {
        const record = item?.case ?? item;
        return {
            ...record,
            stageKey: item?.stage?.key ?? record?.stageKey ?? null,
            activeWork: item?.activeWork ?? record?.activeWork ?? null,
            descendantActiveWorkCount: Number(item?.descendantActiveWorkCount ?? record?.descendantActiveWorkCount ?? 0),
        };
    });
    const dailyCandidates = rows(routines).filter((item: any) => String(item?.description || '').includes('[agent-army:m5:routine:m5-daily-campaign]')
        && (!legacy?.projectId
            || !item?.projectId
    ...
    const dailyDetail = daily?.id
        ...
        : null;
    const scheduleTriggers = rows(dailyDetail?.triggers).filter((item: any) => item.kind === 'schedule');
    const campaignParents = cases.filter((item: any) => !item.parentCaseId && item.fields?.campaignGrant);
    const activeCases = cases.filter((item: any) => item.fields?.campaignGrant?.status === 'active'
    ...
    const completedPlatforms = cases.filter((item: any) => item.fields?.platform
...
export async function inspectM5V2CloneCutover({ adapter, legacyPipelineId, definition, backup, }: any = {}) {
    ...
    const legacyPreconditionsPassed = Object.entries(legacyAudit.checks)
        .filter(([key]: any) => key !== 'exactlyOneDraftCampaign')
        .every(([, passed]: any) => passed === true)
        && legacyCloneEligible;
    ...
    const unversionedRoutineMarkers = targetRoutines
        .filter((routine: any) => routine.marker === `[agent-army:m5:routine:${routine.key}]`)
        .map((routine: any) => routine.key);
    const blockers = [];
...
export function assertM5V2CloneCutoverApplyAllowed(audit: any, confirmation: any) {
    ...
    if (audit?.blockers?.length || audit?.applySupported !== true) {
        const codes = (audit?.blockers || []).map((item: any) => item.code).join(', ');
        throw new Error(`M5 v2 clone cutover 保持 dry-run：${codes || '安全实现尚未完成'}`);
    }
    return true;
}
export async function dryRunM5V2CloneCutover({ definition, adapter = new FakePaperclipAdapter(), legacyCampaignCaseId, bindings = {}, budgetCents = 625, now = () => new Date(), }: any = {}) {
...
export async function applyM5V2CloneCutover({ definition, adapter, legacyPipelineId, legacyCampaignCaseId, backup, bindings = {}, budgetCents, confirmation, now = () => new Date(), }: any = {}) {
...
export async function rollbackM5V2CloneDraft({ adapter, v2CampaignCaseId, confirmation, now = () => new Date(), }: any = {}) {
    if (confirmation !== M5_V2_CLONE_CUTOVER_CONFIRMATION) {
    ...
    const descendants = (await adapter.listPipelineCases(current.pipelineId))
        .filter((item: any) => item.parentCaseId === current.id);
...
async function executeCloneCutover({ adapter, definition, targetDefinition, bootstrap, legacyCampaignCaseId, now, mode, audit = null, }: any) {
    const targetPipelineId = bootstrap.operations
        .find((item: any) => item.type === 'pipeline')?.id;
    if (!targetPipelineId)
        throw new Error('clone cutover 没有取得 v2 Pipeline ID');
    const targetTrigger = bootstrap.operations.find((item: any) => item.type === 'routine-trigger');
    if (targetTrigger?.enabled !== false)
        throw new Error('clone cutover v2 Cron 必须保持关闭');
    ...
    if (v2Draft.fields?.campaignGrant?.status !== 'draft'
...
export async function verifyGzipBackupReference(reference: any) {
    ...
    await pipeline(createReadStream(reference), createGunzip(), new Writable({
