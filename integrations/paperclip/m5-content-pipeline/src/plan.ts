import { validateBudgetPayload, validateCompiledStage, validateDefinition, validateGoalPayload, validateProjectPayload, validateRoutinePayload, validateTriggerPayload, } from './validate.ts';
const marker = (kind: any, key: any) => `[agent-army:m5:${kind}:${key}]`;
...
function collectRequiredAgentKeys(definition: any) {
    return [...new Set([
            ...(definition?.stages ?? []).map((stage: any) => stage.owner),
            ...PARALLEL_ROUTINE_SPECS.map(([, owner]: any) => owner),
        ])];
}
export function listM5RequiredAgentKeys(definition: any) {
    return collectRequiredAgentKeys(definition);
}
export function buildBootstrapPlan(definition: any, bindings: any = {}) {
    ...
    const routineIdentityMarker = (key: any) => resourceNamespace
        ? `[agent-army:m5:deployment:${resourceNamespace}:routine:${key}]`
        : marker('routine', key);
    const routineDescriptionMarkers = (key: any) => resourceNamespace
    ...
    const dailyControllerUrl = String(bindings.dailyControllerUrl || 'http://127.0.0.1:4321/api/paperclip/m5-daily-heartbeat').trim();
    const metricsControllerUrl = String(bindings.metricsControllerUrl || 'http://127.0.0.1:4321/api/paperclip/m5-metrics-heartbeat').trim();
    const publisherControllerUrl = String(bindings.publisherControllerUrl || 'http://127.0.0.1:4321/api/paperclip/m5-publisher-heartbeat').trim();
    const retrospectiveControllerUrl = String(bindings.retrospectiveControllerUrl || 'http://127.0.0.1:4321/api/paperclip/m5-retrospective-heartbeat').trim();
    const learningControllerUrl = String(bindings.learningControllerUrl || 'http://127.0.0.1:4321/api/paperclip/m5-learning-heartbeat').trim();
    const parallelControllerUrl = String(bindings.parallelControllerUrl || 'http://127.0.0.1:4321/api/paperclip/m5-parallel-heartbeat').trim();
    const stageRoutines = valid.stages
        .filter((stage: any) => stage.routineKey)
        .map((stage: any) => {
        ...
        } as Record<string, any>)[stage.routineKey] || null;
    ...
    const parallelRoutines = PARALLEL_ROUTINE_SPECS.map(([key, owner, title, boundary]: any) => ({
        key,
    ...
    const stages = valid.stages.map((stage: any, position: any) => {
    ...
    const businessStages = valid.stages.filter((stage: any) => stage.kind !== 'cancelled');
    const transitions = businessStages.slice(0, -1).map((stage: any, index: any) => ({
        fromStageKey: stage.key,
        toStageKey: businessStages[index + 1].key,
        label: '推进',
    })).filter((edge: any) => !(edge.fromStageKey === 'script' && edge.toStageKey === 'render'));
    transitions.push({ fromStageKey: 'script', toStageKey: 'parallel_join_gate', label: '脚本完成，等待配音和其余并行产物' }, { fromStageKey: 'parallel_join_gate', toStageKey: 'render', label: '五分支汇聚完成' }, { fromStageKey: 'retrospective', toStageKey: 'done', label: '样本不足，结束本次复盘' });
    for (const stage of valid.stages.filter((item: any) => item.review)) {
        const review = stage.review!;
        for (const [decision, toStageKey] of Object.entries({
            approve: review.approveTo,
            reject: review.rejectTo,
            request_changes: review.requestChangesTo,
        })) {
            if (!transitions.some((edge: any) => edge.fromStageKey === stage.key && edge.toStageKey === toStageKey)) {
    ...
    if (projectId)
        validateBudgetPayload({ ...budgetPayload, amount: 1 });
    return {
        version: 1,
        sourcePaperclipVersion: valid.paperclipVersion,
        unresolved: {
            agentKeys: requiredAgentKeys.filter((key: any) => !agentIds[key]),
            routineKeys: routines.map((item: any) => item.key).filter((key: any) => !routineIds[key]),
            goalId: !goalId,
            projectId: !projectId,
        },
        resources: {
...
function stageBoundaryInstruction(stageKey: any) {
    if (stageKey === 'render') {
