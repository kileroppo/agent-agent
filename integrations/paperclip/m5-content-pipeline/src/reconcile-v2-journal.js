...
export function logicalTransitions(pipeline) {
...
export function transitionSetsEqual(left, right) {
...
export function transitionsHash(transitions) {
  return createHash('sha256')
    .update(canonicalTransitions(normalizeTransitions(transitions)))
    .digest('hex');
}

export function normalizeRoutinePayload(payload) {
...
export function assertRollbackSnapshot(snapshot, companyId) {
  if (
    snapshot?.schemaVersion !== 'agent.army/m5-existing-v2-rollback/v2'
...
export function progressOperation(now, step, operation) {
  return {
