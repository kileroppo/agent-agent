...
export function platformCaseKey(campaignId, date, platform, contentVersion = 'v1') {
  if (!campaignId || campaignId.includes(':')) throw new Error('campaignId 必须非空且不能包含冒号');
  assertIsoDate(date);
  if (!PLATFORMS.includes(platform)) throw new Error(`不支持的平台: ${platform}`);
  if (!/^v[1-9]\d*$/.test(contentVersion)) throw new Error(`无效内容版本: ${contentVersion}`);
  return `${campaignId}:${date}:${platform}:${contentVersion}`;
}

export function buildParallelWorkCaseBatch({
  campaignId,
  scheduledDate,
  contentVersion = 'v1',
}) {
  ...
  const branches = PARALLEL_WORK_BRANCHES.map((branch) => {
  ...
  join.blockedByCaseKeys = branches.map((item) => item.caseKey);
  join.fields.parallelJoin.branchCaseKeys = [...join.blockedByCaseKeys];
  join.fields.parallelJoin.requiredWorkProducts = branches.map(
...
export function buildCampaignCaseBatch({
  campaignId,
  startDate,
  themes,
  ...
  if (!Array.isArray(themes) || themes.length !== 7 || themes.some((theme) => !String(theme).trim())) {
  ...
  const platformCases = days.flatMap((day) => PLATFORMS.map((platform) => ({
...
export function assertReviewDecision(definition, stageKey, decision) {
  const stage = definition.stages.find((item) => item.key === stageKey);
  if (!stage || stage.kind !== 'review') throw new Error(`不是 review 阶段: ${stageKey}`);
  const destinations = {
    approve: stage.review.approveTo,
    reject: stage.review.rejectTo,
    request_changes: stage.review.requestChangesTo,
  };
  if (!Object.hasOwn(destinations, decision)) throw new Error(`不支持的审核决定: ${decision}`);
  return destinations[decision];
}

export async function ingestCampaignCaseBatch(adapter, pipelineId, batch) {
...
export async function ingestParallelWorkCaseBatch(adapter, pipelineId, batch, dayCase) {
  ...
  const branchPayloads = batch.branches.map((item) => ({
  ...
  const branches = typeof adapter.ingestCases === 'function'
    ...
    : await Promise.all(branchPayloads.map((payload) => adapter.ingestCase(pipelineId, payload)));
  branches.forEach((item, index) => {
    assertIngestedCase(item, batch.branches[index].caseKey, join.id);
  });

  const blockedByCaseIds = branches.map((item) => item.id);
...
export async function ingestCampaignDraftCase(adapter, pipelineId, batch) {
  if (!adapter?.ingestCase || !pipelineId) throw new Error('adapter 和 pipelineId 必填');
...
export async function ingestCampaignExecutionCases(adapter, pipelineId, batch, parent) {
  ...
  return {
    days: batch.days.map((item) => createdByLogicalId.get(item.logicalId)),
    platformCases: batch.platformCases.map((item) => createdByLogicalId.get(item.logicalId)),
  };
}
...
function assertParallelWorkBatch(batch) {
  ...
  const caseKeys = batch.branches.map((item) => item.caseKey);
  if (
    new Set(caseKeys).size !== 5
    || !batch.branches.every((item) => (
      item.parentLogicalId === batch.join?.logicalId
      && item.stageKey === 'draft'
  ...
  if (
    batch.join?.parentLogicalId !== batch.dayLogicalId
...
function assertIngestedCase(item, expectedCaseKey, expectedParentCaseId) {
  if (
    !item?.id
