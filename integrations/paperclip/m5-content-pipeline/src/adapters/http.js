...
export class HttpPaperclipAdapter {
  ...
  async findByMarker(type, marker) {
    ...
    const rows = await this.request('GET', paths[type]);
    return rows.find((item) =>
      item.description?.includes(marker) || item.key === marker,
    ) ?? null;
  }
  ...
  async create(type, payload) {
    ...
    return this.request('POST', paths[type], payload);
  }
  ...
  async ensureSystemAgent(payload) {
    ...
    const coreMatchesDesired = [
      ...
      'adapterType',
      'budgetMonthlyCents',
    ].every((key) => structurallyEqual(current[key] ?? null, payload[key] ?? null))
      ...
      && current.status === 'idle';
    const permissionsMatch = structurallyEqual(
      current.permissions || {},
      payload.permissions || {},
    );
  ...
  async reconcilePipeline(pipeline, payload) {
    ...
    const byKey = new Map((detail.stages ?? []).map((stage) => [stage.key, stage]));
    for (const declared of payload.stages) {
      ...
      if (!existing) {
        ...
        await this.request(
          'PATCH',
          `/api/pipelines/${pipeline.id}/stages/${existing.id}`,
          declared,
        );
        updated = true;
      }
    }
    if (updated) detail = await this.request('GET', `/api/pipelines/${pipeline.id}`);
  ...
  async ensureRoutineTrigger(routine, payload) {
    const detail = await this.request('GET', `/api/routines/${routine.id}`);
    const existing = detail.triggers?.find((item) =>
      item.kind === payload.kind && item.label === payload.label,
    );
  ...
  async getProjectDailyTrigger(projectId) {
    const routines = await this.request(
      'GET',
      `/api/companies/${encodeURIComponent(this.companyId)}/routines`,
    );
    ...
    const triggers = (detail.triggers || []).filter((item) => item.kind === 'schedule');
  ...
  async listPipelineCases(pipelineId) {
    const result = await this.request(
      'GET',
      `/api/pipelines/${encodeURIComponent(pipelineId)}/cases`,
    );
    const rows = Array.isArray(result) ? result : Array.isArray(result?.items) ? result.items : [];
    return rows.map((item) => item?.case ?? item);
  }
  ...
  async runParallelRoutine({ branch, routineKey, idempotencyKey }) {
    const routine = await this.findByMarker(
      'routine',
      `[agent-army:m5:routine:${routineKey}]`,
    );
    if (!routine?.id) throw new Error(`M5 并行分支 Routine 不存在: ${routineKey}`);
  ...
  async linkCaseIssue(caseId, issueId, role = 'automation') {
    const existing = await this.listCaseIssueLinks(caseId);
    const rows = Array.isArray(existing) ? existing : existing?.items || [];
    const match = rows.find((item) => (item?.issue?.id || item?.issueId) === issueId);
    if (match) return match;
    return this.request(
      'POST',
      `/api/cases/${encodeURIComponent(caseId)}/issue-links`,
      { issueId, role },
    );
  }

  async completeParallelGateIssues(caseId, comment) {
    ...
    await Promise.all(targets.map((issue) => this.request(
      'PATCH',
