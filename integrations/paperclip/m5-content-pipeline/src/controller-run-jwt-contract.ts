...
export const CONTROLLER_RUN_JWT_APPLY_CONFIRMATION = 'I_ACCEPT_M5_CONTROLLER_RUN_JWT_APPLY';
export const CONTROLLER_RUN_JWT_ROLLBACK_CONFIRMATION = 'I_ACCEPT_M5_CONTROLLER_RUN_JWT_ROLLBACK';
export const CONTROLLER_RUN_JWT_SNAPSHOT_SCHEMA = 'agent.army/m5-controller-run-jwt-snapshot/v1';
...
export class M5ControllerRunJwtCutoverError extends Error {
    ...
    readonly rollbackErrors: any[];

    constructor(message: any, { recoveryRequired = false, rollbackErrors = [] }: any = {}) {
        super(message);
...
export class PaperclipControllerClient {
    ...
    constructor({ apiBase = PAPERCLIP_CONTROLLER_API_BASE, fetchImpl = fetch, }: any = {}) {
        if (canonicalOrigin(apiBase) !== PAPERCLIP_CONTROLLER_API_BASE) {
            throw cutoverError(`控制器部署只允许 ${PAPERCLIP_CONTROLLER_API_BASE}。`);
        }
    ...
    async getController(id: any) {
        return this.#request('GET', `/api/agents/${id}`);
    }
    async updateController(id: any, adapterConfig: any) {
        return this.#request('PATCH', `/api/agents/${id}`, { adapterConfig });
    }
    async #request(method: any, pathname: any, body: any = undefined) {
        ...
        if (!response.ok) {
            throw cutoverError(`Paperclip ${method} ${pathname} 失败：HTTP ${response.status}。`);
        }
        return parsed;
    }
}
function validateController(current: any, declared: any) {
    ...
    assertExactKeys(adapterConfig, ['url', 'forwardRunJwt'], `${declared.key} adapterConfig`, { optional: ['forwardRunJwt'] });
    if (adapterConfig.url !== declared.url) {
        throw cutoverError(`${declared.key} 控制器 URL 不匹配固定 loopback 路径。`);
    }
    if ('forwardRunJwt' in adapterConfig
...
function targetAdapterConfig(item: any) {
    return { ...structuredClone(item.adapterConfig), forwardRunJwt: true };
}
function configSha(adapterType: any, adapterConfig: any) {
    return sha256(canonicalJson({ adapterType, adapterConfig }));
}
async function assertPaperclipVersion(client: any) {
    const version = await client.getVersion();
    if (version !== PAPERCLIP_CONTROLLER_CUTOVER_VERSION) {
        throw cutoverError(`只允许 Paperclip ${PAPERCLIP_CONTROLLER_CUTOVER_VERSION}，当前为 ${version || 'unknown'}。`);
    }
}
function validateSnapshotController(item: any) {
    ...
    const declared = M5_RUN_JWT_CONTROLLERS.find(({ id }: any) => id === item.id);
    ...
    const targetConfigSha256 = configSha(item.adapterType, targetAdapterConfig({ adapterConfig: normalized.adapterConfig }));
    if (item.configSha256 !== configSha256
...
function declarationFor(item: any) {
    const declared = M5_RUN_JWT_CONTROLLERS.find(({ id }: any) => id === item.id);
...
function assertNoSecretKeys(value: any, pathValue: any = 'snapshot') {
    if (Array.isArray(value)) {
        value.forEach((item: any, index: any) => assertNoSecretKeys(item, `${pathValue}[${index}]`));
        return;
    }
    if (!isRecord(value))
        return;
...
function assertExactKeys(value: any, allowed: any, label: any, { optional = [] }: any = {}) {
    if (!isRecord(value))
        throw cutoverError(`${label} 必须是对象。`);
    const allowedSet = new Set(allowed);
    const unknown = Object.keys(value).filter((key: any) => !allowedSet.has(key));
    if (unknown.length)
        throw cutoverError(`${label} 包含未知字段。`);
    const optionalSet = new Set(optional);
    const missing = allowed.filter((key: any) => !optionalSet.has(key) && !(key in value));
    if (missing.length)
        throw cutoverError(`${label} 缺少必填字段。`);
}
function canonicalOrigin(value: any) {
    try {
...
function canonicalJson(value: any): string {
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            throw cutoverError('快照包含非有限数字。');
        return JSON.stringify(value);
    }
    if (Array.isArray(value)) {
        return `[${value.map((item: any) => canonicalJson(item)).join(',')}]`;
    }
    if (isRecord(value)) {
        return `{${Object.keys(value)
            .sort()
            .map((key: any) => `${JSON.stringify(key)}:${canonicalJson(value[key])}`)
            .join(',')}}`;
    }
    throw cutoverError('快照包含不支持的值。');
}
function validIsoDate(now: any) {
    const value = now();
...
function validIsoDateValue(value: any) {
...
function assertClient(client: any) {
    if (!client
...
function cutoverError(message: any, options: any = {}) {
