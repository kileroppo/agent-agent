...
import {
  ...
  CONTROLLER_RUN_JWT_SNAPSHOT_SCHEMA,
  M5_RUN_JWT_CONTROLLERS,
  PAPERCLIP_CONTROLLER_API_BASE,
  PAPERCLIP_CONTROLLER_CUTOVER_VERSION,
  controllerRunJwtContract,
} from './controller-run-jwt-contract.js';
import { controllerRunJwtSnapshotStore } from './controller-run-jwt-snapshot-store.js';
...
const {
  ...
  targetAdapterConfig,
  configSha,
} = controllerRunJwtContract.controller;
const {
  canonicalJson,
  validIsoDate,
  sha256,
} = controllerRunJwtContract.snapshot;
const { create:cutoverError, safeMessage } = controllerRunJwtContract.errors;
const {
  write:writePrivateSnapshot,
  read:readTrustedSnapshot,
} = controllerRunJwtSnapshotStore;

async function snapshotControllerRunJwtCutover({
  client,
  snapshotPath,
  now = () => new Date(),
  fileSystem = fs,
...
async function applyControllerRunJwtCutover({
  client,
  snapshotPath,
  confirmation,
  fileSystem = fs,
...
async function rollbackControllerRunJwtCutover({
  client,
  snapshotPath,
  confirmation,
  fileSystem = fs,
