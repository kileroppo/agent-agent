import { APPLY_CONFIRMATION, M5_EXISTING_V2_RECONCILE_CONFIRMATION, M5_EXISTING_V2_RECOVERY_CONFIRMATION, M5_V2_CLONE_CUTOVER_CONFIRMATION, HttpPaperclipAdapter, applyExistingM5V2Reconcile, applyM5V2CloneCutover, applyBootstrap, buildM5V2CloneDefinition, createM5V2ProgressJournalAppender, defaultDefinition, dryRunBootstrap, inspectM5V2CloneCutover, inspectM5V2Migration, inspectExistingM5V2Reconcile, recoverExistingM5V2Reconcile, validateDefinition, verifyGzipBackupReference, writeM5V2RollbackSnapshotFile, } from './index.ts';
import { readFile } from 'node:fs/promises';
import { isAbsolute } from 'node:path';
import { resolveLiveAgentBindings } from './live-bindings.ts';
...
if (command === 'validate') {
    ...
    console.log(JSON.stringify({
        valid: true,
        controlStages: result.stages.filter((stage: any) => controlStageKeys.has(stage.key)).length,
        businessStages: result.stages.filter((stage: any) => !controlStageKeys.has(stage.key) && !['done', 'cancelled'].includes(stage.kind)).length,
        successTerminalStages: result.stages.filter((stage: any) => stage.kind === 'done').length,
        systemTerminalStages: result.stages.filter((stage: any) => stage.kind === 'cancelled').length,
    ...
    if (!apiBase || !companyId || !legacyPipelineId || !backupReference) {
        throw new Error('migrate-v2-dry-run 需要 --api-base、--company-id、--legacy-pipeline-id 和 --backup');
    }
    ...
    const backupReference = option('--backup');
    if (!apiBase || !companyId || !legacyPipelineId || !backupReference) {
        throw new Error('clone-cutover-v2-dry-run 需要 --api-base、--company-id、--legacy-pipeline-id 和 --backup');
    }
    const adapter = new HttpPaperclipAdapter({ apiBase, companyId });
    const backup = await verifyGzipBackupReference(backupReference);
    ...
    console.log(JSON.stringify(audit, null, 2));
    if (!audit.legacyPreconditionsPassed || !audit.applySupported)
        process.exitCode = 3;
}
    ...
    if (!apiBase
        ...
        || !backupReference) {
        throw new Error('clone-cutover-v2-apply 需要 API、公司、v1 Pipeline/草案 Case、备份和预算参数');
    }
    const adapter = new HttpPaperclipAdapter({ apiBase, companyId });
    const backup = await verifyGzipBackupReference(backupReference);
    ...
    if (!apiBase || !companyId || !pipelineId || !projectId) {
        throw new Error('reconcile-existing-v2-dry-run 需要 --api-base、--company-id、--pipeline-id 和 --project-id');
    }
    const adapter = new HttpPaperclipAdapter({ apiBase, companyId });
    ...
    if (!apiBase
        ...
        || !rollbackOutput
        || !progressOutput) {
        throw new Error('reconcile-existing-v2-apply 需要 API、公司、v2 Pipeline/Project 和绝对回滚快照路径');
    }
    ...
    try {
        const result = await applyExistingM5V2Reconcile({
            ...
            projectId,
            confirmation,
            writeRollbackSnapshot: (snapshot: any) => writeM5V2RollbackSnapshotFile(rollbackOutput, snapshot),
    ...
    if (!apiBase || !companyId || !snapshotPath || !progressOutput) {
        throw new Error('reconcile-existing-v2-recover 需要 API、公司、回滚快照和进度 journal');
    }
    ...
    const adapter = new HttpPaperclipAdapter({ apiBase, companyId });
    const progress = createM5V2ProgressJournalAppender(progressOutput, { append: true });
    try {
