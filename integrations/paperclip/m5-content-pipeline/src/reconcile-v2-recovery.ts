import { assertDraftCampaign, check, rows, unwrap, } from './reconcile-v2-inspection.ts';
import { M5_EXISTING_V2_RECOVERY_CONFIRMATION, M5V2RecoveryRequiredError, assertRollbackSnapshot, logicalTransitions, normalizeRoutinePayload, payloadHash, progressOperation, safeErrorMessage, transitionsHash, } from './reconcile-v2-journal.ts';
export async function recoverExistingM5V2Reconcile({ adapter, snapshot, confirmation, appendRecoveryProgress, now = () => new Date(), }: any = {}) {
    ...
    if (!initial.safeToRecover) {
        const error = new Error(`M5 v2 恢复遇到未知漂移: ${initial.blockers.map((item: any) => item.code).join(', ')}`);
        (error as any).code = 'M5_V2_RECOVERY_BLOCKED';
        (error as any).recovery = initial;
        throw error;
    }
    const needsRecovery = Object.values(initial.states).some((state: any) => state === 'target');
    if (!needsRecovery) {
        return {
            mode: 'recover',
    ...
    try {
        if (initial.states.transitions === 'target') {
            ...
            const currentDocument = await adapter.request('GET', `/api/pipelines/${encodeURIComponent(snapshot.pipelineId)}`);
            const current = unwrap(currentDocument, 'pipeline');
            if (transitionsHash(logicalTransitions(current))
                !== snapshot.pipelineTransitions.targetTransitionsSha256) {
                throw new Error('恢复前 transitions 已不再是本次 target 状态');
            }
            await adapter.setPipelineTransitions(snapshot.pipelineId, snapshot.pipelineTransitions.oldTransitions);
            const operation = {
                method: 'PUT',
        ...
        if (initial.states.visual === 'target') {
            ...
            const currentDocument = await adapter.request('GET', `/api/routines/${encodeURIComponent(initial.visual.id)}`);
            const current = unwrap(currentDocument, 'routine');
            if (current.latestRevisionId !== initial.visual.latestRevisionId
            ...
            const resource = await adapter.request('PATCH', `/api/routines/${encodeURIComponent(current.id)}`, { status: 'archived', baseRevisionId: current.latestRevisionId });
            const operation = {
                method: 'PATCH',
                resource: 'routine',
        ...
        if (initial.states.assets === 'target') {
            ...
            const currentDocument = await adapter.request('GET', `/api/routines/${encodeURIComponent(snapshot.assetsRoutine.id)}`);
            const current = unwrap(currentDocument, 'routine');
            if (current.latestRevisionId !== initial.assets.latestRevisionId
                || payloadHash(normalizeRoutinePayload(current))
                    !== snapshot.assetsRoutine.targetPayloadSha256) {
                throw new Error('恢复前 assets Routine revision 或 payload 已变化');
            }
            const resource = await adapter.request('PATCH', `/api/routines/${encodeURIComponent(current.id)}`, {
        ...
        if (!verification.safeToRecover
            || Object.values(verification.states).some((state: any) => state !== 'old')) {
        ...
        throw new M5V2RecoveryRequiredError(`M5 v2 恢复未完成: ${recovery.cause}`, recovery);
    }
}
async function inspectRecoveryState({ adapter, snapshot }: any) {
    const [pipelineDocument, projectDocument, routineDocument, caseDocument,] = await Promise.all([
        ...
        adapter.request('GET', `/api/pipelines/${encodeURIComponent(snapshot.pipelineId)}/cases`),
    ]);
    ...
    const routines = rows(routineDocument)
        .filter((item: any) => item.projectId === snapshot.projectId);
    const cases = rows(caseDocument);
    const blockers: any[] = [];
    check(blockers, pipeline?.id === snapshot.pipelineId
        && pipeline?.key === snapshot.pipelineKey
        && pipeline?.projectId === snapshot.projectId
        && project?.id === snapshot.projectId, 'recovery_target_identity_mismatch', '恢复目标 Pipeline/Project identity 不匹配');
    assertDraftCampaign({
        blockers,
        cases,
    ...
    const dailyMatches = routines.filter((item: any) => String(item.description || '').includes(`[agent-army:m5:deployment:${snapshot.pipelineKey}:routine:m5-daily-campaign]`));
    const daily = dailyMatches.length === 1
        ? unwrap(await adapter.request('GET', `/api/routines/${encodeURIComponent(dailyMatches[0].id)}`), 'routine')
        : null;
    const scheduleTriggers = (daily?.triggers ?? [])
        .filter((item: any) => item.kind === 'schedule');
    check(blockers, (daily?.triggers ?? []).length === 1
        && scheduleTriggers.length === 1
        && scheduleTriggers[0].enabled === false, 'recovery_daily_trigger_not_off', '恢复时每日 Trigger 必须保持唯一且关闭');
    const assetsDocument = await adapter.request('GET', `/api/routines/${encodeURIComponent(snapshot.assetsRoutine.id)}`);
    ...
    const assetsState = assetsHash === snapshot.assetsRoutine.oldPayloadSha256
        ...
        : assetsHash === snapshot.assetsRoutine.targetPayloadSha256
            ? 'target'
            : 'unexpected';
    const visualMatches = routines.filter((item: any) => String(item.description || '').includes(snapshot.visualRoutine.marker));
    let visual = null;
    ...
    if (snapshot.visualRoutine.priorState === 'present') {
        if (visualMatches.length === 1
            ...
            visual = unwrap(await adapter.request('GET', `/api/routines/${encodeURIComponent(visualMatches[0].id)}`), 'routine');
    ...
    const transitionState = transitionHash === snapshot.pipelineTransitions.oldTransitionsSha256
        ? 'old'
        : transitionHash === snapshot.pipelineTransitions.targetTransitionsSha256
            ? 'target'
            : 'unexpected';
    check(blockers, assetsState !== 'unexpected', 'recovery_assets_unexpected_drift', 'assets 既不是 old 也不是本次 target');
    check(blockers, visualState !== 'unexpected', 'recovery_visual_unexpected_drift', 'visual 既不是 old/archived 也不是本次 target');
    check(blockers, transitionState !== 'unexpected', 'recovery_transitions_unexpected_drift', 'transitions 既不是 old 也不是本次 target');
    return {
