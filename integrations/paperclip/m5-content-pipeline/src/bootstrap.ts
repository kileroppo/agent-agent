...
async function ensure(adapter: any, type: any, key: any, marker: any, payload: any) {
...
export async function dryRunBootstrap({ definition, bindings = {}, adapter = new FakePaperclipAdapter(), budgetCents = 625, }: any = {}) {
...
export async function applyBootstrap({ definition, bindings = {}, adapter, budgetCents, confirmLiveWrite, }: any = {}) {
    ...
    const unsafeScheduledVariables = preflight.resources.scheduleRoutine.payload.variables
        .filter((variable: any) => variable.required && variable.defaultValue == null);
    if (unsafeScheduledVariables.length > 0) {
        throw new Error(`live apply 拒绝创建无法调度的Routine变量: ${unsafeScheduledVariables.map((item: any) => item.name).join(', ')}`);
    }
    return executeBootstrap({ definition, bindings, adapter, budgetCents, mode: 'apply' });
}
async function executeBootstrap({ definition, bindings, adapter, budgetCents, mode }: any) {
    ...
    const retrospectiveController = await adapter.ensureSystemAgent(plan.resources.retrospectiveController.payload);
    operations.push({
        type: 'agent',
    ...
    const learningController = await adapter.ensureSystemAgent(plan.resources.learningController.payload);
    operations.push({
        type: 'agent',
    ...
    const parallelController = await adapter.ensureSystemAgent(plan.resources.parallelController.payload);
    operations.push({
        type: 'agent',
    ...
    const goalResult = await ensure(adapter, 'goal', plan.resources.goal.key, plan.resources.goal.marker, plan.resources.goal.payload);
    ...
    const projectResult = await ensure(adapter, 'project', plan.resources.project.key, plan.resources.project.marker, plan.resources.project.payload);
    ...
    const pipelineResult = await ensure(adapter, 'pipeline', plan.resources.pipeline.payload.key, pipelineMarker, plan.resources.pipeline.payload);
