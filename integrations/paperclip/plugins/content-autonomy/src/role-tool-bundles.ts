...
export function validateExactAgentToolPolicy(config: any) {
    const bindings = plainObject(config?.agentRoleBindings);
    const grants = plainObject(config?.agentToolGrants);
    const errors = [];
    if (!bindings)
        errors.push('缺少 agentRoleBindings 岗位 UUID 绑定。');
    if (!grants)
        errors.push('缺少 agentToolGrants 岗位工具授权。');
    if (!bindings || !grants)
    ...
    const boundAgentIds = roleKeys.map((role: any) => String(bindings[role] || '').trim());
    if (boundAgentIds.some((agentId: any) => !agentId)
    ...
    for (const role of M5_CONTENT_ROLES) {
        ...
        const expected = (M5_ROLE_TOOL_BUNDLES as Record<string, readonly string[]>)[role];
        if (!Array.isArray(actual)
            || actual.some((tool: any) => typeof tool !== 'string')
...
function sameSet(left: any, right: any) {
    return left.length === right.length
        && left.every((value: any) => right.includes(value));
}
