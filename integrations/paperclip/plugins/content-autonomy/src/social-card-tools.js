...
const DEFAULT_RENDERER_SCRIPT = fileURLToPath(new URL(
  '../scripts/render-social-card-package.mjs',
  import.meta.url,
));
const CARD_KINDS = Object.freeze(['cover', 'evidence', 'checklist']);

export async function renderM5SocialCardPackage(ctx, params, run, options = {}) {
  const validation = validateM5SocialCardProps(params?.props);
  if (!validation.passed) throw coded('social_card_props_invalid', validation.errors.join(' '));
  ...
  try {
    ...
    const manifestPath = path.join(temporary, 'social-card-render-manifest.json');
    ...
    return {
      ...
      data:{
        ...
        manifestPath:path.posix.join(output.relative, 'social-card-render-manifest.json'),
    ...
    if (error?.code && String(error.code).startsWith('social_card_')) throw error;
    throw coded('social_card_render_failed', `受控社交卡渲染失败：${String(error?.code || 'renderer_error')}。`);
  }
}

export function validateM5SocialCardProps(props) {
  const errors = [];
  if (!props || typeof props !== 'object' || Array.isArray(props)) {
    return { passed:false, errors:['社交卡 props 必须是对象。'] };
  }
  if (props.platform !== M5_PLATFORM_IDS.XIAOHONGSHU) errors.push('社交卡平台必须是 xiaohongshu。');
  if (!boundedText(props.title, 1, 40)) errors.push('社交卡标题长度必须是 1–40 个字符。');
  if (!boundedText(props.subtitle, 1, 120)) errors.push('社交卡副标题长度必须是 1–120 个字符。');
  if (!boundedText(props.sourceLabel, 1, 80)) errors.push('社交卡来源标签长度必须是 1–80 个字符。');
  if (!boundedText(props.rightsBasis, 1, 500)) errors.push('社交卡必须提供素材版权依据。');
  ...
  const ledgerPaths = ledger.map((item) => String(item?.relativePath || ''));
  if (
    ledger.length < 1
    || ledger.length > 12
    || new Set(ledgerPaths).size !== ledger.length
    || ledger.some((item) => !safeImagePath(item?.relativePath) || !sha256Value(item?.checksum))
  ) {
    errors.push('社交卡素材账本必须包含 1–12 个图片相对路径和 sha256。');
  }
  const assetPaths = new Set(ledger.map((item) => String(item.relativePath || '')));
  ...
  if (cards.length < 3 || cards.length > 9) {
    ...
    for (const [index, card] of cards.entries()) {
      ...
      ids.add(card?.id);
      if (!CARD_KINDS.includes(card?.kind)) errors.push(`第 ${index + 1} 页版式不在白名单。`);
      const limits = cardTextLimits(card?.kind);
      if (!boundedText(card?.headline, 1, limits.headline)) errors.push(`第 ${index + 1} 页标题长度无效。`);
      if (!boundedText(card?.body, 1, limits.body)) errors.push(`第 ${index + 1} 页正文长度无效。`);
      const bullets = Array.isArray(card?.bullets) ? card.bullets : [];
      if (bullets.length > limits.bullets || bullets.some((item) => !boundedText(item, 1, limits.bullet))) {
    ...
    if (cards[0]?.kind !== 'cover') errors.push('社交卡第一页必须是 cover。');
  }
  return { passed:errors.length === 0, errors };
}

async function verifiedRenderedCards(directory, manifest, requestedCards) {
  if (
...
async function workspaceRoot(ctx, companyId, writable) {
...
async function newWorkspaceDirectory(root, relativePath) {
  const relative = safeRelativePath(relativePath);
  const candidate = path.resolve(root, relative);
  if (!candidate.startsWith(`${root}${path.sep}`)) throw coded('path_escape', '社交卡输出路径逃逸工作区。');
  await fs.mkdir(path.dirname(candidate), { recursive:true });
  const parent = await fs.realpath(path.dirname(candidate));
  if (parent !== root && !parent.startsWith(`${root}${path.sep}`)) throw coded('symlink_escape', '社交卡输出目录逃逸工作区。');
...
async function hardenDirectory(directory) {
  ...
  for (const entry of await fs.readdir(directory, { withFileTypes:true })) {
    if (entry.isFile()) await fs.chmod(path.join(directory, entry.name), 0o600);
  }
}
...
function parseJson(bytes, code, message) {
  try { return JSON.parse(bytes.toString('utf8')); } catch { throw coded(code, message); }
}
...
function cardTextLimits(kind) {
  if (kind === 'cover') return { headline:14, body:60, bullets:3, bullet:18 };
  if (kind === 'evidence') return { headline:16, body:60, bullets:3, bullet:24 };
