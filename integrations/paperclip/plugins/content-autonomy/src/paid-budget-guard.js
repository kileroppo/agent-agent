...
import { verifySignedBudgetTicket } from './signed-budget-ticket.js';
 ...
 */
export function paidBudgetCheckerFromContext(ctx) {
  const checker = ctx?.paperclipBudgets?.reservePaidToolBudget;
  if (typeof checker === 'function') return checker.bind(ctx.paperclipBudgets);
...
export async function reservePaidToolBudget({
  checker,
  run,
  actionId,
  operation,
  ...
  const requestedCents = positiveInteger(
    maximumCostCents,
    'paid_budget_estimate_invalid',
    '付费工具缺少有效的最大费用估算，Provider 未调用。',
  );
  if (typeof checker !== 'function') {
    throw coded(
      'paid_budget_checker_unavailable',
      'Paperclip 未向插件注入可信预算检查器，Provider 未调用。',
    );
  }
  ...
  try {
    ...
    throw coded(
      'paid_budget_check_failed',
      'Paperclip 预算检查失败，Provider 未调用。',
    );
  }
  ...
  const reservationId = validUuid(
    decision.reservationId,
    'paid_budget_reservation_invalid',
    'Paperclip 未返回有效的原子预算预留，Provider 未调用。',
  );
  const reservedCents = positiveInteger(
    decision.reservedCents,
    'paid_budget_reservation_invalid',
    'Paperclip 预算预留金额无效，Provider 未调用。',
  );
  ...
  for (const [scopeType, idField] of REQUIRED_SCOPES) {
    ...
    if (
      ...
      throw coded(
        ...
        `Paperclip ${scopeType} 预算回执缺失、不匹配或余额不足，Provider 未调用。`,
      );
    }
  }
...
function trustedRunContext(run) {
  return {
...
function assertDecisionContext(decision, context) {
  if (
    !decision
    ...
    throw coded(
      'paid_budget_context_mismatch',
      'Paperclip 预算回执与当前公司、岗位、Project 或 Run 不匹配，Provider 未调用。',
    );
  }
}
...
function positiveInteger(value, code, message) {
  ...
  if (!Number.isSafeInteger(amount) || amount <= 0) throw coded(code, message);
  return amount;
}

function validUuid(value, code, message) {
  const id = String(value || '').trim();
  if (!UUID.test(id)) throw coded(code, message);
  return id;
}
