...
import { coded, sha256 } from './policy.js';
...
export function createSignedBudgetTicket({
  privateKey,
  run,
  actionId,
  operation,
  ...
  if (!Number.isInteger(issuedAt)) throw new Error('预算票据签发时间无效。');
...
export function verifySignedBudgetTicket({
  ...
  if (!encoded || !signature || extra) denied('预算票据格式无效。');
  let payload;
  try {
    if (!crypto.verify(
      null,
    ...
    if (error?.code === 'paid_budget_ticket_invalid') throw error;
    denied('预算票据无法验证。');
  }
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (payload[key] !== value) denied(`预算票据与当前 ${key} 不匹配。`);
  }
  ...
  return Object.freeze({
    ...
    scopes:Object.freeze(Object.fromEntries(scopes.map((item) => [
      item.scopeType,
      Object.freeze({
...
export function budgetTicketParametersChecksum(parameters) {
  return sha256(Buffer.from(stableJson(withoutTicket(parameters))));
}

function withoutTicket(value) {
...
function canonicalScopes(value, reservedCents) {
  const source = Array.isArray(value)
    ? Object.fromEntries(value.map((item) => [item?.scopeType, item]))
    : value;
  const definitions = [
    ['company', 'companyId'],
    ['agent', 'agentId'],
...
function validActionId(value) {
  ...
  if (!/^[A-Za-z0-9:_-]{8,160}$/.test(actionId)) throw new Error('预算票据 actionId 无效。');
  return actionId;
}

function positiveInteger(value, message) {
  const amount = Number(value);
  if (!Number.isSafeInteger(amount) || amount <= 0) throw new Error(message);
  return amount;
}
