...
const DEFAULT_RENDERER_SCRIPT = fileURLToPath(new URL('../../../../../apps/animated-chart/scripts/render-m5-controlled.mjs', import.meta.url));
export async function writeM5RenderProps(ctx: any, params: any, run: any) {
    const definition = (COMPOSITIONS as Record<string, any>)[params.composition];
    if (!definition)
        throw coded('remotion_composition_denied', '只允许 M5Master、M5Douyin 或 M5Xiaohongshu。');
    ...
    if (!validation.passed)
        throw coded('remotion_props_invalid', validation.errors.join(' '));
    ...
    if (!readBack.equals(encoded))
        throw coded('remotion_props_write_failed', 'Remotion props 写回校验失败。');
    return {
...
export async function renderM5Composition(ctx: any, params: any, _run: any, options: any = {}) {
    const definition = (COMPOSITIONS as Record<string, any>)[params.composition];
    if (!definition)
        throw coded('remotion_composition_denied', '只允许 M5Master、M5Douyin 或 M5Xiaohongshu。');
    ...
    try {
        ...
        if (!bytes.length)
            throw coded('remotion_render_empty', 'Remotion 没有生成视频数据。');
        ...
        if (error?.code && String(error.code).startsWith('remotion_'))
            throw error;
        throw coded('remotion_render_failed', `受控 Remotion 渲染失败：${String(error?.code || 'renderer_error')}。`);
    }
}
export async function validateSubtitleLayoutFromProps(ctx: any, params: any, run: any) {
...
export function validateM5RenderProps(props: any, composition: any) {
    const errors = [];
    const definition = (COMPOSITIONS as Record<string, any>)[composition];
    if (!definition)
        return { passed: false, errors: ['Composition 不在白名单。'], subtitleLayout: null };
    if (!props || typeof props !== 'object' || Array.isArray(props)) {
        return { passed: false, errors: ['Remotion props 必须是对象。'], subtitleLayout: null };
    }
    if (props.platform !== definition.platform)
        errors.push('props.platform 与 Composition 不匹配。');
    if (!boundedText(props.title, 1, 80))
        errors.push('标题长度必须是 1–80 个字符。');
    if (!boundedText(props.subtitle, 1, 120))
        errors.push('副标题长度必须是 1–120 个字符。');
    if (!boundedText(props.sourceLabel, 1, 80))
        errors.push('来源标签长度必须是 1–80 个字符。');
    const assetLedger = Array.isArray(props.assetLedger) ? props.assetLedger : [];
    if (assetLedger.length < 1
        || assetLedger.length > 12
        || assetLedger.some((asset: any) => !safeAssetPath(asset?.relativePath)
            || !/^sha256:[0-9a-f]{64}$/i.test(String(asset?.checksum || '')))) {
        errors.push('真实画面素材账本必须包含 1–12 个相对路径和 sha256。');
    }
    const assetPaths = new Set(assetLedger.map((asset: any) => String(asset.relativePath || '')));
    if (!safeAssetPath(props.coverSrc) || !assetPaths.has(props.coverSrc)) {
        errors.push('封面必须绑定真实画面素材账本。');
    }
    if (props.voiceoverSrc != null) {
        if (!safeAudioPath(props.voiceoverSrc)
    ...
    if (!subtitleLayout.passed)
        errors.push(...subtitleLayout.errors);
    return { passed: errors.length === 0, errors, subtitleLayout };
}
export function validateSubtitleLayout(captions: any) {
    const errors = [];
    ...
    for (const [index, caption] of captions.entries()) {
        ...
        if (caption.endFrame > M5_DURATION_FRAMES)
            errors.push(`第 ${index + 1} 条字幕超出固定成片范围。`);
        if (caption.startFrame < previousEnd)
            errors.push(`第 ${index + 1} 条字幕与上一条重叠。`);
        previousEnd = Number.isInteger(caption?.endFrame) ? caption.endFrame : previousEnd;
        if (!text.trim())
            errors.push(`第 ${index + 1} 条字幕为空。`);
        if (lines.length > 3)
            errors.push(`第 ${index + 1} 条字幕超过 3 行。`);
        if (lines.some((line: any) => displayUnits(line) > 28))
            errors.push(`第 ${index + 1} 条字幕单行超过安全宽度。`);
        if (displayUnits(text.replaceAll('\n', '')) > 72)
            errors.push(`第 ${index + 1} 条字幕总长度超过安全区域。`);
    }
...
async function validateReferencedAssets(root: any, props: any) {
    const references = [
        props.voiceoverSrc,
        props.coverSrc,
        ...(props.scenes || []).map((scene: any) => scene.imageSrc)
    ].filter(Boolean);
...
async function existingWorkspacePath(ctx: any, companyId: any, relativePath: any) {
    ...
    if (!absolute.startsWith(`${root}${path.sep}`))
        throw coded('symlink_escape', 'Remotion 输入路径逃逸工作区。');
...
async function writableWorkspacePath(ctx: any, companyId: any, relativePath: any) {
    ...
    if (!candidate.startsWith(`${root}${path.sep}`))
        throw coded('path_escape', 'Remotion 输出路径逃逸工作区。');
    await fs.mkdir(path.dirname(candidate), { recursive: true });
    const parent = await fs.realpath(path.dirname(candidate));
    if (parent !== root && !parent.startsWith(`${root}${path.sep}`))
        throw coded('symlink_escape', 'Remotion 输出目录逃逸工作区。');
...
function parseJson(text: any, code: any, message: any) {
    try {
...
function safeAssetPath(value: any) {
...
function safeAudioPath(value: any) {
    const text = String(value || '').trim().replaceAll('\\', '/');
    return Boolean(text)
        && !text.startsWith('/')
        && text.split('/').every((part: any) => part && part !== '.' && part !== '..')
...
function displayUnits(value: any) {
    return [...String(value || '')].reduce((sum: any, character: any) => sum + (/[\u0000-\u00ff]/.test(character) ? 0.55 : 1), 0);
}
