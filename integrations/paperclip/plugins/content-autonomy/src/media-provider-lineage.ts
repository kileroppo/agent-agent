...
import { paidActionStateKey } from './stepfun-tools.ts';
...
async function verifyProviderAction(ctx: any, params: any, run: any) {
    const keys = Object.keys(params || {}).sort();
    if (keys.join(',') !== 'actionId,companyId,costEventId,operation,runContext') {
        throw coded('provider_action_verify_input_denied', 'Provider action 只读核验只接受 Paperclip 注入的 companyId、actionId、costEventId、operation 和 runContext。');
    }
    const actionId = String(params.actionId || '');
    const costEventId = String(params.costEventId || '');
    const operation = String(params.operation || '');
    const expectedModel = (PROVIDER_MODELS as Record<string, string>)[operation];
    ...
    if (state?.state !== 'confirmed'
        ...
        throw coded('provider_action_unconfirmed', `StepFun ${operation} action 无法由原 Paperclip Run 的已确认插件状态证明。`);
    }
    return {
...
async function buildStepFunArtifactLineageFromConfirmedActions({ ctx, run, actionRefs, sources, lineage, }: any) {
    ...
    if (visionPath !== imagePath || visionChecksum !== imageChecksum) {
        throw coded('provider_vision_source_mismatch', 'StepFun 视觉 action 没有核验本次选用的生成图片。');
    }
    await Promise.all([
    ...
    const generated = nextSources?.aiGeneratedMedia?.find((item: any) => item?.ref === imagePath
    ...
    if (!generated
        ...
        throw coded('provider_artifact_binding_mismatch', '来源账本中的 StepFun 图片或旁白与 confirmed action 不一致。');
    }
    ...
    nextLineage.parents = [
        ...(Array.isArray(nextLineage.parents)
            ? nextLineage.parents.filter((item: any) => item?.kind !== parent.kind)
            : []),
        parent,
    ];
    return {
...
function confirmedActionRefs(value: any) {
    const refs = {
        image: String(value?.image || ''),
        vision: String(value?.vision || ''),
        tts: String(value?.tts || ''),
    };
    if (Object.values(refs).some((actionId: any) => !/^[A-Za-z0-9:_-]{8,160}$/.test(actionId))) {
        throw coded('provider_action_refs_invalid', '逐阶段 Provider 血缘必须同时提供有效的 image、vision、tts actionId。');
    }
...
async function confirmedProviderAction(ctx: any, run: any, actionId: any, operation: any) {
    const state = await ctx.state.get(paidActionStateKey(run.projectId, actionId));
    const data = state?.resultData;
    const record = data?.callRecord;
    ...
    if (state?.state !== 'confirmed'
        ...
        throw coded('provider_action_unconfirmed', `StepFun ${operation} action 无法由当前 Project 的插件状态证明已确认费用。`);
    }
    return state;
}
function providerActionSummary(state: any) {
    return {
...
async function assertWorkspaceChecksum(ctx: any, run: any, relativePath: any, expectedChecksum: any) {
...
function buildStepFunArtifactLineage({ ledger, ledgerPath, ledgerChecksum, themeId, sources, lineage, }: any) {
    if (ledger?.schemaVersion !== 'agent.army/stepfun-seven-theme-batch/v1'
    ...
    const theme = ledger.themes.find((item: any) => item?.id === themeId);
    if (!theme)
        throw coded('provider_theme_missing', 'Provider ledger 中不存在指定主题。');
    const selected = theme.candidates?.find((candidate: any) => candidate?.outputPath === theme.selectedPath
    ...
    if (!/^sha256:[a-f0-9]{64}$/.test(String(selected.promptChecksum || theme.promptChecksum || ''))
    ...
    const nextSources = structuredClone(sources);
    const generated = nextSources?.aiGeneratedMedia?.find((item: any) => item?.ref === selected.outputPath
    ...
    if (!generated
        ...
        throw coded('provider_artifact_binding_mismatch', '来源账本中的 StepFun 图片或旁白与 Provider ledger 不一致。');
    }
    Object.assign(generated, {
...
function requireConfirmedProviderAction(action: any, operation: any) {
