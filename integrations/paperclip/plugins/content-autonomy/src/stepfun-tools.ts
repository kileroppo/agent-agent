...
import { callRecord, costForImage, costForTts, costForVision, reportCostMetric } from './call-record.ts';
import { requirePaperclipSecretRef } from './secret-ref.ts';
import { DEFAULT_LEGACY_RATE_LIMIT_ACTIONS, PaidProviderActionProtocol, } from './paid-provider-action-protocol.ts';
export { paidActionStateKey, paidParameterChecksum } from './paid-provider-action-protocol.ts';
const MODELS = Object.freeze({
    vision: M5_STEPFUN_MODELS.vision,
    image: M5_STEPFUN_MODELS.image_generate,
    tts: M5_STEPFUN_MODELS.tts,
});
const VISION_MAX_INPUT_TOKENS = 128000;
...
export class StepFunContentTools {
    readonly ctx: any;
    readonly paidBudgetChecker: any;
    readonly inflight: Map<any, any>;
    readonly legacyRateLimitActions: any;
    readonly paidActions: PaidProviderActionProtocol;
    readonly rateLimit: Record<string, any>;

    constructor({ ctx, paidBudgetChecker = null, minRequestIntervalMs = DEFAULT_MIN_REQUEST_INTERVAL_MS, maxRateLimitRetries = DEFAULT_MAX_RATE_LIMIT_RETRIES, maxRetryDelayMs = DEFAULT_MAX_RETRY_DELAY_MS, sleep = (milliseconds: number) => new Promise((resolve) => setTimeout(resolve, milliseconds)), now = () => Date.now(), random = Math.random, legacyRateLimitActions = DEFAULT_LEGACY_RATE_LIMIT_ACTIONS, }: any) {
        this.ctx = ctx;
        ...
        this.paidActions = new PaidProviderActionProtocol({
            ...
            assertConfirmedOutput: (existing: any, run: any) => this.assertConfirmedOutput(existing, run),
            withCostTransition: (run: any, actionId: any, execute: any) => this.withCostTransition(run, actionId, execute),
        });
        this.rateLimit = {
    ...
    async vision(params: any, run: any) {
        ...
        const maximumCostCents = Math.ceil((VISION_MAX_INPUT_TOKENS * Number(config.costRatesCents.visionInputPerMillionTokens)
            + VISION_MAX_OUTPUT_TOKENS * Number(config.costRatesCents.visionOutputPerMillionTokens)) / 1000000);
    ...
    async image(params: any, run: any) {
        ...
        return this.paidAction(params, run, 'image_generate', maximumCostCents, async () => {
            ...
            if (!encoded)
                throw coded('stepfun_image_empty', 'StepFun 没有返回图片数据。');
            const bytes = providerBase64Bytes(encoded, 'stepfun_image_invalid', 'StepFun 返回的图片 Base64 无效。');
    ...
    async imageEdit(params: any, run: any) {
        assertImageOutputPath(params.outputPath);
        const { root, absolute } = await this.inputPath(run.companyId, params.inputPath);
        const bytes = await fs.readFile(absolute);
        if (bytes.length > 12 * 1024 * 1024)
            throw coded('image_too_large', '图片编辑输入超过 12MB。');
        const detectedMime = detectImageMime(bytes);
        const declaredMime = imageMimeFromPath(absolute);
        if (!detectedMime || detectedMime !== declaredMime) {
        ...
        return this.paidAction(params, run, 'image_edit', maximumCostCents, async () => {
            ...
            if (!response.ok)
                throw httpError('stepfun_image_edit_failed', 'StepFun 图片编辑', response, requested);
            const payload = await response.json();
            const encoded = payload?.data?.[0]?.b64_json;
            if (!encoded)
                throw coded('stepfun_image_empty', 'StepFun 图片编辑没有返回图片数据。');
            const outputBytes = providerBase64Bytes(encoded, 'stepfun_image_invalid', 'StepFun 图片编辑返回的 Base64 无效。');
    ...
    async tts(params: any, run: any) {
        if (/clone|克隆|复刻/i.test(String(params.voice)))
            throw coded('voice_clone_denied', '首版只允许官方音色，禁止克隆真人音色。');
        ...
        return this.paidAction(params, run, 'tts', maximumCostCents, async () => {
            const apiKey = await this.resolveApiKey(config);
            const requested = await this.fetchWithRateLimit('media', () => this.ctx.http.fetch(`${config.stepfunMediaBaseUrl}/audio/speech`, {
                method: 'POST',
            ...
            const response = requested.response;
            if (!response.ok)
                throw httpError('stepfun_tts_failed', 'StepFun TTS', response, requested);
    ...
    async claimCostEvent(params: any, run: any) {
        return this.paidActions.claim(params, run);
    }
    async confirmCostEvent(params: any, run: any) {
    ...
    async paidAction(params: any, run: any, operation: any, maximumCostCents: any, execute: any) {
        return this.paidActions.execute(params, run, operation, maximumCostCents, execute);
    }
    async assertConfirmedOutput(existing: any, run: any) {
        return this.paidActions.assertConfirmedArtifact(existing, run);
    }
    async withCostTransition(run: any, actionId: any, execute: any) {
        return this.paidActions.withTransition(run, actionId, execute);
    }
    assertBillableRate(operation: any, rates: any) {
        ...
        if (!valid)
            throw coded('billing_rate_missing', `付费工具 ${operation} 没有有效费率，调用已在外发前拒绝。`);
    }
    async request(config: any, endpoint: any, body: any, route: any = 'reasoning') {
        const apiKey = await this.resolveApiKey(config);
        const baseUrl = route === 'media' ? config.stepfunMediaBaseUrl : config.stepfunBaseUrl;
        const requested = await this.fetchWithRateLimit(route, () => this.ctx.http.fetch(`${baseUrl}${endpoint}`, {
            method: 'POST',
            headers: { 'content-type': 'application/json', authorization: `Bearer ${apiKey}` },
    ...
    async fetchWithRateLimit(route: any, request: any) {
        ...
        for (let retry = 0; retry <= this.rateLimit.maxRateLimitRetries; retry += 1) {
            ...
            if (retry === this.rateLimit.maxRateLimitRetries) {
                const error = httpError('stepfun_rate_limited', 'StepFun', response, { providerAttempts, rateLimitRejections });
                (error as any).retryableWithoutCharge = true;
                throw error;
            }
    ...
    async acquireRateSlot(route: any) {
        const pool = route === 'media' ? 'media' : 'reasoning';
        const previous = this.rateLimit.tails.get(pool) || Promise.resolve();
        let release: () => void = () => undefined;
        const current = new Promise((resolve: any) => { release = resolve; });
        this.rateLimit.tails.set(pool, current);
        await previous;
        try {
            const lastStartedAt = Number(this.rateLimit.lastStartedAt.get(pool) || 0);
            const waitMs = Math.max(0, lastStartedAt + this.rateLimit.minRequestIntervalMs - this.rateLimit.now());
            if (waitMs > 0)
                await this.rateLimit.sleep(waitMs);
            this.rateLimit.lastStartedAt.set(pool, this.rateLimit.now());
        }
        finally {
            release();
            if (this.rateLimit.tails.get(pool) === current)
                this.rateLimit.tails.delete(pool);
        }
    }
    rateLimitDelay(response: any, retry: any) {
        ...
        const jitter = Math.floor(Math.max(0, Math.min(1, Number(this.rateLimit.random()))) * 1000);
        return Math.min(this.rateLimit.maxRetryDelayMs, Math.max(this.rateLimit.minRequestIntervalMs, hinted, exponential) + jitter);
    }
    async config() {
        const config = await this.ctx.config.get();
        const stepfunBaseUrl = String(config.stepfunBaseUrl || 'https://api.stepfun.com/v1').replace(/\/+$/, '');
        const stepfunMediaBaseUrl = String(config.stepfunMediaBaseUrl || config.stepfunBaseUrl || 'https://api.stepfun.com/step_plan/v1').replace(/\/+$/, '');
        assertOfficialStepFunUrl(stepfunBaseUrl, ['/v1']);
        assertOfficialStepFunUrl(stepfunMediaBaseUrl, ['/v1', '/step_plan/v1']);
        return {
            stepfunSecretRef: requirePaperclipSecretRef(config.stepfunSecretRef, (message: any) => coded('stepfun_secret_ref_invalid', message)),
            stepfunBaseUrl,
            stepfunMediaBaseUrl,
    ...
    async resolveApiKey(config: any) {
        try {
    ...
    async inputPath(companyId: any, relativePath: any) {
        ...
        if (!absolute.startsWith(`${root}${path.sep}`))
            throw coded('symlink_escape', '素材路径通过符号链接逃逸了工作区。');
        return { root, absolute };
    }
    async writeBinary(companyId: any, relativePath: any, bytes: any) {
        const status = await this.ctx.localFolders.status(companyId, 'content-workspace');
...
export async function paperclipResponseBytes(response: any) {
...
function assertOfficialStepFunUrl(value: any, allowedPaths: any) {
    let parsed;
    try {
...
function httpError(code: any, label: any, response: any, attempts: any) {
...
function retryAfterMilliseconds(value: any, now: any) {
...
function providerBase64Bytes(value: any, code: any, message: any) {
