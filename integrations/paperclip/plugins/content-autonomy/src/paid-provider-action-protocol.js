...
export class PaidProviderActionProtocol {
  constructor({
    getContext,
    getPaidBudgetChecker,
    getLegacyRateLimitActions,
    getInflight,
    assertConfirmedOutput,
    withCostTransition,
  }) {
  ...
  async execute(params, run, operation, maximumCostCents, invoke) {
    const actionId = validActionId(params.actionId);
    const parameterChecksum = paidParameterChecksum(params);
    const stateKey = paidActionStateKey(run.projectId, actionId);
    const memoryKey = `${run.projectId}:${actionId}`;
    if (this.getInflight().has(memoryKey)) throw coded('paid_action_in_progress', '相同付费 action 正在执行，禁止并发重放。');
  ...
  async assertConfirmedArtifact(existing, run) {
  ...
  async withTransition(run, actionId, invoke) {
    const memoryKey = `cost-transition:${run.projectId}:${actionId}`;
    if (this.getInflight().has(memoryKey)) throw coded('cost_transition_in_progress', '相同费用事件正在变更状态。');
    this.getInflight().set(memoryKey, true);
    try {
      return await invoke();
    } finally {
