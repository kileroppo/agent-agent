...
async function mediaProbe(ctx, params, run) {
...
async function mediaValidate(ctx, params, run) {
  const { absolute, relative } = await existingWorkspacePath(ctx, run.companyId, params.relativePath);
  const probe = await probeFile(absolute);
...
async function mediaFinalize(ctx, params, run) {
...
function buildFinalEncodeArgs(inputPath, outputPath) {
  return [
