...
const FEATURE_PATCH_UNITS = Object.freeze({
  ...
  timeout: upgradeCommanderIngressTimeoutPatch,
  precedence: upgradeCommanderIngressPrecedencePatch,
});

const FEATURE_PATCH_PLANS = Object.freeze({
  baseline: ['completion', 'persistence', 'progress', 'taskControl', 'trialReadiness', 'timeout', 'precedence'],
  notifyV3: ['progress', 'taskControl', 'trialReadiness', 'timeout'],
  notifyV4: ['taskControl', 'trialReadiness', 'timeout', 'precedence'],
  ingressPrecedence: ['taskControl', 'trialReadiness', 'timeout'],
});
...
const LEGACY_MIGRATION_MATRIX = Object.freeze([
  ...
  { name: 'commander-task-notify-v4', marker: 'AJUN_COMMANDER_TASK_NOTIFY_V4', plan: 'notifyV4' },
  { name: 'commander-task-notify-v3', marker: 'AJUN_COMMANDER_TASK_NOTIFY_V3', plan: 'notifyV3' },
  { name: 'legacy-or-unpatched', marker: null, plan: 'baseline', prepare: installLegacyCommanderRouter },
]);
...
function installLegacyCommanderRouter(source) {
  let result = installCommanderIngress(source);
...
function installCommanderIngress(source) {
  ...
  if (source.includes('def _route_ajun_agent_proposal_event(')) {
    return upgradeLegacyProposalPatch(source);
  }
