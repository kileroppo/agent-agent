...
export function loadConfig(env = process.env) {
  ...
  if (workerToken.length < 32) throw new Error('AGENT_ARMY_WORKER_TOKEN 至少需要 32 个字符。');
  ...
  if (!workerId) throw new Error('AGENT_ARMY_WORKER_ID 不能为空。');
  return {
    cloudUrl,
    workerToken,
    workerId,
    ...
    pollMs:positiveNumber(env.AGENT_ARMY_WORKER_POLL_MS, 5_000, 1_000, 60_000),
    requestTimeoutMs:positiveNumber(env.AGENT_ARMY_WORKER_REQUEST_TIMEOUT_MS, 10_000, 1_000, 60_000)
  };
}
