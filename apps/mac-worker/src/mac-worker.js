import { WorkerHttpError } from './agent-army-client.js';

export class MacWorker {
  constructor({ cloud, xiaod, stateStore, workerId, now = () => Date.now() } = {}) {
    this.cloud = cloud;
    this.xiaod = xiaod;
  ...
  async runOnce() {
    ...
    if (!state.activeLease) {
      ...
      if (!leased.job) return { status:'idle', nextPollAfterMs:leased.nextPollAfterMs || 5_000 };
    ...
    try {
      ...
      if (error instanceof WorkerHttpError && error.status === 422) {
        ...
        await this.stateStore.save(state);
        return { status:'lease_lost', nextPollAfterMs:5_000 };
      }
      throw error;
    }
  }

  async processActive(state) {
    ...
    let local;
    try { local = await this.xiaod.get(mapping.xiaodJobId); }
    catch (error) {
      if (error?.status !== 404) throw error;
      delete state.jobs[job.taskId];
      await this.stateStore.save(state);
      return { status:'local_job_missing', nextPollAfterMs:1_000 };
    }
