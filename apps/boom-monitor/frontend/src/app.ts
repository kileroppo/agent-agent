const { createApp, ref, computed, onMounted, reactive }: any = Vue;
createApp({
    setup(): any {
        const state: any = {
            view: ref('dashboard'),
        ...
        const gradeClass: any = (grade: any = 'N0'): any => ({
            T3: 'badge-T3',
            T2: 'badge-T2',
            T1: 'badge-T1',
            N0: 'badge-N0',
        } as Record<string, string>)[grade] || 'badge-N0';
        const fmt: any = (v: any): any => v ? dayjs(v).format('YYYY-MM-DD HH:mm') : '-';
        const api: any = async (path: any, opt: any = {}): Promise<any> => {
            const r: any = await fetch(path, {
                headers: { 'Content-Type': 'application/json' },
                ...opt,
            });
            const data: any = await r.json().catch((): any => ({}));
            if (!r.ok) {
                throw new Error(data.detail || data.error || '请求失败');
            }
            return data;
        };
        const loadDashboard: any = async (): Promise<any> => {
            state.dashboard.value = await api('/api/dashboard');
        };
        const loadWorks: any = async (): Promise<any> => {
            state.loading.value = true;
            try {
                const q: any = new URLSearchParams();
                if (state.filters.grade.value)
                    q.set('grade', state.filters.grade.value);
                if (state.filters.platform.value)
                    q.set('platform', state.filters.platform.value);
                if (state.filters.creatorId.value)
                    q.set('creator_id', state.filters.creatorId.value);
                const data: any = await api(`/api/works?${q.toString()}`);
        ...
        const loadDetail: any = async (id: any): Promise<any> => {
            state.selected.value = await api(`/api/works/${id}`);
        };
        const runScan: any = async (): Promise<any> => {
            await api('/api/scan/run', { method: 'POST' });
            await runWorkerOnce();
            await loadWorks();
        };
        const dispatchAnalysis: any = async (): Promise<any> => {
        ...
        const loadAnalysis: any = async (): Promise<any> => {
            const data: any = await api('/api/analysis');
            state.analysis.value = data.items || [];
        };
        const loadSettings: any = async (): Promise<any> => {
            state.settingsLoading.value = true;
            try {
                const data: any = await api('/api/settings');
                const config: any = data.analysis_auto || {};
        ...
        const saveSettings: any = async (): Promise<any> => {
            const grades: any = String(state.settingsDraft.value.analysis_auto_grades || '')
                .split(',')
                .map((v: any): any => v.trim())
                .filter(Boolean)
                .join(',');
        ...
        const runWorkerOnce: any = async (): Promise<any> => {
            await api('/api/analysis/process', { method: 'POST' });
        };
        const enqueuePlatform: any = async (platform: any): Promise<any> => {
            await api(`/api/scan/enqueue/${platform}`, { method: 'POST' });
        };
        const submitImport: any = async (): Promise<any> => {
            if (!state.importPayload.value)
                return;
            const payload: any = JSON.parse(state.importPayload.value);
            const body: any = {
                source_type: 'manual',
                platform: 'douyin',
                ...payload,
            };
            state.importResult.value = JSON.stringify(await api('/api/import', {
                method: 'POST',
        ...
        const collectUrl: any = async (): Promise<any> => {
            const url: any = String(state.sourceUrl.value || '').trim();
            if (!url)
                return;
        ...
        const start: any = async (): Promise<any> => {
            await Promise.all([loadDashboard(), loadWorks(), loadSettings()]);
        };
        onMounted(start);
        const booms: any = computed((): any => (state.dashboard.value ? state.dashboard.value.boom : {}));
        return {
