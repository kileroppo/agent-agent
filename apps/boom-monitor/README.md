...
- 作品列表: `GET /api/works`
- v1/v2 观察样本: `GET /api/shadow-scores?limit=100`
...
- 首次有效 v2 基线单独冻结为 `url-history-shadow-v2`，和 v1 互不覆盖；
- 平台拿不到发布时间时，v2 明确标记为“累计表现、作品年龄未知”，不能据此声称作品正在爆发。

这些绝对量与质量门槛是首版观察参数。先累计 50–100 条 `GET /api/shadow-scores` 对照样本，检查不同平台、粉丝层级和内容类型的误报/漏报，再决定是否调整或把 v2 升为正式派发规则。
