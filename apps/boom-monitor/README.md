...
首页可直接粘贴小红书、抖音、B站或 YouTube 作品链接。A君在进程内请求本机小D：小红书、抖音和 B站复用受控 MediaCrawlerPro 深度通道，YouTube 复用公开 `yt-dlp` 元数据通道，读取作品详情、作者粉丝数和近期历史作品；Cookie 只在本机 CookieBridge 与 MediaCrawlerPro 之间流转，不进入爆款雷达数据库，YouTube 公开读取不读取浏览器 Cookie。

这条链路不修改 MediaCrawlerPro 官方仓库，也不把观测时间伪装成发布时间。适配层优先使用平台响应中的 `create_time` / `publish_time`；当前 MediaCrawlerPro 下载服务未透传抖音原始 `create_time` 时，从经校验的抖音作品 ID 时间位恢复并记录来源。字段和作品 ID 都无法形成合法时间时仍保持未知，评分器直接使用作者主页返回顺序中的历史指标计算中位数。

JSON/CSV 导入继续保留。批量导入会先完整入库，再按每条作品发布时间读取其之前最多 20 条作品，避免导入顺序改变评分。

旧评分 v1（只保留作回滚对照，不再控制链接评分派发）：

- R：抖音/B站/YouTube 使用点赞，小红书使用点赞+收藏；
