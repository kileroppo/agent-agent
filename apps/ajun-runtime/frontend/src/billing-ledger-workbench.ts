...
export function createBillingLedgerWorkbench({ agentName, formatDate, formatNumber, formatUsd }: any): any {
    const elements: any = {
    ...
    function renderTabs(): any {
        ...
        counts.unknown_cost = state.entries.filter((entry: any): any => entry.cost?.status === 'unknown').length;
        for (const button of elements.viewButtons) {
    ...
    return {
        setEntries,
        setUnavailable,
        setView: (view: any): any => reset({ view }),
        setAgent: (agentId: any): any => reset({ agentId }),
    };
}
