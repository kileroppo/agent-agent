...
export function createBoomMonitorConsole({ root, api, formatDate }: any): any {
    ...
    function workMetricChips(work: any): any {
        const rValue: any = Number(work.r_value);
        const mValue: any = Number(work.m_value);
        const likes: any = Number(work.likes);
        const mThreshold: any = boomMThreshold(work.tier);
        const volumeFloor: any = boomVolumeFloor(work.platform);
        return [
...
function metricChip({ icon, value, label, title, fill, pass }: any): any {
    const percent: any = Math.round(Math.max(0, Math.min(1, Number(fill) || 0)) * 100);
...
function gateRows(score: any): any {
    ...
    const gateRow: any = (icon: any, label: any, gate: any, value: any, passed: any, extra: any = '') => `<div class="boom-gate-row${passed ? ' is-pass' : ''}">`
        + `<svg class="boom-metric-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${boomMetricIcons[icon] || ''}</svg>`
