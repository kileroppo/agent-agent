...
export function createBoomMonitorConsole({ root, api, formatDate }: any): any {
    ...
    function workVerdict(work: any): any {
        const grade: any = ['T1', 'T2', 'T3'].includes(work.grade) ? work.grade : 'N0';
        const rValue: any = Number(work.r_value);
        const multiplier: any = Number.isFinite(rValue) ? `${rValue.toFixed(1)}×` : '';
        const label: any = ({ T3: '大爆', T2: '小爆', T1: '跑赢历史', N0: '未达爆款门槛' } as Record<string, string>)[grade];
        const icon: any = ({ T3: 'fire', T2: 'spark', T1: 'trend' } as Record<string, string>)[grade] || '';
    ...
    async function showWorkDetail(workId: any, triggerButton: any): Promise<any> {
        ...
        if (!output.hidden) {
            ...
            if (triggerButton) triggerButton.textContent = '查看判断依据';
            return;
        }
        output.hidden = false;
        ...
        if (triggerButton) triggerButton.textContent = '收起判断依据';
        try {
        ...
        <small>缺少发布时间时只表示累计表现，不表示作品正在爆发。</small>`;
            ...
            if (triggerButton?.isConnected) triggerButton.textContent = '查看判断依据';
            throw error;
        }
    }
...
function formatCompact(value: any): any {
    const number: any = Number(value);
...
const BOOM_M_THRESHOLDS: any = { high: 0.04, mid: 0.08, mid_small: 0.15, low: 0.3 };
const BOOM_VOLUME_FLOORS: any = { xiaohongshu: { T1: 100, T2: 500, T3: 5000 }, default: { T1: 500, T2: 3000, T3: 10000 } };
const BOOM_QUALITY_REASON_LABELS: any = {
...
function boomMThreshold(tier: any): any {
    return Number(BOOM_M_THRESHOLDS[String(tier || '')] ?? BOOM_M_THRESHOLDS.mid);
}
function boomVolumeFloors(platform: any): any {
    return BOOM_VOLUME_FLOORS[String(platform || '')] || BOOM_VOLUME_FLOORS.default;
}
function boomVolumeFloor(platform: any): any {
    return boomVolumeFloors(platform).T1;
}
function gateMark(passed: any): any {
    return `<span class="boom-gate-mark${passed ? ' is-pass' : ''}" aria-label="${passed ? '达标' : '未达标'}">${passed ? '✓' : '✗'}</span>`;
}
function gateRows(score: any): any {
    const signals: any = score.signals || {};
    const relative: any = signals.relative || {};
    const reach: any = signals.reach || {};
    const quality: any = signals.quality || {};
    const sampleCount: any = Number(score.sample_count || 0);
    const insufficient: any = String(score.status || '') === 'insufficient_history';
    const relativePassed: any = relative.passed === true;
    const mValue: any = Number(reach.m_value ?? score.m_value);
    const mThreshold: any = Number(reach.m_threshold ?? boomMThreshold(score.tier));
    const floors: any = reach.absolute_floors || boomVolumeFloors('');
    const volume: any = Number(score.absolute_interactions || 0);
    const volumePassed: any = volume >= Number(floors.T1);
    const reachPassed: any = Number.isFinite(mValue) && mValue >= mThreshold;
    const qualityPassed: any = quality.passed === true;
    const reasonChips: any = (quality.reasons || []).map((reason: any) => `<span class="boom-quality-chip">${escapeHtml(BOOM_QUALITY_REASON_LABELS[reason] || reason)}</span>`).join('');
    const gateRow: any = (icon: any, label: any, gate: any, value: any, passed: any, extra: any = '', title: any = '') => `<div class="boom-gate-row${passed ? ' is-pass' : ''}"${title ? ` title="${escapeHtml(title)}"` : ''}>`
