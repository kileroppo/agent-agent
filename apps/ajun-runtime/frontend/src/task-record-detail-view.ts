...
export function renderAttentionDetail(attention: any, actionState: any, _escapeHtml: any, options: any = {}): any {
    ...
    const isNeedsInput: boolean = Boolean(task && (
        attention.kind === 'needs_input'
        || task.status === 'needs_input'
    ));

    const needsInputForm: string = isNeedsInput ? html`
    ...
    const fallbackNext: any = !hasActions && !isGeneric && !isNextActionSameAsCause && attention.nextAction
...
export function renderNeedsInputContext(task: any = {}): string {
    if (!task) return '';
