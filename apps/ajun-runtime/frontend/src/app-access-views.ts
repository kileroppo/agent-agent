...
export function createAccessViews({ elements, state, api, statusLabel, formatDate, providerLabel, agentName, replaceChildrenPreservingDisclosureState, setTextIfChanged, modelPolicyConsole, }: any): any {
    ...
    function aiServiceGroups(services: any[], categories: any[] = []): any[] {
        ...
        const leftoverServices: any = services.filter((service: any): any => !assigned.has(service.id));
        if (leftoverServices.length) {
            const readyCount: any = leftoverServices.filter((service: any): any => service.state === 'running' || service.state === 'ready').length;
    ...
    function aiServiceCard(service: any): any {
        ...
        ${[['on_demand', '按需启动'], ['always_on', '保持运行'], ['disabled', '禁用']].map(([value, label]: any): any => `<option value="${escapeHtml(value)}"${service.mode === value ? ' selected' : ''}>${escapeHtml(label)}</option>`).join('')}
      </select></label>`;
        ...
        if (failedConnections.length > 0 && !verifying) {
            ...
            await Promise.allSettled(
                failedConnections.map((connection: any): any =>
                    api('/api/access-connections/' + connection.connectionId + '/verify', { method: 'POST' }).catch(() => null)
                )
            );
            verifying = false;
            renderAccessConnections();
