...
export function createAccessViews({ elements, state, api, statusLabel, formatDate, providerLabel, agentName, replaceChildrenPreservingDisclosureState, setTextIfChanged, modelPolicyConsole, }: any): any {
    ...
    function aiServiceCard(service: any): any {
        ...
        const actions: any = service.actions.map((action: any): any => `<button type="button" class="${action === 'stop' ? 'secondary-action danger-action' : 'secondary-action'}" data-ai-service="${service.id}" data-ai-action="${action}">${aiActionLabel(action)}</button>`).join('');
        node.innerHTML = html`
