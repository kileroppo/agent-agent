...
export function createTaskRecordWorkbench({ api, getAgents, taskTypeLabel, agentName, escapeHtml, initialTaskId = '', }: any): any {
    ...
    function renderList(): any {
        if (!state.items.length) {
            const filtered: any = Boolean(state.q || state.agentId || state.taskType || state.time !== '30d' || state.includeRoutine);
    ...
    function renderDetail(): any {
      ...
      ${renderTechnicalDetails(task, presentation, attention, escapeHtml)}`;
        ...
        for (const button of elements.detail.querySelectorAll('[data-attention-action]')) {
            button.addEventListener('click', (): any => executeAttentionAction(task, button.dataset.attentionAction));
        }
    ...
    async function executeAttentionAction(task: any, actionKey: any): Promise<any> {
        ...
        if (!action || state.actionState.get(task.taskId)?.status === 'submitting')
            return;
        const confirmation: any = action.confirmation || `确认执行“${action.label}”？系统只会执行这条明确的恢复动作。`;
    ...
    function renderFilters(): any {
        ...
        const changed: any = Boolean(state.q || state.agentId || state.taskType || state.time !== '30d' || state.includeRoutine);
...
function renderTechnicalDetails(task: any, presentation: any, attention: any, escapeHtml: any): any {
    ...
    if (!rows.length)
        return '';
    return `<details class="record-technical"><summary>技术详情</summary><dl>${rows.map(([label, value]: any): any => `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`).join('')}</dl></details>`;
}
