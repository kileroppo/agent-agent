...
export function isTaskAdoptable(task: any): boolean {
    ...
    if (task.status === 'succeeded' || task.acceptanceTarget?.decision === 'accepted') return false;
    if (['failed', 'error', 'cancelled', 'stopped', 'needs_input'].includes(task.status)) return false;
...
export async function submitWorkflowAcceptance({ api, target, decision, note, idempotencyKey }: any): Promise<any> {
...
export function confirmAttentionActionHelper({ task, actionKey, state, renderDetail, elements }: any): void {
    const attention: any = taskAttentionView(task);
...
export async function executeAttentionActionHelper({ task, actionKey, state, renderDetail, api, loadSelectedDetail, recoverySubmissionView }: any): Promise<void> {
    const attention: any = taskAttentionView(task);
    const action: any = attention?.actions.find((item: any): any => item.actionKey === actionKey);
    if (!action || state.actionState.get(task.taskId)?.status === 'submitting')
        return;
...
export async function executeAcceptanceDecisionHelper({ task, decision, state, renderDetail, elements, submitAcceptance, api, loadRecords, withAcceptanceTarget }: any): Promise<void> {
    const target: any = acceptanceTargetView(task);
