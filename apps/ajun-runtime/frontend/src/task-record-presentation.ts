...
export function displaySubtaskTitle(childTask: any, parentTask: any = null): string {
    const rawTitle: string = String(childTask?.input?.title || childTask?.title || '子任务').trim();
    const parentRawTitle: string = String(parentTask?.input?.title || parentTask?.title || '').trim();
    const parsed = parseTaskTitle(rawTitle);
