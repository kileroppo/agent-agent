...
MAX_DOWNLOAD_RPC_TIMEOUT_SECONDS = 120
MAX_DOWNLOAD_WAIT_SECONDS = 45.0
...
class PlaywrightBrowserSession:
    ...
    def run(
        ...
        *,
        ...
        global LAST_DOWNLOAD_ERROR_CODE
        try:
            if list(args[:2]) == ["wait", "--fn"] and len(args) == 3:
                ...
                self.rpc(
                    "downloadRef",
                    timeout=min(timeout, MAX_DOWNLOAD_RPC_TIMEOUT_SECONDS),
                    ref=args[1],
                    output=str(output),
                )
                if EXPORT_MODE == "images":
                    normalize_image_download(output, self.download_dir)
            else:
...
def download_wait_wrapper(function: Any, stage: str) -> Any:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> Any:
        ...
        bounded_kwargs["timeout"] = min(
            float(bounded_kwargs.get("timeout", MAX_DOWNLOAD_WAIT_SECONDS)),
            MAX_DOWNLOAD_WAIT_SECONDS,
        )
        try:
...
def normalize_image_download(output: Path, download_dir: Path) -> Path:
    ...
    with zipfile.ZipFile(normalized, "x", compression=zipfile.ZIP_DEFLATED) as archive:
        for index, candidate in enumerate(source_candidates, start=1):
            ...
            archive.write(candidate, arcname=f"{index:02d}{suffix}")
