...
export function healthyM5StageWorkProducts(outputs, contract) {
  ...
  return m5StageWorkProductCandidates(outputs, contract).filter((item) => {
    ...
    const expectedPayloadValid = expected.type === 'ContentVersion'
      ? validContentVersionWorkProduct(metadata?.contentVersion, artifact?.contentVersion)
      : expected.type === 'MachineReview'
        ? validMachineReviewWorkProduct(metadata?.reviewReport, artifact?.reviewReport)
        : true;
    ...
    && metadata?.kind === expected.type
    && metadata?.stageKey === contract.stageKey
...
export function m5StageWorkProductCandidates(outputs, contract) {
  const expected = contract?.expectedWorkProduct;
  if (!expected) return [];
  return listItems(outputs, 'items').filter((item) => {
    if (item?.kind !== 'work_product' || item?.type !== 'artifact') return false;
    const metadata = item?.metadata;
    return metadata?.stageKey === contract.stageKey
      || metadata?.schemaVersion === expected.schemaVersion
      || metadata?.kind === expected.type;
  });
}
