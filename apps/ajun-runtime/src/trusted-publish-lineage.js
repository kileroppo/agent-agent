...
import { trustedM5WorkProducts } from './m5-work-product-trust.js';
...
const CONTENT_SHA256 = /^sha256:[a-f0-9]{64}$/i;
const EVIDENCE_SHA256 = /^sha256:[a-f0-9]{64}$/;
const MAX_METRIC_OBSERVATION_AGE_MS = 5 * 60_000;

export function trustedContentVersionProducts(outputs) {
...
export function trustedMachineReviewProducts(outputs) {
  return trustedM5WorkProducts(outputs, {
    provider:CONTENT_PROVIDER,
...
export function trustedPublishReceiptProducts(outputs) {
  return trustedM5WorkProducts(outputs, {
...
export function trustedMetricSnapshotProducts(outputs) {
  return trustedM5WorkProducts(outputs, {
    provider:PUBLISHER_PROVIDER,
...
export function assertContentVersionIdentity(contentVersion, { invalid } = {}) {
  if (
    !contentVersion
...
export function assertPublishReceiptIdentity(receipt, { invalid, requireMetricIdentity = false } = {}) {
  const metricIdentityInvalid = requireMetricIdentity && (
...
export function assertMetricSnapshotLineage({
  snapshot,
  receipt,
...
function optionalIdentityMatches(snapshot, receipt) {
  return ['campaignId', 'accountRef', 'connectorMode', 'externalContentId']
    .every((field) => !Object.hasOwn(snapshot, field) || snapshot[field] === receipt[field]);
}
...
function validXhsMetricEvidence(snapshot, receipt) {
  ...
  const metricKeys = ['comments', 'likes', 'saves', 'views'];
  const sourceKeys = [
  ...
  return snapshot.accountRef === receipt.accountRef
    ...
    && metricKeys.every((key) => exactRawMetricValue(source.rawMetrics[key]));
}
...
function invalidError(factory, message, code) {
  if (typeof factory === 'function') return factory(message, code);
