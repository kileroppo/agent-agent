export class LocalGithubResearch {
  ...
  async search(task, input) {
    try {
      ...
      const report = {
        query:search.query, searchedAt:search.searchedAt, results:search.results.slice(0, limit).map((item) => ({ ...item, assessment:assess(item), suitability:suitability(item) })),
...
function suitability(item) {
  const basis = `${item.fullName || ''} ${item.description || ''} ${(item.topics || []).join(' ')}`.toLowerCase();
  if (/(?:trading|finance|financial|stock)/.test(basis)) return '适合参考垂直业务中的多智能体分工，不宜直接作为通用军团控制面。';
  if (/(?:software|developer|metagpt|code|engineering)/.test(basis)) return '适合参考软件研发角色与任务编排，接入前仍需核对权限和人工审批边界。';
  if (/(?:swarm|handoff|lightweight|minimal)/.test(basis)) return '适合参考轻量 Agent 交接与编排，不适合直接承担完整治理、审计和预算控制。';
  return '适合进入多智能体编排候选清单，落地前仍需核对治理、审计和故障恢复能力。';
}

function requestedResultLimit(input) {
  const text = `${input?.title || ''}\n${input?.description || ''}`;
  const raw = text.match(/(?:前|取|返回|比较|选择)\s*([1-9]\d?)/)?.[1]
