...
import { ContentCampaignError } from './content-campaign-service.ts';
...
import { M5PublisherBindingError } from './m5-publisher-bindings.ts';
import { routeM5PublisherApi } from './m5-publisher-api.ts';
import { PublicWebFetchError } from './public-web-fetch.ts';
import { OfficialFeishuCompletionWatcherError } from './official-feishu-completion-watcher.ts';
import { assertTaskCardOwnership, presentCommanderReply, presentTaskStatus, resolveTaskCardAction } from './runtime-http-feishu.ts';
import { createMissionHttpResult, createTaskHttpResult, } from './contracts/agent-army-adapter-projection.ts';
import { ValidationError } from './task-service.ts';
import { AgentArmyTaskInputError } from './contracts/agent-army-task-input.ts';
import { normalizeMissionHttpInput, normalizeTaskHttpInput, } from './contracts/agent-army-http-input.ts';
...
export function createAjunHttpHandler({ environment, publicDir, dataDir, detailBaseUrl, development = {}, network, paperclip, work, connections, localAi, feishu, m5, runtimeRelease, }: any): any {
    ...
    return async function ajunHttpHandler(request: any, response: any): Promise<any> {
        try {
            ...
            if (paperclipResult)
                return sendJson(response, paperclipResult.status, paperclipResult.payload);
            if (request.url?.startsWith('/api/worker/')) {
                if (deploymentMode !== 'cloud')
                    return sendJson(response, 404, { error: '当前运行台不是云端办公室。' });
                if (!macWorker.enabled())
                    return sendJson(response, 503, { error: '云端尚未配置 Mac 工作间令牌。' });
                if (!macWorker.authorize(request.headers.authorization))
                    return sendJson(response, 401, { error: 'Mac 工作间身份校验失败。' });
                if (request.method === 'POST' && request.url === '/api/worker/lease')
                    return sendJson(response, 200, await macWorker.lease(await readJsonBody(request)));
                const workerTaskMatch: any = request.url.match(/^\/api\/worker\/tasks\/([0-9a-f-]+)\/(heartbeat|complete)$/i);
            ...
            if (request.method === 'GET' && request.url === '/api/local-share') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '共享口令只能在本机查看。' });
            ...
            if (request.method === 'POST' && request.url === '/api/local-share/rotate') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '共享口令只能在本机轮换。' });
            ...
            const taskTimelineUrl: any = request.method === 'GET' && request.url?.startsWith('/api/tasks/')
                ? new URL(request.url, 'http://127.0.0.1')
                : null;
            const taskTimelineMatch: any = taskTimelineUrl?.pathname.match(/^\/api\/tasks\/([0-9a-f-]{36})\/timeline$/i);
            if (taskTimelineMatch) {
                const task: any = await store.getTask(taskTimelineMatch[1]);
                if (!task)
                    return sendJson(response, 404, { error: '没有找到这条任务。' });
                const audience: any = isLocalAddress(request.socket.remoteAddress) ? 'local-owner' : 'lan';
            ...
            if (request.method === 'GET' && taskDetailMatch) {
                const audience: any = isLocalAddress(request.socket.remoteAddress) ? 'local-owner' : 'lan';
                const task: any = await tasks.taskRecordDetail(taskDetailMatch[1], { audience });
                if (!task)
                    return sendJson(response, 404, { error: '没有找到这条任务。' });
                return sendJson(response, 200, { task });
            }
            const transcriptRevisionMatch: any = request.url?.match(/^\/api\/tasks\/([0-9a-f-]{36})\/transcript-revision$/i);
            ...
            const transcriptRevisionsMatch: any = request.url?.match(/^\/api\/tasks\/([0-9a-f-]{36})\/transcript-revisions$/i);
            if (request.method === 'POST' && transcriptRevisionsMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '任务字幕只能由老板在本机补正。' });
            ...
            if (request.method === 'GET' && request.url === '/api/employee-feishu-connections') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '员工飞书接线资料只能在老板这台设备上查看。' });
            ...
            if (request.method === 'POST' && request.url === '/api/access-login/open') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '账号登录只能由老板在本机打开。' });
            ...
            if (request.method === 'POST' && request.url === '/api/access-connections') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '账号连接只能由老板在本机创建。' });
                return sendJson(response, 201, { connection: await accessConnections.create(await readJsonBody(request)) });
            }
            const accessConnectionRevokeMatch: any = request.url?.match(/^\/api\/access-connections\/([0-9a-f-]{36})\/revoke$/i);
            if (request.method === 'POST' && accessConnectionRevokeMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '账号连接只能由老板在本机撤销。' });
                return sendJson(response, 200, { connection: await accessConnections.revoke(accessConnectionRevokeMatch[1]) });
            }
            const accessConnectionDisableMatch: any = request.url?.match(/^\/api\/access-connections\/([0-9a-f-]{36})\/disable$/i);
            if (request.method === 'POST' && accessConnectionDisableMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '账号连接只能由老板在本机禁用。' });
                return sendJson(response, 200, { connection: await accessConnections.disable(accessConnectionDisableMatch[1]) });
            }
            const accessConnectionDefaultMatch: any = request.url?.match(/^\/api\/access-connections\/([0-9a-f-]{36})\/default$/i);
            if (request.method === 'POST' && accessConnectionDefaultMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '默认账号只能由老板在本机设置。' });
                return sendJson(response, 200, { connection: await accessConnections.setDefault(accessConnectionDefaultMatch[1]) });
            }
            const accessConnectionReauthorizeMatch: any = request.url?.match(/^\/api\/access-connections\/([0-9a-f-]{36})\/reauthorize$/i);
            ...
            if (request.method === 'POST' && employeeFeishuConnectionMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '员工飞书接线只能由老板在本机完成。' });
            ...
            const employeeModelSetupMatch: any = request.url?.match(/^\/api\/employee-model-setup\/([a-z][a-z0-9-]{0,63})$/);
            ...
            if (request.method === 'GET' && request.url === '/api/agent-proposals')
                return sendJson(response, 200, { proposals: await store.listProposals(), testInstances: await store.listTestInstances() });
            if (request.method === 'POST' && request.url === '/api/agent-proposals')
                return sendJson(response, 201, { proposal: await proposals.create(await readJsonBody(request)) });
            if (request.method === 'POST' && request.url === '/api/feishu/agent-proposals') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书入口只能由本机 Hermes 适配器调用。' });
                const proposal: any = await proposals.create(await readJsonBody(request), { source: 'feishu' });
            ...
            if (request.method === 'POST' && request.url === '/api/feishu/channel/messages') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书官方收发入口只能由本机适配器调用。' });
            ...
            if (request.method === 'POST' && request.url === '/api/feishu/channel/cards') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书官方卡片回调只能由本机适配器调用。' });
            ...
            if (request.method === 'POST' && request.url === '/api/feishu/task-status') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书任务状态只能由本机 Hermes 适配器读取。' });
                const input: any = await readJsonBody(request);
                const taskId: any = String(input.taskId || '').trim();
                const task: any = (await store.list()).find((item: any): any => item.taskId === taskId);
                const cardIdentity: any = assertTaskCardOwnership(task, input);
                const chatRef: any = String(input.chatId || input.chatRef || '').trim();
                const notification: any = await tasks.notificationStatus(taskId, chatRef);
                return sendJson(response, 200, presentTaskStatus(notification, task, { ...(await loadTaskCardContext({ store, tasks }, task)), ...cardIdentity }));
            }
            if (request.method === 'POST' && request.url === '/api/feishu/task-card-actions') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书任务卡动作只能由本机 Hermes 适配器调用。' });
            ...
            if (request.method === 'POST' && request.url === '/api/mcp/completion-watches/resolve') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书投递核对只能由本机执行。' });
                const input: any = await readJsonBody(request);
                const taskId: any = String(input.taskId || '').trim();
                const chatRef: any = String(input.chatRef || '').trim();
                const task: any = (await store.list()).find((item: any): any => item.taskId === taskId);
                if (!task)
                    throw new ValidationError('找不到要核对的任务。');
                if (task.source?.channel !== 'feishu' || String(task.source?.chatRef || '').trim() !== chatRef)
                    throw new ValidationError('只能核对任务原飞书会话的投递。');
            ...
            if (request.method === 'POST' && request.url === '/api/mcp/completion-watches') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: 'Hermes 任务跟进只能由本机 MCP 登记。' });
                const input: any = await readJsonBody(request);
                const taskId: any = String(input.taskId || '').trim();
                const chatRef: any = String(input.chatRef || '').trim();
                const task: any = (await store.list()).find((item: any): any => item.taskId === taskId);
            ...
            const assignmentRoutes: any = new Map([
            ...
            if (m5CampaignResult)
                return sendJson(response, m5CampaignResult.status, m5CampaignResult.payload);
            const m5PublisherResult: any = await routeM5PublisherApi({
                method: request.method,
                url: request.url,
                local: isLocalAddress(request.socket.remoteAddress),
                getService: campaigns,
            ...
            if (m5PublisherResult)
                return sendJson(response, m5PublisherResult.status, m5PublisherResult.payload);
            const feishuProposalApprovalMatch: any = request.url?.match(/^\/api\/feishu\/proposal-approvals\/([0-9a-f-]+)\/(approve|reject)$/i);
            if (request.method === 'POST' && feishuProposalApprovalMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书岗位审批回调只能由本机 Hermes 适配器调用。' });
            ...
            const proposalAction: any = request.url?.match(/^\/api\/agent-proposals\/([0-9a-f-]+)\/(submit|approve-for-test|activate|reject|archive|test-instance|test-evidence|acceptance|run-acceptance)$/i);
            if (request.method === 'POST' && proposalAction) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '草案审核与测试操作只能由本机主人发起。' });
                const [, proposalId, action] = proposalAction;
                const input: any = await readJsonBody(request);
                if (action === 'submit')
                    return sendJson(response, 200, { proposal: await proposals.submit(proposalId) });
                if (action === 'approve-for-test')
                    return sendJson(response, 200, { proposal: await proposals.approveForTest(proposalId) });
                if (action === 'activate')
                    return sendJson(response, 200, { proposal: await proposals.activate(proposalId) });
                if (action === 'reject')
                    return sendJson(response, 200, { proposal: await proposals.reject(proposalId) });
                if (action === 'archive')
                    return sendJson(response, 200, { proposal: await proposals.archive(proposalId, { archivedBy: '本机负责人', reason: String(input.reason || '').trim() || undefined }) });
                if (action === 'test-instance')
                    return sendJson(response, 201, { testInstance: await proposals.createTestInstance(proposalId, { hermesProfileName: String(input.hermesProfileName || '').trim() || null }) });
                if (action === 'test-evidence')
                    return sendJson(response, 200, { proposal: await proposals.recordTestEvidence(proposalId, input) });
                if (action === 'run-acceptance')
                    return sendJson(response, 200, { proposal: await proposals.runRestrictedAcceptance(proposalId, input) });
            ...
            if (request.method === 'POST' && request.url === '/api/tasks') {
                const input: any = await readJsonBody(request);
                if (!isLocalAddress(request.socket.remoteAddress) && !String(input.requesterName || '').trim())
                    throw new ValidationError('局域网协作者请先填写自己的称呼。');
            ...
            if (request.method === 'GET' && request.url === '/api/integrations/boom-monitor/health') {
                if (boomMonitorEnabled !== false)
                    return sendJson(response, 503, { error: '旧版爆款雷达回滚桥仅在 native writer 已关闭时开放。' });
            ...
            if (request.method === 'POST' && request.url === '/api/mcp/missions') {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: 'Hermes MCP 多人任务只能由本机调用。' });
                const input: any = await readJsonBody(request);
            ...
            const rejectMatch: any = request.url?.match(/^\/api\/approvals\/([0-9a-f-]+)\/reject$/i);
            if (request.method === 'POST' && rejectMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '只有本机主人可以拒绝审批。' });
                return sendJson(response, 200, { task: await tasks.rejectApproval(rejectMatch[1]) });
            }
            const approveMatch: any = request.url?.match(/^\/api\/approvals\/([0-9a-f-]+)\/approve$/i);
            if (request.method === 'POST' && approveMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '只有本机主人可以批准审批。' });
                return sendJson(response, 200, { task: await tasks.approveApproval(approveMatch[1], await readJsonBody(request)) });
            }
            const privateGrantRevokeMatch: any = request.url?.match(/^\/api\/approvals\/([0-9a-f-]+)\/revoke-private-read-grant$/i);
            if (request.method === 'POST' && privateGrantRevokeMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '只有本机主人可以撤销微信临时授权。' });
            ...
            const feishuPrivateGrantRevokeMatch: any = request.url?.match(/^\/api\/feishu\/approvals\/([0-9a-f-]+)\/revoke-private-read-grant$/i);
            if (request.method === 'POST' && feishuPrivateGrantRevokeMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书微信授权回调只能由本机 Hermes 适配器调用。' });
                const input: any = await readJsonBody(request);
            ...
            const feishuApprovalMatch: any = request.url?.match(/^\/api\/feishu\/approvals\/([0-9a-f-]+)\/(approve|reject)$/i);
            if (request.method === 'POST' && feishuApprovalMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书审批回调只能由本机 Hermes 适配器调用。' });
            ...
            const feishuGovernanceApprovalMatch: any = request.url?.match(/^\/api\/feishu\/governance-approvals\/([0-9a-f-]+)\/(approve|reject)$/i);
            if (request.method === 'POST' && feishuGovernanceApprovalMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: '飞书组织级审批回调只能由本机 Hermes 适配器调用。' });
            ...
            if (request.method === 'POST' && mcpTaskControlMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: 'Hermes MCP 任务控制只能由本机调用。' });
            ...
            const mcpTaskFeedbackMatch: any = request.url?.match(/^\/api\/mcp\/tasks\/([0-9a-f-]+)\/feedback$/i);
            if (request.method === 'POST' && mcpTaskFeedbackMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: 'Hermes MCP 任务评价只能由本机调用。' });
                const input: any = await readJsonBody(request);
                const task: any = await store.getTask(mcpTaskFeedbackMatch[1]);
                if (!task)
            ...
            const mcpApprovalMatch: any = request.url?.match(/^\/api\/mcp\/approvals\/([0-9a-f-]+)\/(approve|reject)$/i);
            if (request.method === 'POST' && mcpApprovalMatch) {
                if (!isLocalAddress(request.socket.remoteAddress))
                    return sendJson(response, 403, { error: 'Hermes MCP 审批只能由本机调用。' });
                const [, approvalId, decision] = mcpApprovalMatch;
                const approval: any = (await store.listApprovals()).find((item: any): any => item.approvalId === approvalId);
                if (!approval)
                    throw new ValidationError('找不到这条审批。');
                const options: Record<string, any> = { decisionBy: 'Hermes MCP 人工确认', decisionReason: '由 Hermes 当前会话的原生审批界面确认。' };
                if (approval.governanceMode === 'paperclip')
                    return sendJson(response, 200, { task: await tasks.resolvePaperclipApproval(approvalId, decision, options) });
            ...
            if (request.method === 'GET' && (publicPath === '/stepfun-model-policy-console.js' || publicPath === '/runtime-release-console.js' || publicPath === '/data-lifecycle-console.js' || publicPath === '/refresh-scheduler.js' || publicPath === '/boom-monitor-console.js' || publicPath === '/billing-entry-filter.js' || publicPath === '/billing-ledger-workbench.js'))
                return sendFile(response, publicDir, publicPath.slice(1), 'text/javascript; charset=utf-8');
            if (request.method === 'GET' && (publicPath === '/billing-usage-cache.js' || publicPath === '/console-labels.js' || publicPath === '/html.js' || publicPath === '/console-navigation.js' || publicPath === '/disclosure-state.js' || publicPath === '/task-record-filter.js' || publicPath === '/task-record-detail-view.js' || publicPath === '/task-record-workbench.js' || publicPath === '/task-record-presentation.js' || publicPath === '/task-timeline-view.js' || publicPath === '/task-flow-view.js' || publicPath === '/task-tree-view.js'))
                return sendFile(response, publicDir, publicPath.slice(1), 'text/javascript; charset=utf-8');
...
async function loadTaskCardContext({ store, tasks }: any, task: any): Promise<any> {
...
export function createOwnerActionSession({ clock = (): any => Date.now(), ttlMs = OWNER_ACTION_NONCE_TTL_MS }: any = {}): any {
    let nonce: any = '';
    let expiresAtMs: any = 0;
    function issue(): any {
        const now: any = Number(clock());
        if (!nonce || now >= expiresAtMs) {
            nonce = crypto.randomBytes(24).toString('base64url');
            expiresAtMs = now + Math.max(1000, Number(ttlMs) || OWNER_ACTION_NONCE_TTL_MS);
        }
        return { nonce, expiresAt: new Date(expiresAtMs).toISOString() };
    }
    function authorize(value: any): any {
        const supplied: any = String(value || '');
        const now: any = Number(clock());
        if (!nonce || now >= expiresAtMs || supplied.length !== nonce.length)
            return false;
...
async function readJsonBody(request: any): Promise<any> {
    ...
    if (tooLarge)
        throw new JsonBodyError(413, '请求体超过 1 MiB 限制。');
    if (!chunks.length)
        return {};
    try {
...
class JsonBodyError extends Error {
    httpStatus: any;
    name: any;
    constructor(httpStatus: any, message: any) {
        super(message);
        this.name = 'JsonBodyError';
        this.httpStatus = httpStatus;
    }
}
async function sendFile(response: any, publicDir: any, name: any, type: any): Promise<any> {
