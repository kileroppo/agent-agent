...
export class CrossAgentMissionService {
    ...
    store: any;
    tasks: any;
    constructor({ tasks, store, governance, missionChildPolicy = null }: any = {}) { this.tasks = tasks; this.store = store; this.governance = governance; this.missionChildPolicy = missionChildPolicy; }
    ...
    async createBusinessMission({ title, items, requester, source, idempotencyKey, productMaturityBatchId = null }: any): Promise<any> {
        ...
        if (!missionTitle)
            throw new Error('请说明这组工作的总目标。');
        if (!normalized.length)
            throw new Error('请提供 1 到 11 项可执行的员工分工；每个依赖必须引用同一总任务中的分工 key，且不能形成循环。');
        const description: any = normalized.map((item: any, index: any): any => `${index + 1}. ${item.title}${item.description ? `：${item.description}` : ''}`).join('\n');
    ...
    async createMission({ title, description = '', requester, source, idempotencyKey, context }: any = {}): Promise<any> {
        let mission: any = await this.tasks.create({
            title,
            description,
        ...
        if (mission.status === 'waiting_approval') {
            const approvalId: any = mission.approvalRefs?.[0];
            const approval: any = approvalId ? (await this.store.listApprovals()).find((item: any): any => item.approvalId === approvalId && item.status === 'pending') : null;
            return {
                mission, children: [],
        ...
        const plan: any = mission.artifactRefs?.find((item: any): any => item.type === 'cross_agent_mission_plan')?.data;
        if (!plan) {
            return {
                mission,
                children: [],
    ...
    async dispatch(missionOrId: any): Promise<any> {
        let mission: any = typeof missionOrId === 'string' ? (await this.store.list()).find((item: any): any => item.taskId === missionOrId) : missionOrId;
        if (mission?.input?.context?.productMaturityBatchId && typeof this.store?.list === 'function') {
            mission = (await this.store.list()).find((item: any): any => item.taskId === mission.taskId) || mission;
        }
        if (!mission)
            throw new Error('找不到要继续的多人协作任务。');
        const existing: any = mission.artifactRefs?.find((item: any): any => item.type === 'cross_agent_mission_summary');
        if (mission.status === 'succeeded' || existing?.data?.completed === true)
            return { mission, children: [], reply: '这项多人工作已经完成汇总，不会重复安排员工。' };
        const plan: any = mission.artifactRefs?.find((item: any): any => item.type === 'cross_agent_mission_plan')?.data;
        if (!plan)
            throw new Error('多人协作计划未生成，未创建分工。');
        const parentIssueId: any = mission.governance?.paperclipIssueId || null;
        const existingChildren: any = this.store?.list ? (await this.store.list()).filter((task: any): any => task.parentTaskId === mission.taskId) : [];
        const childByKey: any = new Map();
        for (const subtask of plan.subtasks) {
            const idempotencyKey: any = `${mission.idempotencyKey || `mission:${mission.taskId}`}:${subtask.key}`;
            const matches: any = existingChildren.filter((item: any): any => item.idempotencyKey === idempotencyKey);
            if (mission?.input?.context?.productMaturityBatchId && matches.length > 1) {
                throw new Error('产品成熟度固定分工出现重复幂等任务，已停止恢复。');
            }
            const child: any = matches[0];
            if (child)
                childByKey.set(subtask.key, child);
        }
        await this.resumeQueuedMaturityChildren({ mission, plan, childByKey });
        const attemptedApprovalResume: any = new Set();
        const runSubtask: any = async (subtask: any): Promise<any> => {
            this.missionChildPolicy?.assertAuthorized({ mission, subtask });
            const idempotencyKey: any = `${mission.idempotencyKey || `mission:${mission.taskId}`}:${subtask.key}`;
            const dependencyKeys: any = dependenciesFor(plan.subtasks, subtask);
            const fixedSourceTaskIds: any[] = [...new Set(Array.isArray(subtask.context?.sourceTaskIds)
                    ? subtask.context.sourceTaskIds.map((taskId: any): any => clean(taskId, 200)).filter(Boolean)
                    : [])];
            const dependencyTaskIds: any[] = [...new Set(dependencyKeys
                    .map((key: any): any => childByKey.get(key)?.taskId)
                    .filter(Boolean))];
            const signedMaturityItem: any = subtask.context?.productMaturityAuthorization?.kind === 'product-maturity-validation';
            const sourceTaskIds: any = signedMaturityItem
                ? fixedSourceTaskIds
                : fixedSourceTaskIds.length ? fixedSourceTaskIds : dependencyTaskIds;
            let child: any = existingChildren.find((task: any): any => task.idempotencyKey === idempotencyKey) || null;
            if (!child)
                child = await this.tasks.create({
                    title: subtask.title,
        ...
        while (true) {
            const activeCount: any = [...childByKey.values()].filter((item: any): any => isActivelyRunning(item.status)).length;
            const availableSlots: any = Math.max(0, 4 - activeCount);
            if (!availableSlots)
                break;
            const ready: any = plan.subtasks.filter((subtask: any): any => {
                const child: any = childByKey.get(subtask.key);
                if (child && (child.status !== 'waiting_approval' || attemptedApprovalResume.has(subtask.key)))
                    return false;
                return dependenciesFor(plan.subtasks, subtask).every((key: any): any => {
                    const dependency: any = childByKey.get(key);
                    return dependency && dependencySatisfied(mission, dependency.status);
                });
            });
            if (!ready.length)
                break;
            const batch: any = ready.slice(0, availableSlots);
            batch.forEach((subtask: any): any => {
                if (childByKey.get(subtask.key)?.status === 'waiting_approval')
                    attemptedApprovalResume.add(subtask.key);
            });
            const created: any = await Promise.all(batch.map(runSubtask));
        ...
        if (maturityMission && state.status === 'succeeded' && mission.attempt === 2) {
            ...
            const latestMission: any = (await this.store.list()).find((item: any): any => item.taskId === mission.taskId);
    ...
    async resumeQueuedMaturityChildren({ mission, plan, childByKey }: any): Promise<any> {
        if (!mission?.input?.context?.productMaturityBatchId)
            return;
        const recoverable: any = plan.subtasks.filter((subtask: any): any => {
            const child: any = childByKey.get(subtask.key);
        ...
        for (const subtask of recoverable) {
            ...
            if (child?.status !== 'queued'
                ...
                && !isExactQueuedMaturityContentRetry(child))
                continue;
            if (!dependenciesFor(plan.subtasks, subtask).every((key: any): any => dependencySatisfied(mission, childByKey.get(key)?.status)))
                continue;
            this.missionChildPolicy.assertAuthorized({ mission, subtask });
            const idempotencyKey: any = `${mission.idempotencyKey || `mission:${mission.taskId}`}:${subtask.key}`;
            const resumed: any = await this.tasks.resumeVerifiedQueuedMissionChild({
                missionTaskId: mission.taskId,
                taskId: child.taskId,
                idempotencyKey,
            });
...
function missionSummary(mission: any, plan: any, children: any, state: any): any {
    ...
    const statuses: any = plan.subtasks.map((subtask: any): any => {
        const child: any = children.find((item: any): any => item.idempotencyKey === `${mission.idempotencyKey || `mission:${mission.taskId}`}:${subtask.key}`)
            || children.find((item: any): any => item.assigneeAgentId === subtask.agentId && item.taskType === subtask.taskType);
        return {
            key: subtask.key,
            title: subtask.title,
            ...
            artifactTypes: (child?.artifactRefs || []).filter(isVerifiedArtifact).map((item: any): any => item.type)
        };
    });
    return {
        ...
        data: {
            ...
            childTaskIds: children.map((item: any): any => item.taskId),
            completed: state.allDone,
            terminal: state.allTerminal,
            employeeIds: plan.subtasks.map((item: any): any => item.agentId),
            statuses,
            decision: plan.kind === 'business' ? businessDecision(statuses, children, state) : decisionFor(children, state.allDone)
        }
    };
}
function replyFor(mission: any, plan: any, children: any, state: any): any {
    const names: Record<string, any> = { operator: '运维官', architect: '架构师', xiaod: '小D', 'intel-researcher': '小R', 'office-assistant': '办公执行助理' };
    const work: any = plan.subtasks.map((item: any): any => names[item.agentId] || item.agentId).join('、');
    if (plan.kind === 'business') {
        const summary: any = mission.artifactRefs?.find((item: any): any => item.type === 'cross_agent_mission_summary')?.data;
        if (!state.allTerminal)
            return `总任务已建立，已安排${work}处理。我会以这一个总任务持续跟进，不让你分别追问每个人。`;
        const done: any = summary?.statuses?.filter((item: any): any => item.status === 'succeeded').length || 0;
        const open: any = summary?.statuses?.filter((item: any): any => item.status !== 'succeeded') || [];
        if (!open.length)
            return `${plan.summary}已完成：${work}的 ${done} 项分工均已交付并进入统一汇总。`;
        return `${plan.summary}已完成阶段汇总：${done} 项已交付；${open.map((item: any): any => `${names[item.employeeId] || item.employeeId}为${statusLabel(item.status)}`).join('，')}。未完成部分没有被冒充为成功。`;
    }
    if (!state.allDone)
        return `已安排${work}协同处理这次军团盘点。我会在结论出来后直接告诉你现在的情况和下一步。`;
    const decision: any = mission.artifactRefs?.find((item: any): any => item.type === 'cross_agent_mission_summary')?.data?.decision;
    if (!decision)
        return `已安排${work}完成这次军团盘点。结论已保存；没有触发登录、外发、付费或扩权。`;
    const owner: any = decision.ownerAction ? `现在需要你处理的是：${decision.ownerAction}` : '现在没有必须由你决定的事情。';
    return `盘点完成：${decision.health}。下一步建议：${decision.nextAction}。${owner}`;
}
function decisionFor(children: any, allDone: any): any {
    const health: any = children.find((item: any): any => item.taskType === 'operations.health-review')?.artifactRefs?.find((item: any): any => item.type === 'health_report')?.data;
    const architecture: any = children.find((item: any): any => item.taskType === 'governance.architecture-review')?.artifactRefs?.find((item: any): any => item.type === 'architecture_review')?.data;
    const unhealthy: any = (health?.components || []).filter((item: any): any => item.status !== 'healthy').map((item: any): any => item.name || '一项本机组件');
    const healthText: any = !health ? '本机检查结果还在整理' : health.overall === 'healthy' ? '本机运行正常' : `发现需要检查的部分：${unhealthy.length ? unhealthy.join('、') : '本机组件'}`;
    const nextAction: any = String(architecture?.nextAction || (allDone ? '继续按真实工作积累记录；有新的卡点我会主动说明。' : '等待员工完成当前分工。')).replace(/。$/, '');
    const ownerAction: any = health?.overall === 'healthy' ? null : '请在方便时查看本机运行状态；我不会自行重置、登录或扩大权限';
    return { health: healthText, nextAction, ownerAction };
}
function normalizeBusinessItems(items: any): any {
    if (!Array.isArray(items) || items.length < 1 || items.length > 11)
        return [];
    const seen: any = new Set();
    const normalized: any = items.map((item: any, index: any): any => {
        const title: any = clean(item?.title, 500);
        const taskType: any = clean(item?.taskType, 120);
        const agentId: any = clean(item?.agentId, 80);
        if (!title || !taskType || !agentId)
            return null;
        let key: any = clean(item?.key, 80) || `work-${index + 1}`;
        while (seen.has(key))
            key = `${key}-${index + 1}`;
        seen.add(key);
        return canonicalizeBusinessAssignment({
            ...
            sourceUrls: Array.isArray(item?.sourceUrls) ? [...new Set(item.sourceUrls.map((url: any): any => clean(url, 2000)).filter(Boolean))].slice(0, 5) : [],
            ...
            platforms: Array.isArray(item?.platforms) ? [...new Set(item.platforms.map((platform: any): any => clean(platform, 40)).filter(Boolean))].slice(0, 3) : [],
    ...
    if (normalized.length !== items.length)
        return [];
    const keys: any = new Set(normalized.map((item: any): any => item.key));
    if (normalized.some((item: any): any => item.dependsOn.some((key: any): any => !keys.has(key) || key === item.key)))
        return [];
...
function businessDecision(statuses: any, children: any, state: any): any {
    const office: any = children.find((item: any): any => item.assigneeAgentId === 'office-assistant')
        ?.artifactRefs?.find((item: any): any => item.type === 'office_briefing_package' && isVerifiedArtifact(item))?.data;
    const open: any = statuses.filter((item: any): any => item.status !== 'succeeded');
    return {
        ...
        openItems: open.map((item: any): any => ({ employeeId: item.employeeId, title: item.title, status: item.status })),
        briefing: office ? {
            title: clean(office.title, 500),
            summary: clean(office.summary, 1000),
            openItems: Array.isArray(office.openItems) ? office.openItems.map((item: any): any => clean(item, 500)).filter(Boolean).slice(0, 5) : [],
            nextAction: clean(office.nextAction, 500)
        } : null
    };
}
function isActivelyRunning(status: any): any {
...
function dependenciesFor(subtasks: any, subtask: any): any {
    const explicit: any = Array.isArray(subtask.dependsOn) ? subtask.dependsOn : [];
    if (explicit.length)
        return explicit;
    if (subtask.dependsOnPrevious !== true)
        return [];
    const index: any = subtasks.findIndex((item: any): any => item.key === subtask.key);
    return index > 0 ? subtasks.slice(0, index).map((item: any): any => item.key) : [];
}
function hasDependencyCycle(subtasks: any): any {
    const visiting: any = new Set();
    const visited: any = new Set();
    const visit: any = (subtask: any): any => {
        if (visiting.has(subtask.key))
            return true;
        if (visited.has(subtask.key))
            return false;
        visiting.add(subtask.key);
        for (const key of dependenciesFor(subtasks, subtask)) {
            const dependency: any = subtasks.find((item: any): any => item.key === key);
...
function isVerifiedArtifact(artifact: any): any {
