...
import { analysisIntentLabel, resolveAnalysisIntent } from './analysis-intent.js';
...
export class LocalVideoContentAnalyst {
  ...
  async performanceReview(task, { sourceArtifacts = null } = {}) {
    ...
    const learning = buildMetricLearning(task.input, metrics, normalizedContentMetrics);
    const data = {
...
function buildMetricLearning(input, metrics, derivedMetrics) {
  ...
  const comparableSampleCount = Math.max(0, Number(input?.comparableSampleCount) || 0);
  const binding = {
    platform,
  ...
  const enoughComparableSamples = comparableSampleCount >= 5 && Boolean(contentType) && observationWindow === '72h';
  if (!metricBindingComplete || !enoughComparableSamples) {
    return {
      status:'insufficient_sample',
      reason:!metricBindingComplete
        ? '指标缺少平台内容ID、发布时间、平台或内容版本绑定。'
        : '需要至少五条同平台、同内容类型、统一72小时口径的可比较样本。',
  ...
  return {
    status:'proposed',
    comparableSampleCount,
    metricBindingComplete:true,
    binding,
    proposal:{
      kind:'LearningProposal',
      schemaVersion:'agent.army/content-learning-proposal/v1',
      status:'waiting_reviewer_approval',
      keep:['保留当前已有证据支持的开场与结构变量。'],
      testNext:['下一版只调整一个主要变量，并继续关联原任务、内容版本和72小时指标。'],
      evidence:{ metrics, derivedMetrics, scope:{ platform, contentType, observationWindow } },
      claimBoundary:'这是基于同类样本的待审核建议，不构成确定因果或增长承诺。',
    },
    requiresHumanReview:true,
    productionMutationAllowed:false,
  };
...
function buildDigest(segments, modules, evidenceMode) {
  const selected = segments.slice(0, 3);
  const corePoints = modules.slice(0, 3).map((module) => ({
    point:clean(module.finding, 100),
    evidence:module.evidence,
  }));
  const goldenQuotes = selected.slice(0, 2).map((segment) => ({
