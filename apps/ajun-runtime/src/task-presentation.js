...
export function formatTaskPresentation(value) {
  ...
  if (presentation.detailUrl) lines.push(`查看任务：${presentation.detailUrl}`);
  return lines.join('\n');
}

export function taskDetailBaseUrl(value, fallback = '') {
...
function taskDetailUrl(baseUrl, detailPath) {
  const base = taskDetailBaseUrl(baseUrl);
