const LEGACY_DEDICATED_FEISHU_AGENT_IDS = new Set(['intel-researcher', 'office-assistant']);

function supportsDedicatedFeishu(agent) {
  return agent?.status === 'active' && (
    LEGACY_DEDICATED_FEISHU_AGENT_IDS.has(agent?.agentId)
    || (
...
export class EmployeeFeishuConnectionService {
  constructor({ registry, store, fleet } = {}) {
  ...
  async list() {
    ...
    const states = this.fleet.snapshot();
    const results = [];
    for (const agent of agents.filter(supportsDedicatedFeishu)) {
      const app = apps.find((item) => item.agentId === agent.agentId);
      const credentialed = app ? Boolean(await this.store.getSecret(agent.agentId)) : false;
      results.push({
  ...
  async connect(agentId, input) {
    const id = String(agentId || '').trim();
    const agent = (await this.registry.list()).find((item) => item.agentId === id && supportsDedicatedFeishu(item));
    if (!agent) throw new EmployeeFeishuConnectionError('这名员工未声明独立飞书入口，不能从这里接线。');
    const appId = String(input?.appId || '').trim();
    const appSecret = String(input?.appSecret || '').trim();
    const allowedUserId = String(input?.allowedUserId || '').trim();
    if (!/^cli_[a-zA-Z0-9]{8,}$/.test(appId)) throw new EmployeeFeishuConnectionError('飞书 App ID 格式不正确。');
    if (appSecret.length < 16 || appSecret.length > 512) throw new EmployeeFeishuConnectionError('飞书 App Secret 格式不正确。');
    if (!/^ou_[a-zA-Z0-9_-]{8,}$/.test(allowedUserId)) throw new EmployeeFeishuConnectionError('允许人员必须填写自己的飞书 open_id。');
    const app = await this.store.upsertApp({
      agentId:id,
      appId,
    ...
    const channel = await this.fleet.startApp(app);
    return {
      agentId:id,
      name:agent.name,
      configured:true,
...
function modelReadiness(independentRuntime) {
  const state = independentRuntime?.state;
