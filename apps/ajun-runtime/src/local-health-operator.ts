import { DeterministicLocalHealthProbe } from './deterministic-local-health-probe.ts';
const DEFAULT_COMPONENT_REGISTRY: any[] = [
...
export class LocalHealthOperator {
    ...
    now: any;
    runtimeProbe: any;
    constructor({ governance, runtimeProbe = new DeterministicLocalHealthProbe(), componentRegistry = DEFAULT_COMPONENT_REGISTRY, now = (): any => new Date() }: any = {}) {
        this.governance = governance;
        this.runtimeProbe = runtimeProbe;
        this.componentRegistry = normalizeRegistry(componentRegistry);
        this.now = now;
    }
    async execute(task: any): Promise<any> {
        if (task.taskType === 'operations.failure-recovery')
            return this.reviewFailure(task);
        const checkedAt: any = this.now().toISOString();
        ...
        ].map((component: any): any => attachDependencies(component, this.componentRegistry));
        const overall: any = components.every((item: any): any => item.status === 'healthy') ? 'healthy' : 'degraded';
        const incidents: any = correlateHealthIncidents(components, this.componentRegistry);
        const report: Record<string, any> = {
            schemaVersion: 'agent.army/operations-health/v1',
            checkedAt,
            overall,
            components,
            dependencyGraph: this.componentRegistry.map((item: any): any => ({ componentId: item.id, dependencies: item.dependencies })),
            incidents,
            ...
            recommendedAction: overall === 'healthy'
                ? '无需恢复动作。'
                : `先检查 Paperclip 本机服务，并按登记依赖逐项验证：${incidents.map((item: any): any => item.componentId).join('、') || '异常组件'}；不要尝试重置账号、凭据或外部连接。`
        };
        return {
    ...
    async reviewFailure(task: any): Promise<any> {
        const checkedAt: any = this.now().toISOString();
        const context: any = task.input?.context || {};
        const failure: any = context.failure || {};
        const registry: any = this.componentRegistry;
        const componentId: any = String(context.componentId || failure.componentId || '').trim() || inferComponentId(failure);
        const component: any = registry.find((item: any): any => item.id === componentId) || null;
        const incidentCorrelation: any = correlateFailureEvents({
            componentId,
            failure,
        ...
        const highRisk: any = failure.highRisk === true || ['authorization_or_permission', 'data_loss', 'security'].includes(String(context.failureClassification?.failureClass || failure.category || ''));
        const canRetry: any = failure.retryable === true
...
function selectRecoveryPlaybook({ canRetry, component, failure, highRisk }: any): any {
    if (!component)
        return { playbookId: 'unregistered_component_escalation', selectedFromRegistry: false, steps: ['登记组件及依赖', '由技术专家在隔离环境诊断'], externalSideEffects: false };
    if (highRisk)
        return { playbookId: 'high_risk_escalation', selectedFromRegistry: true, steps: ['冻结自动恢复', '保留脱敏证据', '升级技术专家和负责人'], externalSideEffects: false };
    if (canRetry)
        return {
...
function correlateHealthIncidents(components: any, registry: any): any {
    const statusById: any = new Map(components.map((item: any): any => [item.id, item.status]));
    return components.filter((item: any): any => item.status !== 'healthy').map((item: any): any => {
        const registered: any = registry.find((entry: any): any => entry.id === item.id);
        const unhealthyDependencies: any = (registered?.dependencies || []).filter((dependency: any): any => statusById.get(dependency) !== 'healthy');
        const affectedDependents: any = registry.filter((entry: any): any => entry.dependencies.includes(item.id)).map((entry: any): any => entry.id);
        return {
...
function correlateFailureEvents({ componentId, failure, events, registry, checkedAt }: any): any {
    const now: any = Date.parse(checkedAt);
    const dependencyIds: any = new Set(registry.find((item: any): any => item.id === componentId)?.dependencies || []);
    const related: any = events.filter((event: any): any => {
        const occurredAt: any = Date.parse(event?.occurredAt || '');
        const closeInTime: any = Number.isFinite(occurredAt) && Number.isFinite(now) && Math.abs(now - occurredAt) <= 15 * 60 * 1000;
        const sameComponent: any = String(event?.componentId || '') === componentId;
        const dependencyEvent: any = dependencyIds.has(String(event?.componentId || ''));
        const sameCode: any = failure.code && String(event?.code || '') === String(failure.code);
        return closeInTime && (sameComponent || dependencyEvent || sameCode);
    }).slice(-12).map((event: any): any => ({
...
function postRecoveryChecks({ component, failure, context }: any): any {
    return [
...
function attachDependencies(component: any, registry: any): any {
    const registered: any = registry.find((item: any): any => item.id === component.id);
    return { ...component, registered: Boolean(registered), dependencies: registered?.dependencies || [] };
}
function normalizeRegistry(value: any): any {
    return (Array.isArray(value) ? value : []).filter((item: any): any => item?.id).map((item: any): any => ({
        id: String(item.id),
...
function inferComponentId(failure: any): any {
    const value: any = `${failure.code || ''} ${failure.stage || ''}`;
    if (/xiaod/i.test(value))
        return 'xiaod';
    if (/paperclip/i.test(value))
        return 'paperclip';
    if (/ajun|executor/i.test(value))
        return 'ajun-runtime';
    return 'unknown';
}
