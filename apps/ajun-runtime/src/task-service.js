...
export class TaskService {
  ...
  async overview({ includeTasks = true } = {}) {
    ...
    const billing = this.billingOverview(tasks, [...agents, ...(manager ? [manager] : [])], startOfRecentDays(7));
    return {
  ...
  async usageOverview() {
    ...
    return {
      ...
      billing:this.billingOverview(tasks, await this.registry.list(), since),
    };
  }
  ...
  billingOverview(tasks, agents, since) {
    ...
    return reconcileUsageBilling(tasks, this.usageLedger.summarize({ since, agentIds }), { since });
  }
