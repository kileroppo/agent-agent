...
export class TaskService {
  ...
  async executeTask(task, agent) {
    if (
      ...
      && agent?.status === 'active'
      && this.officePresentationWorkspaceRoot
    ) {
      return this.executeDeterministicPresentation(task, agent);
    }
    return this.executionCoordinator.execute(task, agent);
  }

  async executeDeterministicPresentation(task, agent) {
    const executor = this.capabilityCatalog.executor(agent.agentId, this.executors);
    ...
    try {
      ...
      if (updated.status === 'succeeded') {
        await this.syncOfficePresentationWorkProducts(updated);
      }
    } catch (error) {
      updated = await this.store.updateTask(updated.taskId, {
        status:'waiting_test',
  ...
  async syncOfficePresentationWorkProducts(task) {
    ...
    const persisted = outputItems(await this.governance.getIssueWorkProducts(issueId));
    const persistedTypes = new Set(persisted
