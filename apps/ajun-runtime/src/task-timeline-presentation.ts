const TIMELINE_SCHEMA_VERSION: any = 'agent.army/task-timeline/v1';
const LAN_VISIBLE_EVENTS: any = new Set([
    'task_received',
...
const EVENT_PRESENTATION: any = Object.freeze({
...
export function presentTaskTimelinePage(page: any, { taskId, audience = 'lan', filters = [] }: any = {}): any {
    const resolvedAudience: any = audience === 'local-owner' ? 'local-owner' : 'lan';
    const items: any = (Array.isArray(page?.items) ? page.items : [])
        .filter((event: any): any => resolvedAudience === 'local-owner' || LAN_VISIBLE_EVENTS.has(event?.eventType))
        .map((event: any): any => presentTaskTimelineEvent(event, { audience: resolvedAudience }))
        .filter(Boolean);
    return {
        ...
        filters: [...new Set((Array.isArray(filters) ? filters : []).map((item: any): any => cleanText(item, 24)).filter(Boolean))],
    };
}
export function presentTaskTimelineEvent(event: any, { audience = 'lan' }: any = {}): any {
    ...
    const publicSummary: any = cleanText(event.publicSummary, 360);
    ...
    if (audience === 'local-owner')
        item.technical = technicalProjection(event);
    return item;
}
function technicalProjection(event: any): any {
    const cost: any = finiteNumber(event.costAmount ?? event.cost?.amount);
    ...
    return Object.fromEntries(Object.entries(technical).filter(([, value]: any): any => (value !== null && value !== '' && (!Array.isArray(value) || value.length))));
}
function summaryForStatus(eventType: any, status: any, fallback: any): any {
    if (eventType === 'quality_check_completed') {
        if (['failed', 'revise', 'blocked', 'quality_failed'].includes(status))
            return '质量检查发现问题，结果不会直接进入下游。';
        if (['passed', 'success', 'succeeded'].includes(status))
            return '当前结果已经通过质量检查。';
    }
...
function toneForStatus(status: any, fallback: any): any {
    if (['failed', 'blocked', 'denied', 'error'].includes(status))
        return 'danger';
    if (['warning', 'revise', 'ambiguous', 'degraded', 'quality_failed'].includes(status))
        return 'warning';
    if (['success', 'succeeded', 'passed', 'completed'].includes(status))
        return 'success';
    return fallback;
}
function cleanQualityResult(value: any): any {
    ...
    const result: Record<string, any> = {
        ...
        failedCriteria: (Array.isArray(value.failedCriteria) ? value.failedCriteria : [])
            .slice(0, 20).map((item: any): any => cleanText(item, 160)).filter(Boolean),
    };
    return result.status || result.gateId || result.failedCriteria.length ? result : null;
}
function cleanArtifactRefs(values: any): any {
    if (!Array.isArray(values))
        return [];
    return values.slice(0, 20).map((value: any): any => {
        if (typeof value === 'string')
            return { artifactId: cleanText(value, 160) };
        if (!value || typeof value !== 'object')
            return null;
        return {
    ...
    }).filter((value: any): any => value?.artifactId);
}
function cleanRef(value: any): any {
    const text: any = cleanText(value, 240);
    return /^(?:runtime|artifact|checkpoint):\/\/[a-z0-9._~:/?#\[\]@!$&'()*+,;=%-]+$/i.test(text) ? text : '';
}
function cleanHash(value: any): any {
    const text: any = cleanText(value, 160);
    return /^(?:sha256:)?[a-f0-9]{32,128}$/i.test(text) ? text : '';
}
function cleanCursor(value: any): any {
    const text: any = cleanText(value, 512);
    return /^[A-Za-z0-9_-]+$/.test(text) ? text : null;
}
function cleanText(value: any, limit: any): any {
    return String(value ?? '').replace(/[\u0000-\u001f\u007f]/g, ' ').replace(/\s+/g, ' ').trim().slice(0, limit);
}
function redactSensitiveText(value: any, limit: any): any {
...
function validTimestamp(value: any): any {
    const text: any = cleanText(value, 40);
    return text && Number.isFinite(Date.parse(text)) ? new Date(text).toISOString() : null;
}
function finiteNumber(value: any): any {
...
export const taskTimelinePresentationContract: any = Object.freeze({
    schemaVersion: TIMELINE_SCHEMA_VERSION,
