...
import { buildM5PlatformCopy, deriveM5ContentVersionId, validM5MediaChecksum, } from '@agent-army/m5-kernel/content-version';
import { CONTENT_PLATFORM_PLAYBOOKS, contentQualityChecklist, normalizeContentBrief, normalizeContentChannel, } from '@agent-army/m5-contracts';
import { validM5ProductionTemplateBinding } from './m5-production-template-resolver.ts';
import { findArtifact, needsInput, readArtifactText, referencedArtifacts, successResult, writeArtifact, } from './local-content-artifacts.ts';
import { evidenceSegments } from './local-content-analysis.ts';
export class LocalContentCreator {
    ...
    scriptPackage: any;
    store: any;
    constructor({ store, artifactsDir, allowedArtifactRoots = [], advisor = null, scriptPackage = null, now = (): any => new Date(), }: any = {}) {
        this.store = store;
    ...
    supports(agent: any): any { return agent?.agentId === 'content-creator'; }
    async execute(task: any, { sourceArtifacts = null, allowAdvisor = true }: any = {}): Promise<any> {
        if (task.taskType === 'content.video-script-package') {
            if (!this.scriptPackage?.execute)
                return needsInput(this.now(), 'video_script_package_unavailable', '可拍脚本能力暂时不可用。');
        ...
        const transcriptArtifact: any = findArtifact(sources, 'confirmed_transcript');
        const analysisArtifact: any = findArtifact(sources, 'video_content_analysis_report');
        if (!transcriptArtifact)
            return needsInput(this.now(), 'confirmed_transcript_required', '小创必须引用系统质量确认稿或人工确认稿。');
        if (!analysisArtifact || analysisArtifact.data?.evidenceMode !== 'formal')
            return needsInput(this.now(), 'formal_analysis_required', '小创必须引用小拆基于确认稿生成的正式分析。');
        const platforms: any = normalizePlatforms(task.input?.platforms, `${task.input?.title || ''}\n${task.input?.description || ''}`);
        if (!platforms.length)
            return needsInput(this.now(), 'platform_required', '请明确至少一个目标平台。');
        if (platforms.length > 3)
            return needsInput(this.now(), 'platform_limit_exceeded', '首版一次最多生成三个平台版本。');
        const transcript: any = await readArtifactText(transcriptArtifact, this.allowedArtifactRoots);
        const evidence: any = evidenceSegments(transcript);
        const fallback: any = buildDrafts({
        ...
        let drafts: any = fallback;
        let modelUsage: any = null;
        let advisorApplied: any = false;
        if (allowAdvisor && this.advisor?.draft) {
            try {
                const advisedResult: any = await this.advisor.draft({ title: task.input?.title, contentGoal: task.input?.contentGoal, platforms, transcript, analysis: analysisArtifact.data });
                const advised: any = advisedResult?.data || advisedResult;
        ...
        const contentBrief: any = normalizeContentBrief({
            ...
            coreJudgment: analysisArtifact.data?.summary || (evidence as any)[0]?.text || '只表达确认稿和正式分析可以支持的判断。',
        ...
        const data: Record<string, any> = {
            ...
            contentBrief,
            contentStrategy: {
                primaryPlatform: (platforms as any)[0],
        ...
        const artifact: any = await writeArtifact({
            ...
            validation: {
                ...
                semanticQualityGateCount: drafts.every((draft: any): any => draft.qualityChecklist?.length === 6) ? 6 : 0,
                visualAnchorPresent: drafts.every((draft: any): any => Boolean(draft.visualAnchor)),
                advisorApplied,
            },
            completedAt
        });
    ...
    async m5PlatformAdapt(task: any, sources: any): Promise<any> {
        const script: any = findArtifact(sources, 'video_script_package');
        const render: any = findArtifact(sources, 'render_package');
        const context: any = task.input?.context || {};
        const platform: any = String(context.pipelineCase?.fields?.platform || '').trim();
        const renderOutput: any = (render?.data?.outputs as any)?.[platform] || render?.data;
        const mediaPath: any = safeM5RelativePath(renderOutput?.relativePath || renderOutput?.outputPath);
        const checksum: any = String(renderOutput?.checksum || '').trim().toLowerCase();
        const audioHash: any = String(renderOutput?.audioHash || '').trim().toLowerCase();
        const voiceProviderActionId: any = String(renderOutput?.voiceProviderActionId || '').trim();
        const pipelineCaseId: any = String(context.pipelineCaseId || '').trim();
        const dayCaseId: any = String(context.pipelineCase?.parentCaseId || '').trim();
        const scheduledDate: any = String(context.pipelineCase?.fields?.scheduledDate || '').trim();
        ...
        const contentVersionId: any = deriveM5ContentVersionId({
            pipelineCaseId,
            platform,
        ...
        const renderVariantKey: any = String(renderOutput?.variantKey || 'baseline');
        const grayScriptVariant: any = script.data?.variants?.gray_douyin || null;
        ...
        const expectedVariantKey: any = renderVariantKey;
        const scriptVariant: any = (script.data?.variants as any)?.[expectedVariantKey]
            || (expectedVariantKey === 'baseline' ? {
                ...script.data,
                variantKey: 'baseline',
                templateBinding: script.data?.templateLifecycle?.templateBinding,
            } : null);
        const templateBinding: any = scriptVariant?.templateBinding;
        const exactGrayTarget: any = templateBinding?.source === 'approved_single_gray'
        ...
        if (expectedVariantKey === 'baseline'
        ...
        const copy: any = buildM5PlatformCopy(scriptVariant, platform);
        const templateApplication: Record<string, any> = {
        ...
        const contentVersion: Record<string, any> = {
            contentVersionId,
            platform,
            dayCaseId,
            ...
            publishingStatus: 'draft_only',
            generatedAt: completedAt,
            templateVersionId: String(templateBinding?.templateVersionId || 'm5-template-default-v1'),
...
function buildDrafts({ title, contentGoal, platforms, analysis, evidence }: any): any {
    const proof: any = (evidence as any)[0] || { timestamp: null, text: '确认稿缺少可展示片段' };
    const second: any = (evidence as any)[1] || proof;
    return enrichDrafts(platforms.map((platform: any): any => ({
        platform,
        titleCandidates: [
...
function enrichDrafts(drafts: any): any {
    return drafts.map((draft: any): any => {
        const platform: any = normalizeContentChannel(draft?.platform) || String(draft?.platform || '');
        const playbook: any = (CONTENT_PLATFORM_PLAYBOOKS as any)[platform] || null;
        const qualityChecklist: any = playbook ? contentQualityChecklist(platform) : [];
        return {
            ...draft,
            platform,
            ...
            humanChecklist: [
                ...
                ...qualityChecklist.map((item: any): any => item.instruction),
            ],
        };
    });
}
function safeM5RelativePath(value: any): any {
    const relative: any = String(value || '').trim().replaceAll('\\', '/');
    if (!relative
        || relative.startsWith('/')
        || relative.split('/').some((part: any): any => !part || part === '.' || part === '..')
        || !/\.mp4$/i.test(relative))
        return null;
    return relative;
}
function normalizePlatforms(value: any, text: any): any {
    const explicit: any = Array.isArray(value) ? value : [];
    const inferred: any = [
        [/抖音|douyin/i, 'douyin'],
    ...
    ].filter(([pattern]: any): any => pattern.test(text)).map(([, platform]: any): any => platform);
    return [...new Set([...explicit, ...inferred].map((item: any): any => (normalizeContentChannel(item) || clean(item, 40).toLowerCase())).filter(Boolean))];
}
...
function platformAdaptation(platform: any): any {
    return (({
    ...
    }) as any)[platform] || '按目标平台长度和展示形式调整，但不改变已确认事实。';
}
function validAdvisedDrafts(value: any, platforms: any): any {
    return Array.isArray(value) && value.length === platforms.length && value.every((draft: any): any => Array.isArray(draft?.humanChecklist) && draft.humanChecklist.length > 0);
}
