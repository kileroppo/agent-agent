export async function routeM5PublisherApi({
  method,
  url,
  local,
  getService,
  ...
  if (method === 'POST' && url === '/api/tool-executions') {
    if (!local) return { status:403, payload:{ error:'岗位工具调用只能由本机受控执行面发起。' } };
  ...
  const receiptMatch = String(url || '').match(
    /^\/api\/publish-receipts\/([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})$/i,
  );
  if (method === 'GET' && receiptMatch) {
    if (!local) return { status:403, payload:{ error:'发布凭证只允许本机负责人读取。' } };
    const service = await getService();
    return {
      status:200,
