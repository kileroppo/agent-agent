...
export class PublicWebTransport {
  constructor({ command = '/usr/bin/curl', run = runBoundedCommand, lookupImpl = dns.lookup } = {}) {
  ...
  async fetch(url, {
    ...
    const responseByteLimit = Math.max(1, Math.min(
      Number.isSafeInteger(maxBytes) ? maxBytes : MAX_BYTES,
      MAX_TRANSPORT_BYTES,
    ));
    ...
    const pinnedAddress = await publicResolvedAddress(url, resolvedAddress, this.lookup);
    const resolveArgs = curlResolveArgs(url, pinnedAddress);
    let raw;
    try {
...
async function publicResolvedAddress(value, suppliedAddress, lookup) {
  let url;
  try {
  ...
  const supplied = String(suppliedAddress || '').trim();
  if (supplied) {
    if (!net.isIP(supplied) || privateHost(supplied)) throw new Error('公开网页固定解析地址无效。');
    return supplied;
  }
  if (net.isIP(url.hostname)) return url.hostname.replace(/^\[|\]$/g, '');
  const addresses = await lookup(url.hostname, { all:true, verbatim:true });
  if (!addresses.length || addresses.some(({ address }) => privateHost(address))) {
    throw new Error('公开网页域名解析到本机或内网地址。');
  }
  return addresses[0].address;
}

function curlResolveArgs(value, resolvedAddress) {
  const address = String(resolvedAddress || '').trim();
  if (!address) return [];
  if (!net.isIP(address)) throw new Error('公开网页固定解析地址无效。');
  const url = new URL(String(value));
  const port = url.port || (url.protocol === 'https:' ? '443' : '80');
  const curlAddress = net.isIP(address) === 6 ? `[${address}]` : address;
...
export function runBoundedCommand(command, args, { maxBuffer = MAX_BYTES + 64 * 1024 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.stderr.on('data', (chunk) => {
      ...
      if (current < 64 * 1024) stderr.push(chunk.subarray(0, 64 * 1024 - current));
    });
...
function parseCurlResponse(raw, maxBodyBytes = MAX_BYTES) {
  ...
  const body = bytes.subarray(split + headerSeparatorLength(bytes, split));
  if (body.length > maxBodyBytes) throw new Error('公开网页响应超过传输上限。');
