...
export class TaskIntake {
  ...
  async resolveRoute(requested) {
    const requestedAgentId = requested.agentId || null;
    let candidates = await this.registry.candidates(requested.taskType);
  ...
  async applyOpenTaskGuard(task, agent) {
  ...
  async createRequiredApproval(task, agent) {
    const { taskType } = task;
    const title = task.input?.title || '';
    const description = task.input?.description || '';
    const wechatChat = task.input?.wechatChat;
    ...
    if (
      taskType !== WECHAT_CHAT_TASK_TYPE
  ...
  async projectGovernance(task, agent) {
    ...
    const approval = task.approvalRefs?.length
      ...
      : null;
    if (!usesPaperclipHermesExecution(agent) && !shouldProjectToPaperclip(task, approval)) return task;
    const parentIssueId = String(task.input?.context?.parentPaperclipIssueId || '').trim();
    const projection = parentIssueId && this.governance.projectChild
  ...
  async reload(taskId) {
    return (await this.store.list()).find((item) => item.taskId === taskId);
  }
}
...
function taskRecord(input, requested, route, idempotencyKey) {
  ...
  return attachDeliveryQualityContracts({
    ...
    routing:{
      ...
      candidateAgentIds:route.candidates.map((item) => item.agentId),
...
function hasAffirmativeHighRiskIntent(value) {
  ...
  return HIGH_RISK_ACTIONS.some((action) => {
    let startAt = 0;
...
function requiresOrganizationGovernance(title, description) {
