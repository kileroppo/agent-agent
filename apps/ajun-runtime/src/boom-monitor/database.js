...
export class BoomMonitorDatabase {
  ...
  ensureColumn(table, column, definition) {
    const columns = new Set(this.connection.prepare(`PRAGMA table_info(${table})`).all().map((row) => row.name));
    if (!columns.has(column)) this.connection.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`);
  }

  upsertCreator(platform, creatorId, creatorName, followerCount) {
    const now = this.now();
  ...
  upsertWork(creatorDbId, platform, work) {
    const workId = String(work.work_id ?? '').trim();
    if (!workId) throw new Error('work_id 必须提供');
    const existing = this.connection.prepare('SELECT id FROM works WHERE platform=? AND work_id=?').get(platform, workId);
    const plays = Object.hasOwn(work, 'plays') && work.plays == null ? null : integer(work.plays);
    const now = this.now();
    this.connection.prepare(`
    ...
    `).run(
      creatorDbId, platform, workId, String(work.creator_work_id ?? '').trim(), String(work.title ?? '').trim(),
      String(work.publish_at ?? '').trim(), integer(work.likes), integer(work.favorites), plays,
      String(work.cover_url ?? '').trim(), String(work.source_url ?? '').trim(), normalizeJson(work.metadata ?? {}), now, now,
    );
    const dbWorkId = Number(this.connection.prepare('SELECT id FROM works WHERE platform=? AND work_id=?').get(platform, workId).id);
    this.connection.prepare(`
  ...
  historyMetrics(creatorDbId, platform, excludeWorkId, limit = 20) {
    ...
    `).all(excludeWorkId, creatorDbId, platform, limit).map((item) => (
      item.platform === 'xiaohongshu' ? integer(item.likes) + integer(item.favorites) : integer(item.likes)
    ));
  }

  getScore(workId) { return row(this.connection.prepare('SELECT * FROM scores WHERE work_id=?').get(integer(workId))); }

  upsertScore(workId, score) {
    const now = this.now();
    this.connection.prepare(`
      INSERT INTO scores(work_id,score_version,r_value,m_value,grade,tier,baseline_metric,baseline_sample_count,follower_snapshot,baseline_at,baseline_version,created_at,updated_at)
    ...
    `).run(
      workId, String(score.version ?? score.score_version ?? 'legacy-v1'), Number(score.r_value), Number(score.m_value),
      String(score.grade), String(score.tier), score.baseline_metric == null ? null : Number(score.baseline_metric),
      integer(score.sample_count ?? score.baseline_sample_count), integer(score.follower_snapshot), score.baseline_at ?? null,
      score.baseline_version ?? null, now, now,
    );
  }

  upsertShadowScore(workId, score) {
    const version = String(score.version ?? '').trim();
    const grade = String(score.grade ?? '').trim();
    if (!version || !grade) throw new Error('影子评分缺少版本或等级。');
    const now = this.now();
    this.connection.prepare(`
  ...
  getShadowScore(workId, version = 'v2') {
    const value = this.connection.prepare('SELECT score_json FROM shadow_scores WHERE work_id=? AND version=?').get(integer(workId), version);
    return value ? parseJson(value.score_json, null) : null;
  }

  listShadowScores(version = 'v2', limit = 100) {
    return this.connection.prepare(`
    ...
    `).all(version, clamp(limit, 1, 500)).map((value) => {
      const item = row(value); const scoreJson = item.score_json; delete item.score_json;
  ...
  queueScanJob(sourceType, creatorRef = null, payload = null) {
    const now = this.now();
    const result = this.connection.prepare('INSERT INTO scan_jobs(source_type,creator_ref,payload_json,created_at,updated_at) VALUES(?,?,?,?,?)')
  ...
  takeNextScanJob() {
    ...
    try {
      const item = row(this.connection.prepare("SELECT * FROM scan_jobs WHERE status='queued' ORDER BY created_at ASC LIMIT 1").get());
      if (!item) { this.connection.exec('COMMIT'); return null; }
      const now = this.now();
  ...
  finishScanJob(jobId, status, errorMessage = null) {
    const now = this.now();
  ...
  upsertAnalysisQueue(workId, grade, scoreSnapshot = {}, analysisDepth = 'fast') {
    const priority = { T3:100, T2:50, T1:10 }[grade] ?? 0;
    const now = this.now();
    this.connection.prepare(`
  ...
  cancelPendingAnalysis(workId, reason) {
  ...
  cancelIneligibleQueuedAnalysis(reason) {
    const result = this.connection.prepare(`
      UPDATE analysis_queue SET status='cancelled',dispatch_error=?,updated_at=?
  ...
  nextDispatchBatch(limit = 20, workId = null) {
    const where = workId == null ? "aq.status='queued'" : "aq.status='queued' AND aq.work_id=?";
  ...
  beginDispatch(queueId) {
    const now = this.now();
    const result = this.connection.prepare(`UPDATE analysis_queue SET status='dispatching',started_at=?,updated_at=?,
      dispatch_attempt_count=dispatch_attempt_count+1,dispatch_error=NULL WHERE id=? AND status='queued'`)
      .run(now, now, queueId);
    return result.changes > 0;
  }

  finishDispatch(queueId, status, { taskId = null, result = {}, error = null } = {}) {
    const now = this.now();
  ...
  getSetting(key) {
    const result = this.connection.prepare('SELECT value_json FROM app_settings WHERE key=?').get(String(key));
  ...
  setAnalysisSettings(analysisAuto, analysisDailyLimit) {
    const statement = this.connection.prepare(`INSERT INTO app_settings(key,value_json,updated_at) VALUES(?,?,?)
      ON CONFLICT(key) DO UPDATE SET value_json=excluded.value_json,updated_at=excluded.updated_at`);
    this.connection.exec('BEGIN IMMEDIATE');
    try {
      const now = this.now();
  ...
  listWorksWithScores({ grade = null, platform = null, creatorId = null, limit = 50 } = {}) {
    const conditions = ['1=1']; const parameters = [];
    if (grade) { conditions.push('s.grade=?'); parameters.push(grade); }
    if (platform) { conditions.push('w.platform=?'); parameters.push(platform); }
    if (creatorId) { conditions.push('c.creator_id=?'); parameters.push(creatorId); }
  ...
  dashboardSummary() {
    const scalar = (sql) => Number(this.connection.prepare(sql).get().count);
    return {
  ...
  listScanJobs(limit = 20) { return this.connection.prepare('SELECT * FROM scan_jobs ORDER BY created_at DESC LIMIT ?').all(integer(limit)).map(row); }
  listAnalysisQueue(limit = 200) {
    return this.connection.prepare(`
...
function normalizeJson(value) { return JSON.stringify(value ?? {}); }
function parseJson(value, fallback) { try { return JSON.parse(value); } catch { return fallback; } }
function row(value) { return value == null ? null : { ...value }; }
