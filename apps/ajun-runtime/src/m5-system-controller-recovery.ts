...
import { consumeM5SystemPlanRevision, M5StageRecoveryController, } from './m5-stage-recovery-controller.ts';
const UUID_PATTERN: any = /[0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}/i;
const RECOVERABLE_SYSTEM_FAILURE: any = Symbol('m5RecoverableSystemFailure');
export function markM5SystemControllerFailure(error: any): any {
...
export function isRecoverableM5SystemControllerFailure(error: any): any {
    return error?.[RECOVERABLE_SYSTEM_FAILURE] === true;
}
export async function consumeM5SystemControllerPlanRevision({ governance, pipelineCaseId, runId, routineKey, systemRole, }: any = {}): Promise<any> {
...
export async function recoverM5SystemControllerFailure({ governance, issueId, runId, agentId, routineKey, systemRole, error, }: any = {}): Promise<any> {
    const contract: any = getM5RoutineExecutionContract(routineKey);
    if (!contract
        || contract.executionMode !== 'system_controller'
        || contract.systemController !== systemRole) {
    ...
    const verified: any = await governance.verifySystemAssignment({
        issueId,
        runId,
    ...
    const pipelineCaseId: any = String(issue.description || '').match(new RegExp(`当前 Case 为 (${UUID_PATTERN.source})`, 'i'))?.[1] || '';
    if (!pipelineCaseId) {
    ...
    const controller: any = new M5StageRecoveryController({ governance });
...
function safeSystemFailureSummary(error: any): any {
    const code: any = String(error?.code || error?.name || 'system_controller_error')
        .replace(/[^a-z0-9_.-]/gi, '_')
        .slice(0, 120);
    const message: any = String(error?.message || '系统控制器执行失败。')
