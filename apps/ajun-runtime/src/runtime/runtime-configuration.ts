...
import { isLoopbackHost } from '../lan-access.ts';

export type RuntimeConfigurationInput = Readonly<{
  environment?: NodeJS.ProcessEnv;
  compositionRootUrl?: string | URL;
...
export function createRuntimeConfiguration({
  ...
  compositionRootUrl = new URL('../runtime-composition-root.ts', import.meta.url),
...
function parseEmployeeIds(value: unknown) {
  return String(value || '')
    ...
    .filter((item: string) => /^[a-z][a-z0-9-]{0,63}$/.test(item));
}
