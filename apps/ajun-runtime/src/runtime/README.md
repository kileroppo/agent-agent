...
| `role-execution-composition.js` | 岗位执行总装、提案、TaskService 与失败恢复连接，不直接认识各能力实现 |
| `role-research-execution-composition.js` | 公开网页、动态网页、PDF、GitHub、Hermes 研究 Advisor 与研究岗位装配 |
| `role-content-execution-composition.js` | 办公、演示文稿、内容生产、本机视觉和岗位工具装配 |
| `role-technical-execution-composition.js` | 隔离修理房、诊断、推广、技术专家和停滞 watchdog 装配 |
