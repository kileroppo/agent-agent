...
export class TaskLocalAiRunEventRecorder {
  constructor({ eventStore, registry, resolveAssignment }) {
  ...
  async record(input = {}) {
    if (!this.eventStore?.appendTaskRunEvent) throw new ValidationError('任务运行事件存储不可用。');
    ...
    const capabilityId = String(event.capabilityId || '').trim();
    ...
    const provider = String(event.provider || 'local-ai').trim().slice(0, 120) || 'local-ai';
    const saved = this.eventStore.appendTaskRunEvent({
      taskId:task.taskId,
...
function eventSummary(eventType, capabilityId, provider) {
  if (eventType === 'capability_call_started') return `${capabilityId} 已开始调用本机登记路线。`;
  if (eventType === 'capability_call_succeeded') return `${capabilityId} 已由 ${provider} 返回确认回执。`;
  if (eventType === 'capability_result_ambiguous') return `${capabilityId} 的本机调用结果无法确认；已停止，不会自动重试。`;
