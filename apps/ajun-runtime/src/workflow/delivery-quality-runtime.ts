...
export class DeliveryQualityRuntime {
  ...
  constructor({ store, createTask, taskRunEvents = null, syncTask = null }: {
    ...
    this.lifecycleEvents = createLifecycleRecorder(taskRunEvents);
  ...
  async continue(task: RuntimeTask) {
    ...
    try {
      ...
      this.record(stopped, 'quality_review_start_failed', 'waiting_test', {
        error:{ code:'delivery_quality_review_start_failed' },
      });
      return stopped;
    }
...
function createLifecycleRecorder(eventStore: RuntimeEventStore | null) {
  return {
    recordPersisted(task: RuntimeTask, { previousTask = null } = {}) {
      if (!eventStore || !task?.taskId || sameLifecycle(previousTask, task)) return;
      try {
        const blocked = ['failed', 'waiting_test', 'needs_input', 'waiting_approval', 'paused'].includes(task.status);
        eventStore.appendTaskRunEvent({
          ...
          eventType:task.status === 'succeeded' ? 'workflow_completed' : blocked ? 'workflow_blocked' : 'workflow_state_changed',
          status:task.status,
...
function sameLifecycle(left: RuntimeTask | null, right: RuntimeTask) {
  if (!left) return false;
  return [left.status, left.currentStage, left.error?.code, left.execution?.outcome, left.deliveryQualityRuntime?.status]
    .every((value, index) => value === [right.status, right.currentStage, right.error?.code, right.execution?.outcome, right.deliveryQualityRuntime?.status][index]);
}
