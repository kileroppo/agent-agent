...
export class TechnicalRepairPromotion {
  constructor({
    ...
    sourceMode = 'legacy_runtime_git_root',
    sourceIdentity = null,
    verifySourceRoot = null,
    fsImpl = fs,
  } = {}) {
    this.projectRoot = path.resolve(projectRoot || process.cwd());
    this.allowedWorkspaceRoots = allowedWorkspaceRoots.map((item) => path.resolve(item));
  ...
  async promoteVerified(task, evidence) {
    const proof = evidence?.metadata?.agentArmyRepairEvidence;
    const workspaceInput = String(task.execution?.workspace?.path || '');
    const allowedFiles = repairFiles(task);
    const changedFiles = Array.isArray(proof?.changedFiles)
      ? proof.changedFiles.map((item) => String(item || '').trim()).filter(Boolean)
      : [];
    if (
      !workspaceInput
      ...
      || !changedFiles.length
      || changedFiles.some((item) => !allowedFiles.includes(item))
    ) {
      return {
        status:'rejected',
    ...
    const workspace = await canonicalAllowedDirectory(
      this.fs,
      workspaceInput,
      this.allowedWorkspaceRoots.length
    ...
    let snapshot;
    try {
      const snapshotFile = await readSafeRegularFile(this.fs, workspace, snapshotPath);
    ...
    for (const relativePath of changedFiles) {
      ...
      const sourceFile = await readSafeRegularFile(this.fs, this.projectRoot, source);
      const candidateFile = await readSafeRegularFile(this.fs, workspace, candidate);
      const current = sourceFile.content;
      const originalHash = hash(current);
  ...
  async commitAll(candidates) {
    const transaction = crypto.randomUUID();
    const staged = [];
    try {
      for (const item of candidates) {
        const temporary = `${item.source}.agent-army-new-${transaction}`;
        const backup = `${item.source}.agent-army-old-${transaction}`;
        const handle = await this.fs.open(temporary, 'wx', item.mode);
        try {
      ...
      for (const item of staged) {
        const currentFile = await readSafeRegularFile(
          this.fs,
          this.projectRoot,
          item.source,
        );
        ...
        const backupStat = await this.fs.lstat(item.backup);
        if (
          !backupStat.isFile()
      ...
      return {
        status:this.sourceMode === 'external_writable_git_root'
          ? 'candidate_promoted'
          : 'promoted',
        changedFiles:candidates.map((item) => item.relativePath),
      ...
      for (const item of [...staged].reverse()) {
        if (!item.replacementCommitted) continue;
        try {
      ...
      for (const item of staged) {
        await this.fs.unlink(item.temporary).catch(() => {});
        if (
          item.backupCreated
          && !item.replacementCommitted
          && !item.recoveryRequired
        ) {
          await this.fs.unlink(item.backup).catch(() => {});
          item.backupCreated = false;
        }
      }
    }
...
async function canonicalAllowedDirectory(fsImpl, input, allowedRoots) {
  if (!path.isAbsolute(input)) throw new Error('修理副本路径必须是绝对路径。');
  ...
  for (const root of allowedRoots) {
    try { allowed.push(await fsImpl.realpath(root)); } catch {}
  }
  if (!allowed.some((root) => isInsideOrSame(canonical, root))) {
    throw new Error('修理副本不在允许目录内。');
  }
  return canonical;
}

async function assertSafeRegularFile(fsImpl, root, candidate) {
  ...
  for (const segment of relative.split(path.sep).filter(Boolean)) {
    ...
    if (result.isSymbolicLink()) throw new Error('修复文件或路径祖先是符号链接。');
    if (cursor !== candidate && !result.isDirectory()) {
      throw new Error('修复文件的路径祖先不是目录。');
    }
  }
  if (!result?.isFile()) throw new Error('修复对象必须是普通文件。');
  const canonical = await fsImpl.realpath(candidate);
  if (canonical !== candidate) throw new Error('修复对象没有解析到原始文件。');
  return result;
}
...
function hashFromSnapshotCandidate(item, candidates) {
  const original = candidates.find((candidate) => candidate.source === item.source);
