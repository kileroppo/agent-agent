...
export class PaperclipRepairReconciler {
  constructor({ store, governance, evidenceRelay = null, now = () => Date.now(), intervalMs = 10_000 } = {}) {
    this.store = store;
  ...
  async reconcileOnce() {
    ...
    await Promise.all(tasks.filter(isDelegatedRepair).map((task) => this.reconcileTask(task)));
  }
...
function isDelegatedRepair(task) {
...
function validatedRepairEvidence(product) {
...
function repairArtifact(task, evidence, proof, now) {
  return {
    ...
    data:{ changedFiles:proof.changedFiles.map((item) => String(item).slice(0, 240)), testSummary:String(proof.testSummary || '相关测试已通过。').slice(0, 500), recoverySummary:String(proof.recoverySummary || '恢复检查已通过。').slice(0, 500), remainingTests:Array.isArray(proof.remainingTests) ? proof.remainingTests.map((item) => String(item).slice(0, 240)) : [] }
  };
}
