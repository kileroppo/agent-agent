...
export const TASK_CARD_SCHEMA_VERSION: any = 'agent.army/task-card/v1';
const TASK_CARD_RENDER_REVISION: any = 'card-ux4';
const ACTION_LABELS: any = Object.freeze({
    approve: '批准',
    reject: '拒绝',
    pause: '暂停任务',
    resume: '继续任务',
});
const ALLOWED_ACTIONS: any = new Set(Object.keys(ACTION_LABELS));
const TERMINAL_STATES: any = new Set(['rejected']);
/**
 ...
 */
export function presentTaskCard(task: any = {}, { approvals = [], recoveryView = null, owner = null, agentId = null, profileId = null, chatId = null, taskCardPolicy = null, }: any = {}): any {
    const relevantApprovals: any = taskApprovals(task, approvals);
    const pendingApproval: any = relevantApprovals.find((approval: any): any => approval?.status === 'pending') || null;
    const presentation: any = presentTask(task, { approvals: relevantApprovals, recoveryView });
    const taskCardCopy: any = publicTaskCardCopy(task, presentation);
    const state: any = cardState(task, { pendingApproval, recoveryView });
    const updatedAt: any = latestTimestamp(task, relevantApprovals, recoveryView);
...
function taskDetails(task: any): any {
    const taskType: any = safeIdentifier(task?.input?.taskType || task?.taskType);
    return {
        taskType: DEFAULT_TASK_DEFINITION_REGISTRY.taskLabel(taskType) || '军团任务',
        createdAt: validTimestamp(task?.createdAt) || null,
    };
}
function cardAgentId(task: any, value: any): any {
    return safeIdentifier(value || task?.source?.targetAgentId) || null;
}
function cardProfileId(task: any, value: any): any {
    return safeIdentifier(value || task?.source?.profileId) || null;
}
function cardChatId(task: any, value: any): any {
    return safeIdentifier(value || task?.source?.chatRef) || null;
}
function cardPolicy(value: any): any {
    const policy: any = safeIdentifier(value);
    return ['disabled', 'routed-task', 'durable-task', 'incident-only'].includes(policy) ? policy : null;
}
function deliveryLink(task: any): any {
    const delivery: any = (Array.isArray(task?.artifactRefs) ? task.artifactRefs : [])
        .find((artifact: any): any => artifact?.type === 'xiaod_media_delivery');
    const value: any = String(delivery?.data?.larkUrl || '').trim();
    if (!value || delivery?.data?.larkPermissionGranted !== true)
        return null;
    try {
        const url: any = new URL(value);
        const trustedHost: any = url.hostname === 'feishu.cn' || url.hostname.endsWith('.feishu.cn');
        if (url.protocol !== 'https:' || !trustedHost || !url.pathname.startsWith('/docx/'))
            return null;
...
function publicTaskCardCopy(task: any, presentation: any): any {
    if (task?.error?.code === 'xiaod_delivery_pending') {
        const title: any = publicText(task.input?.title || task.title, 160) || '当前视频任务';
        return {
...
export const projectTaskCard: any = presentTaskCard;
function taskApprovals(task: any, approvals: any): any {
    if (!Array.isArray(approvals))
        return [];
    const taskId: any = safeIdentifier(task?.taskId);
    const refs: any = new Set((Array.isArray(task?.approvalRefs) ? task.approvalRefs : [])
        .map(safeIdentifier)
        .filter(Boolean));
    return approvals.filter((approval: any): any => {
        const approvalId: any = safeIdentifier(approval?.approvalId);
        return (approvalId && refs.has(approvalId))
            || (taskId && safeIdentifier(approval?.taskId) === taskId);
    }).sort((left: any, right: any): any => approvalSortKey(right).localeCompare(approvalSortKey(left)));
}
function cardState(task: any, { pendingApproval, recoveryView }: any): any {
    if (pendingApproval)
        return 'waiting_approval';
    const taskStatus: any = normalizedState(task?.status);
    const recoveryState: any = normalizedState(recoveryView?.state || recoveryView?.status);
    if (['recovery_pending', 'technical_repair'].includes(recoveryState))
        return recoveryState;
    if (taskStatus === 'pending_approval')
        return 'waiting_approval';
    return taskStatus || 'unknown';
}
function cardTone(state: any, fallback: any): any {
    if (state === 'succeeded')
        return 'success';
    if (state === 'failed')
        return 'danger';
    if (['cancelled', 'rejected'].includes(state))
        return 'muted';
    if (['pausing', 'paused', 'waiting_approval', 'needs_input', 'waiting_test', 'recovery_pending', 'technical_repair'].includes(state))
        return 'attention';
    return ['active', 'attention', 'success', 'danger', 'muted'].includes(fallback) ? fallback : 'active';
}
function cardActions({ task, pendingApproval, recoveryView, state }: any): any {
    const actions: any[] = [];
    if (pendingApproval) {
        const approvalId: any = safeIdentifier(pendingApproval.approvalId);
        const governanceMode: any = safeIdentifier(pendingApproval.governanceMode);
        if (approvalId) {
            const labels: any = approvalActionLabels(pendingApproval.action);
    ...
    for (const candidate of Array.isArray(recoveryView?.actions) ? recoveryView.actions : []) {
        const action: any = normalizeAction(candidate?.action || candidate?.actionKey || candidate?.taskControlAction);
        if (!action || actions.some((item: any): any => item.action === action))
            continue;
    ...
    if (!pendingApproval && task?.execution?.executor === 'xiaod' && task.execution?.xiaodJobId) {
        if (['queued', 'running', 'pausing'].includes(state) && !actions.some((item: any): any => item.action === 'pause'))
            actions.push(actionView('pause'));
        if (state === 'paused' && !actions.some((item: any): any => item.action === 'resume'))
            actions.push(actionView('resume'));
    }
    ...
    if (TERMINAL_STATES.has(state) || isTaskCardTerminalStatus(state))
        return [];
    return actions.filter((item: any): any => ALLOWED_ACTIONS.has(item.action)).slice(0, 4);
}
function approvalActionLabels(action: any): any {
    if (action === 'pause-task')
        return { approve: '确认暂停', reject: '保持运行' };
    if (action === 'resume-task')
        return { approve: '确认继续', reject: '保持暂停' };
    return { approve: ACTION_LABELS.approve, reject: ACTION_LABELS.reject };
}
function actionView(action: any, { approvalId = '', governanceMode = '', label = '' }: any = {}): any {
    return {
        action,
...
function normalizeAction(value: any): any {
    const action: any = String(value || '').trim().toLowerCase();
    return ALLOWED_ACTIONS.has(action) ? action : '';
}
function ownerLabel(owner: any, task: any): any {
    const value: any = typeof owner === 'string'
        ? owner
...
function latestTimestamp(task: any, approvals: any, recoveryView: any): any {
    const timestamps: any = [
        ...
        ...approvals.flatMap((approval: any): any => [approval?.updatedAt, approval?.decidedAt, approval?.createdAt]),
    ].map(validTimestamp).filter(Boolean).sort();
    return timestamps.at(-1) || null;
}
function sourceRevision(task: any, approvals: any, recoveryView: any, updatedAt: any): any {
    const explicit: any = [
        ...
        ...approvals.map((approval: any): any => approval?.revision),
    ].map(normalizedRevision).filter(Boolean);
    if (updatedAt)
        return `${updatedAt}:${explicit.join('.') || '0'}`;
    const legacyTruth: Record<string, any> = {
        taskStatus: normalizedState(task?.status),
        approvalStates: approvals.map((approval: any): any => ({
            approvalId: safeIdentifier(approval?.approvalId),
            status: normalizedState(approval?.status),
        })).sort((a: any, b: any): any => String(a.approvalId).localeCompare(String(b.approvalId))),
...
function hashProjection(projection: any): any {
...
function approvalSortKey(approval: any): any {
    const timestamp: any = validTimestamp(approval?.updatedAt || approval?.decidedAt || approval?.createdAt);
...
function normalizedState(value: any): any {
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 80);
}
function normalizedRevision(value: any): any {
    if (typeof value === 'number' && Number.isFinite(value))
        return String(value);
    return safeIdentifier(value);
}
function safeIdentifier(value: any): any {
    return String(value || '').replace(/[\u0000-\u001f\u007f]/g, '').trim().slice(0, 240);
}
function publicText(value: any, limit: any): any {
