...
const OPEN_TASK_DELEGATES = DEFAULT_TASK_CAPABILITY_CATALOG.openTaskDelegates();
function supportsOpenTask(task, agent) {
  return Boolean(
    task
...
function routeOpenTaskForExecutor(task, agent) {
...
function inspectOpenTaskManifestCapabilities(task, agent) {
...
function decideIntelResearchOpenTask({
  task,
  agent,
  observation,
  progress = {},
...
export const openTaskRoutingPolicy = Object.freeze({
