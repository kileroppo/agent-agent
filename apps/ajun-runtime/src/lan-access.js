...
export function isLoopbackHost(host) { return ['127.0.0.1', 'localhost', '::1'].includes(String(host).toLowerCase()); }
export function isLocalAddress(address) { return isLoopbackHost(String(address).replace(/^::ffff:/, '')); }

export async function loadLanShareKey(filePath, enabled) {
...
export async function rotateLanShareKey(filePath, enabled) { return enabled ? writeLanShareKey(filePath) : null; }

async function writeLanShareKey(filePath) {
...
export function canAccessApi(request, { enabled, key }) {
