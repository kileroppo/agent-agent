import { agentArmyApprovalView as approvalView, agentArmyTaskView as taskView, } from './agent-army-client-task-view.ts';
import { armyStatusReadView, capabilitiesReadView, capabilityTruthView } from './agent-army-read-views.ts';
import { AgentArmyTaskInputError, prepareMissionCreateRequest, prepareTaskCreateRequest, } from './contracts/agent-army-task-input.ts';
...
export class AgentArmyClient {
    ...
    sleep: any;
    timeoutSignal: any;
    constructor({ baseUrl = process.env.AGENT_ARMY_BASE_URL || 'http://127.0.0.1:4321', fetchImpl = globalThis.fetch, timeoutSignalImpl = (ms: any): any => AbortSignal.timeout(ms), now = (): any => Date.now(), sleepImpl = (ms: any): any => new Promise((resolve: any): any => setTimeout(resolve, ms)), missionWaitMs = Number(process.env.AGENT_ARMY_MISSION_WAIT_MS || 45000), missionPollMs = Number(process.env.AGENT_ARMY_MISSION_POLL_MS || 1000), detailBaseUrl = process.env.AJUN_TASK_DETAIL_BASE_URL || '' }: any = {}) {
        this.baseUrl = loopbackBaseUrl(baseUrl);
        this.detailBaseUrl = taskDetailBaseUrl(detailBaseUrl, this.baseUrl);
        if (typeof fetchImpl !== 'function')
            throw new AgentArmyClientError('Agent Army MCP 缺少 HTTP 客户端。');
        this.fetchImpl = fetchImpl;
        this.timeoutSignal = timeoutSignalImpl;
        this.now = now;
        this.sleep = sleepImpl;
        this.missionWaitMs = boundedDuration(missionWaitMs, 0, 120000, 45000);
        this.missionPollMs = boundedDuration(missionPollMs, 50, 10000, 1000);
    }
    async capabilities(): Promise<any> {
        return capabilitiesReadView(await this.overview());
    }
    async armyStatus(): Promise<any> {
        return armyStatusReadView(await this.overview());
    }
    async employeeStatus(employee: any): Promise<any> {
        const overview: any = await this.overview();
        const agent: any = findEmployee(overview.agents || [], employee);
        if (!agent)
            throw new AgentArmyClientError(`没有找到员工“${safeText(employee, 80)}”。`);
        const recentTasks: any = (overview.tasks || [])
            .filter((task: any): any => task.assigneeAgentId === agent.agentId || task.routing?.requestedAgentId === agent.agentId)
            .sort(newestFirst)
            .slice(0, 5)
            .map((task: any): any => taskView(task, overview.approvals || [], this.detailBaseUrl));
        return {
    ...
    async listTasks({ status = null, employee = null, limit = 10 }: any = {}): Promise<any> {
        const overview: any = await this.overview();
        const agent: any = employee ? findEmployee(overview.agents || [], employee) : null;
        if (employee && !agent)
            throw new AgentArmyClientError(`没有找到员工“${safeText(employee, 80)}”。`);
        const wantedStatuses: any = normalizeStatuses(status);
        return (overview.tasks || [])
            .filter((task: any): any => !wantedStatuses.length || wantedStatuses.includes(task.status))
            .filter((task: any): any => !agent || task.assigneeAgentId === agent.agentId || task.routing?.requestedAgentId === agent.agentId)
            .sort(newestFirst)
            .slice(0, boundedLimit(limit))
            .map((task: any): any => taskView(task, overview.approvals || [], this.detailBaseUrl));
    }
    async getTask(taskId: any, { chatRef = '', agentId = '', profileId = '' }: any = {}): Promise<any> {
        const id: any = requiredId(taskId, '任务编号无效。');
        const overview: any = await this.overview();
        const task: any = (overview.tasks || []).find((item: any): any => item.taskId === id);
        if (!task)
            throw new AgentArmyClientError('没有找到这条任务。');
        ...
        try {
            ...
            const unavailableWithoutFeishuContext: any = [
                '当前会话不能读取',
                '只能读取或操作由飞书创建的任务卡',
                '任务卡缺少原飞书会话',
            ].some((fragment: any): any => message.includes(fragment));
            if (chatRef || !unavailableWithoutFeishuContext)
                throw error;
        }
        return {
    ...
    async createTask(input: any = {}): Promise<any> {
        const prepared: any = taskInputContract((): any => prepareTaskCreateRequest(input, { now: this.now }));
        if (prepared.kind === 'mission')
            return this.createMission(prepared.missionInput);
        const response: any = await this.request('/api/tasks', {
            method: 'POST',
            body: prepared.body,
        });
        const completionWatch: any = await this.ensureCompletionWatch(response.completionWatch, response.task?.taskId, prepared.chatRef, prepared.completionDelivery);
        return {
    ...
    async createMission(input: any = {}): Promise<any> {
        const prepared: any = taskInputContract((): any => prepareMissionCreateRequest(input, { now: this.now }));
        const response: any = await this.request('/api/mcp/missions', {
            method: 'POST',
            body: prepared.body,
        });
        const completionWatch: any = await this.ensureCompletionWatch(response.completionWatch, response.mission?.taskId, prepared.chatRef, prepared.completionDelivery);
        let overview: any = await this.overview();
        let missionTask: any = findMissionTask(overview, response);
        if (input.waitForTerminal === true && missionTask && !isTaskNotificationTerminalStatus(missionTask.status)) {
            const maxPolls: any = Math.ceil(this.missionWaitMs / this.missionPollMs);
            for (let poll: any = 0; poll < maxPolls && !isTaskNotificationTerminalStatus(missionTask.status); poll += 1) {
                await this.sleep(this.missionPollMs);
                overview = await this.overview();
                missionTask = findMissionTask(overview, response);
            }
        }
        const missionId: any = missionTask?.taskId || response.mission?.taskId;
        const childIds: any = new Set((response.children || []).map((task: any): any => task.taskId));
        const children: any = (overview.tasks || []).filter((task: any): any => (task.parentTaskId === missionId || childIds.has(task.taskId)));
        const missionView: any = taskView(missionTask, overview.approvals || [], this.detailBaseUrl);
        return {
            mission: missionView,
            children: children.map((task: any): any => taskView(task, overview.approvals || [], this.detailBaseUrl)),
            completionWatch,
            ...(prepared.completionDelivery ? { completionDelivery: prepared.completionDelivery } : {}),
            userMessage: completionWatchMessage(missionResultMessage(missionView, response.reply), completionWatch)
        };
    }
    async ensureCompletionWatch(serverResult: any, taskId: any, chatRef: any, completionDelivery: any = null): Promise<any> {
        const delivery: any = normalizeCompletionDelivery(completionDelivery || serverResult?.completionDelivery);
        ...
        if (serverResult?.registered === true || (serverResult?.required === false && !delivery))
            return serverResult;
        return this.registerCompletionWatch(taskId, chatRef);
    }
    async registerCompletionWatch(taskId: any, chatRef: any): Promise<any> {
        const task: any = safeText(taskId, 100);
        const chat: any = safeText(chatRef, 240);
        if (!task || !chat)
    ...
    async controlTask(taskId: any, action: any): Promise<any> {
        const id: any = requiredId(taskId, '任务编号无效。');
        const normalized: any = String(action || '').trim().toLowerCase();
        if (!['pause', 'resume'].includes(normalized))
            throw new AgentArmyClientError('任务控制只支持 pause 或 resume。');
        const response: any = await this.request(`/api/mcp/tasks/${encodeURIComponent(id)}/${normalized}`, { method: 'POST', body: {} });
        const overview: any = await this.overview();
        return {
    ...
    async recordTaskFeedback(taskId: any, { sentiment, note = '', chatRef = '' }: any = {}): Promise<any> {
        const id: any = requiredId(taskId, '任务编号无效。');
        const normalized: any = String(sentiment || '').trim().toLowerCase();
        if (!['useful', 'needs_improvement'].includes(normalized)) {
            throw new AgentArmyClientError('任务评价只支持 useful 或 needs_improvement。');
        }
        const response: any = await this.request(`/api/mcp/tasks/${encodeURIComponent(id)}/feedback`, {
            method: 'POST',
            body: {
    ...
    async listApprovals({ status = 'pending', limit = 10 }: any = {}): Promise<any> {
        const overview: any = await this.overview();
        const normalized: any = String(status || '').trim();
        return (overview.approvals || [])
            .filter((approval: any): any => !normalized || normalized === 'all' || approval.status === normalized)
            .sort((left: any, right: any): any => String(right.createdAt || '').localeCompare(String(left.createdAt || '')))
            .slice(0, boundedLimit(limit))
            .map(approvalView);
    }
    async resolveApproval(approvalId: any, decision: any): Promise<any> {
        const id: any = requiredId(approvalId, '审批编号无效。');
        const normalized: any = String(decision || '').trim().toLowerCase();
        if (!['approve', 'reject'].includes(normalized))
            throw new AgentArmyClientError('审批决定只支持 approve 或 reject。');
        const response: any = await this.request(`/api/mcp/approvals/${encodeURIComponent(id)}/${normalized}`, { method: 'POST', body: {} });
        const overview: any = await this.overview();
        return {
            task: taskView(response.task, overview.approvals || [], this.detailBaseUrl),
            approval: approvalView((overview.approvals || []).find((approval: any): any => approval.approvalId === id) || { approvalId: id, status: normalized === 'approve' ? 'approved' : 'rejected' })
        };
    }
    async revokePrivateReadGrant(approvalId: any, { chatRef }: any = {}): Promise<any> {
        const id: any = requiredId(approvalId, '审批编号无效。');
        const boundChatRef: any = safeText(chatRef, 240);
        if (!boundChatRef)
            throw new AgentArmyClientError('从飞书撤销微信临时授权时必须带原会话标识。');
        const response: any = await this.request(`/api/feishu/approvals/${encodeURIComponent(id)}/revoke-private-read-grant`, {
            method: 'POST',
    ...
    async getPaperclipAssignment(input: any = {}): Promise<any> {
        const response: any = await this.request('/api/mcp/paperclip-assignment', {
            method: 'POST',
    ...
    async recordPaperclipLocalAiRunEvent(input: any = {}): Promise<any> {
        const event: any = input.event && typeof input.event === 'object' && !Array.isArray(input.event)
            ? input.event
            : {};
        return this.request('/api/mcp/local-ai-run-event', {
            ...
            body: {
                ...
                }).filter(([, value]: any): any => value !== undefined && value !== '')),
            },
            paperclipApiKey: process.env.PAPERCLIP_API_KEY,
        });
    }
    async completePaperclipAssignment(input: any = {}): Promise<any> {
    ...
    async executeAgentProposal(input: any = {}): Promise<any> {
    ...
    async executeTechnicalRepair(input: any = {}): Promise<any> {
    ...
    async executeOperationsHealth(input: any = {}): Promise<any> {
    ...
    async executeContentGrowth(input: any = {}): Promise<any> {
    ...
    async executeM5Stage(input: any = {}): Promise<any> {
    ...
    async request(pathname: any, { method = 'GET', body = null, paperclipApiKey = '', timeoutMs = 30000 }: any = {}): Promise<any> {
        let response: any;
        try {
            response = await this.fetchImpl(`${this.baseUrl}${pathname}`, {
                ...
                signal: this.timeoutSignal(Math.max(1000, Math.min(Number(timeoutMs) || 30000, 900000)))
            });
        }
        catch (error: any) {
        ...
        if (!response.ok)
            throw new AgentArmyClientError(safeText(payload?.error || `A君运行时返回 HTTP ${response.status}`, 500));
        return payload;
    }
}
function taskContextCapsuleView(value: any = {}): any {
    return {
        ...
        adoptedArtifactRefs: (Array.isArray(value.adoptedArtifactRefs) ? value.adoptedArtifactRefs : []).slice(0, 10).map((item: any): any => ({
...
function taskInputContract(operation: any): any {
    try {
        return operation();
    }
    catch (error: any) {
        if (error instanceof AgentArmyTaskInputError)
            throw new AgentArmyClientError(error.message);
        throw error;
    }
}
function loopbackBaseUrl(value: any): any {
    let url: any;
    try {
        url = new URL(String(value || ''));
    }
    catch {
        throw new AgentArmyClientError('Agent Army MCP 地址无效。');
    }
...
function paperclipAssignmentIdentity(input: any): any {
    return {
...
function safeMetrics(value: any): any {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return undefined;
    return Object.fromEntries(Object.entries(value).slice(0, 20).map(([key, item]: any): any => [
        safeText(key, 80),
        typeof item === 'number' || typeof item === 'boolean' ? item : safeText(item, 120)
    ]).filter(([key, item]: any): any => key && item !== ''));
}
function technicalRepairScopeView(scope: any = {}): any {
    return {
...
function technicalDiagnosisView(context: any = {}): any {
    const diagnosis: any = context?.diagnosis || {};
...
function m5RecoveryView(value: any = {}): any {
    return {
...
function architectureGroundTruthView(value: any = {}): any {
    return {
        ...
        agents: (Array.isArray(value?.agents) ? value.agents : []).slice(0, 30).map((item: any): any => ({
        ...
        taskTypes: (Array.isArray(value?.taskTypes) ? value.taskTypes : []).slice(0, 50).map((item: any): any => ({
        ...
        taskEvidence: (Array.isArray(value?.taskEvidence) ? value.taskEvidence : []).slice(0, 24).map((item: any): any => ({
...
function architectureEvidenceRefsView(value: any): any {
    return (Array.isArray(value) ? value : []).slice(0, 30).map((item: any): any => ({
        ref: safeText(item?.ref, 500),
        claim: safeText(item?.claim, 1000)
    })).filter((item: any): any => item.ref && item.claim);
}
function qualityReviewView(value: any): any {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return undefined;
    const status: any = safeText(value.status, 40);
...
function architectureFactClaimsView(value: any): any {
    return (Array.isArray(value) ? value : []).slice(0, 20).map((item: any): any => ({
        claim: safeText(item?.claim, 1000),
...
function architectureJudgmentsView(value: any): any {
    return (Array.isArray(value) ? value : []).slice(0, 20).map((item: any): any => ({
...
function architectureCandidateProposalsView(value: any): any {
    return (Array.isArray(value) ? value : []).slice(0, 10).map((item: any): any => ({
...
function findEmployee(agents: any, value: any): any {
    const key: any = String(value || '').trim().toLowerCase();
    return agents.find((agent: any): any => String(agent.agentId || '').toLowerCase() === key)
        || agents.find((agent: any): any => String(agent.name || '').toLowerCase() === key)
        || null;
}
function findMissionTask(overview: any, response: any): any {
    return (overview.tasks || []).find((task: any): any => task.taskId === response.mission?.taskId) || response.mission;
}
function missionResultMessage(mission: any, fallback: any): any {
    if (!isTaskNotificationTerminalStatus(mission?.status))
        return safeText(fallback, 2000);
    const summary: any = (mission.artifacts || []).find((artifact: any): any => artifact.type === 'cross_agent_mission_summary')?.report;
    if (!summary)
        return safeText(fallback, 2000);
    const done: any = (summary.statuses || []).filter((item: any): any => item.status === 'succeeded').length;
    return `总任务已进入终态：${mission.status}，${done}/${summary.statuses?.length || 0} 项分工完成。请根据 mission 和 children 中的已验证产物直接向负责人做最终汇报，不要再返回中间进度。`;
}
function completionWatchMessage(message: any, watch: any): any {
    const base: any = safeText(message, 2000);
...
function normalizeStatuses(value: any): any {
    const values: any = Array.isArray(value) ? value : String(value || '').split(',');
    return [...new Set(values.map((item: any): any => safeText(item, 60)).filter(Boolean))];
}
function requiredId(value: any, message: any): any {
    const id: any = String(value || '').trim();
    if (!/^[0-9a-z-]{8,100}$/i.test(id))
        throw new AgentArmyClientError(message);
    return id;
}
function boundedLimit(value: any): any { const parsed: any = Number(value); return Number.isFinite(parsed) ? Math.max(1, Math.min(50, Math.floor(parsed))) : 10; }
function boundedDuration(value: any, min: any, max: any, fallback: any): any { const parsed: any = Number(value); return Number.isFinite(parsed) ? Math.max(min, Math.min(max, Math.floor(parsed))) : fallback; }
function newestFirst(left: any, right: any): any { return String(right.updatedAt || right.createdAt || '').localeCompare(String(left.updatedAt || left.createdAt || '')); }
