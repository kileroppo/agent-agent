...
import { paperclipHermesAdapterConfig } from './governance-hermes-runtime.ts';
const PROPOSABLE_CAPABILITIES: any = new Set([
    'content.public.fetch',
    'content.public.media',
    'content.public.subtitles',
    'wechat.local-vault.chat.read'
]);
const WECHAT_LOCAL_VAULT_CAPABILITY: any = 'wechat.local-vault.chat.read';
...
export class AgentProposalService {
    ...
    now: any;
    registry: any;
    restrictedAcceptanceRunner: any;
    store: any;
    taskService: any;
    constructor({ store, registry, governance = null, taskService = null, restrictedAcceptanceRunner = null, now = (): any => new Date() }: any = {}) {
        this.store = store;
        this.registry = registry;
        this.governance = governance;
        this.taskService = taskService;
        this.restrictedAcceptanceRunner = restrictedAcceptanceRunner;
        this.now = now;
    }
    async create(input: any, { source = 'ajun-runtime' }: any = {}): Promise<any> {
        const requestedOutcome: any = text(input?.requestedOutcome || input?.title, '请说明新岗位要交付什么。');
        const sourceEventRef: any = optional(input?.sourceEventRef);
        if (sourceEventRef) {
            const existing: any = (await this.store.listProposals()).find((item: any): any => item.sourceEventRef === sourceEventRef);
            if (existing)
                return existing;
        }
        const requestedCapabilities: any = capabilities(input?.requestedCapabilities, requestedOutcome);
        const candidateManifest: any = candidate(input, requestedOutcome, requestedCapabilities);
        const trialReadiness: any = currentTrialReadiness(candidateManifest, requestedCapabilities);
        const now: any = this.now().toISOString();
        let proposal: any = await this.store.createProposal({
    ...
    async reviewRegisteredDrafts(text: any): Promise<any> {
        const manifests: any = (await this.registry.list({ includeInactive: true })).filter((manifest: any): any => ['draft', 'active'].includes(manifest?.status) && mentionsDraft(text, manifest));
        const reviewed: any[] = [];
        for (const manifest of manifests) {
            let proposal: any = (await this.store.listProposals()).find((item: any): any => item.manifestAgentId === manifest.agentId) || null;
            ...
            if (!proposal)
                proposal = await this.store.createProposal(manifestProposal(manifest, this.now()));
            else {
                const projected: any = manifestProposal(manifest, this.now());
            ...
            if (proposal.status === 'draft')
                proposal = await this.submit(proposal.proposalId);
            const trialReadiness: any = currentTrialReadiness(proposal.candidateManifest, proposal.requestedCapabilities);
    ...
    async submit(proposalId: any): Promise<any> {
        let draft: any = await this.get(proposalId);
        if (draft.status !== 'draft')
            throw new ProposalValidationError(`草案状态“${draft.status}”不能提交审核。`);
        const refreshedCandidateManifest: any = draft.requestedCapabilities?.includes(WECHAT_LOCAL_VAULT_CAPABILITY)
            ? applyWechatCandidateContract(draft.candidateManifest)
            : draft.candidateManifest;
        const refreshedReadiness: any = currentTrialReadiness(refreshedCandidateManifest, draft.requestedCapabilities);
        const refreshedAcceptanceTask: any = draft.requestedCapabilities?.includes(WECHAT_LOCAL_VAULT_CAPABILITY)
            ? acceptanceTask({}, refreshedCandidateManifest)
        ...
        const reviewer: any = await this.runReviewerTask(draft);
        await this.store.updateProposal(proposalId, { reviewRefs: [...(draft.reviewRefs || []).filter((item: any): any => item.role !== 'reviewer'), reviewer] });
        let proposal: any = await this.transition(proposalId, 'pending_approval', 'creator', '已提交组织级审核；未启动测试实例。');
        if (this.governance)
            proposal = await this.store.updateProposal(proposalId, { governance: await this.governance.projectProposal(proposal) });
        return proposal;
    }
    async runReviewerTask(proposal: any): Promise<any> {
        const at: any = this.now().toISOString();
        if (!this.taskService)
            return reviewerReview(proposal.candidateManifest, proposal.requestedCapabilities, at);
        const reviewerManifest: any = await this.registry?.get?.('reviewer');
        const reviewerAdapterConfig: any = reviewerManifest ? paperclipHermesAdapterConfig(reviewerManifest) : null;
        const reviewerModelSelection: any = reviewerAdapterConfig
            ? {
            ...
            : null;
        const wechatPrivateReview: any = proposal.requestedCapabilities.includes(WECHAT_LOCAL_VAULT_CAPABILITY);
        const domainReviewLines: any = wechatPrivateReview
            ? [
        ...
        const reviewTask: any = await this.taskService.create({
        ...
        const report: any = reviewTask.artifactRefs?.find((item: any): any => ['review_report', 'employee_role_report'].includes(item.type));
    ...
    async approveForTest(proposalId: any, decisionBy: any = 'A君'): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'pending_approval')
            throw new ProposalValidationError(`草案状态“${proposal.status}”不能进入受限测试。`);
        const trialReadiness: any = currentTrialReadiness(proposal.candidateManifest, proposal.requestedCapabilities);
        if (this.governance) {
            const governance: any = await this.governance.approveProposal(proposal);
            await this.store.updateProposal(proposalId, { governance });
        }
        if (trialReadiness.status !== 'ready') {
            let revised: any = await this.transition(proposalId, 'needs_revision', decisionBy, '负责人已确认岗位方向；当前没有对应的真实执行能力，草案转为待补能力，不会进入试用或上线。');
            if (this.governance)
                revised = await this.store.updateProposal(proposalId, { governance: await this.governance.updateProposal(revised) });
            return revised;
        }
        return this.transition(proposalId, 'testing', decisionBy, '负责人批准受限测试；仅允许白名单能力与验收任务。');
    }
    async createTestInstance(proposalId: any, { hermesProfileName = null }: any = {}): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'testing')
            throw new ProposalValidationError('草案未获批准，不能创建受限测试实例。');
        assertRunnableCandidate(proposal);
        const existing: any = (await this.store.listTestInstances()).find((item: any): any => item.proposalId === proposalId && item.status !== 'stopped');
        if (existing)
            return existing;
        const active: any = (await this.store.listTestInstances()).find((item: any): any => item.status === 'ready' || item.status === 'running');
        if (active)
            throw new ProposalValidationError('当前已有受限测试实例，请先结束后再创建下一条。');
    ...
    async recordAcceptance(proposalId: any, { artifactTitle, artifactRef, passed = false }: any = {}): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'testing')
            throw new ProposalValidationError('只有测试中的草案可以登记验收。');
        if (!artifactTitle || !artifactRef)
            throw new ProposalValidationError('验收必须提供可验证产物标题与引用。');
        if (passed)
            assertRunnableCandidate(proposal);
        const instance: any = (await this.store.listTestInstances()).find((item: any): any => item.proposalId === proposalId && item.status === 'ready');
        if (!instance)
            throw new ProposalValidationError('请先准备受限测试实例。');
        ...
        if (!passed)
            return this.transition(proposalId, 'needs_revision', 'operator', '验收未通过；未创建正式 Agent 或飞书路由。');
        const audit: any[] = [...(proposal.audit || []), { at: this.now().toISOString(), actor: 'operator', action: 'restricted_acceptance_passed', detail: '受限测试通过，等待负责人单独决定是否激活；当前仍不允许派活。' }];
        let updated: any = await this.store.updateProposal(proposalId, { audit, acceptance: { status: 'passed', artifactTitle: String(artifactTitle).slice(0, 160), artifactRef: String(artifactRef).slice(0, 500), passedAt: this.now().toISOString() } });
        if (this.governance)
            updated = await this.store.updateProposal(proposalId, { governance: await this.governance.updateProposal(updated) });
        return updated;
    }
    async activate(proposalId: any, decisionBy: any = 'A君'): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'testing')
            throw new ProposalValidationError('只有完成受限测试且仍在测试中的草案可以激活。');
        if (proposal.acceptance?.status !== 'passed')
            throw new ProposalValidationError('受限测试尚未通过，不能激活岗位。');
        assertRunnableCandidate(proposal);
        const formal: any = typeof this.registry?.formal === 'function'
            ? await this.registry.formal(proposal.candidateManifest.agentId)
            : (await this.registry.list({ includeInactive: true })).find((item: any): any => item.agentId === proposal.candidateManifest.agentId);
        if (!formal || formal.status !== 'active') {
            throw new ProposalValidationError('受限测试通过后仍需落盘并启用正式 Manifest，不能只靠运行时提案生成活动员工。');
        }
        let updated: any = await this.transition(proposalId, 'active', decisionBy, '负责人确认受限测试产物后激活岗位。');
        if (this.governance)
            updated = await this.store.updateProposal(proposalId, { governance: await this.governance.updateProposal(updated) });
        const rosterSync: any = await this.syncActivatedRoster();
        if (rosterSync)
            updated = await this.store.updateProposal(proposalId, { governance: { ...(updated.governance || {}), rosterSync } });
        return updated;
    }
    async runRestrictedAcceptance(proposalId: any, input: any = {}): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'testing')
            throw new ProposalValidationError('只有测试中的草案可以运行受限试用。');
        const instance: any = (await this.store.listTestInstances()).find((item: any): any => item.proposalId === proposalId && item.status === 'ready');
        if (!instance)
            throw new ProposalValidationError('请先准备受限测试实例。');
        if (!this.restrictedAcceptanceRunner)
            throw new ProposalValidationError('当前没有可用的受限试用执行器。');
        let result: any;
        try {
        ...
        const artifact: any = result?.artifactRefs?.find((item: any): any => item.validation?.exists && item.validation?.readable && item.validation?.nonEmpty);
    ...
    async recordTestEvidence(proposalId: any, { artifactTitle, artifactRef }: any = {}): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'testing')
            throw new ProposalValidationError('只有测试中的草案可以登记测试产物。');
        if (!artifactTitle || !artifactRef)
            throw new ProposalValidationError('测试产物必须提供标题与引用。');
        const instance: any = (await this.store.listTestInstances()).find((item: any): any => item.proposalId === proposalId && item.status === 'ready');
        if (!instance)
            throw new ProposalValidationError('请先准备受限测试实例。');
        const evidence: any[] = [...(instance.evidence || []), { title: String(artifactTitle).slice(0, 160), ref: String(artifactRef).slice(0, 500), recordedAt: this.now().toISOString() }];
        await this.store.updateTestInstance(instance.testInstanceId, { evidence });
        const audit: any[] = [...(proposal.audit || []), { at: this.now().toISOString(), actor: 'operator', action: 'test_evidence_recorded', detail: `已登记测试产物：${evidence.at(-1).title}；尚未激活。` }];
        let updated: any = await this.store.updateProposal(proposalId, { audit });
        if (this.governance)
            updated = await this.store.updateProposal(proposalId, { governance: await this.governance.updateProposal(updated) });
        return updated;
    }
    async reject(proposalId: any, decisionBy: any = 'A君'): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status !== 'pending_approval')
            throw new ProposalValidationError(`草案状态“${proposal.status}”不能拒绝。`);
        if (this.governance) {
            const governance: any = await this.governance.rejectProposal(proposal);
            await this.store.updateProposal(proposalId, { governance });
        }
        return this.transition(proposalId, 'rejected', decisionBy, '负责人拒绝草案；未启动任何运行实例。');
    }
    async archive(proposalId: any, { archivedBy = 'A君', reason = '岗位或测试实例已结束，保留历史审计但不再进入活动名单。' }: any = {}): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (proposal.status === 'archived')
            return proposal;
        const archivedAt: any = this.now().toISOString();
        const updated: any = await this.transition(proposalId, 'archived', String(archivedBy).slice(0, 120), String(reason).slice(0, 500));
    ...
    async get(proposalId: any): Promise<any> {
        const proposal: any = (await this.store.listProposals()).find((item: any): any => item.proposalId === proposalId);
        if (!proposal)
            throw new ProposalValidationError('找不到 Agent 草案。');
        return proposal;
    }
    async syncActivatedRoster(): Promise<any> {
        if (!this.governance?.syncRoster || !this.registry?.list)
            return null;
        try {
    ...
    async transition(proposalId: any, target: any, actor: any, detail: any): Promise<any> {
        const proposal: any = await this.get(proposalId);
        if (!(TRANSITIONS as any)[proposal.status]?.has(target))
            throw new ProposalValidationError(`草案状态“${proposal.status}”不能转换为“${target}”。`);
        const audit: any[] = [...(proposal.audit || []), { at: this.now().toISOString(), actor, action: `status_${target}`, detail }];
        return this.store.updateProposal(proposalId, { status: target, audit });
    }
}
export class ProposalValidationError extends Error {
}
function candidate(input: any, outcome: any, requestedCapabilities: any = []): any {
    const agentId: any = slug(input?.agentId || input?.candidateName || outcome);
    const wechatPrivateRead: any = requestedCapabilities.includes(WECHAT_LOCAL_VAULT_CAPABILITY);
    const nonResponsibilities: any = strings(input?.nonResponsibilities, wechatPrivateRead
        ? [
    ...
    const dataScopes: any = Array.isArray(input?.dataScopes) && input.dataScopes.length
        ? input.dataScopes
        : [{ scope: 'public-material-only', access: ['read'], boundary: '仅公开素材；无账号、无私密数据、无写入。' }];
    const approvalPolicies: any = Array.isArray(input?.approvalPolicies) && input.approvalPolicies.length
        ? input.approvalPolicies
        : [{ action: 'external-or-sensitive-action', riskLevel: 'high', decision: 'require-approval' }];
    const qualityGates: any = Array.isArray(input?.qualityGates) && input.qualityGates.length
        ? input.qualityGates
        : [{ gate: 'acceptance-artifact-verified', required: true }];
    if (wechatPrivateRead) {
        nonResponsibilities.push('不得执行外发、发布、付费、扩权、发送微信消息或其他外部副作用。', '不得抓取或回显微信数据库密钥、salt、wxid、Cookie、签名或完整明文数据库。', '不得全量扫描、后台持续采集、读取未被负责人逐次指定的联系人、群聊或时间范围。', '不得把私密聊天原文写入 Paperclip、任务描述、日志、测试夹具或项目工作区。');
        ...
        qualityGates.push({ gate: 'single-chat-and-time-range-explicitly-approved', required: true }, { gate: 'no-key-database-or-unscoped-chat-in-artifact', required: true });
    }
    return {
...
function wechatCandidateContract(): any {
    return {
        ...
        runtimeCapabilities: {
            ...
            skillReviews: [{
                    ...
                    evidenceRefs: [
                        'integrations/access/wechat-local-vault-adapter.ts',
...
function applyWechatCandidateContract(manifest: any): any {
    const contract: any = wechatCandidateContract();
    return {
        ...manifest,
        ...contract,
        runtimeCapabilities: {
...
function acceptanceTask(input: any, manifest: any): any {
    const wechatPrivateRead: any = manifest.toolAllowlist?.includes(WECHAT_LOCAL_VAULT_CAPABILITY);
    return {
        taskType: (manifest.acceptedTaskTypes as any)[0],
...
function architectReview(manifest: any, capabilities: any, at: any): any { return { reviewId: `architect:${at}`, role: 'architect', result: 'recommend_pending_approval', summary: `复用 A君能力边界；候选岗位仅请求：${capabilities.join('、') || '无外部能力'}。`, at }; }
function reviewerReview(manifest: any, capabilities: any, at: any): any { return { reviewId: `reviewer:${at}`, role: 'reviewer', result: 'human_owner_required', summary: `默认拒绝账号、发布、付费与扩权；测试预算仅一次。`, at }; }
function promptFor(manifest: any, outcome: any): any {
    const wechatPrivateRead: any = manifest.toolAllowlist?.includes(WECHAT_LOCAL_VAULT_CAPABILITY);
    return [
...
function capabilities(value: any, outcome: any = ''): any {
    const inferred: any = /公开.*(?:网页|页面|文章|资料|素材)|(?:网页|页面|文章|资料|素材).*?(?:摘要|报告|整理)|公开.*(?:摘要|报告|整理)/.test(String(outcome)) ? ['content.public.fetch'] : [];
    const selected: any = strings(value, inferred);
    if (selected.some((item: any): any => !PROPOSABLE_CAPABILITIES.has(item))) {
        throw new ProposalValidationError('草案只能请求已审核或已登记且有明确风险边界的能力；未知能力不能借创建岗位自动扩权。');
    }
    return selected;
}
function assertRunnableCandidate(proposal: any): any {
...
function currentTrialReadiness(manifest: any, requestedCapabilities: any): any {
    const taskTypes: any = manifest?.acceptedTaskTypes || [];
    ...
    const supportsPublicReport: any = taskTypes.length === 1
        && (taskTypes as any)[0] === 'report.public-material'
        && requestedCapabilities?.includes('content.public.fetch');
    const supportsIntelResearch: any = manifest?.agentId === 'intel-researcher'
    ...
    const supportsVideoContentAnalyst: any = manifest?.agentId === 'video-content-analyst'
    ...
    const supportsContentCreator: any = manifest?.agentId === 'content-creator'
        && ((taskTypes.length === 1
            && (taskTypes as any)[0] === 'content.platform-draft'
    ...
    if (supportsIntelResearch)
        return { status: 'ready', message: '可进入一次受限公开只读测试；负责人批准前不会启用，测试必须交出可验证产物。' };
    if (supportsVideoContentAnalyst)
        return { status: 'ready', message: '可引用一份受控确认稿进入一次证据化拆解试用；测试不能抓取、登录、发布或绕过人工确认。' };
    if (supportsContentCreator)
        return { status: 'ready', message: '可生成待审草稿或一版可拍脚本与受控生产包；不能生成成片或发布。' };
...
function manifestProposal(manifest: any, now: any): any {
    const requestedCapabilities: any = strings(manifest.toolAllowlist);
    const trialReadiness: any = currentTrialReadiness(manifest, requestedCapabilities);
    return {
        ...
        acceptanceTask: { taskType: (manifest.acceptedTaskTypes as any)?.[0] || '', title: `验证 ${manifest.name} 的受限公开资料任务`, constraints: [...(manifest.nonResponsibilities || []), '仅执行一次白名单验收任务', '必须返回可验证产物引用'] },
...
function sameItems(items: any, expected: any): any { return Array.isArray(items) && items.length === expected.length && expected.every((item: any): any => items.includes(item)); }
function registeredProjectionChanged(proposal: any, projected: any): any {
...
function mentionsDraft(text: any, manifest: any): any {
    const value: any = String(text || '').toLowerCase().replace(/\s+/g, '');
    const agentId: any = String(manifest?.agentId || '').toLowerCase();
    const name: any = String(manifest?.name || '').toLowerCase().replace(/\s+/g, '');
    return Boolean(value && (value.includes(agentId) || value.includes(name)));
}
function strings(value: any, fallback: any = []): any { const items: any = Array.isArray(value) ? value : value ? [value] : fallback; return [...new Set(items.map((item: any): any => String(item).trim()).filter(Boolean))]; }
function optional(value: any): any { return String(value || '').trim() || null; }
function text(value: any, message: any): any {
    const result: any = optional(value);
    if (!result)
        throw new ProposalValidationError(message);
    return result.slice(0, 500);
}
function slug(value: any): any { const ascii: any = String(value).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''); return ascii || `candidate-${Math.random().toString(36).slice(2, 8)}`; }
function capabilityBoundary(value: any): any {
    return (({
    ...
    }) as any)[value] || `${value}：仅按 Manifest 数据范围使用，未知副作用默认拒绝`;
}
function reviewContractKey(proposal: any, reviewerModelSelection: any = null): any {
