...
export class FeishuAgentProvisioningPlanner {
    plan(agent: any, { collaborationGroup = false, existingApp = false }: any = {}): any {
        validateAgent(agent);
        if (agent.status !== 'active') {
            throw new FeishuAgentProvisioningPlanError('岗位尚未上岗，不能创建飞书智能体应用。');
        }
        const capabilities: any[] = ['私聊接收工作', '原聊天回复结果', '审批卡片按钮'];
        const events: any[] = ['im.message.receive_v1', 'card.action.trigger'];
...
function validateAgent(agent: any): any {
...
export function feishuRegisterAppOptions(agent: any, { collaborationGroup = false }: any = {}): any {
    const events: any[] = ['im.message.receive_v1'];
    const scopes: any[] = ['im:message.p2p_msg:readonly', 'im:message:send_as_bot'];
    if (collaborationGroup) {
