...
export class TaskNotification {
  ...
  async status(taskId, chatRef = '') {
    ...
    const immediate = technicalOrPausedStatus(root, chain);
  ...
  async succeededStatus(root, current, retried) {
    const result = verifiedDelivery(current, root);
...
function assertSameChat(task, chatRef) {
  const expected = String(task.source?.chatRef || '').trim();
  const actual = String(chatRef || '').trim();
  if (expected && actual !== expected) throw new ValidationError('当前会话不能读取这条任务。');
}
...
function verifiedDelivery(current, root) {
  ...
  const delivery = {
    'operations.health-review':() => {
      const report = artifact(current, 'health_report')?.data;
    ...
    'report.public-material':() => {
      const report = artifact(current, 'public_web_report')?.data;
    ...
    'research.github-search':() => {
      const report = artifact(current, 'research_github_report')?.data;
      const read = artifact(current, 'github_code_read')?.data;
      if (report?.results?.length) return formatGithubSearchDelivery(report);
      if (read?.summary) return `小R已读取 ${read.repo} 的 ${read.path}：${read.summary}\n来源：${read.source || `https://github.com/${read.repo}`}`;
      return `小R已经完成“${title}”，但公开 GitHub 产物不可读；系统不会把它当作完整交付。`;
    },
    'research.intel-report':() => {
      const report = artifact(current, 'intel_research_report')?.data;
    ...
    'office.briefing-package':() => {
      const report = artifact(current, 'office_briefing_package')?.data;
      return report?.summary && report?.markdown ? formatOfficeBriefingReply(report) : `办公执行助理已经完成“${title}”，但汇报包没有通过读取确认；系统不会把它当作完整交付。`;
    },
    'office.presentation-package':() => presentationDelivery(current, title),
    'office.knowledge-summary':() => knowledgeDelivery(current, title),
    'content.video-benchmark-analysis':() => {
      const report = artifact(current, 'video_content_analysis_report')?.data;
    ...
    'content.platform-draft':() => {
      const draft = artifact(current, 'platform_content_draft')?.data;
    ...
    'content.video-script-package':() => {
      const script = artifact(current, 'video_script_package')?.data;
      return script?.fullScript ? formatVideoScriptDelivery(script) : `小创已经完成“${title}”，但可拍脚本没有通过读取确认；系统不会把它当作完整交付。`;
    },
    'content.performance-review':() => {
      const report = artifact(current, 'content_performance_report')?.data;
...
function presentationDelivery(task, title) {
  const source = artifact(task, 'office_presentation_source');
  const pptx = artifact(task, 'office_pptx_document');
  if (!source?.validation?.structuralQaPassed || !source?.location) return `小办已经完成“${title}”，但 PPTD 没有通过结构检查；系统不会把它当作完整交付。`;
  return [
...
function knowledgeDelivery(task, title) {
  const note = artifact(task, 'knowledge_summary_note');
...
function formatHealthReportReply(report) {
  ...
  const components = report.components.slice(0, 12).map((item) => {
    const componentStatus = item.status === 'healthy' ? '正常' : item.status === 'degraded' ? '降级' : String(item.status || '未知');
...
function formatVideoScriptDelivery(report) {
  if (report.templateLifecycle?.approvedForUse === true) return '已采用这版。可拍脚本和制作包已经准备好；没有生成成片，也没有发布。';
  const notes = Array.isArray(report.shootingNotes) ? report.shootingNotes.slice(0, 3).map((item) => `- ${item}`).join('\n') : '';
...
async function missionNotification(task, { chain = [], approvals = [], xiaod = null } = {}) {
  const report = artifact(task, 'cross_agent_mission_summary')?.data;
  ...
  if (briefing?.summary) {
    ...
    if (reviewTask?.execution?.xiaodJobId && typeof xiaod?.getJob === 'function') {
      try {
        ...
        if (job.output?.larkPermissionGranted === true && /^https:\/\//.test(String(job.output?.larkUrl || ''))) reviewUrl = String(job.output.larkUrl);
      } catch { /* 保留文字恢复路径。 */ }
    }
