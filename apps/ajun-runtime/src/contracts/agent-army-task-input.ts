...
const analysisIntentSchema: any = z.enum(['digest', 'deep', 'template', 'style']);
const depthSchema: any = z.enum(['fast', 'full']);
const evidenceModeSchema: any = z.enum(['preliminary', 'formal']);
const reviewPolicySchema: any = z.enum(['optional', 'required']);
const visualModeSchema: any = z.enum(['auto', 'off', 'required']);
export const httpUrlSchema: any = z.string().url().refine((value: any): any => {
    const protocol: any = new URL(value).protocol;
    return protocol === 'http:' || protocol === 'https:';
}, '只支持 HTTP(S) 链接');
const missionItemSchema: any = z.object({
...
const completionDeliverySchema: any = z.object({
    mode: z.literal('dynamic_card'),
    owner: z.literal('hermes_gateway'),
});
export const taskCreateToolInputSchema: any = z.object({
...
export const missionCreateToolInputSchema: any = z.object({
...
export function taskClientInputFromTool(input: any, scope: any = {}): any {
    assertSingleTaskRequest(input);
    const assignment: any = canonicalizeBusinessAssignment({
        title: input.title,
...
export function missionClientInputFromTool(input: any, scope: any = {}): any {
    if (!scope.allowMissions) {
        throw new AgentArmyTaskInputError('当前员工身份不能创建多人总任务；请交给 A君。');
    }
    const assignments: any = input.items.map((item: any, index: any): any => canonicalizeBusinessAssignment({
        key: item.key,
        title: item.title,
...
export function prepareTaskCreateRequest(input: any = {}, { now = Date.now }: any = {}): any {
    const title: any = safeText(input.title, 500);
    const taskType: any = safeText(input.taskType, 120);
    if (!title)
        throw new AgentArmyTaskInputError('请说明要完成什么。');
    if (!taskType)
        throw new AgentArmyTaskInputError('请提供任务类型；不确定时先调用 capabilities。');
    const description: any = safeText(input.description, 2000);
    const chatRef: any = safeText(input.chatRef, 240);
    const requestRef: any = safeText(input.requestRef, 240);
    const sourceAgentId: any = safeText(input.sourceAgentId, 80);
    const sourceProfileId: any = safeText(input.sourceProfileId, 80);
    const taskCardPolicy: any = safeText(input.taskCardPolicy, 40);
    const sourceUrls: any = validHttpUrls(input.sourceUrls, 5);
    const sourceTaskIds: any = safeStringList(input.sourceTaskIds, 20, 100);
    const connectionId: any = optionalConnectionId(input.connectionId);
    const goalSpec: any = normalizeGoalSpecInput(input.goalSpec);
    const completionDelivery: any = completionDeliveryInput(input.completionDelivery);
    const evidenceMode: any = input.evidenceMode === 'preliminary' ? 'preliminary' : 'formal';
    const analysis: any = resolveTaskAnalysis({ ...input, title, taskType, description });
    const visualMode: any = input.visualMode === undefined
    ...
    const idempotencyKey: any = requestRef
        ? `hermes:${requestRef}`
        : `hermes:${chatRef || 'local'}:${shortHash([title, taskType, input.agentId || '', Math.floor(now() / 30000)].join('|'))}`;
    const source: any = sourceEnvelope({ chatRef, requestRef, sourceAgentId, sourceProfileId, taskCardPolicy });
    return {
        kind: 'task',
        chatRef,
        sourceAgentId,
...
export function prepareMissionCreateRequest(input: any = {}, { now = Date.now }: any = {}): any {
    const title: any = safeText(input.title, 500);
    const items: any = normalizeMissionItems(input.items);
    if (!title)
        throw new AgentArmyTaskInputError('请说明这组工作的总目标。');
    if (!items.length) {
    ...
    const idempotencyKey: any = requestRef
        ...
        : `hermes-mission:${chatRef || 'local'}:${shortHash([title, JSON.stringify(items), Math.floor(now() / 30000)].join('|'))}`;
    return {
        title,
        items,
        chatRef,
...
function goalSpecFromTool(input: any): any {
    if (!input.goal)
        return undefined;
    return {
        outcome: input.goal,
        ...
        capabilityRequests: (input.capability_requests || []).map((request: any): any => ({
...
function assertTaskScope({ taskType, agentId }: any, scope: any): any {
    const targetAgent: any = String(agentId || onlyValue(scope.agentIds) || '').trim();
...
function assertSingleTaskRequest({ title, description }: any): any {
    const text: any = `${String(title || '')}\n${String(description || '')}`;
    const numberedItems: any = [...text.matchAll(/(?:^|\n)\s*([1-3])[.、)]\s+\S/g)].map((match: any): any => match[1]);
    const distinctItems: any = new Set(numberedItems);
    const explicitlyGrouped: any = /(总任务|多人任务|[两二三3]\s*项工作|前两项|统一汇报)/.test(text);
    if (distinctItems.has('1') && distinctItems.has('2') && explicitlyGrouped) {
        throw new AgentArmyTaskInputError('检测到负责人一次交办多项工作，禁止压成单员工任务。请改用 mission_create，并把每项工作分别映射到已上岗员工。');
    }
}
function sourceEnvelope({ chatRef, requestRef, sourceAgentId, sourceProfileId, taskCardPolicy }: any): any {
    if (!chatRef)
        return { channel: 'hermes-native' };
    return {
        channel: 'feishu',
        chatRef,
...
function resolveTaskAnalysis(input: any): any {
    const analysis: any = input.taskType === 'content.video-benchmark-analysis'
...
function videoAnalysisMissionInput(input: any): any {
    const acquisitionTaskType: any = DEFAULT_TASK_DEFINITION_REGISTRY.prerequisiteTaskType(input.taskType);
    const acquisitionAgentId: any = DEFAULT_TASK_DEFINITION_REGISTRY.defaultAgentId(acquisitionTaskType);
    const analysisAgentId: any = DEFAULT_TASK_DEFINITION_REGISTRY.defaultAgentId(input.taskType);
...
function normalizeMissionItems(value: any): any {
    if (!Array.isArray(value) || value.length < 1 || value.length > 11)
        return [];
    const items: any = value.map((item: any, index: any): any => {
        const analysis: any = resolveMissionAnalysisIntent(item);
        return {
    ...
    const keys: any = new Set(items.map((item: any): any => item.key));
    return items.every((item: any): any => (item.title
        && item.taskType
        && item.agentId
        && item.dependsOn.every((key: any): any => keys.has(key) && key !== item.key))) ? items : [];
}
function resolveMissionAnalysisIntent(item: any): any {
...
function normalizeGoalSpecInput(value: any): any {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return null;
    const outcome: any = safeText(value.outcome || value.goal, 1000);
    if (!outcome)
        return null;
    const budget: any = value.budget && typeof value.budget === 'object' && !Array.isArray(value.budget)
        ? {
    ...
    return {
        ...
        capabilityRequests: (Array.isArray(value.capabilityRequests) ? value.capabilityRequests : []).slice(0, 12).map((request: any): any => ({
        ...
        })).filter((request: any): any => request.capabilityId && request.purpose),
        ...(budget ? { budget } : {}),
    };
}
function completionDeliveryInput(value: any): any {
    if (value === undefined || value === null)
        return null;
    const normalized: any = normalizeCompletionDelivery(value);
    if (!normalized) {
...
function optionalConnectionId(value: any): any {
    const id: any = safeText(value, 100);
    if (!id)
        return undefined;
...
function validHttpUrls(value: any, maxItems: any): any {
    const urls: any = safeStringList(value, maxItems, 2000);
...
function optionalDurationSeconds(value: any): any {
    if (value === undefined || value === null || value === '')
        return undefined;
    const duration: any = Number(value);
...
function normalizeVisualMode(value: any): any {
    return value === 'off' || value === 'required' ? value : 'auto';
}
function boundedDuration(value: any, min: any, max: any, fallback: any): any {
