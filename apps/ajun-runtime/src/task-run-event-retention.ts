export class TaskRunEventRetention {
    ...
    mode: any;
    retentionDays: any;
    retentionDaysByClass: any;
    constructor({ eventStore, retentionDays = null, retentionDaysByClass = { transient: 7, detail: 30, audit: 365 }, mode = 'dry-run', clock = (): any => new Date().toISOString(), }: any) {
        if (!eventStore?.cleanupExpiredDetails)
            throw new Error('运行事件清理器需要 Event Store。');
        if (!['dry-run', 'apply'].includes(mode))
            throw new Error('运行事件清理模式无效。');
