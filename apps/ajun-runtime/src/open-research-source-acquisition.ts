...
const DISCOVERY_RESULTS_PER_LANE: any = 2;
...
export class OpenResearchSourceAcquisition {
    ...
    async discover(topic: any, roleToolContext: any = null): Promise<any> {
        ...
        if (this.publicWebSearch?.search) {
            ...
            if (candidates.size) {
                const selected: any = selectDiverseCandidates(candidates);
                return {
...
function selectDiverseCandidates(candidates: any): any {
...
function sourceDiscoveryMetadata(candidate: any): any {
    if (!candidate)
        return null;
    return {
        laneIds: [...candidate.laneIds],
...
function publicSourceUrl(value: any): any {
    try {
