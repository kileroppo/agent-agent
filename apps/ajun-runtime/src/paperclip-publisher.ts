...
export const paperclipPublisherMethods: Record<string, any> = {
    async authorizePublisherRequest(input: any = {}): Promise<any> {
        ...
        const expectedVersion: any = integerVersion(context.targetCase.version);
        const records: any = boundedRecordMap(context.targetCase.fields?.m5PublisherAuthorizations, input.authorizationId, {
            ...canonical,
            consumedAt: validClock(this.clock()).toISOString(),
        });
        const updated: any = normalizeM5Case(await this.patchPipelineCaseFields(context.targetCase.id, {
            expectedVersion,
        ...
        const persisted: any = publisherAuthorizationRecord(updated.fields?.m5PublisherAuthorizations?.[input.authorizationId]);
    ...
    async getPublisherConnectorApprovalSnapshot(input: any = {}): Promise<any> {
        const context: any = await this.publisherAuthorizationContext(input);
        const approvals: any = await this.publisherConnectorApprovals(context);
        if (approvals.length === 0) {
            throw new Error('Paperclip 中没有当前 Campaign 可用的 Publisher connector 批准。');
        }
        const publicApprovals: any = approvals
            .map((approval: any): any => ({
            status: 'approved',
            ...
            .sort((left: any, right: any): any => (`${left.platform}:${left.capability}`.localeCompare(`${right.platform}:${right.capability}`)));
        const snapshotHash: any = crypto.createHash('sha256')
    ...
    async resolvePublisherCredentialReference(input: any = {}): Promise<any> {
        assertExactKeys(input, ['accountRef', 'platform', 'purpose'], 'Publisher 凭据请求');
        const platform: any = publisherPlatform(input.platform);
        const purpose: any = publisherCapability(input.purpose);
        const accountRef: any = publisherReference(input.accountRef, 'Publisher accountRef 无效。');
        const context: any = await this.publisherRunContextForConnector();
        if (purpose !== context.controllerCapability) {
            throw new Error('Publisher 当前控制器不能读取另一类 connector 凭据。');
        }
        const approval: any = uniquePublisherApproval(await this.publisherConnectorApprovals(context), { platform, capability: purpose, accountRef });
        ...
        let response: any;
        try {
            response = await this.request(`/api/agents/me/secrets/${encodeURIComponent(approval.secretKey)}/value`, {
                method: 'POST',
                runId: context.credential.runId,
    ...
    async verifyPublisherAccountIdentity(input: any = {}): Promise<any> {
        assertExactKeys(input, ['platform', 'accountRef', 'providerIdentity'], 'Publisher 账号身份核验请求');
        const platform: any = publisherPlatform(input.platform);
        const accountRef: any = publisherReference(input.accountRef, 'Publisher accountRef 无效。');
        const identity: any = publisherIdentity(input.providerIdentity);
        const context: any = await this.publisherRunContextForConnector();
        const approvals: any = (await this.publisherConnectorApprovals(context)).filter((approval: any): any => (approval.platform === platform
            && approval.accountRef === accountRef
            && approval.capability === context.controllerCapability));
        if (approvals.length === 0 || approvals.some((approval: any): any => !samePublisherIdentity(approval.providerIdentity, identity))) {
    ...
    async assertPublisherCampaignBudget(input: any = {}): Promise<any> {
        assertExactKeys(input, ['campaignId', 'connectorMode', 'operation', 'checkedAt'], 'Publisher 预算请求');
        const campaignId: any = publisherReference(input.campaignId, 'Publisher Campaign 标识无效。');
        const connectorMode: any = String(input.connectorMode || '');
        const operation: any = String(input.operation || '');
        if (!PUBLISHER_CONNECTOR_MODE.test(connectorMode)
            || !PUBLISHER_BUDGET_OPERATIONS.has(operation)) {
            throw new Error('Publisher connector 模式或预算操作无效。');
        }
        validClock(input.checkedAt);
        const context: any = await this.publisherRunContextForConnector(campaignId);
        if (operation !== context.controllerCapability) {
            throw new Error('Publisher 预算操作与当前控制器能力不一致。');
        }
        const overview: any = await this.request(`/api/companies/${encodeURIComponent(context.credential.companyId)}/budgets/overview`, {
            runId: context.credential.runId,
            apiKey: context.credential.apiKey,
        });
        const scopes: any[] = [
        ...
        const matched: any = scopes.map(([scopeType, scopeId]: any): any => {
            const rows: any = policies.filter((policy: any): any => (policy?.scopeType === scopeType
            ...
            const remainingAmount: any = Number(rows[0].remainingAmount);
            if (!Number.isFinite(remainingAmount) || remainingAmount < 0) {
                throw new Error(`Paperclip ${scopeType} Publisher 剩余预算无效。`);
            }
            return { ...rows[0], remainingAmount };
        });
        const projectPolicy: any = matched.find((policy: any): any => policy.scopeType === 'project');
        if (!Number.isInteger(Number(projectPolicy?.amount))
        ...
        const allowed: any = matched.every((policy: any): any => (policy.paused !== true
        ...
        return {
            ...
            remainingAmountUsd: Math.min(...matched.map((policy: any): any => policy.remainingAmount)) / 100,
        };
    },
    async recordPublisherConnectorAttempt(input: any = {}): Promise<any> {
        ...
        const costRecordId: any = publisherReference(input.costRecordId, 'Publisher 费用幂等标识无效。');
        const campaignId: any = publisherReference(input.campaignId, 'Publisher Campaign 标识无效。');
        const connectorMode: any = String(input.connectorMode || '');
        const operation: any = String(input.operation || '');
        const amountUsd: any = Number(input.amountUsd);
        const occurredAt: any = validClock(input.occurredAt).toISOString();
        if (!PUBLISHER_CONNECTOR_MODE.test(connectorMode)
        ...
        const sourceRef: any = publisherReference(input.providerRequestId || input.receiptRef, 'Publisher 费用来源引用无效。');
        const context: any = await this.publisherRunContextForConnector(campaignId);
        ...
        const existing: any = publisherCostRecord(context.targetCase.fields?.m5PublisherCostRecords?.[costRecordId]);
        if (existing) {
        ...
        let targetCase: any = await this.patchPublisherCostRecord(context, canonical, {
            state: 'submitting',
            claimedAt: validClock(this.clock()).toISOString(),
        });
        let created: any;
        try {
            created = await this.request(`/api/companies/${encodeURIComponent(context.credential.companyId)}/cost-events`, {
                method: 'POST',
                runId: context.credential.runId,
                apiKey: context.credential.apiKey,
                body: {
        ...
        if (!UUID.test(String(created?.id || ''))
        ...
        targetCase = await this.patchPublisherCostRecord({ ...context, targetCase }, canonical, {
            state: 'reported',
            costEventId: created.id,
            reportedAt: validClock(this.clock()).toISOString(),
        });
        const confirmed: any = publisherCostRecord(targetCase.fields?.m5PublisherCostRecords?.[costRecordId]);
    ...
    async patchPublisherCostRecord(context: any, canonical: any, patch: any): Promise<any> {
        const records: any = boundedRecordMap(context.targetCase.fields?.m5PublisherCostRecords, canonical.costRecordId, { ...canonical, ...patch });
        return normalizeM5Case(await this.patchPipelineCaseFields(context.targetCase.id, {
    ...
    async publisherAuthorizationContext(input: any = {}): Promise<any> {
        assertExactKeys(input, ['action', 'runId', 'issueId', 'campaignId', 'agentId', 'authorizationId'], 'Publisher 授权请求');
        const action: any = String(input.action || '');
        const expectedRole: any = PUBLISHER_ACTION_ROLES[action];
        if (!expectedRole)
            throw new Error('Publisher action 不属于批准的系统控制器动作。');
        const credential: any = await this.currentPublisherRunCredential();
        ...
        const verified: any = await this.verifySystemAssignment({
    ...
    async publisherRunContextForConnector(campaignId: any = null): Promise<any> {
        const credential: any = await this.currentPublisherRunCredential();
        const roles: any = Object.values(PUBLISHER_ACTION_ROLES);
        ...
        const expectedStage: any = verified.systemRole === 'm5-publisher-controller' ? 'publish' : 'metrics';
        const context: any = await this.publisherCampaignContext({
            credential,
            issue: verified.issue,
            campaignId,
            expectedStage,
    ...
    async publisherCampaignContext({ credential, issue, campaignId, expectedStage }: any): Promise<any> {
        const targetCaseId: any = publisherIssueCaseId(issue);
        await this.assertCaseIssueLink(targetCaseId, credential.issueId);
        const targetDetail: any = await this.getPipelineCase(targetCaseId);
        const targetCase: any = normalizeM5Case(targetDetail);
        if (targetCase.stageKey !== expectedStage || !targetCase.parentCaseId) {
            throw new Error(`Publisher 当前 Case 不在 ${expectedStage} 阶段。`);
        }
        const dayCase: any = normalizeM5Case(await this.getPipelineCase(targetCase.parentCaseId));
        if (!dayCase.parentCaseId)
            throw new Error('Publisher 日期 Case 缺少活动父 Case。');
        const campaignCase: any = normalizeM5Case(await this.getPipelineCase(dayCase.parentCaseId));
        const canonicalCampaignId: any = String(campaignCase.id || '');
        if (campaignCase.parentCaseId
        ...
        const campaignGrant: any = campaignCase.fields?.campaignGrant;
        const now: any = validClock(this.clock()).getTime();
        const startsAt: any = Date.parse(campaignGrant?.startsAt);
        const expiresAt: any = Date.parse(campaignGrant?.expiresAt);
        if (campaignGrant?.status !== 'active'
        ...
        const projectId: any = m5CaseProjectId(targetDetail, targetCase)
            || m5CaseProjectId({}, campaignCase);
        if (!UUID.test(projectId))
            throw new Error('Publisher Campaign 缺少可信 Paperclip Project。');
        return {
            credential,
            issue,
            targetCase,
    ...
    async publisherConnectorApprovals(context: any): Promise<any> {
        const payload: any = await this.request(`/api/companies/${encodeURIComponent(context.credential.companyId)}/approvals`, {
            runId: context.credential.runId,
            apiKey: context.credential.apiKey,
        });
        const approvals: any = Array.isArray(payload) ? payload : Array.isArray(payload?.items) ? payload.items : [];
        const now: any = validClock(this.clock()).getTime();
        const grant: any = context.campaignGrant;
        return approvals
            .filter((approval: any): any => approval?.status === 'approved')
            .map((approval: any): any => normalizePublisherConnectorApproval(approval))
            .filter(Boolean)
            .filter((approval: any): any => (approval.campaignId === context.campaignCase.id
    ...
    async currentPublisherRunCredential(): Promise<any> {
        ...
        const credential: any = value && typeof value === 'object' && !Array.isArray(value)
            ? value
            : {};
