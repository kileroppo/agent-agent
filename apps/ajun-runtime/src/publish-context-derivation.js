...
export function derivePublishContext({
  outputs,
  targetCase,
  campaignCase,
  grant,
  ...
  const contentVersion = assertContentVersionIdentity(contentVersionCandidate, {
    invalid:() => invalid('ContentVersion 与当前平台 Case 不一致或缺少发布产物。'),
  });
...
function immediatePublishDateMismatch({ scheduledDate, executionDate, invalid }) {
  const error = invalid(
