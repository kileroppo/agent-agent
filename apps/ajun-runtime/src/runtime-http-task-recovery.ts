export async function routeTaskRecoveryApi({
  ...
  const match = request.url?.match(/^\/api\/tasks\/([0-9a-f-]+)\/recovery-actions\/(resume_approved_mission|use_confirmed_transcript_only|request_safe_recovery|request_read_only_diagnosis|retry_visual_analysis_after_recovery)$/i);
