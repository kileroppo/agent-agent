const REGISTERED_HTTP_CHECKS: any = Object.freeze([
    Object.freeze({
        id: 'ajun-runtime',
        name: 'A君运行台',
        port: 4321,
...
const CHECK_BY_ID: any = new Map(REGISTERED_HTTP_CHECKS.map((check: any): any => [check.id, check]));
export class DeterministicLocalHealthProbe {
    fetch: any;
    now: any;
    timeoutMs: any;
    constructor({ fetchImpl = fetch, timeoutMs = 2500, now = (): any => new Date() }: any = {}) {
        if (typeof fetchImpl !== 'function')
            throw new Error('本机健康探针需要受控 HTTP 读取能力。');
        this.fetch = fetchImpl;
        this.timeoutMs = normalizeTimeout(timeoutMs);
        this.now = now;
    }
    registeredChecks(): any {
        return REGISTERED_HTTP_CHECKS.map(publicCheck);
    }
    async check(): Promise<any> {
        return Promise.all(REGISTERED_HTTP_CHECKS.map((target: any): any => this.#checkRegistered(target)));
    }
    async checkOne(targetId: any): Promise<any> {
        const target: any = CHECK_BY_ID.get(String(targetId || ''));
        if (!target)
            throw new Error('本机健康探针拒绝未登记目标。');
        return this.#checkRegistered(target);
    }
    async #checkRegistered(target: any): Promise<any> {
        const checkedAt: any = this.now().toISOString();
        const endpoint: any = registeredEndpoint(target);
        const startedAt: any = Date.now();
        try {
            const response: any = await this.fetch(endpoint, {
                method: 'GET',
                headers: { accept: 'application/json' },
                redirect: 'error',
                signal: AbortSignal.timeout(this.timeoutMs)
            });
            const body: any = await response.json().catch((): any => null);
            const contractSatisfied: any = response.ok === true && target.verify(body);
...
function degradedObservation({ target, checkedAt, latencyMs, httpStatus, errorCode }: any): any {
    return {
        schemaVersion: 'agent.army/local-health-observation/v1',
        id: target.id,
        name: target.name,
...
function publicCheck(target: any): any {
    return {
        targetId: target.id,
        transport: 'http',
        host: '127.0.0.1',
...
function registeredEndpoint(target: any): any {
...
function elapsed(startedAt: any): any {
    return Math.max(0, Date.now() - startedAt);
}
function finiteStatus(value: any): any {
    const parsed: any = Number(value);
