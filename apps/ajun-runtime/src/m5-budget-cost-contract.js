...
const PRODUCER_SOURCES = Object.freeze({
  'content-plugin':'provider-usage',
  publisher:'publisher-receipt',
});

export function assertM5BudgetCoverage({
  overview,
  companyId,
  agentId,
  projectId,
  ...
  const accepted = requirements.map(([scopeType, scopeId]) => {
    const matches = policies.filter((item) =>
    ...
    const remainingAmount = Number(policy?.remainingAmount);
    if (
      !policy
      ...
      throw new M5BudgetCostContractError(
        `Paperclip ${scopeType} 预算未唯一覆盖本次最大估算 ${maximumCostCents} 美分。`,
...
export function createM5CostEventDraft({
  producer,
  actionId,
  source,
  runContext,
  ...
  const expectedSource = PRODUCER_SOURCES[producer];
  if (!expectedSource) {
  ...
  const sourceRecord = strictObject(source, ['kind', 'receiptChecksum', 'connectorMode']);
  if (
    sourceRecord.kind !== expectedSource
    ...
    throw new M5BudgetCostContractError(
      '费用必须来自可核验的真实 Provider 用量或真实 Publisher 回执。',
      'cost_source_unverified',
    );
  }
  const context = strictObject(runContext, [
    'companyId',
    'agentId',
    'issueId',
    'projectId',
  ...
  const charge = strictObject(cost, [
    'provider',
    'biller',
    'billingType',
    'billingCode',
...
function strictObject(value, allowedKeys) {
  ...
  if (extra) {
    throw new M5BudgetCostContractError(
      `费用契约不接受自由字段 ${extra}；不得夹带凭据、Prompt、文件路径或平台正文。`,
      'cost_event_extra_field',
    );
  }
  return value;
}
