...
type TaskRunEventCursor = Readonly<{ startedAt: string; eventId: string }>;
...
const MAX_LENGTH: Readonly<Record<string, number>> = Object.freeze({
...
export function normalizeTaskRunEvent(
  input: JsonRecord,
  { now = new Date().toISOString() }: Readonly<{ now?: string }> = {},
) {
...
function defaultRetentionClass(eventType: string): string {
  ...
  if (/(?:approval|receipt|cost|artifact_committed|workflow_(?:completed|blocked)|review_|quality_|revision_)/.test(eventType)) return 'audit';
  return 'detail';
}

export function normalizeTaskRunEventQuery(input: JsonRecord = {}) {
  const taskId = cleanIdentifier(input.taskId, MAX_LENGTH.taskId);
  if (!taskId) throw codedError('task_run_event_task_required', '查询运行事件必须指定任务。');
  const filterInput = Array.isArray(input.filters)
    ? { flags:input.filters }
    : recordOf(input.filters) || {};
  return {
    taskId,
    limit:Math.max(1, Math.min(Number.parseInt(String(input.limit || ''), 10) || 50, 200)),
...
export function encodeTaskRunEventCursor(event: Partial<TaskRunEventCursor> | null | undefined): string | null {
...
export function decodeTaskRunEventCursor(value: unknown): TaskRunEventCursor | null {
...
export function redactTaskRunSummary(value: unknown): string | null {
...
function normalizeEnumList(value: unknown, allowed: readonly string[]): string[] {
...
function normalizeDuration(value: unknown, startedAt: string, finishedAt: string | null): number | null {
...
function cleanIdentifier(value: unknown, limit = 160): string {
