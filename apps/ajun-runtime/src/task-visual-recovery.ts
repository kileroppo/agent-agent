import { paperclipIssueIdFor, uniqueStrings } from './task-recovery-policy.ts';
const BILLING_FAILURE_CODES: any = new Set([
    'provider_http_402',
    'provider_balance_insufficient',
    'provider_quota_exhausted',
]);
export async function visionCapabilityReadiness(capabilityStatus: any, { failureCode = '', failureAt = '' }: any = {}): Promise<any> {
    let status: any = null;
    try {
    ...
    const row: any = (Array.isArray(status?.capabilities) ? status.capabilities : [])
        .find((item: any): any => item?.capability === 'vision.analyze');
    ...
    const baseReady: any = capability.configured && capability.healthy && capability.e2eVerified;
    if (!BILLING_FAILURE_CODES.has(String(failureCode || '').toLowerCase())) {
        return { ...capability, ready: baseReady };
    }
    const verifiedAt: any = String(row?.verifiedAt || '').trim() || null;
    const failureTime: any = Date.parse(String(failureAt || ''));
    const verifiedTime: any = Date.parse(verifiedAt || '');
    const billingRecoveryVerified: any = Number.isFinite(failureTime)
...
export async function retryVisualAnalysis({ task, requestId, requestedBy, createTask, record, clock, errorFactory, }: any): Promise<any> {
    if (typeof createTask !== 'function') {
        throw errorFactory('视觉恢复重跑入口暂不可用，未创建子任务。', 'task_recovery_unavailable', 503);
    }
    const paperclipIssueId: any = paperclipIssueIdFor(task);
    if (!paperclipIssueId) {
