...
import { createBackgroundLifecycleComposition } from './runtime/background-lifecycle-composition.js';
  ...
  try {
    const contentCampaign = await createContentCampaignComposition({
      ...
      resolveTaskIdForPaperclipCase:async (caseId) => {
        const task = (await runtimeState.store.list()).find((item) =>
    ...
    backgroundLifecycle = createBackgroundLifecycleComposition({
      configuration,
      runtimeSource,
      store:runtimeState.store,
      governance,
    ...
    tasks.setWorkerStatus((currentTasks) => deployment.mode === 'cloud'
      ? backgroundLifecycle.macWorker.snapshot(currentTasks)
    ...
    const handler = createAjunHttpHandler({
      ...
      work:{
        ...
        proposals,
        missions:backgroundLifecycle.missions,
        macWorker:backgroundLifecycle.macWorker,
        xiaod:localExecution.xiaod,
