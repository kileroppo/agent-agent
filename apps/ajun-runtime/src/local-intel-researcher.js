...
import {
  ...
  OpenResearchSourceAcquisition,
} from './open-research-source-acquisition.js';
...
export class LocalIntelResearcher {
  ...
  async execute(task, { roleToolContext = null } = {}) {
    ...
    const acquisition = await this.sourceAcquisition.acquire({
      task,
      topic,
      roleToolContext,
  ...
  async synthesizeVerifiedReport({
    ...
    const sources = this.sourceAcquisition.fromVerifiedObservations(sourceObservations);
    if (sources.length < 2) {
  ...
  async readSources(
    ...
    return this.sourceAcquisition.readSources(
      urls,
      roleToolContext,
      readModes,
      candidatesByUrl,
