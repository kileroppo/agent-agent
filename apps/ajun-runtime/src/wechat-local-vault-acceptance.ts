...
const CAPABILITY: any = 'wechat.local-vault.chat.read';
export class WeChatLocalVaultAcceptance {
    artifactsDir: any;
    now: any;
    constructor({ artifactsDir, now = (): any => new Date() }: any = {}) {
        this.artifactsDir = path.resolve(artifactsDir);
        this.now = now;
    }
    async run({ proposal, testInstance }: any): Promise<any> {
        const taskId: any = `proposal-test:${testInstance.testInstanceId}`;
        const agentId: any = proposal.candidateManifest.agentId;
        const approvalRef: any = `synthetic_${crypto.randomUUID().replaceAll('-', '')}`;
        const startTime: any = '2026-07-01T00:00:00.000Z';
        const endTime: any = '2026-07-01T00:10:00.000Z';
        const scope: Record<string, any> = {
            ...
            endTime,
            maxMessages: 3,
            expiresAt: new Date(this.now().getTime() + 60000).toISOString()
        };
        ...
        const adapter: any = new WeChatLocalVaultAdapter({
            scopeResolver: async (value: any): Promise<any> => value === approvalRef ? scope : null,
            runVaultQuery: async (): Promise<any> => ({ messages: syntheticMessages }),
        ...
        if (!acquired.ok)
            throw new Error(acquired.safeMessage);
        const slice: any = acquired.contentPackage.contentItems.chat_slice;
        if (slice.messageCount !== syntheticMessages.length)
            throw new Error('合成聊天切片数量不符合验收预期。');
        ...
        const directory: any = path.resolve(this.artifactsDir, safeSegment(proposal.proposalId), safeSegment(testInstance.testInstanceId));
        if (!directory.startsWith(`${this.artifactsDir}${path.sep}`))
            throw new Error('微信受限验收产物路径越界。');
        await fs.mkdir(directory, { recursive: true });
        const filePath: any = path.join(directory, 'wechat-local-vault-synthetic-acceptance.json');
