...
function createRecoveryServer({
  releaseHash = process.env.AJUN_RECOVERY_RELEASE_HASH,
  payloadHash = process.env.AJUN_RECOVERY_PAYLOAD_HASH,
  reason = process.env.AJUN_RECOVERY_REASON || 'cutover_recovery',
  now = () => new Date(),
  pid = process.pid,
...
export async function startRecoveryServer({
  host = process.env.AJUN_HOST || HOST,
  port = Number(process.env.PORT || 4321),
  ...options
} = {}) {
  if (host !== HOST) throw new Error('recovery server只允许监听127.0.0.1');
...
function json(response, statusCode, value, headOnly = false) {
...
function rejectWrite(request, response) {
