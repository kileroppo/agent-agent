...
export class ContentCampaignService extends ContentCampaignKernel {
  constructor({ adapter, definition, controlPlane = null, ...ports } = {}) {
    const resolvedControlPlane = controlPlane || (typeof adapter?.request === 'function'
  ...
  async executeTool(input = {}, authentication = {}) {
    ...
    const rawCase = await this.getRawCase(input.campaignId);
    const caseItem = normalizePaperclipCase(rawCase) || rawCase;
