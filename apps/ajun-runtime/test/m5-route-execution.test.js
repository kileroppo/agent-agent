...
test('routeChanged 只由真实输入、工具或策略差异产生', () => {
  const changed = createM5RouteExecution({
    runId:'run-route-changed',
    stageKey:'voice',
    recovery,
    strategy:'same_stage_rebuild_inputs',
    toolIds:rejectedExecution.toolIds,
...
test('同一恢复版本再次使用同一路线时拒绝伪 routeChanged', () => {
  ...
  const repeated = createM5RouteExecution({
    runId:'run-route-repeat',
    stageKey:'voice',
    recovery,
    previousExecution:first,
  ...
  assert.throws(
    () => assertChangedM5RecoveryRoute(repeated, recovery),
    /输入、工具或策略没有真实变化/,
  );
});
...
test('Observation驱动决策保留Paperclip血缘但不复制真实工具正文', () => {
  const decision = createM5ObservationDecision({
    runId:'run-open-research',
    issueId:'issue-open-research',
    observation:{
      ...
      output:{ text:'真实网页正文不应进入路线回执' },
      token:'never-copy',
    },
    action:'switch_adapter',
    selectedToolId:'content.public.dynamic.read',
    successCondition:'读取公开动态网页并得到可核验正文。',
    budget:{
      stepsRemaining:6,
      safeRetriesRemaining:2,
      replansRemaining:3,
      remainingUnitsAfterDecision:8,
    },
