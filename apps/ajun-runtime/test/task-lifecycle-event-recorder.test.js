...
test('相同状态的新执行尝试保留独立运行事件', () => {
  const { events, recorder } = eventFixture();
  recorder.recordPersisted(task({ attempt:1 }));
  recorder.recordPersisted(task({ attempt:2 }), { previousTask:task({ attempt:1 }) });
  const workflowEvents = [...events.values()].filter((event) => event.eventType === 'workflow_state_changed');
  assert.equal(workflowEvents.length, 2);
  assert.deepEqual(workflowEvents.map((event) => event.attempt), [1, 2]);
});
...
test('取消等阻塞终态统一保留为审计事件', () => {
  const { events, recorder } = eventFixture();
  recorder.recordPersisted(task({ status:'cancelled', currentStage:'cancelled_by_owner' }), {
    previousTask:task({ status:'running', currentStage:'executing' }),
  });

  const [event] = [...events.values()];
  assert.equal(event.eventType, 'workflow_blocked');
  assert.equal(event.retentionClass, 'audit');
});

test('运行事件存储失败不改变任务持久化结果', () => {
  const recorder = new TaskLifecycleEventRecorder({ eventStore:{
    appendTaskRunEvent() { throw new Error('event store unavailable'); },
  } });
  assert.doesNotThrow(() => recorder.recordPersisted(task({ status:'succeeded' }), {
    previousTask:task({ status:'running' }),
  }));
});
