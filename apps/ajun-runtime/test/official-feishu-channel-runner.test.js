...
test('启用后只把官方规范化消息交给 A君，并回复原消息和审批卡', async () => {
  ...
  assert.deepEqual(state.sent[1].input.card.elements[1].actions[0].value, { approvalId:'approval-a', governanceMode:'local', action:'approve' });
});
...
test('官方卡片回调只传递批准或拒绝所需的信息，并给点击者明确结果', async () => {
  ...
  assert.deepEqual(response, { toast:{ type:'success', content:'已批准，任务会按确认范围继续。' } });
});
...
test('审批卡不会夹带未定义字段，只保留原审批编号和决定', () => {
  ...
  assert.deepEqual(card.elements[1].actions[1].value, { approvalId:'approval-a', governanceMode:'proposal', action:'reject' });
});
