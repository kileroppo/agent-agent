...
test('运行事件按时间正序游标分页，并支持快捷和结构化过滤', async () => {
  await withStore(async ({ store }) => {
    const fixtures = [
      ...
      ['e6', 'quality_review_start_failed', 'waiting_test', 'review_start_failed', null, null],
    ];
    ...
    assert.deepEqual(store.queryTaskRunEvents({ taskId:'task-page', filters:['failure'] }).items.map((event) => event.eventId), ['e2', 'e6']);
    assert.deepEqual(store.queryTaskRunEvents({ taskId:'task-page', filters:['fallback'] }).items.map((event) => event.eventId), ['e3']);
    assert.deepEqual(store.queryTaskRunEvents({ taskId:'task-page', filters:['quality'] }).items.map((event) => event.eventId), ['e4', 'e6']);
...
test('生命周期阻塞与交付质量事件默认进入审计留存', () => {
  const database = new DatabaseSync(':memory:');
  const store = new TaskRunEventStore(database);
  const blocked = store.appendTaskRunEvent({ taskId:'task-audit', eventType:'workflow_blocked', status:'waiting_test' });
  const quality = store.appendTaskRunEvent({ taskId:'task-audit', eventType:'quality_review_start_failed', status:'waiting_test' });
  const progress = store.appendTaskRunEvent({ taskId:'task-audit', eventType:'workflow_state_changed', status:'running' });
  assert.equal(blocked.retentionClass, 'audit');
  assert.equal(quality.retentionClass, 'audit');
  assert.equal(progress.retentionClass, 'transient');
  store.close();
  database.close();
});
