...
function setup({ taskPatch = {}, getJob } = {}) {
  const task = {
  ...
  return { task, reconciler: new XiaodReconciler({ store, xiaod: { baseUrl: 'http://127.0.0.1:4318', getJob }, now }) };
}

test('central reconciler settles a persisted running task after restart', async () => {
  const { task, reconciler } = setup({ getJob: async () => ({ id: 'xiaod-1', status: 'completed', title: '素材', output: { markdownPath: '/tmp/result.md' }, quality: { passed: true } }) });
...
test('Xiaod retryable failure is preserved on the parent task', async () => {
  const { task, reconciler } = setup({ getJob: async () => ({ id: 'xiaod-1', status: 'failed', error: '服务重启导致任务中断，请重试。', failure: { category: 'retryable', retryable: true, recovery: '请重试小D任务。' } }) });
  await reconciler.reconcile();
