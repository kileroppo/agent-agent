...
export function createTaskRecordWorkbench({
  api,
  getAgents,
  taskTypeLabel,
  agentName,
  ...
  return {
    async setActive(active) {
      ...
      if (!state.loaded) await loadRecords();
    },
  ...
  function bindEvents() {
    for (const button of elements.viewButtons) button.addEventListener('click', async () => {
      if (state.view === button.dataset.recordView && !state.selectedTaskId) return;
    ...
    elements.list.addEventListener('click', async (event) => {
      ...
      if (task) await selectTask(task, { updateUrl:true, revealDetail:true });
    });
    elements.routineSummary.addEventListener('click', async (event) => {
      if (!event.target.closest('[data-show-routine]')) return;
  ...
  async function loadRecords({ append = false } = {}) {
    ...
    else elements.loadMore.disabled = true;
    try {
      ...
      if (state.selectedTaskId) {
        ...
        if (selected) await selectTask(selected, { updateUrl:false, revealDetail:Boolean(initialTaskId) });
        else await loadSelectedDetail({ revealDetail:Boolean(initialTaskId) });
  ...
  async function checkForUpdates() {
    try {
      ...
      if (state.selectedTaskId) await loadSelectedDetail({ revealDetail:false, quiet:true });
    } catch {
  ...
  async function loadSelectedDetail({ revealDetail = false, quiet = false } = {}) {
    ...
    try {
      ...
      if (!quiet) renderDetailError(error);
    }
  }
  ...
  function recordQueryUrl(cursor) {
    ...
    if (state.taskType) params.set('taskType', state.taskType);
    if (state.time !== 'all') params.set('since', sinceFor(state.time));
    if (state.includeRoutine) params.set('includeRoutine', '1');
...
function stateForTask(status) {
  if (['failed', 'needs_input', 'pending_approval', 'waiting_approval', 'waiting_test', 'paused', 'blocked', 'error'].includes(status)) return 'needs_action';
  if (['succeeded', 'cancelled', 'rejected', 'stopped'].includes(status)) return 'completed';
  return 'active';
}
...
function relativeTime(value) {
  ...
  if (absolute < 7 * 86_400_000) return formatter.format(Math.round(delta / 86_400_000), 'day');
...
function resultSummary(task) {
  ...
  if (intake?.nextAction) return { label:'判断结果', text:intake.nextAction };
  const review = artifacts.find((item) => item?.type === 'review_report')?.data;
  if (review?.nextAction) return { label:'审核结论', text:review.nextAction };
  const publicReport = artifacts.find((item) => item?.type === 'public_web_report')?.data;
  if (publicReport?.summary) return { label:'结果摘要', text:publicReport.summary };
  if (task.status === 'succeeded') return { label:'完成情况', text:`任务已完成${artifacts.length ? `，留下 ${artifacts.length} 项交付或证据` : ''}。` };
  return null;
}
