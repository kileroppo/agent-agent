export function createAccessViews({
  elements,
  state,
  api,
  escapeHtml,
  ...
  const {
    shareInfo, employeeConnections, accessConnections, aiControl, aiServiceList,
    aiControlMessage, aiRoutingList, campaignList, campaignMessage,
    employeeConnectionList, accessConnectionList, accessConnectionMessage,
    contentAccessSummary, accessLoginDisclosure, accessLoginForm,
    accessLoginProvider, accessLoginAlias, accessLoginAccount, accessLoginMessage,
    saveAccessConnection, cancelAccessReauthorize, accessStepPanels,
    accessStepIndicators,
  } = elements;
...
async function renderContentCampaigns() {
  if (!state.localOwner || !campaignList || !campaignMessage) return;
  try {
...
async function renderAccessLoginOptions() {
  ...
  replaceChildrenPreservingDisclosureState(
    accessLoginProvider,
    state.accessLoginOptions.providers.map((provider) => new Option(provider.label, provider.id)),
  );
  if (state.accessLoginOptions.providers.some((provider) => provider.id === selectedProvider)) accessLoginProvider.value = selectedProvider;
