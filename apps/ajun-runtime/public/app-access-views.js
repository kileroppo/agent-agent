...
export function createAccessViews({ elements, state, api, statusLabel, formatDate, providerLabel, agentName, replaceChildrenPreservingDisclosureState, setTextIfChanged, modelPolicyConsole, }) {
    ...
    async function renderLocalShare() {
        ...
        try {
            ...
            if (share.enabled) {
                const shareAddresses = document.querySelector('#share-addresses');
                const shareKey = document.querySelector('#share-key');
                shareAddresses.textContent = `同一局域网可通过：${share.addresses.map((address) => `http://${address}:4321`).join(' · ')}`;
    ...
    async function renderAccessLoginOptions() {
        ...
        replaceChildrenPreservingDisclosureState(accessLoginProvider, state.accessLoginOptions.providers.map((provider) => new Option(provider.label, provider.id)));
        if (state.accessLoginOptions.providers.some((provider) => provider.id === selectedProvider))
    ...
    function renderAccessLoginAccounts() {
        ...
        const accounts = state.accessLoginOptions.accounts.filter((account) => account.connected && account.platforms.includes(provider));
        ...
        if (accounts.some((account) => account.clientId === current))
    ...
    async function renderTaskSubmitForm() {
        if (!state.localOwner)
            return;
        const container = document.querySelector('#task-submit-section');
        if (!container)
            return;
        container.hidden = false;
        container.innerHTML = html `<details class="task-submit-disclosure" data-disclosure-key="task-submit"><summary><div class="task-submit-summary-left"><span class="task-submit-icon"><svg aria-hidden="true"><use href="#icon-radar"></use></svg></span><div class="task-submit-copy"><strong>快速交办备用通道</strong><small>仅在飞书不便时使用</small></div></div><svg class="chevron" aria-hidden="true"><use href="#icon-chevron"></use></svg></summary><div class="task-submit-body"><form id="task-submit-form" data-refresh-protected><div class="task-submit-fields"><div class="field-row"><label class="task-field-label"><span>任务标题 <em class="req">*</em></span><input name="title" required placeholder="描述要完成的工作，如：整理本周 AI 动态"></label><label class="task-field-label agent-select-label"><span>指定岗位</span><select name="agentId"><option value="">🤖 智能自动路由</option></select></label></div><label class="task-field-label"><span>补充要求 (可选)</span><textarea name="description" rows="2" placeholder="补充交付格式、重点参考链接或具体要求..."></textarea></label></div><div class="task-submit-footer"><p class="task-submit-message" role="status"></p><button type="submit" class="task-submit-btn"><svg aria-hidden="true" width="14" height="14" viewBox="0 0 24 24"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg><span>提交任务</span></button></div></form></div></details>`;
        try {
            const options = await api('/api/tasks/submit/options');
            const select = container.querySelector('select[name="agentId"]');
