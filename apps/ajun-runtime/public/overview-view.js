...
export function createOverviewView({ elements, state, api, employeeView, formatDate }) {
    ...
    async function fetchChainDiagnosis() {
        ...
        try {
            ...
            `;
            body.querySelector('[data-copy-cmd]')?.addEventListener('click', async (e) => {
            ...
            body.querySelector('[data-reload-diagnosis]')?.addEventListener('click', () => {
    ...
    function getUserGuidance(verdict) {
    ...
    function statCard(label, value, note, icon, statusTone = 'normal', href = '') {
