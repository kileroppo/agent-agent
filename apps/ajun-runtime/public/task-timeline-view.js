...
export function createTaskTimelineLoader({ api, escapeHtml = defaultEscapeHtml } = {}) {
  if (typeof api !== 'function') throw new TypeError('任务时间线需要 api 读取函数。');
  let taskId = '';
  let filters = [];
  ...
  return {
    ...
    async loadMore() {
      if (!page.nextCursor) return renderTaskTimeline(page, { escapeHtml });
...
function renderTechnical(technical, escapeHtml) {
  ...
  if (!rows.length && !(technical.artifactRefs || []).length && !technical.qualityResult) return '';
