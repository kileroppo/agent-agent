...
export function createHotReloadMonitor({
  ...
  return Object.freeze({
    async check() {
      if (reloaded) return { status:'reloaded' };
      const response = await fetchImpl('/api/dev/hot-reload', { cache:'no-store' });
      if (!response.ok) return { status:'disabled' };
      const state = await response.json();
      if (!state?.enabled || !state.revision) return { status:'disabled' };
      if (revision === null) {
        revision = state.revision;
        return { status:'baseline', revision };
      }
      if (state.revision === revision) return { status:'unchanged', revision };
      reloaded = true;
      reload();
...
export function startBrowserHotReload({
  ...
  const tick = async () => {
    try {
      ...
      if (result.status === 'reloaded' || result.status === 'disabled') stop();
    } catch {
