export function replaceChildrenPreservingDisclosureState(container, nodes) {
  ...
  const openKeys = new Set(
    [...container.querySelectorAll('details[data-disclosure-key]')]
...
function sameRenderedNode(current, replacement) {
  if (current.isEqualNode(replacement)) return true;
  if (typeof current.cloneNode !== 'function' || typeof replacement.cloneNode !== 'function') return false;
...
function clearDisclosureState(node) {
  if (node.matches?.('details[data-disclosure-key]')) node.open = false;
