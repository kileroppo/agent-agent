...
export function cleanHumanReadableText(text) {
...
export function formatStructuredReportText(data, artifactType = '') {
    ...
    if (typeof data.markdown === 'string' && data.markdown.trim().length > 20)
        return cleanHumanReadableText(data.markdown.trim());
    if (typeof data.text === 'string' && data.text.trim().length > 20 && !data.text.trim().startsWith('{'))
        return cleanHumanReadableText(data.text.trim());
    const sections = [];
    ...
    if (artifactType === 'cross_agent_mission_summary' || (Array.isArray(data.statuses) && data.decision)) {
        ...
        if (data.decision?.briefing) {
            ...
            if (b.summary)
                bLines.push(`核心结论：${cleanHumanReadableText(b.summary)}`);
            if (b.keyFindings)
    ...
    if (artifactType === 'video_content_analysis_report' || Array.isArray(data.modules) || data.sourceMetadata) {
        ...
        if (Array.isArray(data.actionItems) && data.actionItems.length > 0) {
            const actions = data.actionItems.map((a, idx) => `  ${idx + 1}. ${cleanHumanReadableText(typeof a === 'string' ? a : (a.action || a.title || ''))}`).join('\n');
        ...
        if (Array.isArray(data.modules) && data.modules.length > 0) {
            const moduleLines = data.modules.map((m, idx) => {
                ...
                if (m.finding)
                    mLines.push(`     分析结论：${cleanHumanReadableText(m.finding)}`);
                if (Array.isArray(m.originalAnalysis?.claims) && m.originalAnalysis.claims.length > 0) {
                    const claims = m.originalAnalysis.claims.map((c) => cleanHumanReadableText(c.claim || c.text || c)).join('；');
                    mLines.push(`     原文提炼：${claims}`);
                }
                if (Array.isArray(m.diagnosis?.issues) && m.diagnosis.issues.length > 0) {
                    const issues = m.diagnosis.issues.map((i) => cleanHumanReadableText(i.issue || i.text || i)).join('；');
                    mLines.push(`     诊断要点：${issues}`);
                }
                if (Array.isArray(m.optimization?.actions) && m.optimization.actions.length > 0) {
                    const opts = m.optimization.actions.map((o) => cleanHumanReadableText(o.action || o.text || o)).join('；');
    ...
    if (data.topic)
        sections.push(`【调研主题】${cleanHumanReadableText(data.topic)}`);
    ...
    if (Array.isArray(data.opportunitySignals) && data.opportunitySignals.length > 0) {
        const signals = data.opportunitySignals.map((s, idx) => `  ${idx + 1}. ${cleanHumanReadableText(s.signal || s.text || s.title || '')}`).join('\n');
        sections.push(`【热门选题机会信号】\n${signals}`);
    }
    else if (Array.isArray(data.claims) && data.claims.length > 0) {
        const claims = data.claims.map((c, idx) => `  ${idx + 1}. ${cleanHumanReadableText(c.text || c.claim || c.statement || '')}`).join('\n');
        sections.push(`【关键事实与选题发现】\n${claims}`);
    }
    if (Array.isArray(data.originalAngles) && data.originalAngles.length > 0) {
        const angles = data.originalAngles.map((a, idx) => `  💡 角度 ${idx + 1}：${cleanHumanReadableText(a.premise || a.angle || '')}\n     建议创作用法：${cleanHumanReadableText(a.treatment || '提炼核心观点重新创作')}`).join('\n');
        sections.push(`【原创切入角度建议】\n${angles}`);
    }
    if (Array.isArray(data.sources) && data.sources.length > 0) {
        const sources = data.sources.map((s, idx) => `  🔗 [来源 ${idx + 1}] ${cleanHumanReadableText(s.title || '公开资料')} ${s.url ? `(${s.url})` : ''}`).join('\n');
