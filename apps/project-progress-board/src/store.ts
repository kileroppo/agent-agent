...
function normalizeTask(row: any) { return { ...row, sourceLinks: parseLinks(row.source_links), source_links: undefined }; }
function normalizeProject(row: any) {
    ...
    delete project.current_phase;
    delete project.created_at;
    delete project.updated_at;
    delete project.source_links;
    return project;
}
function projectStats(projectId: any) {
    ...
    const progress = total ? Math.round(tasks.reduce((sum: any, task: any) => sum + (task.status === 'done' ? 100 : task.progress), 0) / total) : 0;
    return { total, done, progress, doing: tasks.filter((task: any) => task.status === 'doing').length, blocked: tasks.filter((task: any) => task.status === 'blocked').length, todo: tasks.filter((task: any) => task.status === 'todo').length };
}
function withStats(row: any) { return { ...normalizeProject(row), stats: projectStats(row.id) }; }
function seed() {
    ...
    projects.forEach((project: any, index: any) => {
        ...
        taskSets[index].forEach(([title, status, progress, nextAction]: any, taskIndex: any) => insertTask.run(result.lastInsertRowid, title, index === 0 ? ['M1', 'M2', 'M3'][taskIndex] || 'M2' : project.phase, status, progress, taskIndex === 0 ? 'high' : 'normal', '本人', '', nextAction, status === 'blocked' ? '需要补齐后台阶段更新的真实回归证据' : '', '', '[]', timestamp, timestamp));
    });
}
seed();
...
export function getProject(id: any) {
    ...
    project.phases = [...new Set(project.tasks.map((task: any) => task.phase))].map((name: any) => {
        const tasks = project.tasks.filter((task: any) => task.phase === name);
        return { name, progress: tasks.length ? Math.round(tasks.reduce((sum: any, task: any) => sum + task.progress, 0) / tasks.length) : 0, done: tasks.filter((task: any) => task.status === 'done').length, total: tasks.length, status: tasks.every((task: any) => task.status === 'done') ? 'completed' : tasks.some((task: any) => task.status === 'doing' || task.status === 'blocked') ? 'active' : 'planning' };
    });
    return project;
}
export function dashboard() {
    const projects = listProjects();
    const tasks = db.prepare('SELECT t.*, p.name AS project_name FROM tasks t JOIN projects p ON p.id = t.project_id ORDER BY CASE t.status WHEN \'blocked\' THEN 1 WHEN \'doing\' THEN 2 ELSE 3 END, t.updated_at DESC').all().map((task: any) => ({ ...normalizeTask(task), projectName: task.project_name }));
    const total = tasks.length;
    return { projects, focusTasks: tasks.filter((task: any) => ['doing', 'blocked'].includes(task.status)).slice(0, 8), stats: { projects: projects.length, progress: total ? Math.round(tasks.reduce((sum: any, task: any) => sum + task.progress, 0) / total) : 0, doing: tasks.filter((task: any) => task.status === 'doing').length, blocked: tasks.filter((task: any) => task.status === 'blocked').length, done: tasks.filter((task: any) => task.status === 'done').length, todo: tasks.filter((task: any) => task.status === 'todo').length } };
}
...
export function updateProject(id: any, input: any = {}) {
...
export function createTask(projectId: any, input: any = {}) {
    if (!db.prepare('SELECT id FROM projects WHERE id = ?').get(Number(projectId)))
        return null;
    ...
    return getProject(projectId).tasks.find((task: any) => task.id === Number(result.lastInsertRowid));
}
export function updateTask(id: any, input: any = {}) {
    ...
    return getProject(existing.project_id).tasks.find((task: any) => task.id === Number(id));
}
export { db };
