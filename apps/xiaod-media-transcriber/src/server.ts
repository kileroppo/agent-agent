...
import { IntakeError, createFeishuMediaJob } from './feishu-media-intake.ts';
import { MediaPipeline } from './pipeline.ts';
import { LarkDeliveryCoordinator } from './lark-delivery.ts';
import { JobPauseController, JobPauseError } from './job-pause-controller.ts';
import { JobStore, JobStoreConflictError } from './store.ts';
import { ConnectionSelectionError, createContentRuntime } from './content-runtime.ts';
...
const onRunEvent = (event: unknown) => taskRunEvents.appendTaskRunEvent(
...
app.get('/api/cookie-bridge/accounts', async (_req, res) => {
  ...
  try {
    ...
    res.json({ accounts: accounts.map((account: DynamicRecord) => ({
    ...
    })).filter((account: DynamicRecord) => account.clientId) });
  } catch {
...
app.get('/api/jobs/:id/download', async (req, res, next) => {
  try {
    ...
    const markdownPath = requiredOutputPath(job.output.markdownPath);
    await fs.access(markdownPath);
    res.download(markdownPath, `${String(job.title)}-分享式整理稿.md`);
  } catch (error) { next(error); }
});
app.get('/api/jobs/:id/download/:asset', async (req, res, next) => {
  try {
    ...
    const assetPath = requiredOutputPath(job.output[asset[0]]);
    await fs.access(assetPath);
    res.download(assetPath, asset[1]);
  } catch (error) { next(error); }
});
...
app.post('/api/jobs/:id/redeliver', async (req, res, next) => {
  try {
    ...
    const markdown = await fs.readFile(requiredOutputPath(job.output.markdownPath), 'utf8');
    const requestedTitle = typeof req.body?.title === 'string' ? req.body.title.trim().replace(/\s+/g, ' ').slice(0, 200) : '';
    const title = requestedTitle || String(job.title || '未命名素材');
    ...
    const reviewComplete = ['confirmed', 'auto_confirmed'].includes(String(job.output?.reviewStatus || ''));
...
app.post('/api/jobs/:id/lark-delivery/resolve', async (req, res, next) => {
  try {
    ...
    const patch = knownLarkDeliveryRecoveryPatch(existing);
    if (!patch) return res.status(409).json({ error:'飞书交付仲裁结果无法结算。' });
    const job = await store.update(req.params.id, patch, { stage:String(patch.status), message:'飞书交付已由操作者核对并结算' });
    res.json({ job });
  } catch (error) { next(error); }
});
...
app.use((error: unknown, _req: Request, res: Response, _next: NextFunction) => {
  ...
  const dynamicError = error as DynamicRecord;
  if (dynamicError?.code === 'lark_delivery_uncertain') return res.status(dynamicError.status || 409).json({ error:dynamicError.message, code:dynamicError.code });
  if (dynamicError?.code === 'lark_delivery_resolution_invalid') return res.status(dynamicError.status || 422).json({ error:dynamicError.message, code:dynamicError.code });
...
function localCookieBridgeAccountsEndpoint(value: unknown): URL | null {
  try {
    const endpoint = new URL('/api/accounts', String(value || ''));
    return ['127.0.0.1', 'localhost', '::1'].includes(endpoint.hostname) ? endpoint : null;
  } catch { return null; }
}

function requiredOutputPath(value: unknown): string {
  const filePath = String(value || '');
  if (!path.isAbsolute(filePath)) throw new Error('任务交付物路径无效。');
  return filePath;
}
