...
export function createTaskRunEventBridge({ onRunEvent = null, clock = () => new Date().toISOString() } = {}) {
  ...
  return Object.freeze({
    async recordExecutionReceipt(receipt, { qualityResult = null } = {}) {
    ...
    async recordVisualResult({ job = {}, result = null, startedAt = null, completedAt = null, error = null } = {}) {
      ...
      const failureCode = failed ? clean(error?.code || error?.accessFailure?.code || 'visual_evidence_unavailable', 120) : null;
    ...
    async recordLarkDelivery({ job = {}, delivery = null } = {}) {
...
async function recordQualityResult(receipt, quality, emit) {
  await emit({
...
function receiptCommon(receipt) {
  return {
...
function jobCommon(job, taskId, capabilityId) {
  return {
    taskId,
...
function sanitizeEvent(input, fallbackTime) {
...
function eventTypeForOutcome(value) {
