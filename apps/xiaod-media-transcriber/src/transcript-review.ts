...
import { composeDelivery } from './domain.ts';
import { larkDeliveryUncertainFailure } from './recovery.ts';
import { confirmedTranscriptDocument } from './transcript-evidence.ts';
type DynamicRecord = Record<string, any>;
type Clock = () => Date;
type JobStoreInterface = Readonly<{
...
type DeliveryInterface = Readonly<{
  deliver(input: Readonly<{ jobId: string; title: string; markdown: string }>): Promise<DynamicRecord>;
}>;
...
export async function createTranscriptConfirmationFiles({
  ...
  const controlledDirectory = requiredPath(directory);
  const confirmedTranscriptPath = path.join(controlledDirectory, `confirmed-transcript-v${version}.md`);
  ...
  const attestation = {
    ...
    basedOnVersion:typeof basedOnVersion === 'number' && Number.isSafeInteger(basedOnVersion) && basedOnVersion > 0 ? basedOnVersion : null,
...
export async function readTranscriptRevision(job: DynamicRecord | null) {
...
export async function reviseTranscript({ store, job, input = {}, now = () => new Date() }: Readonly<{
  store: JobStoreInterface;
  job: DynamicRecord | null;
  input?: DynamicRecord;
  now?: Clock;
}>) {
  if (!job) throw new TranscriptReviewError('任务不存在。', 404);
  ...
  let confirmation: Awaited<ReturnType<typeof createTranscriptConfirmationFiles>>;
  try {
    ...
    if (errorCode(error) === 'EEXIST') throw new TranscriptReviewError('字幕版本已经变化，请刷新后再保存。', 409);
    throw error;
  }
...
export async function reviewTranscript({ store, job, input = {}, now = () => new Date(), delivery = null }: Readonly<{
  store: JobStoreInterface;
  job: DynamicRecord | null;
  input?: DynamicRecord;
  now?: Clock;
  delivery?: DeliveryInterface | null;
}>) {
  if (!job) throw new TranscriptReviewError('任务不存在。', 404);
  ...
  if (delivery && !localOnly) {
    try {
      ...
      const uncertain = errorCode(error) === 'lark_delivery_uncertain';
      ...
      updated = await store.update(job.id, {
        ...
        error:uncertain ? errorMessage(error) : null,
        failure,
...
async function readJson(filePath: unknown): Promise<DynamicRecord | null> {
  try { return JSON.parse(await fs.readFile(requiredPath(filePath), 'utf8')); }
  catch { return null; }
}

async function readText(filePath: unknown): Promise<string | null> {
...
function stripConfirmedTranscriptDocument(value: unknown): string {
  return stripDocumentHeading(String(value || '').replace(/^---\n[\s\S]*?\n---\n+/m, '')).trim();
}

function normalizeCorrection(value: unknown): string {
...
function transcriptVersionRecord(output: DynamicRecord = {}, version = 1): DynamicRecord {
  return {
    version,
    basedOnVersion:null,
    correctedAt:null,
...
function mergeTranscriptVersions(existing: unknown, previous: DynamicRecord, revision: DynamicRecord): DynamicRecord[] {
  const versions: DynamicRecord[] = Array.isArray(existing) ? existing.filter((item): item is DynamicRecord => Boolean(item && typeof item === 'object' && Number.isSafeInteger(item.version))) : [];
