...
type CommandResult = Readonly<{ stdout: string | Buffer; stderr: string; code: number | null }>;
type RunCommand = (command: string, args: readonly string[], options?: Readonly<{ encoding?: BufferEncoding | null; allowFailure?: boolean }>) => Promise<CommandResult>;
type FrameCandidate = Readonly<{ timestampSeconds: number; reason: string; priority: number }>;
type EvidenceFrame = {
  frameId: string;
  timestamp: string;
...
type Storyboard = Readonly<{ storyboardId: string; localRef: string; frameRefs: string[]; checksum: string }>;
...
export async function createVisualEvidencePackage({
  ...
  outputDir?: string;
  depth?: string;
  transcriptSegments?: readonly Readonly<{ startSeconds?: unknown }>[];
...
export function selectFrameCandidates({ durationSeconds, transcriptSegments = [], sceneTimes = [], maxFrames = FAST_FRAME_LIMIT }: Readonly<{
  durationSeconds?: unknown;
  transcriptSegments?: readonly Readonly<{ startSeconds?: unknown }>[];
  sceneTimes?: readonly unknown[];
  maxFrames?: number;
}> = {}): FrameCandidate[] {
  ...
  const byTime = new Map<number, FrameCandidate>();
...
export function hammingDistance(left: unknown, right: unknown): number {
...
async function probeVideo(videoPath: string, run: RunCommand): Promise<{ hasVideo: boolean; durationSeconds: number }> {
  ...
  try { payload = JSON.parse(String(output.stdout || '{}')) as Record<string, any>; } catch { payload = {}; }
  return {
...
async function detectSceneTimes(videoPath: string, run: RunCommand): Promise<number[]> {
...
async function framePerceptualHash(videoPath: string, timestampSeconds: number, run: RunCommand): Promise<bigint | null> {
  const result = await run('ffmpeg', [
    '-hide_banner',
    '-loglevel', 'error',
...
async function renderTimestampedFrame(videoPath: string, timestampSeconds: number, outputPath: string, run: RunCommand): Promise<void> {
...
async function writeTimestampLabel(filePath: string, text: string): Promise<void> {
...
async function renderStoryboard(framePaths: readonly string[], outputPath: string, workspace: string, run: RunCommand): Promise<void> {
...
function spreadValues<T>(values: readonly T[], limit: number): T[] {
...
function spreadCandidatesByTime(values: readonly FrameCandidate[], limit: number, endSeconds: number): FrameCandidate[] {
...
function coveragePriorityOrder(values: readonly FrameCandidate[]): FrameCandidate[] {
  ...
  const selected: FrameCandidate[] = [remaining.shift()!];
...
function timestampSlug(value: unknown): string {
...
function runCommand(command: string, args: readonly string[], { encoding = 'utf8', allowFailure = false }: Readonly<{ encoding?: BufferEncoding | null; allowFailure?: boolean }> = {}): Promise<CommandResult> {
  return new Promise<CommandResult>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      reject(Object.assign(new Error(`${command} 执行失败（退出码 ${code}）：${result.stderr.slice(-500)}`), {
        code:command === 'ffmpeg' || command === 'ffprobe' ? 'visual_tool_failed' : 'tool_failed',
      }));
    });
  });
}
