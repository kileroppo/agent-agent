...
11 个正式岗位均保留原专有任务，并增加一个岗位域内开放任务。开放任务必须生成 GoalSpec、DAG 工作计划、checkpoint 和能力审计产物；未知或敏感能力不得自动加入白名单。统一自主硬上限为 60 分钟、20 次模型调用、4 个并发子任务、2 层委派和 5 美元付费审批线。主模型元数据统一为 `stepfun/step-3.7-flash`，回退链为空；微信私密只读检索岗位是 A君本地适配能力，不计入这 11 个正式岗位。新主模型真实调用验证前 Profile 仍显示传输待验证。
