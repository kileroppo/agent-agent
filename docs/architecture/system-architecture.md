...
| 字段 | 内容 |
| --- | --- |
| 状态 | 生效；M1 真实飞书闭环已验收，M2 其余外部能力按账本继续验证 |
| 负责人 | 技术负责人 / Codex 工作台 |
| 版本 | v1.7 |
| 最后更新 | 2026-07-26 |
...
飞书中的“ A君·军团总管”是主要日常总管入口；老板也可以按需直接私聊任何已上岗员工。每个入口由该员工独立 Hermes Gateway 将聊天映射为自己的持久 Session/Profile/Memory，群内仅在 @ 提及时接收命令；官方 Channel SDK 保留已验证卡片、群聊能力和应急回退。飞书适配层不保存业务执行 checkpoint，也不自行判断任务完整成功。
