...
flowchart LR
    U["用户手机"] --> F["飞书：A君·军团总管\n日常任务、结果与审批卡"]
    F --> H["Hermes Gateway：常驻 Mac 收发与路由"]
    H --> R["协调官与执行运行时"]
...
飞书中的“ A君·军团总管”是唯一日常入口：私聊接收命令，群内仅在 @ 提及时接收命令；展示用户级状态、请求补充信息或审批、交付文档和附件。它不保存业务执行 checkpoint，也不自行判断任务完整成功。用户在任何地点只需手机飞书，不需要访问 A君局域网页；Mac 未在线时必须明确说明不可执行或已排队。
