...
`office.presentation-package` 沿用 MCP/飞书 → A君任务目录 → 小办受控本地执行 → Paperclip Issue/Work Product 的现有治理链路。结构化演示任务不调用模型；A君保留任务与阶段 checkpoint，Paperclip 只记录治理状态和脱敏产物引用。`OfficePresentationAdapter` 组合两个窄适配器：
...
3. `export`：导出到适配器临时目录，补齐 Office 兼容字体声明与每页唯一根级 fade 转场，验证 OOXML 顺序和 ZIP CRC 后，以 no-overwrite 方式写入 A君受控任务工作区。
...
- 只读取任务允许的 `sourceTaskIds` 与当前 A君受控任务工作区；
