...
当前固定版 Paperclip `2026.722.0` 使用内置 `zh-CN` 语言包，并由本机兼容层翻译主控制台仍然硬编码为英文的导航、状态、操作和说明；任务标题、员工名、产物正文等业务数据不会被改写。页面右上角固定显示 **EN / 中文** 按钮，点击后会保存本机语言偏好并刷新页面，以后不需要改代码。中文补丁同时使用独立缓存标识，是版本和源码哈希锁定的本机补丁；Paperclip 升级后不会盲目修改新版本，需先重新核验并更新补丁。若已打开的旧页面仍显示英文，刷新一次；仍未恢复时检查补丁和运行版本，不要用浏览器机翻代替运行验证。
