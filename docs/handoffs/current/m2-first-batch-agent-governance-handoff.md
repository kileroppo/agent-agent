...
- Exact next action: 修复独立“ A君·军团总管”应用的飞书消息事件投递：机器人聊天会话已创建，但两次“检查系统状态”均未进入 Hermes。入站恢复后，确认同会话回执；随后发送“外发本次健康摘要”点击 local 审批卡，再发送“公开发布系统摘要”点击组织级审批卡，确认卡片点击后 Paperclip 审批状态与 A君任务状态一致。最后发送“小D整理 + 公开链接”和“创建一个 Agent”。
...
- 飞书 `open_id` 按应用隔离：同一用户在小D与A君等不同自建应用下会得到不同 ID；不得复用旧机器人的允许名单、审批人映射、幂等键或会话关联。
- 新增或切换飞书机器人时，先验证“真实私聊消息 → Hermes 入站日志 → A君任务 → 同会话回执”；长连接成功、事件已订阅和机器人会话已创建都不能替代该验证。
- 本次 A君没有配置用户允许名单，因此当前未回执不能直接归因于 `open_id` 拒绝；该经验作为后续多应用接入的强制检查项保留。
...
| 外部平台 | Paperclip 审核任务与批准记录曾真实创建；2026-07-21 已恢复为 `ai.agent-army.paperclip` LaunchAgent；Hermes 隔离 Profile 已创建。独立的“ A君·军团总管”飞书机器人已启用长连接消息订阅，本机 Hermes 已建立 WebSocket 连接；飞书客户端中机器人会话已真实创建 | 两次真实私聊均未投递到 Hermes；回执、local/组织级真实卡片点击、小D/创建 Agent 真实路由与候选 Agent 上线未验证 |
...
- “ A君·军团总管”现为独立飞书应用，而非此前挂接的 Hermes 应用；本机凭证仅在当前用户 LaunchAgent 中保存，未进入仓库或交接材料。机器人能力、长连接接收消息事件和 1.0.2 发布已完成，Hermes 已建立 WebSocket 连接；用户已真实打开机器人会话，但两条“检查系统状态”未进入 Hermes，必须以事件投递日志和 A君任务记录定位，不能从连接日志推断入站可用。
