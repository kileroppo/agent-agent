...
- 唯一下一步：由主人明确指定一次微信会话与范围，验收临时授权、撤销和本机脱敏摘要；不要自行选择真实私聊。
...
| 本地运行时 | Hermes 的小R、小D、办公助理已同步岗位技能；A君 Profile 已加载微信授权撤销工具；回环 Qwen3.5-9B 已随 release `6a41ec6a…` 切入 4321 live | Hermes Profile 0 drift、18081 `/health`、4321 PID/cwd/entrypoint、`/api/overview` 与 release manifest | live 接线已验证；真实私聊未读取 |
...
| 运行时 | PASS WITH PRIVACY GATE | Hermes 三岗位技能与 A君 撤销工具同步完成；Qwen3.5-9B 只监听 `127.0.0.1:18081` 且健康；4321 新 release 强制重启后回读 `wechat-private-read=ready`；合成消息摘要不含原文和发送者标识 | 真实微信摘要仍待主人给定会话与范围 |
