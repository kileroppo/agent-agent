...
- [标准产品仓库结构改造交接](./current/standard-product-repository-architecture-handoff.md)：已关闭。产品目录、装配根、TaskService 深层 Module、任务关注、安全详情和受控恢复已落地；提交 `d071098…` 的不可变 release `869473aa…` 已切到 4321，并通过核心路由、真实失败任务、三档视口和两个刷新周期验收。真实恢复及外部平台动作仍需逐次授权。
