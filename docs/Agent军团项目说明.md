...
> 文档状态：方向说明与第一阶段建设基线（v1.3）
> 更新日期：2026-07-18
> 项目目录：`agent-agent`

## 1. 一句话定义

**Agent军团是一套以飞书为工作入口、以 Hermes 为智能体运行和执行任务底座、从 M2 引入 Paperclip 组织治理控制面、并以 Codex 等 AI 工具持续建设的数字员工系统。**
...
flowchart LR
    ...
    PC["Paperclip 军团治理（M2）<br/>组织、预算、审批、跨 Agent 审计"] -.治理投影.-> RT
    RT -.结果摘要.-> PC
    H["人工审批与最终责任"] --> PC
    PC --> H
```
...
岗位信息不由 Paperclip 或 Hermes 的后台配置单方包办。版本化 `AgentManifest` 是岗位发布基线：Hermes 管运行配置，M2 起由 Paperclip 管组织与治理投影，二者通过 Manifest 版本和部署记录发现漂移。

### 5.3 Paperclip：M2 起的军团组织治理控制面

Paperclip 适合作为 Agent军团的组织治理控制平面：

- 管理公司、部门、岗位、汇报关系、负责人和 Agent 生命周期；
- 关联标准任务 ID，保存组织级归属和结果投影；
- 汇总 Agent 健康、任务结果和交付物引用；
- 管理预算、成本、审批、暂停和终止；
- 支持定时任务、Webhook 和 API 触发；
- 为多个 Agent 提供共享但隔离的工作状态。

Paperclip 不是聊天界面、推理框架或第二套执行队列。它不能替代 Hermes 和飞书，需要通过适配器读取组织级投影，不能反向覆盖 Hermes 的执行状态。
...
- 接入 Paperclip 组织治理控制面；
- 建立组织投影、预算硬限制、结构化审批和跨 Agent 审计；
